...
/** Seeded H/V chrome slot from Forest paneRect (no gap). */
export function seededChildRect(wm, liveChild) {
  if (!wm?._liveForestSeeded || !wm.forest || !liveChild) return null;
  const id = forestIdFromLive(wm, liveChild);
  if (!id) return null;
  const tom = wm.forest.nodes[id];
  if (!tom) return null;
  const slot = forestSlotRect(wm.forest, tom);
  if (!slot || !(Number(slot.width) > 0) || !(Number(slot.height) > 0)) return null;
  return { x: slot.x, y: slot.y, width: slot.width, height: slot.height };
}

/** Forest MONITOR id `moNwsW` for chrome identity; null if none (FLOAT/orphan). */
export function forestMonitorIdForChrome(tree, node) {
  try {
    const wm = tree?.extWm;
    if (wm?._liveForestSeeded && wm.forest && node) {
      ...
      if (tom) {
        ...
        return tomMon?.id ? String(tomMon.id) : null;
      }
    }
    const mon = tree.findAncestorMonitor?.(node) ?? null;
    const val = mon?.nodeValue;
...
/** True when the node's Forest `moNwsW` pair is the live pair (active WS + that output). */
export function decorationOnActiveWorkspace(tree, node) {
  const monId = forestMonitorIdForChrome(tree, node);
  if (!monId) return true;
  try {
    const wsIdx = Utils.workspaceIndex(monId);
...
function hideInactivePairChrome(tree, node) {
  if (!node) return;
  const deco = node.decoration;
  if (deco) {
    if (!NodeChrome.actorAlive(deco)) {
      destroyDeadDecoration(node, "pair-hide");
    } else {
      try {
        deco.hide();
      } catch (e) {
        if (NodeChrome.isDisposedActorError(e)) {
  ...
  if (typeof node.isWindow === "function" && node.isWindow()) {
    const wm = tree.extWm;
    const actor = node.windowActor || node.nodeValue?.get_compositor_private?.();
    if (actor && typeof wm?.hideActorBorder === "function") {
      wm.hideActorBorder(actor);
    }
  }
  const kids = liveChildrenForPresent(tree.extWm, node);
  for (const child of kids) hideInactivePairChrome(tree, child);
}
...
export function processNode(tree, node) {
  ...
  if (node.nodeType === NODE_TYPES.MONITOR || node.nodeType === NODE_TYPES.CON) {
    if (node.nodeType === NODE_TYPES.MONITOR && !decorationOnActiveWorkspace(tree, node)) {
      hideInactivePairChrome(tree, node);
      return;
    }
    ...
    const seeded = !!tree.extWm?._liveForestSeeded && !!tree.extWm?.forest;
    const splitLayout = node.layout === LAYOUT_TYPES.HSPLIT || node.layout === LAYOUT_TYPES.VSPLIT;
    let sizes = null;
    if (!(seeded && splitLayout)) {
      sizes = computeSizes(tree, node, tiledChildren);
    }
    ...
    tiledChildren.forEach((child, index) => {
      if (groupChrome && node.layout === LAYOUT_TYPES.STACKED) {
        ...
        const slot = seeded ? seededChildRect(tree.extWm, child) : null;
        if (slot) {
          child.rect = slot;
        } else {
          if (!params.sizes) {
            params.sizes = computeSizes(tree, node, tiledChildren);
          }
...
export function applyDecorationRect(tree, node, child, params, barSize, tabExpand, tabHost = null) {
  ...
  let borderWidth = 0;
  const winBorder = child.isWindow() ? child.actor?.border : null;
  if (NodeChrome.actorAlive(winBorder)) {
    try {
      borderWidth = winBorder.get_theme_node()?.get_border_width?.(St.Side.TOP) || 0;
    } catch (_e) {
      borderWidth = 0;
    }
  }
...
export function computeSizes(tree, node, childItems) {
  ...
  return TreeLayout.computeSizes(node, childItems, (items) => getTiledChildren(tree, items), {
    skipWriteBack: !!tree.extWm?.openLayoutBatchActive || seeded,
    renormalize: seeded,
  });
}
