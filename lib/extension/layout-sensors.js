 ...
 * Attribution decides chrome-only vs markUnsettled + diagnostic verify (no forest apply).
 */
 ...
 *  - stack `_suppressGeometrySignalRetile` (in-call re-entrancy during move/apply), or
 *  - active per-window command echo epoch (post-stack client snap residual, AC2).
 *
 * @param {{
 *   _suppressGeometrySignalRetile?: boolean,
...
export function isForgeCausedGeometrySignal(wm, metaWindow) {
  if (!wm) return false;
  if (wm._suppressGeometrySignalRetile) return true;
  if (
    metaWindow &&
    wm.layoutEpoch &&
