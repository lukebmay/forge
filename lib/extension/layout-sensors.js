...
/**
 * True when a TILE node's Meta frame already matches its paint target within ε
 * (and is not fullscreen / maximized). Full re-layout is unnecessary then —
 * chrome (borders) only (W-storm in-slot path).
 *
 * `options.targetRect` is the paint rect (zoom-aware); else `renderRect` / `rect`.
 *
...
export function shouldChromeOnlyGeometry(
  ...
  const slot = options?.targetRect || node.renderRect || node.rect;
...
/**
 * True when a TILE should snap back to its paint target (D026).
 * Lone-tile maximize-on-single is left alone. Live grab uses `forGrab`
 * so resize/move percents are not treated as drift.
 *
 * Zoom (D030): compare / restore against `options.targetRect` (paint), not the
 * unzoomed layout slot — otherwise tab-peer heal and Chrome snap leave a
 * zoomed border over a slot-sized frame.
 *
 * @param {{ mode?: string, renderRect?: object, rect?: object, zoomMode?: string|null }|null|undefined} node
 * @param {{
 *   get_frame_rect?: () => object,
 *   is_fullscreen?: () => boolean,
 *   get_maximized?: () => number,
