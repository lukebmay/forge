...
export function afterFocus(wm, node, opts = {}) {
  ...
  // Workspace switch: Meta may focus a non-open tab sibling (WS focus memory).
  // Open leaf ≠ keyboard focus — do not adopt; re-raise lastTabFocus.
  if (
    opts.source === "meta-focus" &&
    wm._workspaceChanging &&
    typeof wm.restoreOpenLeafIfWorkspaceFocusSteal === "function" &&
    wm.restoreOpenLeafIfWorkspaceFocusSteal(node)
  ) {
    Logger.debug("afterFocus ws-change preserve open-leaf");
    try {
      wm.updateBorderLayout?.();
    } catch (_e) {
      /* best-effort */
  ...
  try {
    ...
    // G5d: pair border to compositor in the same turn as raise (before strip).
    try {
      const meta = node.nodeValue;
      if (meta) {
        if (typeof wm.restackBorderForMeta === "function") {
          wm.restackBorderForMeta(meta);
        } else {
          wm.decorationManager?.restackBorderForMeta?.(meta);
        }
      }
    } catch (_e) {
      /* best-effort */
    ...
    // D069: no reassert on afterFocus (PWA frame-lie thrash). Slot heal is
    // commit / epoch-end / revealGroupChild (R025) only.
 ...
 * Write LTF → pin or adopt live pin (R026) → raise → optional R025 slot
 * heal → settleTabFocus; keyboard → focus + activate + afterFocus (restack last).
 *
 * D069: peers are sized on join / slot change. Tab click is raise+focus;
 * R025 reassert is a safety net only when still off-slot after raise
 * (Chrome/PWA often lie about buried frames until raised).
 *
 * @param {import('./window.js').WindowManager} wm
 * @param {import('./tree.js').Node|null|undefined} node
...
export function revealGroupChild(wm, node, opts = {}) {
  ...
  const tabClick = source === "tab-click";
  ...
  // Non-tab reveal: size before raise (FLOAT / old-slot recovery).
  // Tab click: raise first so Meta reports a real frame, then heal only if
  // still off-slot — avoids move_resize thrash when join already sized peers.
  const reassertBeforeRaise = !tabClick;
  if (reassertBeforeRaise) {
    try {
  ...
  // G5d: same-turn border pair after raise (keyboard path also pairs in afterFocus).
  try {
    if (meta) {
      if (typeof wm.restackBorderForMeta === "function") {
        wm.restackBorderForMeta(meta);
  ...
  if (!reassertBeforeRaise) {
    try {
      const moved = !!wm.reassertNodeToSlot?.(node);
      if (moved) Logger.trace(`reveal R025 moved source=${source}`);
    } catch (_e) {
      /* best-effort */
    }
  }
