...
/**
 * Action pipeline stage composers (FocusChanged, later Structure/Open).
...
export function afterFocus(wm, node, opts = {}) {
  ...
  // Unfreeze so F / Dfocus are not no-ops after grab/batch freeze.
  try {
    wm.unfreezeRender?.();
  } catch (_e) {
    /* ignore */
  ...
  // F — tab/stack lastTabFocus + ε sibling reassert + raise leaf
  try {
