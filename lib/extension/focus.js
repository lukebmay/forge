...
export class FocusManager extends GObject.Object {
  ...
  updateStackedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.STACKED && !this._extWm._freezeRender) {
      // Stable chrome order: do not appendChild (that reordered stack labels on
      // every focus/click). Reassert off-slot TILE siblings, then raise focused.
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      this._reassertTabStackSiblingSlots(parentNode);
  ...
  updateTabbedFocus(focusNodeWindow) {
    if (!focusNodeWindow || !focusNodeWindow.parentNode) return;
    const parentNode = focusNodeWindow.parentNode;
    if (parentNode.layout === LAYOUT_TYPES.TABBED && !this._extWm._freezeRender) {
      // Same as stack: record open leaf; reassert sibling geometry; raise.
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      this._reassertTabStackSiblingSlots(parentNode);
      focusNodeWindow.nodeValue.raise?.();
    }
  ...
  /**
   * TABBED/STACKED: move TILE siblings whose Meta frame drifted off
   * renderRect/rect (ε). No full renderTree — slot reassert only.
   * @param {object} parentNode
   */
  _reassertTabStackSiblingSlots(parentNode) {
    ...
    for (const child of kids) {
      ...
      if (!meta) continue;
      // Prefer WM helper when present (chrome-only / maximize / fullscreen).
      if (typeof wm._tiledWindowAtTreeSlot === "function") {
