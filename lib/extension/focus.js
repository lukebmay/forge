...
import { setLastTabFocus } from "../tom/atomics.js";
...
export class FocusManager extends GObject.Object {
  ...
  movePointerWith(nodeWindow, { force = false } = {}) {
    // Invent Nodes use `_data`; Forest WINDOW ducks use `nodeValue` only.
    if (!nodeWindow || !(nodeWindow._data || nodeWindow.nodeValue)) return;
  ...
  /**
   * TABBED/STACKED parent for open-leaf writes: Forest live parent, else GObject.
   * @param {object|null|undefined} focusNodeWindow
   * @returns {object|null}
   */
  _openLeafParent(focusNodeWindow) {
    if (!focusNodeWindow) return null;
    let parentNode = liveParentForPresent(this._extWm, focusNodeWindow);
    const gParent = focusNodeWindow.parentNode;
    const isTabOrStack = (n) =>
      !!n && (n.layout === LAYOUT_TYPES.TABBED || n.layout === LAYOUT_TYPES.STACKED);
    if (!isTabOrStack(parentNode) && isTabOrStack(gParent)) parentNode = gParent;
    return isTabOrStack(parentNode) ? parentNode : null;
  }
  ...
  setOpenLeaf(focusNodeWindow) {
    if (!focusNodeWindow) return false;
    const parentNode = this._openLeafParent(focusNodeWindow);
    ...
    try {
      ...
      if (wm?._liveForestSeeded && forest?.nodes) {
        const parentId = forestIdFromLive(wm, parentNode);
        const childId = forestIdFromLive(wm, focusNodeWindow);
        const tom = parentId ? forest.nodes[parentId] : null;
        if (
          tom &&
          (tom.kind === "CON" || tom.kind === "MONITOR") &&
          (tom.layout === "TABBED" || tom.layout === "STACKED") &&
          childId &&
          forest.nodes[childId]
        ) {
          setLastTabFocus(tom, childId);
        }
      }
    } catch (_e) {
      /* Forest ids optional in Tree-only fixtures */
    }
    ...
    if (
      gParent &&
      gParent !== parentNode &&
      (gParent.layout === LAYOUT_TYPES.TABBED || gParent.layout === LAYOUT_TYPES.STACKED)
    ) {
      gParent.lastTabFocus = next;
    }
    return true;
  }
  ...
  updateStackedFocus(focusNodeWindow) {
    ...
    const parentNode = this._openLeafParent(focusNodeWindow);
    if (!parentNode || parentNode.layout !== LAYOUT_TYPES.STACKED) return;
  ...
  updateTabbedFocus(focusNodeWindow) {
    if (!focusNodeWindow) return;
    const parentNode = this._openLeafParent(focusNodeWindow);
    if (!parentNode || parentNode.layout !== LAYOUT_TYPES.TABBED) return;
  ...
  _reassertTabStackSiblingSlots(parentNode, opts = {}) {
    ...
    for (const child of kids) {
      ...
      if (!isWin) continue;
      // TILE only — GRAB_TILE / FLOAT skip (createEnum string).
      if (String(child.mode) !== "TILE") continue;
      ...
      if (typeof wm.reassertNodeToSlot === "function") {
        if (wm.reassertNodeToSlot(child, { force })) n += 1;
  ...
  /**
   * Explicit/debug: size every TABBED/STACKED TILE peer to its group slot.
   * D069 visible-first; not the happy-path render wave (D095 S5).
   * @param {{ force?: boolean }} [opts]
   * @returns {number} groups that needed at least one reassert
   */
  reassertAllTabStackSlots(opts = {}) {
    const tree = this._tree;
    if (!tree || typeof tree.getNodeByType !== "function") return 0;
    const force = !!opts.force;
    let cons = [];
    try {
      cons = tree.getNodeByType(NODE_TYPES.CON) ?? [];
    } catch (_e) {
      cons = [];
    }
    const groups = [];
    ...
    // Pass 1: open leaves (on-screen content) before any buried peer work.
    for (const con of groups) {
      if (this._reassertTabStackSiblingSlots(con, { mode: "open-leaf", force }) > 0) {
        touched.add(con);
      }
    }
    // Pass 2: buried peers (still mapped under the open leaf).
    for (const con of groups) {
      if (this._reassertTabStackSiblingSlots(con, { mode: "all", force }) > 0) {
        touched.add(con);
      }
    }
    return touched.size;
