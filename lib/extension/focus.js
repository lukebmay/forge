...
export class FocusManager extends GObject.Object {
  ...
  updateStackedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.STACKED && !this._extWm._freezeRender) {
      ...
      // every focus/click). Reassert open leaf only, then raise.
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      this._reassertTabStackSiblingSlots(parentNode, { mode: "open-leaf" });
  ...
  updateTabbedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.TABBED && !this._extWm._freezeRender) {
      // Open leaf only: buried tabs skip move_resize (Wayland Chrome reflow).
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      this._reassertTabStackSiblingSlots(parentNode, { mode: "open-leaf" });
      focusNodeWindow.nodeValue.raise?.();
    }
   ...
   * Default mode `open-leaf`: only lastTabFocus — buried siblings stay put so
   * focus/z-order does not move_resize Chrome PWAs under the active tab.
   * Mode `all`: every off-slot TILE (verify recovery / force settle).
   * No full renderTree.
   *
