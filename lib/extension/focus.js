...
export class FocusManager extends GObject.Object {
  ...
  updateStackedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.STACKED && !this._extWm._freezeRender) {
      // lastTabFocus + raise only. Geometry reassert belongs to render/verify —
      // not focus (Chrome PWA flicker on first tab selects).
  ...
  updateTabbedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.TABBED && !this._extWm._freezeRender) {
      // lastTabFocus + raise only (no move_resize on tab switch).
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      focusNodeWindow.nodeValue.raise?.();
    }
  }
