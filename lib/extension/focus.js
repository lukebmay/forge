...
export class FocusManager extends GObject.Object {
  ...
  movePointerWith(nodeWindow, { force = false } = {}) {
    ...
    // Floats (Guake): never auto-warp unless force. move-pointer-focus is for
    // keyboard tile nav / attachNode; warping into dropdowns steals the cursor.
    const isFloat = typeof nodeWindow.isFloat === "function" && nodeWindow.isFloat();
    const shouldWarp =
      force || (!isFloat && this._extWm.ext.settings.get_boolean("move-pointer-focus-enabled"));
    if (shouldWarp) {
  ...
  updateStackedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.STACKED && !this._extWm._freezeRender) {
      ...
      // every focus/click). Raise only the focused window; track lastTabFocus.
  ...
  updateTabbedFocus(focusNodeWindow) {
    ...
    if (parentNode.layout === LAYOUT_TYPES.TABBED && !this._extWm._freezeRender) {
      // Same as stack: record open leaf so install/session save restores it.
      parentNode.lastTabFocus = focusNodeWindow.nodeValue;
      focusNodeWindow.nodeValue.raise();
    }
  }
