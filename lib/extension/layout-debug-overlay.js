 ...
 * @param {{ parentLayout?: string|null, percent?: number|null, monWsId?: string|null, minW?: number|null, minH?: number|null }} opts
 * @returns {string}
 */
export function formatLayoutDebugLabel({
  ...
  const pct = typeof percent === "number" && percent > 0 ? `${Math.round(percent * 100)}%` : "auto";
  const mon = monWsId || "?";
...
export function layoutDebugInfoFromNode(node, tree) {
  ...
  return { parentLayout, percent, monWsId, minW, minH };
}
