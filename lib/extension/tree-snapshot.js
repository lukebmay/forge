 ...
 *
 * Descriptor schema version 1 (in-memory only; disk is layout later):
...
export function applyMonitorSnapshot(mon, monDesc, ctx) {
  ...
  // Classify extras vs cohort *before* rebuild detaches windows (indices shift).
...
/**
 * Mon-level child that owns `node` (node itself if a direct child).
 * @param {any} node
 * @param {any} mon
 * @returns {any|null}
 */
 ...
 * snapshot mon is empty (monitor-recovery / thrash cross-mon pile).
 * @param {any} forest
 * @param {{
 *   findMonitor: (id: string) => any,
 *   findNode: (win: any) => any,
