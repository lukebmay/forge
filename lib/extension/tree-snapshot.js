 ...
 *
 * Descriptor schema version 1 (in-memory only; disk is T7 / workon later):
 *   Forest:  { version: 1, monitors: MonitorDesc[] }
 *   Monitor: { id: "moNwsW", layout, children: NodeDesc[] }
 ...
 * @param {any} monNode
 */
export function captureMonitor(monNode) {
 ...
 */
export function captureForest(monitorNodes) {
  const monitors = [];
  for (const mon of monitorNodes || []) {
    if (!mon?.childNodes?.length) continue;
    const desc = captureMonitor(mon);
 ...
 * Prefer the snapshot mon when survivors still live there; otherwise majority
 * mon of surviving windows (mon-agnostic regroup — layout-group style).
 *
 * @param {any} monDesc
 * @param {{ findMonitor: (id: string) => any, findNode: (win: any) => any }} ctx
 * @returns {any|null}
 */
export function resolveTargetMonitor(monDesc, ctx) {
  ...
  if (nodes.length === 0) return preferred;
  ...
  if (preferred) {
    ...
    // All (or sole) survivors still under snapshot mon → keep mon key.
    if (onPreferred > 0 && onPreferred >= best) return preferred;
  }
...
export function renormalizeChildPercents(children) {
  ...
  if (!anyUser) {
    if (sum <= 0) {
      // Equalize magic-zero path (computeSizes treats 0 as equal).
      children.forEach((c) => {
        c.percent = 0;
        c.userSized = false;
      });
...
export function rebuildNode(descriptor, ctx) {
  ...
  // Collapse degenerate single-child CONs (closed sibling).
