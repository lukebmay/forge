...
import { monWsIdFromMeta, workspaceFromId } from "./monitor-identity.js";
...
function observeMonitorId(wm, forest, live, meta) {
  const grabbing =
    live?.mode === "GRAB_TILE" ||
    (wm?._draggedNodeWindow && live && wm._draggedNodeWindow === live);
  // Mutter moves the grabbed window; membership is still Forest until the op.
  if (grabbing) {
    const fid = forestIdFromLive(wm, live);
    const node = fid ? forest.nodes[fid] : null;
    const mon = node ? ancestorMonitor(forest, node) : null;
    if (mon) return mon.id;
  }
  const want = monWsIdFromMeta(meta);
  let cur = live;
  while (cur) {
    const isMon =
    ...
    if (isMon && wm.liveById instanceof Map) {
      ...
      for (const [id, n] of wm.liveById) {
        if (n === cur && forest.nodes[id]?.kind === "MONITOR") {
          hit = id;
          break;
        }
      }
      if (!hit) {
        ...
        if (typeof v === "string" && forest.nodes[v]?.kind === "MONITOR") hit = v;
      }
      if (hit && (!want || hit === want || workspaceFromId(hit) < 0)) return hit;
      if (hit && want && hit !== want) break;
    }
    cur = cur.parentNode;
  }

  if (want && forest.nodes[want]?.kind === "MONITOR") return want;
  return null;
}
