...
export function observeReality(wm, opts = {}) {
  ...
  for (const node of forestWindows(forest)) {
    ...
    const fact = { exists };
    if (exists) {
      const floating = observeFloating(bag, live, forest, node);
 ...
 */
function observeFloating(bag, live, forest, node) {
  ...
  if (liveTilesParented(live)) return undefined;
  try {
