...
// After settle: ignore entered-monitor + skip last-good snapshot so Meta peel
// cannot poison homes (lock+DPMS live: thrash pending cleared then pile).
export const POST_REHOME_COOLDOWN_MS = 2500;
...
export class SoftRehomeManager extends GObject.Object {
  ...
  /**
   * Lock screen: hold thrash pending, do not settle until unlock.
  ...
  setLockScreenThrashGuard(locked) {
    ...
    if (locked && !was) {
      // Freeze last-good; Meta may thrash for the whole lock.
  ...
  softRehomeAfterWorkareas() {
    ...
    // Cooldown before render so last-good snapshot inside render skips thrash frames.
    this.beginPostRehomeCooldown();
