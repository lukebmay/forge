...
// After settle: keep thrash pending + skip last-good so late Meta peel cannot
// poison homes. Slides on further workareas-changed (multi-machine hybrid).
export const POST_REHOME_COOLDOWN_MS = 3000;

/** Compact live monitor geometry fingerprint (count + each rect). */
export function monitorGeometryFingerprint() {
  const n = global.display.get_n_monitors();
  const parts = [`n=${n}`];
  for (let i = 0; i < n; i++) {
    try {
      const g = global.display.get_monitor_geometry(i);
      if (g) parts.push(`${i}:${g.x},${g.y},${g.width},${g.height}`);
    } catch (_e) {
      parts.push(`${i}:?`);
    }
  }
...
/** move_to_monitor when mon differs; swallow Meta errors. */
...
export class SoftRehomeManager extends GObject.Object {
  ...
  /** @returns {boolean} */
  isPostRehomeCooldownActive() {
    const wm = this._extWm;
    const until = wm._softRehomeCooldownUntilUs;
    if (!until) return false;
    if (Utils.monoTimeUs() > until) {
      // Timer may still clear thrash pending; wall-clock alone is not enough mid-callback.
      return wm._softRehomeCooldownSrcId !== 0;
    }
    return true;
  }

  /**
   * Keep thrash pending through cooldown (entered-monitor blocked).
   * Sliding: each call extends the window (hybrid thrash after first settle).
   */
  beginPostRehomeCooldown() {
    const wm = this._extWm;
    wm._workareasThrashPending = true;
    wm._softRehomeCooldownUntilUs = Utils.monoTimeUs() + POST_REHOME_COOLDOWN_MS * 1000;
    wm._clearTimeoutId("_softRehomeCooldownSrcId");
    wm._softRehomeCooldownSrcId = GLib.timeout_add(
      GLib.PRIORITY_DEFAULT,
      POST_REHOME_COOLDOWN_MS,
      () => {
        wm._softRehomeCooldownSrcId = 0;
        wm._softRehomeCooldownUntilUs = 0;
        // Do not clear while still locked, settling, or a new settle is armed.
        if (wm._lockScreenThrashGuard || wm._workareasSettleSrcId) {
          return false;
        }
        wm._workareasThrashPending = false;
        Logger.debug("soft-rehome: post-rehome cooldown end");
        return false;
      }
    );
  }
  ...
  /**
   * Lock screen: freeze last-good, hold thrash pending, do not settle until unlock.
   * Unlock: release guard and queue one settle from pre-lock last-good.
   * @param {boolean} locked
   */
  setLockScreenThrashGuard(locked) {
    const wm = this._extWm;
    const was = !!wm._lockScreenThrashGuard;
    wm._lockScreenThrashGuard = !!locked;
    if (locked && !was) {
      // Snapshot while tree is still quiet, then freeze homes for the whole lock.
      try {
        this.snapshotLastGoodHomes({ force: true });
      } catch (_e) {
        /* best-effort freeze */
      }
      wm._workareasThrashPending = true;
      ...
      wm._softRehomeCooldownSrcId = 0;
      wm._softRehomeCooldownUntilUs = 0;
      Logger.debug("soft-rehome: lock-screen thrash guard on");
    } else if (!locked && was) {
      Logger.debug("soft-rehome: lock-screen thrash guard off → settle");
      // Unlock often coincides with DPMS wake workareas; settle after debounce.
      this.queueSoftRehomeOnWorkareas();
    }
  }
  ...
  queueSoftRehomeOnWorkareas() {
    const wm = this._extWm;
    wm._workareasThrashPending = true;
    // While locked, keep pending but do not settle (avoids clear→entered-monitor pile).
    if (wm._lockScreenThrashGuard) {
      wm._clearTimeoutId("_workareasSettleSrcId");
      wm._workareasSettleSrcId = 0;
      Logger.debug("soft-rehome: deferred while lock-screen");
      return;
    }
    // Late thrash after a settle: slide cooldown so multi-machine hybrid bursts stay covered.
    if (this.isPostRehomeCooldownActive()) {
      this.beginPostRehomeCooldown();
    }
    wm._workareasSettleFingerprint = monitorGeometryFingerprint();
    wm._clearTimeoutId("_workareasSettleSrcId");
    wm._workareasSettleSrcId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, WORKAREAS_SETTLE_MS, () => {
      wm._workareasSettleSrcId = 0;
      try {
        // Fingerprint still moving → re-arm (wall-clock alone is not enough on hybrid).
        const fp = monitorGeometryFingerprint();
        if (fp !== wm._workareasSettleFingerprint) {
          Logger.debug("soft-rehome: mon fingerprint moved, re-arm settle");
          this.queueSoftRehomeOnWorkareas();
          return false;
        }
  ...
  softRehomeAfterWorkareas() {
    const wm = this._extWm;
    if (wm._lockScreenThrashGuard) {
      // Still locked (race); keep pending for unlock settle.
      wm._workareasThrashPending = true;
      return;
    }
    // Transient mon loss (DPMS / KVM): keep pending, do not clear thrash.
    if (global.display.get_n_monitors() === 0) {
      wm._workareasThrashPending = true;
      Logger.debug("soft-rehome: 0 monitors, keep thrash pending");
      return;
    }
    ...
    // Post-restore thrash: reapply shield forest instead of snapshotting broken tree.
    ...
    // Keep thrash pending through move_to_monitor (entered-monitor during moves).
    // beginPostRehomeCooldown() owns clearing after a sliding quiet window.
    if (!structureOk) {
   ...
   * @param {{ force?: boolean }} [opts] force=true freezes homes on lock entry
   */
  snapshotLastGoodHomes(opts = {}) {
    const wm = this._extWm;
    const force = !!opts.force;
    if (!force) {
      if (wm._workareasThrashPending) return;
      if (wm._lockScreenThrashGuard) return;
      if (this.isPostRehomeCooldownActive()) return;
    }
    const nMonitors = global.display.get_n_monitors();
