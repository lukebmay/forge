...
export const EXTENSION_OPS = Object.freeze([
  ...
  "move-in",
  "move-out",
  // R012 / live matrix: synthetic grab-tile drop (zone + optional entered-monitor)
  "dnd-drop",
  "float",
  "order",
  "size",
...
export function validateStep(step, index = 0) {
  ...
  switch (op) {
    ...
    case "dnd-drop": {
      // tile → onto; zone defaults CENTER (tab join). Optional cross-mon rehome probe.
      // R015: onto may be omitted when destMonitor is set (empty-mon drop).
      const tile = step.tile ?? step.selector ?? step.source;
      const onto = step.onto ?? step.target ?? step.dest ?? step.with;
      const destMonRaw = step.destMonitor ?? step.dest_monitor ?? step.monitor ?? step.destMon;
      if (tile == null || String(tile).trim() === "") {
        return { ok: false, error: "dnd-drop requires tile", index };
      }
      const hasOnto = onto != null && String(onto).trim() !== "";
      const hasDestMon =
        destMonRaw != null && String(destMonRaw).trim() !== "" && !Number.isNaN(Number(destMonRaw));
      if (!hasOnto && !hasDestMon) {
        return {
          ok: false,
          error: "dnd-drop requires onto or destMonitor",
          index,
        };
      }
      normalized.tile = String(tile);
      if (hasOnto) normalized.onto = String(onto);
      if (hasDestMon) normalized.destMonitor = Number(destMonRaw);
      const zoneRaw = step.zone ?? step.dropZone ?? "CENTER";
      normalized.zone = String(zoneRaw).trim().toUpperCase() || "CENTER";
      // Default true: fire entered-monitor mid GRAB_TILE (R012 regression path).
      // Empty-mon path also defaults on (probe R012 skip before rehome).
      if (
        step.simulateEnteredMonitor === false ||
        step.simulate_entered_monitor === false ||
        step.simulateEnteredMonitor === 0 ||
        step.simulateEnteredMonitor === "false"
      ) {
        normalized.simulateEnteredMonitor = false;
      } else {
        normalized.simulateEnteredMonitor = true;
      }
      break;
    }
