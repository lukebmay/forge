...
import { MARK2_VIM_KEYS } from "../shared/keybind-presets.js";
...
export class Keybindings extends GObject.Object {
  ...
  _directionalBindings(commandName, keyPrefix) {
    ...
    for (const dir of ["left", "down", "up", "right"]) {
      bindings[`${keyPrefix}-${dir}`] = () => {
        this.extWm.command({ name: `${commandName}.${dir}` });
      };
    }
    return bindings;
  }
  ...
  buildBindingDefinitions() {
    this._bindings = {
      ...
      ...this._directionalBindings("join", "window-swap"),
      ...this._directionalBindings("move", "window-move"),
      ...this._mark2CommandBindings(),
      ...this._simpleCommandBindings({
        "con-split-vertical": { name: "toggleSplit" },
        "con-split-horizontal": { name: "toggleSplit" },
        "con-stacked-layout-toggle": { name: "toggleTabStack" },
        "con-tabbed-layout-toggle": { name: "toggleTabStack" },
        "window-merge-group": { name: "WindowMergeGroup" },
        "window-move-in": { name: "WindowMoveIn" },
        "window-move-out": { name: "WindowMoveOut" },
        "con-tabbed-showtab-decoration-toggle": { name: "ShowTabDecorationToggle" },
        "focus-border-toggle": { name: "FocusBorderToggle" },
        "layout-debug-overlay-toggle": { name: "LayoutDebugOverlayToggle" },
        ...
        "window-zoom-toggle": { name: "ZoomToggle" },
        "window-zoom-horizontal": { name: "ZoomHorizontal" },
        "window-zoom-vertical": { name: "ZoomVertical" },
        ...
        // window-unfocus / WindowUnfocus abandoned (no product keybind)
      }),
      ...
      "prefs-app-launch": () => {
        const command = (this.settings.get_string("launch-app-command") || "").trim();
        if (command) {
          try {
        ...
        try {
          ...
          Logger.error(`Failed to open run dialog: ${e.message}`);
        }
      },
