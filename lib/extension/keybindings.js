  ...
  buildBindingDefinitions() {
    this._bindings = {
      ...
      ...this._simpleCommandBindings({
        ...
        "window-focus-parent": { name: "FocusParent" },
        "window-focus-child": { name: "FocusChild" },
        "window-selection-clear": { name: "ClearSelection" },
        ...
        "split-chrome-show-all-toggle": { name: "SplitChromeShowAllToggle" },
      ...
      "prefs-lock-screen": () => {
        // Product: lock ⇒ panels sleep (not dim-only). Soft-rehome owns wake thrash.
        try {
          GLib.spawn_command_line_async("loginctl lock-session");
        } catch (e) {
          Logger.error(`Failed to lock screen: ${e.message}`);
        }
        // X11: force DPMS off so Super+Delete matches overnight sleep (not dim).
        // Wayland: compositor owns blank; xset is unavailable / wrong display.
        try {
          ...
          Logger.debug(`lock DPMS force off skipped: ${e.message}`);
        }
      },
    };
  }
