...
export class Keybindings extends GObject.Object {
  ...
  buildBindingDefinitions() {
    this._bindings = {
      ...
      // Optional only: default kits leave this unbound. GNOME media-keys screensaver
      // (Super+Delete while Forge enabled) owns lock + blank/DPMS. If a user still
      // binds prefs-lock-screen, call the same session lock without forcing DPMS.
      "prefs-lock-screen": () => {
        try {
