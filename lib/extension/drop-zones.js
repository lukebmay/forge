 ...
 * TOP/BOTTOM take full-width outer strips; LEFT/RIGHT mid-height sides; CENTER = C.
 * Covers every pixel of U (hit test still uses trapezoids — corner paint vs hit may differ).
 *
...
export function zonePaintRects(zones) {
  ...
  return {
    [DROP_ZONES.CENTER]: { x: C.x, y: C.y, width: C.width, height: C.height },
    [DROP_ZONES.TOP]: { x: U.x, y: U.y, width: U.width, height: topH },
    [DROP_ZONES.BOTTOM]: { x: U.x, y: botY, width: U.width, height: botH },
    [DROP_ZONES.LEFT]: { x: U.x, y: C.y, width: leftW, height: C.height },
    [DROP_ZONES.RIGHT]: { x: rightX, y: C.y, width: rightW, height: C.height },
  };
}

/**
 * Paint rect for one zone (partition for edges/center).
 * @param {DropZones|null|undefined} zones
...
export function zonePaintRect(zones, zone) {
  ...
  const rects = zonePaintRects(zones);
