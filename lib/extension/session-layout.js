...
/**
 * Resolve portable leaves. Order: id → pid → class+title → class+frame/mon →
 * unique class. Pid survives Shell HUP; titles (e.g. Ghostty) often do not.
 * @param {Iterable<any>} metaWindows
...
export function createWindowResolver(metaWindows) {
  ...
  for (const w of list) {
    ...
    try {
      ...
      if (pid > 0 && !byPid.has(pid)) byPid.set(pid, w);
    } catch (_e) {
      // ignore
    }
  ...
  /** Best unmatched candidate among same-class windows using frame + mon. */
  const pickByGeometry = (leaf, cands) => {
    ...
    for (const w of cands) {
      ...
      try {
        ...
        if (typeof leaf.monitor === "number" && mon === leaf.monitor) score += 1_000_000;
      } catch (_e) {
        // ignore
      }
  ...
  return (leaf) => {
    ...
    if (leaf.pid > 0) {
      const hit = claim(byPid.get(leaf.pid));
      if (hit) return hit;
    }
