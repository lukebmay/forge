...
/**
 * Shell HUP often assigns new Meta window ids. Match portable leaves by:
 * 1) id, 2) unique (wmClass, title), 3) unique remaining wmClass.
 * Each live window is used at most once.
 *
 * @param {Iterable<any>} metaWindows
...
export function createWindowResolver(metaWindows) {
  ...
  return (leaf) => {
    if (!leaf) return null;
    if (leaf.id !== null && leaf.id !== undefined && byId.has(leaf.id)) {
      const w = byId.get(leaf.id);
      if (w && !used.has(w)) {
        used.add(w);
        return w;
      }
...
/**
 * Count portable leaves and how many resolve via resolver or idMap.
...
export function matchStats(portableForest, idMapOrResolver) {
  ...
  // Resolver consumes matches; for stats only use a fresh resolver when Map given.
  let total = 0;
  let matched = 0;
  if (typeof idMapOrResolver === "function") {
    // Destructive resolve — caller should pass a dedicated stats path
    const walk = (d) => {
      if (!d) return;
  ...
  // Map: non-destructive id-only count (legacy tests)
  const walk = (d) => {
    if (!d) return;
    if (isPortableWindow(d)) {
      total += 1;
      if (idMapOrResolver.has(d.id)) matched += 1;
      return;
    }
    for (const c of d.children || []) walk(c);
  };
...
/**
 * Non-destructive match count using a fresh class/title resolver.
 * @param {{ monitors?: any[] }|null} portableForest
 * @param {Iterable<any>} metaWindows
 */
...
/**
 * Topology richness score — higher means more multi-mon / tab structure.
 * Used so a thrash-flat tree does not overwrite a good last-good file.
 * @param {{ monitors?: any[] }|null} forest
 */
...
export function isSessionLayoutFresh(
  ...
  // CLI GetTree fallback may stamp Python monotonic vs GLib — if mono
  // regresses but wall age is still within max, accept (same session).
...
/**
 * Restore forest using strict mon targets (preferred id/stableKey only).
 * Caller must rehome Meta + reparent nodes under those mons first so cohorts
 * are non-empty under the snapshot mon (not the thrash pile).
 *
 * @param {any} forest
 * @param {any} ctx - tree snapshot ctx + optional findMonitorByStableKey
 * @param {(monDesc: any, ctx: any) => any} resolveMon
 * @param {(mon: any, monDesc: any, ctx: any) => boolean} applyMon
 * @param {(root: any) => void} pruneEmpty
 */
export function restoreForestStrict(forest, ctx, resolveMon, applyMon, pruneEmpty) {
  if (!forest?.monitors?.length || !ctx || !resolveMon || !applyMon) return;
  for (const monDesc of forest.monitors) {
    const mon = resolveMon(monDesc, ctx);
    if (!mon) continue;
    applyMon(mon, monDesc, ctx);
    if (pruneEmpty) pruneEmpty(mon);
  }
}
