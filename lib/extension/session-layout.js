 ...
 *
 * Session identity adapter: T6/epoch forest ↔ JSON-safe portable leaves.
 * Portable `id` = Forest nanoid; Meta = `metaWindowId` match aid
 * (class/title/pid/frame cover HUP churn).
 */
...
/**
 * Fill empty/dead TABBED/STACKED open leaf from keyboard focus before snapshot.
 * Never stomps a live open leaf (D018: LTF ≠ keyboard). focusWindowId stays separate.
 *
 * @param {{ findNode?: (meta: any) => any }|null} tree
 * @param {any} focusMeta
 * @returns {boolean} true if a group lastTabFocus was written
 */
export function syncLastTabFocusFromFocus(tree, focusMeta) {
  if (!tree || !focusMeta) return false;
  let node = null;
  try {
    node = typeof tree.findNode === "function" ? tree.findNode(focusMeta) : null;
  } catch (_e) {
    return false;
  }
  ...
  while (p) {
    const isGroup =
      (typeof p.isStackedOrTabbed === "function" && p.isStackedOrTabbed()) ||
      p.layout === "TABBED" ||
      p.layout === "STACKED";
    if (isGroup) {
      const current = p.lastTabFocus;
      if (current != null && current !== focusMeta) {
        const kids = p.childNodes || [];
        const stillInGroup = kids.some((c) => c && c.nodeValue === current);
        if (stillInGroup) return false;
      }
      if (current === focusMeta) return false;
      p.lastTabFocus = focusMeta;
      return true;
    }
    p = p.parentNode;
  }
...
/**
 * Resolve Meta.Window for session focus save: Mutter focus, then LFT fallback.
 * Prefer the same source for focusWindowId and lastTabFocus sync.
 *
 * @param {any} wm
 * @returns {any|null}
 */
export function resolveFocusMetaForSessionSave(wm) {
  if (!wm) return null;
  let focus = null;
  try {
    focus = wm.focusMetaWindow ?? null;
  } catch (_e) {
    focus = null;
  }
  if (!focus) {
    try {
      const display = (typeof global !== "undefined" && global.display) || wm.display || null;
      if (display) {
        focus =
          (typeof display.get_focus_window === "function" ? display.get_focus_window() : null) ??
  ...
  if (!focus) {
    // DBus flush sometimes races with focus; fall back to LFT tile.
    try {
      const lftNode = wm.lftMru?.globalHead?.() ?? wm.lastFocusedWindow ?? null;
      focus = lftNode?.nodeValue ?? lftNode ?? null;
    } catch (_e) {
      focus = null;
    }
  }
...
/**
 * Epoch/T6 forest → JSON-safe portable (`id` = nanoid; Meta = match aid).
 * Identity extras come from Meta when `.window` is present.
...
export function geometryMatchScore(leaf, metaWindow) {
  ...
  // Title/class break ties when tabs/stacks share one frame (geometry identical).
  try {
    const { wmClass, title } = windowClassTitle(metaWindow);
    if (leaf.wmClass && wmClass && leaf.wmClass === wmClass) score += 1_000;
    if (leaf.title && title && leaf.title === title) score += 1_000_000;
  } catch (_e) {
    // ignore
  }
  return score;
...
/** Portable leaf `id` = nanoid (Meta type preserved when equal). */
function portableLeafId(descriptor) {
  if (!descriptor) return null;
  const fromDoc = descriptor.windowId;
  const fromMeta = windowStableId(descriptor.window);
  if (fromDoc != null && fromDoc !== "") {
    if (fromMeta != null && String(fromMeta) === String(fromDoc)) return fromMeta;
    return fromDoc;
  }
  if (descriptor.id != null && descriptor.id !== "") {
    if (fromMeta != null && String(fromMeta) === String(descriptor.id)) return fromMeta;
...
function portableMetaWindowId(descriptor) {
  if (!descriptor) return null;
  if (descriptor.metaWindowId != null && descriptor.metaWindowId !== "") {
    return descriptor.metaWindowId;
  }
  const fromMeta = windowStableId(descriptor.window);
  const leafId = portableLeafId(descriptor);
  if (fromMeta != null && leafId != null && String(fromMeta) !== String(leafId)) {
    return fromMeta;
  }
  return null;
}

function portableLastTabFocusId(descriptor) {
  if (!descriptor) return undefined;
  const fromDoc = descriptor.lastTabFocusId;
  const fromMeta =
    descriptor.lastTabFocus !== undefined ? windowStableId(descriptor.lastTabFocus) : null;
  if (fromDoc != null && fromDoc !== "") {
    if (fromMeta != null && String(fromMeta) === String(fromDoc)) return fromMeta;
    return fromDoc;
  }
...
function portableNode(descriptor) {
  if (!descriptor) return null;
  if (isWindowDescriptor(descriptor) || isPortableWindow(descriptor)) {
    const id = portableLeafId(descriptor);
    ...
    const metaWid = portableMetaWindowId(descriptor);
    if (metaWid != null && String(metaWid) !== String(id)) out.metaWindowId = metaWid;
    ...
    if (w) {
      ...
      if (descriptor.wmClass !== undefined) out.wmClass = descriptor.wmClass;
      if (descriptor.title !== undefined) out.title = descriptor.title;
      if (descriptor.pid > 0) out.pid = descriptor.pid;
      ...
      if (descriptor.metaWindowId != null && out.metaWindowId == null) {
        out.metaWindowId = descriptor.metaWindowId;
      }
    }
    if (isDingDesktopIconsSurface({ wmClass: out.wmClass, title: out.title })) return null;
    return out;
  }
  ...
  const ltfId = portableLastTabFocusId(descriptor);
  if (ltfId !== undefined) out.lastTabFocusId = ltfId;
  return out;
}
...
/**
 * Resolve portable leaves. Order: metaWindowId/id → unique class+title → pid
 * cohort (global geo assign) → class cohort (global geo assign) → unique class.
 * Ghostty: one pid, many windows, titles churn — geometry ranking is required.
 * Chrome tabs: same pid + identical frames — title match first preserves order.
 * Focus/LTF synthetics match portable `id` (nanoid) or `metaWindowId`.
 *
...
export function createWindowResolver(metaWindows, portableForest = null) {
  ...
  if (portableForest) {
    ...
    // 1) Exact Meta id via metaWindowId (nanoid disks) or legacy leaf.id
    for (const leaf of allLeaves) {
      if (leafAssign.has(leaf)) continue;
      const metaKey =
        leaf.metaWindowId != null && leaf.metaWindowId !== "" ? leaf.metaWindowId : leaf.id;
      if (metaKey === null || metaKey === undefined) continue;
      const hit =
        claim(byId.get(metaKey)) ||
        claim(byId.get(String(metaKey))) ||
        (Number.isFinite(Number(metaKey)) ? claim(byId.get(Number(metaKey))) : null);
      if (hit) leafAssign.set(leaf, hit);
    }

    // 2) Exact class+title before geometry — tab/stack siblings share one frame;
    //    pid-geometry alone would scramble sibling order on ties (Chrome tabs).
    for (const leaf of allLeaves) {
      if (leafAssign.has(leaf)) continue;
    ...
    // 3) Same-pid multi-window: global geometry (+ title boost) assignment
    /** @type {Map<number, any[]>} */
  ...
  /** Match live Meta id (handles number/string JSON drift). */
  const liveById = (id) => {
    if (id === null || id === undefined) return null;
    ...
    if (byId.has(asStr)) return byId.get(asStr);
    return null;
  };
  ...
  /**
   * Synthetic `{ id }` (focusWindowId / lastTabFocusId) is not the portable leaf
   * object. Match nanoid `id` or Meta `metaWindowId` after churn.
   */
  const fromLeafAssignBySavedId = (savedId) => {
    if (savedId === null || savedId === undefined || savedId === "") return null;
    for (const [pLeaf, w] of leafAssign) {
      if (!pLeaf) continue;
      if (idsEqual(pLeaf.id, savedId) || idsEqual(pLeaf.metaWindowId, savedId)) return w;
    }
    return null;
  };
  ...
  const resolveOne = (leaf) => {
    ...
    const hitBySavedId = (savedId) => {
      if (savedId === null || savedId === undefined || savedId === "") return null;
      const byExact = liveById(savedId);
      if (byExact && !used.has(byExact)) {
        used.add(byExact);
        return byExact;
      }
      // lastTabFocus / focusWindowId: already claimed window, still resolve by id
      if (byExact && used.has(byExact)) return byExact;
      return fromLeafAssignBySavedId(savedId);
    };

    const byPortable = hitBySavedId(leaf.id);
    if (byPortable) return byPortable;
    const byMeta = hitBySavedId(leaf.metaWindowId);
...
export function forestRichness(forest) {
  ...
  const walk = (d) => {
    if (!d) return;
    if (isPortableWindow(d) || isWindowDescriptor(d) || (d.window && !d.children)) {
      wins += 1;
      return;
    }
...
/**
 * Portable forest → epoch-shaped live forest (`kind`/`windowId` + resolved Meta).
...
function liveNode(descriptor, resolve) {
  ...
  if (isPortableWindow(descriptor)) {
    ...
    const fromId = descriptor.id;
    const fromMeta = windowStableId(window);
    const windowId =
      fromId != null && fromId !== ""
        ? String(fromId)
    ...
    if (windowId != null) out.windowId = windowId;
    // Keep saved frame/monitor for post-restore last-good seeding (monitor-recovery).
    if (descriptor.frame) out.frame = descriptor.frame;
    if (typeof descriptor.monitor === "number" && descriptor.monitor >= 0) {
      out.monitor = descriptor.monitor;
    }
  ...
  if (descriptor.lastTabFocusId !== undefined && descriptor.lastTabFocusId !== null) {
    out.lastTabFocusId = String(descriptor.lastTabFocusId);
 ...
 * @param {{ focusWindowId?: string|number|null }} [opts]
 */
export function makeEnvelope(portableForest, savedMonotonicUs, savedAtMs = Date.now(), opts = {}) {
  const out = {
  ...
  const focusId = opts?.focusWindowId;
...
/**
 * @param {any} raw
 * @returns {{ kind: string, sessionVersion: number, savedMonotonicUs: number, forest: any, focusWindowId?: any }|null}
 */
...
/**
 * Plan last-good home seeds after session restore so monitor-recovery uses saved
