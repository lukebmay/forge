 ...
 * Portable `id` is epoch windowId; class/title/pid/frame cover HUP churn.
 */
...
/**
 * Epoch/T6 forest → JSON-safe portable (`id` = windowId).
...
/** Same identity as epoch windowId; keep Meta get_id() type when equal. */
function portableLeafId(descriptor) {
  ...
  if (descriptor.id != null && descriptor.id !== "") return descriptor.id;
  return null;
}
...
/**
 * Resolve portable leaves. Order: id → unique class+title → pid cohort
 * (global geo assign) → class cohort (global geo assign) → unique class.
...
export function createWindowResolver(metaWindows, portableForest = null) {
  ...
  /**
   * Synthetic leaves (`{ id }` for focusWindowId / lastTabFocusId) are not the
   * same object as portable forest leaves. After HUP, Meta ids churn — look up
   * the pre-assigned portable leaf with the same saved id.
   */
  const fromLeafAssignBySavedId = (savedId) => {
    if (savedId === null || savedId === undefined) return null;
    for (const [pLeaf, w] of leafAssign) {
      if (!pLeaf || pLeaf.id === null || pLeaf.id === undefined) continue;
      if (pLeaf.id === savedId || String(pLeaf.id) === String(savedId)) return w;
    }
    return null;
  };
  ...
  const resolveOne = (leaf) => {
    ...
    if (leaf.id !== null && leaf.id !== undefined) {
      const byExact = liveById(leaf.id);
      ...
      // Id churn: portable leaf was matched via pid/class/geometry under old id.
      const assigned = fromLeafAssignBySavedId(leaf.id);
