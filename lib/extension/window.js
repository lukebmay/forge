...
export class WindowManager extends GObject.Object {
   ...
   * drop targets so cross-monitor DnD can resolve hints. Dragged / GRAB_TILE
   * windows are excluded. Call again from _handleMoving so targets stay live.
  ...
  trackCurrentMonWs(forWindow = null) {
    ...
    // Active workspace only — both monitors. Do not gate on get_current_monitor()
    // matching the dragged window (that emptied targets mid cross-mon drag).
    const monWindows = currentWsNode
      .getNodeByType(NODE_TYPES.WINDOW)
      .filter((w) => {
        if (!w?.nodeValue || w.nodeValue === metaWindow) return false;
        if (w.isGrabTile?.() || w.mode === WINDOW_MODES.GRAB_TILE) return false;
        if (!w.isTile?.() && w.mode !== WINDOW_MODES.TILE) return false;
        try {
          if (w.nodeValue.minimized) return false;
        } catch (_e) {
          return false;
        }
        return Utils.isWindowAlive(w.nodeValue);
      })
      .map((w) => w.nodeValue);
   ...
   *
   * Apply chrome stays visible until clearLayoutApplyChrome() — residual place
   * runs after batch end (CLI ends chrome via LayoutBatch chrome-clear).
   * @param {string} [reason]
   */
  endOpenLayoutBatch(reason) {
    ...
    // Depth 0: do not clear apply chrome here (residual place still running).
  ...
  /**
   * Clear layout-apply chrome after residual place (or error/finally).
   * Hard timeout and disable() also clear — this is the normal end of apply.
