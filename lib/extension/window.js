...
export class WindowManager extends GObject.Object {
  ...
  _removeSignals() {
    ...
    for (const metaWindow of allWindows) {
      ...
      if (windowActor) {
        ...
        windowActor.splitBorders = undefined;
        windowActor.splitBorder = undefined;
      }
    }

    this.decorationManager?.destroySelectionBorder?.();
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        if (!windowActor.border) {
          ...
          // Pointer-transparent: borders must not steal chrome clicks (Wayland).
  ...
  _retryPlaceHintAfterIdentity(metaWindow) {
    ...
    if (plan.homeMonitor >= 0) {
      ...
      const curMon = this._monitorIndexOfNode(node);
      if (curMon !== plan.homeMonitor) {
        const ws = metaWindow.get_workspace?.()?.index?.() ?? 0;
        const destId = Utils.createMonitorWorkspaceId(plan.homeMonitor, ws);
        ...
        if (dest) {
          this._rehomeWindowPreservingContainer(node, metaWindow, dest);
        }
      }
    }
    return true;
    ...
    if (!metaWindow.windowSignals) {
      let windowSignals = [
        ...
        metaWindow.connect("notify::wm-class", () => {
          ...
          // W2: re-try PlaceNext once class is known (null class skipped match).
        ...
        metaWindow.connect("notify::title", () => {
          ...
          // those until the real title lands. Mirror wm-class re-tile path.
          this.tree.findNode(metaWindow)?.refreshApp();
          this._retryPlaceHintAfterIdentity(metaWindow);
