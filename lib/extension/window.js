...
export class WindowManager extends GObject.Object {
      ...
      if (!this._wmSources.has("renderTree")) {
        ...
        this._wmSources.setIdle("renderTree", () => {
          ...
          try {
            ...
            // Tab/stack peers share one content rect — size all of them on commit
            // (focus path must stay raise-only; R025 on reveal is a safety net).
            try {
              this.reassertAllTabStackSlots({ force: false });
            } catch (_e) {
              /* best-effort */
            }
          ...
          if (renderOk) {
            ...
            this._schedulePostRenderTabSlotHeal();
  ...
  /**
   * Post-commit: size every TABBED/STACKED TILE peer to the shared group slot.
   * Not for focus — tab click uses revealGroupChild (raise + optional R025).
  ...
  /**
   * Coalesced heal after render echo — Chrome/Wayland often snaps late.
   * Also reasserts zoomed TILE paint (D030) after client snap.
   */
  _schedulePostRenderTabSlotHeal() {
    ...
    this._wmSources.set("postRenderTabSlots", delay, () => {
      try {
        if (this.isApplyEpochLive?.()) return;
        if (this._freezeRender) return;
        if (this._draggedNodeWindow) return;
        const n = this.reassertAllTabStackSlots({ force: false });
        if (n > 0) Logger.trace(`post-render-tab-slots groups=${n}`);
        const z = this._reassertZoomedTiles();
        if (z > 0) Logger.trace(`post-render-zoom-paint n=${z}`);
      } catch (_e) {
        /* best-effort */
      }
    });
  ...
  /**
   * Reassert TILE windows whose paint target is zoom (Chrome/PWA late snap).
   * @returns {number}
   */
  _reassertZoomedTiles() {
    if (!this.tree || typeof this.tree.getNodeByType !== "function") return 0;
    let nodes = [];
    try {
    ...
    for (const node of nodes) {
      if (!node || !isZoomMode(node.zoomMode)) continue;
      if (String(node.mode) !== "TILE" && node.mode !== WINDOW_MODES.TILE) continue;
      const meta = node.nodeValue;
      if (!meta) continue;
      if (this._tiledWindowAtTreeSlot(node, meta)) continue;
      if (this.reassertNodeToSlot(node, { force: true })) n += 1;
    }
    return n;
  }
