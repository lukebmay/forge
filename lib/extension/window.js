...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    ...
    // Wayland class mins queue; durable floor load/save on enable (not construct).
    this._minSizeProbeQueue = new Set();
    this._minSizeProbeActive = new Set();
  ...
  disable() {
    ...
    // Teardown: drop probe state only (no restore move_resize into dying Shell).
    this._minSizeProbeQueue?.clear?.();
    this._minSizeProbeActive?.clear?.();
  ...
  move(metaWindow, rect, workArea = null, { skipOffscreenClamp = false, force = false } = {}) {
    ...
    if (!Utils.isWindowAlive(metaWindow)) return;
    // Min-size probe owns geometry until it restores.
    if (metaWindow._forgeMinProbing) return;
  ...
  /**
   * Queue a min-size probe for idle (never during MOVING grab).
   * Skips windows that already have mins or already failed a shrink probe.
   * @param {any} metaWindow
   * @param {{ delayMs?: number }} [opts]
   */
  _queueMinSizeProbe(metaWindow, opts = {}) {
    if (!metaWindow) return;
    if (metaWindow._forgeMinProbeGaveUp) return;
    try {
      const known = readWindowMinSize(metaWindow);
      if (known.width > 0 || known.height > 0) return;
    } catch (_e) {
      /* still queue */
    }
    if (!this._minSizeProbeQueue) this._minSizeProbeQueue = new Set();
    this._minSizeProbeQueue.add(metaWindow);
    if (!this._wmSources) return;
    const delay = Math.max(80, Number(opts.delayMs) || 80);
    this._wmSources.set("minSizeProbeFlush", delay, () => this._flushMinSizeProbeQueue());
  }

  /** Run queued ensureWindowMinSizeKnown when not in a MOVING grab. */
  _flushMinSizeProbeQueue() {
    const q = this._minSizeProbeQueue;
    if (!q?.size) return;
    if (this.grabOp != null) {
      // Still grabbing — try again shortly (do not probe under grab).
      this._wmSources?.set?.("minSizeProbeFlush", 200, () => this._flushMinSizeProbeQueue());
      return;
    }
    const list = [...q];
    q.clear();
    for (const mw of list) {
      try {
        this.ensureWindowMinSizeKnown(mw);
      } catch (_e) {
        /* ignore */
      }
    }
  ...
  /**
   * Abort queued + in-flight shrink probes. Call on MOVING grab-begin so a
   * pending restore cannot fight the gesture or post-drop tile.
   */
  _cancelMinSizeProbes() {
    this._minSizeProbeQueue?.clear?.();
    try {
      this._wmSources?.cancel?.("minSizeProbeFlush");
    } catch (_e) {
      /* ignore */
    }
    const active = this._minSizeProbeActive;
    if (active?.size) {
      for (const mw of [...active]) {
        try {
          this._abortMinSizeProbe(mw);
        } catch (_e) {
          /* ignore */
        }
      }
    ...
    // Cancel stray probe timers even if active set was missed.
    try {
      const slots = this._wmSources?.snapshot?.()?.slots || [];
      for (const s of slots) {
        const name = s?.name;
        if (
          typeof name === "string" &&
          (name.startsWith("minProbe:") || name.startsWith("minProbeClear:"))
        ) {
          this._wmSources.cancel(name);
        }
      }
    } catch (_e) {
      /* ignore */
  ...
  /**
   * Restore + clear flags for one in-flight probe.
   * @param {any} metaWindow
   */
  _abortMinSizeProbe(metaWindow) {
    if (!metaWindow) return;
    let id = "unknown";
    try {
      id = typeof metaWindow.get_id === "function" ? metaWindow.get_id() : String(id);
    ...
    try {
      this._wmSources?.cancel?.(`minProbe:${id}`);
      this._wmSources?.cancel?.(`minProbeClear:${id}`);
    } catch (_e) {
      /* ignore */
    }
    const restore = metaWindow._forgeMinProbeRestore;
    if (restore && metaWindow._forgeMinProbing) {
      try {
        metaWindow._forgeLastResizeRequest = {
          width: restore.width,
          height: restore.height,
          at: Date.now(),
          priorW: restore.width,
          priorH: restore.height,
        };
        this._suppressGeom?.run?.(() => {
          metaWindow.move_resize_frame(true, restore.x, restore.y, restore.width, restore.height);
        });
      } catch (_e) {
        /* ignore */
      }
    }
    metaWindow._forgeMinProbing = false;
    metaWindow._forgeMinProbePending = false;
    delete metaWindow._forgeMinProbeRestore;
    this._minSizeProbeActive?.delete?.(metaWindow);
  }

  /**
   * When Mutter exposes no hints (GNOME 46 Wayland), probe by requesting 1×1
   * and learning the clamped frame. Restores the prior frame afterward.
   * @param {any} metaWindow
   */
  ensureWindowMinSizeKnown(metaWindow) {
    if (!metaWindow || !Utils.isWindowAlive(metaWindow)) return;
    // Never fight a live MOVING / GRAB_TILE gesture.
    if (this.grabOp != null) {
      this._queueMinSizeProbe(metaWindow, { delayMs: 250 });
      return;
    }
    const node = this.findNodeWindow?.(metaWindow);
    if (node?.mode === WINDOW_MODES.GRAB_TILE) {
      this._queueMinSizeProbe(metaWindow, { delayMs: 250 });
      return;
    }
    if (metaWindow._forgeMinProbeGaveUp) return;
    const known = readWindowMinSize(metaWindow);
    if (known.width > 0 || known.height > 0) return;
    if (metaWindow._forgeMinProbePending || metaWindow._forgeMinProbing) return;
    if (!this._wmSources) return;
    ...
    const restore = { x: fr.x, y: fr.y, width: fr.width, height: fr.height };
    metaWindow._forgeMinProbeRestore = restore;
    metaWindow._forgeMinProbePending = true;
    metaWindow._forgeMinProbing = true;
    if (!this._minSizeProbeActive) this._minSizeProbeActive = new Set();
    this._minSizeProbeActive.add(metaWindow);
    // 32px — some toolkits no-op a 1×1 request; still well under real mins.
    const probeW = 32;
    const probeH = 32;
    metaWindow._forgeLastResizeRequest = {
      width: probeW,
    ...
    try {
      this._suppressGeom.run(() => {
        if (typeof metaWindow.unmaximize === "function") {
          try {
        ...
        metaWindow.move_resize_frame(true, fr.x, fr.y, probeW, probeH);
      });
    } catch (_e) {
      metaWindow._forgeMinProbePending = false;
      metaWindow._forgeMinProbing = false;
    ...
    const slot = `minProbe:${id}`;
    // Wayland clamp is often async; wait long enough that a real min can land.
    this._wmSources.set(slot, MIN_CLAMP_LEARN_DELAY_MS + 150, () => {
      let shrank = false;
      try {
        if (!Utils.isWindowAlive(metaWindow)) return;
        const req = metaWindow._forgeLastResizeRequest;
        const now = metaWindow.get_frame_rect?.();
        const nw = Number(now?.width) || 0;
        const nh = Number(now?.height) || 0;
        // Only learn when the frame actually shrank — otherwise Mutter ignored us.
        shrank = (nw > 0 && nw < restore.width - 4) || (nh > 0 && nh < restore.height - 4);
        if (req && now && shrank) noteWindowMinFromClamp(metaWindow, req, now);
        else if (!shrank) {
          // Chrome/Wayland often ignore tiny resize — do not retry every drag.
          metaWindow._forgeMinProbeGaveUp = true;
        }
        // Point last request at restore so post-restore size-changed cannot
        // re-learn the full frame from the probe's tiny request.
        metaWindow._forgeLastResizeRequest = {
          width: restore.width,
          height: restore.height,
          at: Date.now(),
          priorW: nw || restore.width,
          priorH: nh || restore.height,
        };
        this._suppressGeom.run(() => {
          metaWindow.move_resize_frame(true, restore.x, restore.y, restore.width, restore.height);
        });
      ...
      // Keep probing until restore size-changed has been attributed.
      this._wmSources.set(`minProbeClear:${id}`, 80, () => {
        metaWindow._forgeMinProbing = false;
        metaWindow._forgeMinProbePending = false;
        delete metaWindow._forgeMinProbeRestore;
        this._minSizeProbeActive?.delete?.(metaWindow);
  ...
  processFloats() {
    this.allNodeWindows.forEach((nodeWindow) => {
      ...
      if (
        ...
        nodeWindow.float = true;
        this.lftMru?.remove(nodeWindow);
        if (!this._unknownOpenIdentity(metaWindow)) {
        ...
        if (wasFloat) this._adoptOpenIntoTileSlot(nodeWindow);
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        if (placePinned && openPlan.attachLft) {
          const pin =
            openPlan.attachLft.nodeValue != null
              ? this.tree.findNode(openPlan.attachLft.nodeValue) || openPlan.attachLft
              : openPlan.attachLft;
          if (pin) {
        ...
        if (!attachTarget && openMinTabTarget) {
          const tabLive =
            openMinTabTarget.nodeValue != null
              ? this.tree.findNode(openMinTabTarget.nodeValue) || openMinTabTarget
              : openMinTabTarget;
        ...
        if (willTile && !deferHidden && !openMinFloat) {
          ...
          this._queueMinSizeProbe(metaWindow);
        }
    ...
    this.queueEvent(
      {
        ...
        callback: () => {
          ...
          // Learn class floor after tile settle so next free-open can open-min.
          this._queueMinSizeProbe(metaWindow);
        },
      },
      0
  ...
  _tryPlanFromPlaceHint(metaWindow) {
    ...
    if (hint) {
      ...
      Logger.info(
        `place-hint match ${formatPlaceHint(hint)} class=${metaWmClass(metaWindow)} ` +
          `title=${JSON.stringify(metaTitle(metaWindow))} queueLeft=${
  ...
  _tryAdoptLatePlaceHint(metaWindow) {
      ...
      if (!placeHintIdentityReady(metaWindow, provisional)) {
        Logger.debug(
          `place-hint late wait identity ${formatPlaceHint(provisional)} ` +
            `class=${cls} title=${JSON.stringify(title)}`
        );
        return false;
      }
      try {
    ...
    if (idx < 0) {
      Logger.debug(
        `place-hint late no match class=${cls} title=${JSON.stringify(title)} ` +
    ...
    Logger.info(
      `place-hint late adopt ${formatPlaceHint(hint)} class=${cls} title=${JSON.stringify(title)}`
    );
  ...
  _applyPlacePlanToExistingWindow(metaWindow, plan) {
    ...
    try {
      ...
      if (plan.attachLft) {
        const pin =
          plan.attachLft.nodeValue != null
            ? this.tree.findNode(plan.attachLft.nodeValue) || plan.attachLft
            : plan.attachLft;
        if (pin) {
          if (pin.isWindow?.() || pin.nodeType === NODE_TYPES.WINDOW) {
      ...
      try {
        ...
        // Idle Meta move + paint — never move_to_monitor / move_resize from notify.
        this._wmSources?.set?.(`latePlaceHintApply:${winId}`, 50, () => {
          if (this._suppressRehome?.active) return;
          try {
            if (
   ...
   * PlaceNext pins skip this (caller). Fail-open when mins unknown.
   * @param {Meta.Window} metaWindow
  ...
  _decideOpenMinPlacement(metaWindow, lftNode) {
    ...
    const live = lftNode.nodeValue ? this.tree.findNode(lftNode.nodeValue) || lftNode : lftNode;
    if (!live) return null;
    const insertUnit = this._resolveInsertUnit(live) || live;
    const tabOnly = isTabOrStackParent(live.parentNode, LAYOUT_TYPES);
    const startUnit = tabOnly
    ...
    const orientation =
      this._orientationFromUnit(insertUnit) === ORIENTATION_TYPES.VERTICAL
        ? "vertical"
        : "horizontal";
    return resolveOpenMinPlacement({
      lftUnit: startUnit,
      newMins: readWindowMinSize(metaWindow),
      orientation,
      mode: tabOnly ? "tab-only" : "split-or-tab",
  ...
  _ensureTabbedForOpen(unit) {
    if (!unit) return null;
    const live = unit.nodeValue ? this.tree.findNode(unit.nodeValue) || unit : unit;
    if (!live) return null;
  ...
  _adoptOpenIntoTileSlot(nodeWindow) {
    ...
    const liveUnit =
      unit && unit.parentNode && unit !== nodeWindow
        ? unit.nodeValue
          ? this.tree.findNode(unit.nodeValue) || unit
          : unit
        : null;
    ...
    if (liveUnit && !alreadySlotted) {
      ...
      if (leftoverSlot) {
        ...
        if (nodeWindow.parentNode !== leftoverSlot) leftoverSlot.appendChild(nodeWindow);
      } else {
        this.slotSplitForInsert(liveUnit);
        const dest = liveUnit.parentNode;
        if (dest && nodeWindow.parentNode !== dest) dest.appendChild(nodeWindow);
      }
    }
    ...
    if (!metaWindow.windowSignals) {
      let windowSignals = [
        ...
        metaWindow.connect("notify::wm-class", () => {
          ...
          if (
            (metaWindow._forgeProvisionalPlaceHint || this._pendingPlaceHints?.length) &&
            this._tryAdoptLatePlaceHint(metaWindow)
          ) {
            return;
          }
  ...
  updateMetaPositionSize(_metaWindow, from) {
    // Learn mins even on forge-caused size signals (Wayland has no get_size_hints).
    if (_metaWindow && from && /size/i.test(from) && !_metaWindow._forgeMinProbing) {
      try {
        const req = _metaWindow._forgeLastResizeRequest;
        const fr = _metaWindow.get_frame_rect?.();
        if (req && fr) noteWindowMinFromClamp(_metaWindow, req, fr);
      } catch (_e) {
        // ignore
      }
    }
    ...
    if (isForgeCausedGeometrySignal(this, _metaWindow) || _metaWindow?._forgeMinProbing) {
      this.updateBorderLayout();
      return;
    }
