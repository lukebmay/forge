...
/** Session restore diagnostics (file only in debug builds; Logger.info when enabled). */
function sessionLayoutTrace(msg) {
  try {
    Logger.info(msg);
  } catch (_e) {
    // ignore
  }
  // make debug flips production=false; keep install/HUP traces without journal.
...
export class WindowManager extends GObject.Object {
  ...
  _onWindowEnteredMonitor(_display, monitor, metaWindow) {
    // Thrash / session restore / shield: restore forest owns placement.
    if (
  ...
  _sessionLayoutShieldActive() {
    const shield = this._sessionLayoutShield;
  ...
  _reapplySessionLayoutShield(from = "session-layout-shield") {
    const forest = this._sessionLayoutShield?.liveForest;
    if (!forest?.monitors?.length) return false;
    sessionLayoutTrace(`session-layout: shield reapply begin from=${from}`);
    const prev = this._sessionLayoutRestoring;
    this._sessionLayoutRestoring = true;
    try {
      this._rehomeWindowsForSessionForest(forest);
      this._restoreSessionForestStrict(forest);
      this._raiseAfterSessionRestore(forest);
      this._seedLastGoodHomesFromSession(forest, null);
      // Slide shield while thrash is still fighting restore.
      const now =
        typeof GLib.get_monotonic_time === "function"
          ? GLib.get_monotonic_time()
          : Date.now() * 1000;
      this._sessionLayoutShield = {
        liveForest: forest,
        untilMonoUs: now + 3_000_000,
      };
      this.renderTree?.(from);
      sessionLayoutTrace(`session-layout: shield reapply ok from=${from}`);
      this._traceSessionLayoutHomes(forest, "after-shield-reapply");
      return true;
    } catch (e) {
      ...
      this._sessionLayoutRestoring = prev;
    }
  }
  ...
  _traceSessionLayoutHomes(liveForest, tag) {
    try {
      ...
      for (const { window: w, monIndex, monId } of homes) {
        let treeMon = "?";
        try {
          const node = this.tree.findNode(w);
    ...
    this._workareasSettleSrcId = GLib.timeout_add(
      ...
      () => {
        ...
        try {
          ...
          if (
            this._sessionLayoutShieldActive() &&
            this._reapplySessionLayoutShield("soft-rehome-fallback-shield")
          ) {
            return false;
          }
  ...
  _softRehomeAfterWorkareas() {
    ...
    if (this._sessionLayoutRestoring) {
      this._queueSoftRehomeOnWorkareas();
      return;
    }
    ...
    if (this._sessionLayoutShieldActive()) {
      sessionLayoutTrace("session-layout: soft-rehome → shield path");
      if (this._reapplySessionLayoutShield("workareas-session-shield")) {
        this._workareasThrashPending = false;
        return;
      }
    ...
    if (!structureOk) {
      ...
      if (
        this._sessionLayoutShieldActive() &&
        this._reapplySessionLayoutShield("soft-rehome-shield")
      ) {
        this._workareasThrashPending = false;
        return;
      }
  ...
  _saveSessionLayoutForReload(opts = {}) {
    try {
      ...
      // Skip thrash-flat overwrite of a richer last-good.
      if (!opts.force) {
        try {
  ...
  _restoreSessionLayoutAfterTrack() {
    ...
    try {
      ...
      this._traceSessionLayoutHomes(liveForest, "matched-before-rehome");
      this._rehomeWindowsForSessionForest(liveForest);
      this._traceSessionLayoutHomes(liveForest, "after-rehome");
      this._restoreSessionForestStrict(liveForest);
      this._traceSessionLayoutHomes(liveForest, "after-strict-apply");
      this._raiseAfterSessionRestore(liveForest);
      // Seed before any pending soft-rehome settle uses thrash frames.
      this._seedLastGoodHomesFromSession(liveForest, envelope.forest);
      sessionLayoutTrace("session-layout: last-good seeded");
      // Hold restored forest ~3s so post-HUP thrash cannot soft-rehome a broken snapshot.
      const nowMono =
        typeof GLib.get_monotonic_time === "function"
          ? GLib.get_monotonic_time()
          : Date.now() * 1000;
  ...
  _seedLastGoodHomesFromSession(liveForest, portableForest = null) {
    ...
    for (const { window: metaWin, monitorIndex, frame, stableKey } of plans) {
      ...
      const key = stableKey ?? this._monitorLiveMap?.byIndex?.get(monitorIndex) ?? null;
      this._lastGoodHomes.set(metaWin, {
        monitorIndex,
        stableKey: key,
        frame: frame || null,
      });
  ...
  _rehomeWindowsForSessionForest(liveForest) {
    ...
    // One retry if Meta mon or tree parent still wrong (no timers).
  ...
  _raiseAfterSessionRestore(liveForest) {
    const raiseWin = (meta) => {
      try {
        meta?.raise?.();
      } catch (_e) {
    ...
    try {
      ...
      if (focus) {
        raiseWin(focus);
        const node = this.tree.findNode(focus);
