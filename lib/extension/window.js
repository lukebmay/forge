...
export class WindowManager extends GObject.Object {
  ...
  /**
   * CL4: quiet met or max-wait — unmaximize + one layout commit.
   *
   * Prefer requestLayout so open shares the controller debounce + post-render
   * verify. If render is frozen (mid-grab elsewhere), force renderTree once;
   * that path still calls onRenderComplete → requestVerify("post-render").
   *
   * @param {Meta.Window} metaWindow
   */
