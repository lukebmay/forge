...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    ...
    // CL3: in-memory app thrash catalog (Ghostty built-in; learned scores).
    this.appThrashCatalog = new AppThrashCatalog();
    // CL0–CL6: debounced layout/verify + thrash-extra + optional debug interval.
    this.layoutController = new LayoutController(this, {
      catalog: this.appThrashCatalog,
    });
    this._syncLayoutVerifyInterval();
    // CL4: per-window open quiet → one layout commit (open = batch N=1).
    /** @type {Map<object, object>} metaWindow → open-commit state */
    this._openCommitPending = new Map();
    this._openCommitSchedule = glibSchedule;
    this._openCommitCancel = glibCancel;
    // CL5: multi-open layout batch — defer per-app commit until endOpenLayoutBatch.
    this._openLayoutBatchDepth = 0;
    this._openLayoutBatchNeedsCommit = false;
    ...
    // apply/move → size/position-changed must not retile as external (W-storm / CL2).
  ...
  /**
   * Snapshot mon/ws + sortedWindows for drag-target resolution.
  ...
  move(metaWindow, rect, workArea = null, { skipOffscreenClamp = false, force = false } = {}) {
    ...
    // Nested apply may already have the flag; restore only if we raised it here.
    const prevGeomSuppress = this._suppressGeometrySignalRetile;
    this._suppressGeometrySignalRetile = true;
    try {
      this._moveImpl(metaWindow, rect, workArea, { skipOffscreenClamp, force });
    } finally {
  ...
  beginOpenLayoutBatch(layoutName) {
    this._openLayoutBatchDepth = (this._openLayoutBatchDepth || 0) + 1;
    // CL10: show apply chrome on begin when setting on (depth ≥ 1).
    this.layoutApplyChrome?.syncFromBatch(this._openLayoutBatchDepth, {
      layoutName: layoutName ?? null,
    });
    return {
      ok: true,
      depth: this._openLayoutBatchDepth,
      layoutName:
  ...
  endOpenLayoutBatch(reason) {
    ...
    if ((this._openLayoutBatchDepth || 0) <= 0) {
      this._openLayoutBatchDepth = 0;
      this.layoutApplyChrome?.setLayoutName?.(null);
      this.layoutApplyChrome?.syncFromBatch(0);
      return { ok: true, depth: 0, committed: false, wasActive: false };
    }
    this._openLayoutBatchDepth -= 1;
    if (this._openLayoutBatchDepth > 0) {
      this.layoutApplyChrome?.syncFromBatch(this._openLayoutBatchDepth);
      return {
        ok: true,
    ...
    // CL10: clear apply chrome as soon as batch depth hits 0.
    this.layoutApplyChrome?.setLayoutName?.(null);
    this.layoutApplyChrome?.syncFromBatch(0);
    // CL8: unhide deferred maps before residual / batch commit layout.
    this._releaseAllDeferredOpens();
    const need = !!this._openLayoutBatchNeedsCommit;
    this._openLayoutBatchNeedsCommit = false;
    // Residual already scheduled C idle — skip second commit (one C per batch).
    if (need && !this._renderTreeSrcId) {
      // Depth is 0 — commitLayout Cq (or Cf without layoutController).
      this.commitLayout(r, { force: !this.layoutController });
    }
    return {
      ok: true,
      depth: 0,
  ...
  get openLayoutBatchActive() {
    return (this._openLayoutBatchDepth || 0) > 0;
  }
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // CL8: skip mid-batch split — residual will structure once.
        if (!deferHidden) {
        ...
        // Sticky planned mon when we intentionally chose a home (dock / LFT /
        // PlaceNext). Wayland often maps on mon0 then enters the real mon;
        // without grace, entered-monitor rehome flattens under-LFT attach to
        // mon-root. Do not sticky bare mon-root fallback — Meta rehome must win.
        if (
          openPlan.homeMonitor >= 0 &&
          (openPlan.isDock || openPlan.attachLft || openPlan.fromPlaceHint)
        ) {
          this._applyOpenStickyHome(metaWindow, openPlan.homeMonitor, {
            move:
              openPlan.isDock || (deferHidden && shouldStickyMoveHomeMonitor(openPlan.homeMonitor)),
          });
        }
        ...
        // Guake (WR2): Meta often maps on GDK pointer mon; rehome to focus/LFT.
        if (!willTile) {
          this._rehomeFocusFloatMonitor(metaWindow);
        }
  ...
  _markDeferredOpen(metaWindow, windowActor) {
    ...
    markDeferredOpen(this._deferredOpenStore, metaWindow, snap);
  }
  ...
  _scheduleOpenCommit(metaWindow, openPlan) {
    ...
    const now = Date.now();
    this._openCommitPending.set(metaWindow, {
      timerId: null,
      openedAt: now,
      minQuietMs,
  ...
  _cancelOpenCommit(metaWindow) {
    if (!metaWindow || !this._openCommitPending) return;
    const state = this._openCommitPending.get(metaWindow);
    if (!state) return;
    if (state.timerId != null) {
      try {
        this._openCommitCancel(state.timerId);
      } catch (_e) {
        // source already gone
      }
      state.timerId = null;
    }
    this._openCommitPending.delete(metaWindow);
  }

  /** CL4: cancel all pending open commits (disable / teardown). */
  _cancelAllOpenCommits() {
    if (!this._openCommitPending) return;
    for (const mw of [...this._openCommitPending.keys()]) {
      this._cancelOpenCommit(mw);
    }
  }
  ...
  _touchOpenCommitExternalGeometry(metaWindow) {
    if (!metaWindow || !this._openCommitPending) return false;
    const state = this._openCommitPending.get(metaWindow);
    if (!state) return false;

    state.lastExternalGeomAt = Date.now();
    this._refreshOpenCommitIdentity(metaWindow, state);
    this._armOpenCommitTimer(metaWindow);
    return true;
  }
  ...
  _refreshOpenCommitIdentity(metaWindow, state) {
    const st = state || this._openCommitPending?.get(metaWindow);
  ...
  _armOpenCommitTimer(metaWindow) {
    ...
    state.timerId = this._openCommitSchedule(delay, () => {
      state.timerId = null;
      this._fireOpenCommit(metaWindow);
    });
  }
  ...
  _fireOpenCommit(metaWindow) {
    if (!this._openCommitPending?.has(metaWindow)) return;
    this._cancelOpenCommit(metaWindow);
  ...
  _planOpenAppPlacement(metaWindow) {
    ...
    // follow focus, not stale cross-mon LFT. Dock keeps dock mon; only steal
    // attach when focus is also on the dock mon.
  ...
  /**
   * Guake: place float on focus/LFT monitor without tiling or pointer yank.
   * Wayland GDK mouse-display often maps Guake on the wrong head.
   * @param {Meta.Window|null|undefined} metaWindow
   * @returns {boolean}
   */
  _rehomeFocusFloatMonitor(metaWindow) {
    if (!metaWindow) return false;
    if (!isFocusMonitorFloatClass(metaWmClass(metaWindow))) return false;
    if (!Utils.isWindowAlive(metaWindow)) return false;

    // Only while float / float-exempt — never force a user tile override.
    const node = this.tree.findNode(metaWindow);
    if (node) {
      if (!this.floatingWindow(node) && !this.isFloatingExempt(metaWindow)) return false;
    } else if (!this.isFloatingExempt(metaWindow)) {
      return false;
    }

    let focusedTileMonitor = -1;
    let focusWindowMonitor = -1;
    const focusMeta = this.focusMetaWindow;
    if (focusMeta && focusMeta !== metaWindow) {
      const focusNode = this.tree.findNode(focusMeta);
      if (focusNode?.isWindow?.() && focusNode.isTile?.() && !focusNode.isFloat?.()) {
        focusedTileMonitor = this._monitorIndexOfNode(focusNode);
      }
      try {
        const m = focusMeta.get_monitor?.();
        if (typeof m === "number" && m >= 0) focusWindowMonitor = m;
      } catch (_e) {
        /* ignore */
      }
    } else {
      // Guake has focus (or none): prefer last focused tile node if still valid.
      const last = this.lastFocusedWindow;
      if (
        last &&
        last !== node &&
        last.isWindow?.() &&
        ...
        !last.isFloat?.() &&
        last.nodeValue &&
        last.nodeValue !== metaWindow
      ) {
        focusedTileMonitor = this._monitorIndexOfNode(last);
      }
    }

    const globalLft = this.lftMru?.globalHead?.() ?? null;
    const lftMonitor = this._monitorIndexOfNode(globalLft);
    const target = resolveFloatFocusMonitor({
      focusedTileMonitor,
      lftMonitor,
      focusWindowMonitor,
      fallbackMonitor: 0,
    ...
    // Sticky grace so entered-monitor / client thrash cannot instantly undo.
    this._applyOpenStickyHome(metaWindow, target, { move: true });
    return true;
  }
    ...
    if (!metaWindow.windowSignals) {
      let windowSignals = [
        ...
        metaWindow.connect("focus", (_metaWindowFocus) => {
          ...
          // Guake F12 re-show: rehome to LFT/focus mon before raise (no tile).
          this._rehomeFocusFloatMonitor(_metaWindowFocus);
          this.queueEvent({
            name: "focus-update",
            callback: () => {
  ...
  _rehomeAttachAfterMonLft(existNodeWindow, destMonNode) {
    ...
    const parent = attachLft.parentNode;
    if (!parent) return false;

    // Aspect-split LFT when auto-split is on and LFT is not tab/stack (same as open).
    this._maybeAspectSplitForOpen(attachLft);
    const liveLft = attachLft.nodeValue ? this.tree.findNode(attachLft.nodeValue) : attachLft;
    if (!liveLft?.parentNode) return false;

    liveLft.parentNode.insertBefore(existNodeWindow, liveLft.nextSibling);
    try {
   ...
   *
   * CL2 / W-storm attribution:
   *  - Forge apply suppress → chrome only (no unsettle / retile)
   *  - TILE already in slot → chrome only
   *  - External drift → markUnsettled + requestLayout/requestVerify
   */
  updateMetaPositionSize(_metaWindow, from) {
    ...
    if (focusNodeWindow.grabMode && tilingModeEnabled) {
      ...
      if (this._shouldRejectExternalMaximize(changedNode, _metaWindow)) {
        ...
        // Real external drift still goes through the control loop.
        if (!this._tiledWindowAtTreeSlot(changedNode, _metaWindow)) {
          // Prefer requestLayout/verify over naked renderTree (CL2).
  ...
  _resolveExternalMaximize(node, metaWindow, from) {
    if (Compat.isMaximized(metaWindow)) {
      ...
      this.commitLayout(from || "external-maximize-float", { force: true });
      return;
    }
    Compat.unmaximize(metaWindow);
    this.commitLayout(from || "external-edge-snap-reject", { force: true });
  }
