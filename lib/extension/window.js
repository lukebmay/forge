...
export class WindowManager extends GObject.Object {
  ...
  flushSessionLayout() {
    ...
    return this._saveSessionLayoutForReload({ immediate: true });
  }

  /**
   * After flat track on enable, rehydrate from session-layout.json when fresh
   * and most windows still match by Meta id. Rehomes Meta + tree nodes onto the
   * snapshot monitors first so a thrash pile does not empty every cohort.
   */
  _restoreSessionLayoutAfterTrack() {
    try {
      ...
      const idMap = SessionLayout.indexWindowsById(wins);
      const stats = SessionLayout.matchStats(envelope.forest, idMap);
      ...
      const liveForest = SessionLayout.toLiveForest(envelope.forest, idMap);
