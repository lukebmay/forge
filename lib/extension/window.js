...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    ...
    // L1: open-commit timers owned by bag (first wire owner). Injects via fields above.
    this._openCommitAnonSeq = 0;
    /** @type {WeakMap<object, string>} */
    this._openCommitSlotKeys = new WeakMap();
    this._openCommitSources = new SourceBag({
      label: "open-commit",
  ...
  move(metaWindow, rect, workArea = null, { skipOffscreenClamp = false, force = false } = {}) {
    ...
    // After commit, startEcho covers post-stack client snap (AC2 command epoch).
   ...
   * residual place (finally). end() itself does not clear chrome.
   * @param {string} [reason]
   */
  endOpenLayoutBatch(reason) {
    ...
    if (this._openLayoutBatchDepth > 0) {
      // Nested batch still active — keep chrome depth in sync for show policy.
      this.layoutApplyChrome?.syncFromBatch(this._openLayoutBatchDepth);
      return {
        ok: true,
        depth: this._openLayoutBatchDepth,
    ...
    // Residual bind re-sets _layoutBindPending per-op if needed.
    this._layoutBindPending = false;
  ...
  /**
   * Clear layout-apply chrome (after batch end / error/finally).
  ...
  restoreLayoutOpenLeafIfStolen(focusNode) {
    ...
    if (pinNode) {
      try {
        if (typeof this.settleTabFocus === "function") {
          this.settleTabFocus(pinNode);
        } else {
          this.updateStackedFocus?.(pinNode);
          this.updateTabbedFocus?.(pinNode);
          this.updateDecorationLayout?.({ scope: "focus", focusNode: pinNode });
        }
      } catch (_e) {
        /* best-effort */
      }
  ...
  _scheduleOpenCommit(metaWindow, openPlan) {
    ...
    const now = Date.now();
    const slot = this._openCommitSlotName(metaWindow);
    ...
    Logger.debug(
      `[open-commit] schedule slot=${slot} class=${
        wmClass || "?"
      } dock=${isDock} firstOpen=${firstOpen} minQuietMs=${minQuietMs} pending=${
        this._openCommitPending.size
      }`
    );
  ...
  /**
   * Stable SourceBag slot name for an open-commit window.
   * @param {Meta.Window} metaWindow
   * @returns {string|null}
   */
  _openCommitSlotName(metaWindow) {
    if (!metaWindow) return null;
    let key = this._openCommitSlotKeys?.get(metaWindow);
    if (key) return key;
    let id = null;
    try {
    ...
    key = id != null && id !== "" ? `oc:${id}` : `oc:a${++this._openCommitAnonSeq}`;
    this._openCommitSlotKeys?.set(metaWindow, key);
    return key;
  }
  ...
  _cancelOpenCommit(metaWindow) {
    ...
    if (!state) return;
    const slot = state.slot || this._openCommitSlotName(metaWindow);
    if (slot && this._openCommitSources) {
      this._openCommitSources.cancel(slot);
  ...
  _cancelAllOpenCommits() {
    ...
    const n = this._openCommitPending.size;
    if (n > 0) {
      Logger.debug(
        `[open-commit] cancelAll n=${n} bag=${JSON.stringify(
          this._openCommitSources?.snapshot?.() ?? null
        )}`
      );
    }
    for (const mw of [...this._openCommitPending.keys()]) {
      this._cancelOpenCommit(mw);
      // Drop settle samples only on full teardown — fire must keep pending until agree.
      this.layoutController?.clearOpenPendingForSettle?.(mw);
    }
    // Drain any orphaned bag slots (should already be empty).
    this._openCommitSources?.cancelAll();
  }
  ...
  _armOpenCommitTimer(metaWindow) {
    ...
    // SourceBag replaces any prior timer on this slot.
    state.timerId = this._openCommitSources.set(slot, delay, () => {
      state.timerId = null;
      Logger.debug(
  ...
  _planOpenAppPlacement(metaWindow) {
    ...
    // follow focus, not stale cross-mon LFT. Dock sticky mon is never rehomed
    // by focus (left-dock while focus is on right must stay on left).
    const focusMeta = this.focusMetaWindow;
    if (focusMeta && !plan.isDock) {
   ...
   *
   * CL2 / apply-contract AC2 attribution:
   *  - Stack suppress OR active command echo epoch → chrome only
   *  - TILE already in slot → chrome only
   *  - External drift → markUnsettled + diagnostic requestVerify (no forest apply)
   */
