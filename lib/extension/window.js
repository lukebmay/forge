...
export class WindowManager extends GObject.Object {
  ...
  /**
   * window-entered-monitor: re-home unless workareas thrash is pending (soft rehome).
   */
  _onWindowEnteredMonitor(_display, monitor, metaWindow) {
    // During thrash Mutter may shove windows onto the primary; soft rehome settles.
    if (this._workareasThrashPending) return;
  ...
  _queueSoftRehomeOnWorkareas() {
    this._workareasThrashPending = true;
    this._clearTimeoutId("_workareasSettleSrcId");
    this._workareasSettleSrcId = GLib.timeout_add(
      GLib.PRIORITY_DEFAULT,
      WORKAREAS_SETTLE_MS,
      () => {
        this._workareasSettleSrcId = 0;
        try {
          this._softRehomeAfterWorkareas();
        } catch (e) {
          Logger.warn(`soft-rehome settle failed, falling back to reloadTree: ${e}`);
          this._workareasThrashPending = false;
          this.reloadTree("workareas-soft-rehome-fallback");
        }
        return false;
      }
    );
  ...
  _softRehomeAfterWorkareas() {
    ...
    for (const wNode of windowNodes) {
      ...
      if (!Utils.isWindowAlive(metaWindow) || !this._validWindow(metaWindow)) continue;
      const ws = metaWindow.get_workspace();
      if (!ws) continue;

      const targetMon = this._resolveSoftRehomeMonitor(wNode, geometries, nMonitors);
    ...
    if (!structureOk) {
      ...
      this.reloadTree("workareas-soft-rehome-inconsistent");
      return;
    }

    // Force STACKED/TABBED members onto the majority target so CON migrates intact.
    this._alignSoftRehomeGroupTargets(targets, nMonitors);

    for (const [wNode, targetMon] of targets) {
      const metaWindow = wNode.nodeValue;
      if (!Utils.isWindowAlive(metaWindow)) continue;
      if (
        typeof metaWindow.move_to_monitor === "function" &&
        metaWindow.get_monitor() !== targetMon
      ) {
        try {
          metaWindow.move_to_monitor(targetMon);
        } catch (e) {
          Logger.debug(`soft-rehome move_to_monitor skipped: ${e}`);
        }
      }
    }
    ...
    this.tree.restoreTreeIfNeeded(treeSnapshot);
    this._lftTouchFocusAfterRestore();
    this.renderTree("workareas-soft-rehome");
  }
  ...
  _alignSoftRehomeGroupTargets(targets, nMonitors) {
    for (const layout of [LAYOUT_TYPES.STACKED, LAYOUT_TYPES.TABBED]) {
      for (const con of this.tree.getNodeByLayout(layout)) {
        if (con.childNodes.length < 2) continue;
        if (this.tree._hasStackedOrTabbedAncestor(con)) continue;
  ...
  _resolveSoftRehomeMonitor(wNode, geometries, nMonitors) {
    const metaWindow = wNode.nodeValue;
    const home = this._lastGoodHomes.get(metaWindow);

    if (home?.stableKey && this._monitorLiveMap) {
      const byKey = MonitorIdentity.resolveIndexByStableKey(home.stableKey, this._monitorLiveMap);
  ...
  _snapshotLastGoodHomes() {
    ...
    if (!this._monitorLiveMap) this._refreshMonitorIdentityMap();

    for (const wNode of this.tree.getNodeByType(NODE_TYPES.WINDOW)) {
      const metaWindow = wNode.nodeValue;
      if (!Utils.isWindowAlive(metaWindow)) continue;
      const monIdx = this._monitorIndexOfNode(wNode);
      ...
      if (!frame) continue;
      const stableKey = this._monitorLiveMap?.byIndex?.get(monIdx) ?? null;
      this._lastGoodHomes.set(metaWindow, {
        monitorIndex: monIdx,
        stableKey,
        frame,
      });
  ...
  disable() {
    ...
    if (this._sessionLayoutSaveSrcId) {
      try {
        GLib.Source.remove(this._sessionLayoutSaveSrcId);
      } catch (_e) {
        // ignore
      }
      this._sessionLayoutSaveSrcId = 0;
    }
  ...
  enable() {
    ...
    // Protect last-good from thrash auto-save until restore finishes.
  ...
  reloadTree(from) {
    if (!this._reloadTreeSrcId) {
      this._reloadTreeSrcId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
        try {
          // T6 / forge-bqa: full forest snapshot (H/V + tabs/stacks + percents)
          // before reload() wipes the tree; restore after flat re-track so nested
          // splits and size policy survive (tabs/stacks included).
          // On enable the live tree is empty — also try session-layout.json from
          // the previous instance (install/update disable→enable / Shell HUP).
          const treeSnapshot = this.tree.snapshotTree();
          // Delegate tree structure reload to Tree class
          this.tree.reload();
          // Nodes are new identities after reload — drop stale LFT refs.
          this.lftMru?.clear();
          // WindowManager handles window tracking and rendering
          this.trackCurrentWindows();
          this.tree.restoreTree(treeSnapshot);
          // Live snapshot wins when non-empty (in-session reload). Otherwise
          // (enable after HUP) rehydrate portable last-good layout.
  ...
  /**
   * Debounced write of last-good topology so Shell HUP / hard kill still has a
   * file (disable-only save is unreliable on killall -HUP).
   */
  _queueSessionLayoutSave() {
    if (this.disabled || this._sessionLayoutSaveSrcId) return;
    if (this._sessionLayoutSaveHoldUntil) {
      const now =
        typeof GLib.get_monotonic_time === "function"
          ? GLib.get_monotonic_time()
          : Date.now() * 1000;
      if (now < this._sessionLayoutSaveHoldUntil) return;
    }
    this._sessionLayoutSaveSrcId = GLib.timeout_add(GLib.PRIORITY_LOW, 1500, () => {
      this._sessionLayoutSaveSrcId = 0;
      if (!this.disabled) this._saveSessionLayoutForReload();
      return false;
    });
  }

  /**
   * Suppress auto-save briefly after enable so a failed flat thrash does not
   * overwrite a good last-good written by the install flush.
   * @param {number} [holdUs]
   */
  _holdSessionLayoutSave(holdUs = 12_000_000) {
    const now =
      typeof GLib.get_monotonic_time === "function" ? GLib.get_monotonic_time() : Date.now() * 1000;
    this._sessionLayoutSaveHoldUntil = now + holdUs;
  }

  /**
   * Write portable topology so install/update can re-apply splits/tabs.
   * @param {{ immediate?: boolean, force?: boolean }} [opts]
   * @returns {boolean}
   */
  _saveSessionLayoutForReload(opts = {}) {
    try {
      if (!this.tree || this.disabled) return false;
      if (!opts.force && !opts.immediate && this._sessionLayoutSaveHoldUntil) {
        const now =
          typeof GLib.get_monotonic_time === "function"
            ? GLib.get_monotonic_time()
            : Date.now() * 1000;
        if (now < this._sessionLayoutSaveHoldUntil) return false;
      }
      const live = this.tree.snapshotTree?.();
      ...
      // Never clobber a richer fresh last-good with a thrash-flat dump.
      if (!opts.force) {
        try {
          ...
          if (existing && SessionLayout.isSessionLayoutFresh(existing, mono)) {
            ...
            if (newR + 5 < oldR) {
              Logger.debug(`session-layout: skip save (richness ${newR} < existing ${oldR})`);
              return false;
            }
          }
        } catch (_e) {
      ...
      const msg = `session-layout: saved ${portable.monitors.length} monitor(s)${
        opts.immediate ? " (flush)" : ""
      }`;
      if (opts.immediate) Logger.info(msg);
  ...
  /**
   * DBus/install flush: cancel debounce and write now.
   * @returns {boolean}
   */
  flushSessionLayout() {
    if (this._sessionLayoutSaveSrcId) {
    ...
    // Not force: skip if this tree is thrash-flatter than an existing last-good.
    return this._saveSessionLayoutForReload({ immediate: true, force: false });
  }

  /**
   * After flat track on enable, rehydrate from session-layout.json when fresh
   * and most windows still match (id, else class+title). Rehomes Meta + tree
   * nodes onto the snapshot monitors first so a thrash pile does not empty
   * every cohort.
   */
  _restoreSessionLayoutAfterTrack() {
    // Hold auto-save so a flat thrash cannot overwrite a good install flush.
    this._holdSessionLayoutSave(12_000_000);
    try {
      const configMgr = this.ext?.configMgr;
      if (!configMgr?.loadSessionLayout) return false;
      const raw = configMgr.loadSessionLayout();
      const envelope = SessionLayout.parseEnvelope(raw);
      if (!envelope) {
        // No valid file — leave tree flat; do not invent layout.
        return false;
      }
      const now =
        typeof GLib.get_monotonic_time === "function"
      ...
      if (!SessionLayout.isSessionLayoutFresh(envelope, now)) {
        Logger.info("session-layout: stale or post-reboot; discarding");
        configMgr.clearSessionLayout?.();
        return false;
      }
      const wins = this.windowsAllWorkspaces || [];
      // Fresh resolver for stats (class+title survives Shell HUP id churn).
      ...
      if (!SessionLayout.isMatchGoodEnough(stats)) {
        ...
        // Keep file so a later enable (more windows mapped) can retry.
        return false;
      }
      // Second resolver for actual live forest build (consumes matches).
      const resolve = SessionLayout.createWindowResolver(wins);
      const liveForest = SessionLayout.toLiveForest(envelope.forest, resolve);
      if (!liveForest?.monitors?.length) {
        return false;
      }
      // Mutter often piles windows on one head during Shell reload. Move Meta +
      // reparent nodes onto snapshot mons BEFORE topology restore (strict mon).
      this._rehomeWindowsForSessionForest(liveForest);
      this._restoreSessionForestStrict(liveForest);
      configMgr.clearSessionLayout?.();
      Logger.info(
        `session-layout: restored topology (${stats.matched}/${stats.total} windows matched)`
      );
      return true;
    } catch (e) {
  ...
  /**
   * Move each snapshot window onto its mon (Meta + flat tree parent).
  ...
  _rehomeWindowsForSessionForest(liveForest) {
    ...
    const ctx = this.tree._treeSnapshotCtx();
    for (const { window: metaWin, monIndex, monId } of homes) {
      if (!metaWin) continue;
      try {
        const cur = typeof metaWin.get_monitor === "function" ? metaWin.get_monitor() : -1;
        if (cur !== monIndex && typeof metaWin.move_to_monitor === "function") {
          metaWin.move_to_monitor(monIndex);
        }
      } catch (e) {
        Logger.debug(`session-layout: move_to_monitor failed: ${e}`);
      }
      try {
        const node = this.tree.findNode(metaWin);
        if (!node) continue;
        const monDesc = (liveForest.monitors || []).find((m) => m.id === monId) || {
          id: monId,
        };
        const mon = SessionLayout.resolveStrictMonitor(monDesc, ctx) || this.tree.findNode(monId);
        if (!mon) continue;
        if (!TreeSnapshot.hasAncestor(node, mon)) {
          mon.appendChild(node);
        }
      } catch (e) {
        Logger.debug(`session-layout: tree reparent failed: ${e}`);
      }
    }
  }

  /**
   * Apply live forest using snapshot mon ids/stableKeys only (no majority remap).
   * @param {{ monitors: any[] }} liveForest
   */
   ...
   * coherent with live focus (optional T6).
   */
  _lftTouchFocusAfterRestore() {
    try {
      const focusWin = this.focusMetaWindow;
      if (!focusWin) return;
  ...
  _maybeAspectSplitForOpen(lftNode) {
    ...
    const rect = live.nodeValue?.get_frame_rect?.() || live.rect;
  ...
  /**
   * Force Meta monitor for dock launches and mark a short sticky grace so
   * restore-geometry / entered-monitor cannot flip the home immediately.
   * @param {Meta.Window} metaWindow
  ...
  _applyDockStickyHome(metaWindow, monitorIndex) {
    ...
    try {
      ...
      Logger.debug(`dock sticky move_to_monitor skipped: ${e}`);
    }
  }
   ...
   * launches record sticky mon (pointer mon at activate; overview skipped).
  ...
  _enforceDockStickyIfNeeded(metaWindow) {
    ...
    try {
      if (
        metaWindow.get_monitor() !== stickyMon &&
        typeof metaWindow.move_to_monitor === "function"
      ) {
        metaWindow.move_to_monitor(stickyMon);
      }
    } catch (e) {
      Logger.debug(`dock sticky re-force skipped: ${e}`);
    }
  ...
  _resolveDndCenterLayout() {
    const raw = this.ext.settings.get_string("dnd-center-layout") || "tabbed";
    const layout = raw.toUpperCase();
    if (layout === "STACKED" && !this.ext.settings.get_boolean("stacked-tiling-mode-enabled")) {
      return "TABBED";
    }
    return layout;
  }
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    const ctx = {
      ...
      centerLayout: this._resolveDndCenterLayout(),
  ...
  _getDragDropCenterPreviewStyle() {
    return `window-tilepreview-${this._resolveDndCenterLayout().toLowerCase()}`;
  }
