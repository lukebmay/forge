...
export class WindowManager extends GObject.Object {
  ...
  _ensureTiledForSlotPlace(metaWindow, node = null) {
    ...
    const n = node || this.tree.findNode(metaWindow);
  ...
  _logFloatDecision(nodeWindow, metaWindow, decision, wasFloat) {
    ...
    if (HUNT_TILE_SLOT_FLOAT && (applyLive || placeOwned || (willFloat && changed))) {
      huntTileSlotFloat(
        `processFloats id=${id} class=${cls} action=${decision.action} reason=${reason} ` +
          `wasFloat=${wasFloat} metaMon=${metaMon} treeMon=${treeMon} applyLive=${!!applyLive} ` +
          `flags=${formatFloatFlagTags(decision.flags || {})}`
      );
    }
  }
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        if (willTile && !placePinned && !deferHidden) {
          ...
          Logger.debug(
            `track insert min=${minDecision?.kind || "split"} dock=${!!openPlan.isDock} ` +
              `emptyHead=${!!openPlan.isEmptyHead} home=${openPlan.homeMonitor} ` +
              `placePinned=${placePinned} attach=${openPlan.attachMode || "-"}`
          );
          if (minDecision?.kind === "tab" && minDecision.targetUnit) {
            Logger.trace("track insert branch=open-min-tab");
            ...
            Logger.trace("track insert branch=open-min-float");
            openMinFloat = true;
            leftoverSlot = null;
          } else if (leftoverSlot) {
            Logger.trace("track insert branch=leftover-hv-slot");
            leftoverSlot.layout = this._layoutFromOrientation(
              this._orientationFromUnit(leftoverSlot)
            );
          } else {
            Logger.trace("track insert branch=aspect-split+slotSplit");
  ...
  _applyPlacePlanToExistingWindow(metaWindow, plan) {
    ...
    try {
      ...
      if (HUNT_TILE_SLOT_FLOAT) {
        huntTileSlotFloat(
          `place-hint-adopt id=${winId} class=${metaWmClass(metaWindow)} wantMon=${homeMonitor} ` +
            `metaMon=${metaMonBefore} treeMon=${this._monitorIndexOfNode(node)} mode=${
              node.mode || "?"
            } ` +
            `float=${decision.action}/${decision.reason} flags=${formatFloatFlagTags(
              decision.flags || {}
            )}`
        );
      }
  ...
  _scheduleLatePlaceHintMeta(metaWindow, homeMonitor, winId, attempt = 0) {
    ...
    this._wmSources.set(`latePlaceHintApply:${winId}`, delayMs, () => {
      ...
      if (suppress) {
        ...
        if (HUNT_TILE_SLOT_FLOAT) {
          huntTileSlotFloat(
            `place-hint-idle-defer id=${winId} reason=suppress attempt=${n} wantMon=${homeMonitor} metaMon=${metaMon}`
          );
        }
      ...
      try {
        ...
        if (
          ...
          if (HUNT_TILE_SLOT_FLOAT) {
            ...
            huntTileSlotFloat(
              `place-hint-idle-move id=${winId} wantMon=${homeMonitor} before=${metaMon} after=${after} ok=${moved}`
            );
          }
        } else {
          Logger.debug(
          ...
          if (HUNT_TILE_SLOT_FLOAT) {
            huntTileSlotFloat(
              `place-hint-idle-skip id=${winId} reason=already-mon mon=${metaMon} want=${homeMonitor}`
            );
          }
        }
        this.renderTree("late-place-hint");
        if (HUNT_TILE_SLOT_FLOAT) {
          const n2 = this.tree.findNode(metaWindow);
          ...
          huntTileSlotFloat(
            `place-hint-idle-after-render id=${winId} mode=${n2?.mode || "?"} ` +
              `metaMon=${metaAfter} treeMon=${n2 ? this._monitorIndexOfNode(n2) : "?"} ` +
              `float=${d2 ? `${d2.action}/${d2.reason}` : "?"} ` +
              `flags=${d2 ? formatFloatFlagTags(d2.flags || {}) : "-"}`
          );
        }
      } catch (e) {
  ...
  rehomeIfSlotTooSmall(node) {
    ...
    const decision = resolveTileOverflowPlacement({
      selfUnit: startUnit,
      lftUnit: startUnit,
      newMins: readWindowMinSize(meta),
      slotRectFor: (u) => this._slotRectForUnit(u),
  ...
  /**
   * Float last resort: override + peel to monitor; collapse the vacated slot.
   * @param {import('./tree.js').Node} node
   * @returns {boolean}
   */
  ...
  _adoptOpenIntoTileSlot(nodeWindow) {
    ...
    if (liveUnit && !alreadySlotted) {
      ...
      if (minDecision?.kind === "tab" && minDecision.targetUnit) {
        ...
        if (tabCon?.isStackedOrTabbed?.()) {
          if (nodeWindow.parentNode !== tabCon) tabCon.appendChild(nodeWindow);
    ...
    if (!metaWindow.windowSignals) {
      let windowSignals = [
        ...
        metaWindow.connect("notify::title", () => {
          ...
          this.renderTree(adopted ? "title-late-adopt" : "title-changed");
        }),
      ];
  ...
  updateMetaPositionSize(_metaWindow, from) {
    ...
    const covering =
      !!grabMeta &&
      (Compat.isMaximized(grabMeta) || !!(grabMeta.is_fullscreen && grabMeta.is_fullscreen()));
