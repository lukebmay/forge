...
export class WindowManager extends GObject.Object {
  ...
  _scheduleMinClampLearn(metaWindow) {
    ...
    const delay = MIN_CLAMP_LEARN_DELAY_MS + 20;
  ...
  ensureWindowMinSizeKnown(metaWindow) {
    ...
    this._wmSources.set(slot, MIN_CLAMP_LEARN_DELAY_MS + 150, () => {
      try {
        ...
        const shrank = (nw > 0 && nw < restore.width - 4) || (nh > 0 && nh < restore.height - 4);
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        const leftoverSlot = placePinned ? null : this._hvSlotToJoin(insertUnit);
        if (willTile && !placePinned) {
          if (leftoverSlot) {
        ...
        if (nodeWindow && insertUnit && insertUnit !== nodeWindow && !placePinned) {
