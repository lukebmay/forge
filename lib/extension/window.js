...
export class WindowManager extends GObject.Object {
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        } else if (deferHidden && shouldStickyMoveHomeMonitor(openPlan.homeMonitor)) {
          // PlaceNext / planned home mon sticky (same spirit as dock sticky).
          safeMoveToMonitor(metaWindow, openPlan.homeMonitor, "deferred open move_to_monitor");
        }
  ...
  _planOpenAppPlacement(metaWindow) {
    ...
    // Prefer the currently focused tile under the home mon when it is a live
    // TILE (user "selected" that unit). LFT can lag after layout/restore; focus
    // matches what the operator sees. Dock path keeps monLft unless focus is
    // also on that dock mon.
    ...
    if (focusMeta) {
      ...
      if (focusNode?.isWindow?.() && focusNode.isTile?.() && !focusNode.isFloat?.()) {
        ...
        } else if (!plan.isDock && focusMon >= 0 && plan.attachLft == null) {
