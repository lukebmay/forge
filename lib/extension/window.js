...
import { afterFocus as afterFocusPipeline } from "./action-pipeline.js";
...
export class WindowManager extends GObject.Object {
  ...
  constructor(ext) {
    ...
    // unlock-dialog: tree stays loaded; Meta/DPMS thrash must not soft-rehome.
   ...
   * (and no successful renderTree already satisfied it), one requestLayout.
   * @param {string} [reason]
   */
  endOpenLayoutBatch(reason) {
    ...
    this._openLayoutBatchNeedsCommit = false;
    // Residual already scheduled renderTree idle — skip second requestLayout.
    if (need && !this._renderTreeSrcId) {
      // Depth is 0 — requestLayout is no longer gated.
      if (this.layoutController) {
        this.layoutController.requestLayout(r);
      } else {
        this.renderTree(r, true);
      }
    }
    return {
      ok: true,
   ...
   *
   * Prefer requestLayout so open shares the controller debounce + post-render
   * verify. Open-layout batch (CL5 / forge layout multi-open): unmaximize only
   * and latch need-commit — no per-app mid-batch render. If render is frozen
   * mid-grab (not batch), force renderTree once (still onRenderComplete → verify).
   *
   * @param {Meta.Window} metaWindow
   */
    ...
    this.queueEvent(
      {
        ...
        callback: () => {
          ...
          if (this._freezeRender) {
            // Force past freeze so first TILE placement is not dropped.
            this.renderTree("window-create", true);
          } else if (this.layoutController) {
            this.requestLayout("window-create");
          } else {
            this.renderTree("window-create", true);
          }
        },
