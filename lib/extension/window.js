...
import { buildGeomEpsilonFields, logGeomEpsilonSample } from "./geom-epsilon.js";
...
export class WindowManager extends GObject.Object {
  ...
  _moveImpl(metaWindow, rect, workArea = null, { skipOffscreenClamp = false, force = false } = {}) {
    ...
    // D095: ε=4 is interim measurement baseline only — do not treat as locked.
    const frame = metaWindow.get_frame_rect();
    const targetW = Math.max(width, minW);
    const targetH = Math.max(height, minH);
    const eps = LAYOUT_VERIFY_EPSILON_PX;
    ...
    this._scheduleGeomEpsilonObserve(metaWindow, commandRect, known);
  ...
  /**
   * D095 S1: log sent vs observed for the ε campaign.
   * @param {any} metaWindow
   * @param {{ phase: string, sent: object, observed?: object|null, knownMin?: object|null, wrote?: boolean, level?: string }} opts
   */
  _logGeomEpsilon(metaWindow, opts) {
    try {
      ...
      let wmClass = "-";
      try {
        if (typeof metaWindow?.get_id === "function") windowId = metaWindow.get_id();
      } catch (_e) {
        /* ignore */
      }
      try {
   ...
   */
  _scheduleGeomEpsilonObserve(metaWindow, sent, knownMin) {
