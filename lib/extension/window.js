...
export class WindowManager extends GObject.Object {
  ...
  _scheduleMinClampLearn(metaWindow) {
    ...
    this._wmSources.set(slot, delay, () => {
      try {
        ...
        if (node && this._slotTooSmallForTile(node, metaWindow)) {
  ...
  /**
   * Mid-session: TILE mins exceed slot → same-mon tab or float; remove vacated gap.
  ...
  rehomeIfSlotTooSmall(node) {
    ...
    if (meta.is_fullscreen?.()) return false;
    if (Compat.getMaximizeFlags(meta) !== 0) return false;
  ...
  updateMetaPositionSize(_metaWindow, from) {
    ...
    } else if (this._slotTooSmallForTile(changedNode, _metaWindow)) {
  ...
  _restoreTileToSlot(node, metaWindow) {
    ...
    // Overflow owns this settle — do not restore into an illegal slot.
    if (this._slotTooSmallForTile(node, metaWindow)) {
      this._scheduleOverflowRehome(node);
      return;
    }
    this._suppressGeom.run(() => {
