...
export class WindowManager extends GObject.Object {
  ...
  disable() {
    ...
    this._saveSessionLayoutForReload();
    ...
    if (!this._reloadTreeSrcId) {
      this._reloadTreeSrcId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
        try {
          ...
          // the previous instance (install/update disable→enable).
          const treeSnapshot = this.tree.snapshotTree();
          ...
          // Prefer live snapshot when non-empty; else portable session file.
  ...
  /**
   * Write portable topology before disable destroys the tree so install/update
   * can re-apply splits/tabs without flattening to full-height columns.
   */
  _saveSessionLayoutForReload() {
    try {
      if (!this.tree || this.disabled) return;
      const live = this.tree.snapshotTree?.();
      const portable = SessionLayout.toPortableForest(live);
      if (!portable) return;
      const configMgr = this.ext?.configMgr;
      if (!configMgr?.saveSessionLayout) return;
      const mono =
        typeof GLib.get_monotonic_time === "function"
          ? GLib.get_monotonic_time()
          : Date.now() * 1000;
      configMgr.saveSessionLayout(SessionLayout.makeEnvelope(portable, mono));
      Logger.debug("session-layout: saved portable forest for reload");
    } catch (e) {
   ...
   * and most windows still match by Meta id.
   */
  _restoreSessionLayoutAfterTrack() {
    try {
      ...
      if (!SessionLayout.isSessionLayoutFresh(envelope, now)) {
        Logger.debug("session-layout: stale or post-reboot; discarding");
        configMgr.clearSessionLayout?.();
        return false;
      }
      ...
      this.tree.restoreTree(liveForest);
      configMgr.clearSessionLayout?.();
      Logger.info(
      ...
      try {
        this.ext?.configMgr?.clearSessionLayout?.();
      } catch (_e2) {
        // ignore
      }
      return false;
