...
export class WindowManager extends GObject.Object {
  ...
  /**
   * Pin TABBED/STACKED open leaf for a short residual after layout/focus.
   ...
   */
  pinLayoutOpenLeaf(parentCon, meta, residualMs = 3000) {
    ...
    this._layoutOpenLeafPins.set(parentCon, { meta, until: Date.now() + ms });
  }
  ...
  getLayoutOpenLeafPin(parentCon) {
    ...
    if (Date.now() >= pin.until) {
      this._layoutOpenLeafPins.delete(parentCon);
      return null;
    }
    return pin;
  }
  ...
  restoreLayoutOpenLeafIfStolen(focusNode) {
    ...
    if (!pin?.meta) return false;
    if (focusNode.nodeValue === pin.meta) return false;
