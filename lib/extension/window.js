...
export class WindowManager extends GObject.Object {
  ...
  _softRehomeAfterWorkareas() {
    ...
    // T6: full forest before rehome (H/V + tabs + percents). If structure is
    // inconsistent we abandon this snapshot and call reloadTree, which takes its
    // own snapshot around the wipe — do not assume this capture is reused.
  ...
  /**
   * Pick monitor index for a window during soft rehome: max intersection with
  ...
  _resolveSoftRehomeMonitor(wNode, geometries, nMonitors) {
    ...
    if (home && home.monitorIndex >= 0 && home.monitorIndex < nMonitors) {
      return home.monitorIndex;
    }
  ...
  _snapshotLastGoodHomes() {
    ...
    for (const wNode of this.tree.getNodeByType(NODE_TYPES.WINDOW)) {
      ...
      if (!frame) continue;
      this._lastGoodHomes.set(metaWindow, { monitorIndex: monIdx, frame });
    }
  }
  ...
  /**
   * OP1: resolve monitor home + LFT attach head for a new window.
   * @param {Meta.Window} metaWindow
   */
