  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // D032 wrap only when this map will tile. Unknown identity is
        // float-exempt until notify — wrapping now reserves a ghost TILE
        // for always-float (R031). R028 adopts into the slot on FLOAT→TILE.
        ...
        // wrap CON — createNode(bag) would append a tab. Use isCon (not
        // !isWindow) so MONITOR PlaceNext stays mon-root, not workspace.
        ...
        if (nodeWindow && insertUnit && insertUnit !== nodeWindow) {
        ...
        sessionLayoutTrace(
          `layout-track: attached class=${metaWindow.get_wm_class()} title=${JSON.stringify(
            metaWindow.get_title()
          )} dest=${metaMonWsNode.nodeValue} deferred=${!!deferHidden}`
        );
      }
    } else {
      sessionLayoutTrace(
