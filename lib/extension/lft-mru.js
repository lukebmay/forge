...
/**
 * Monitor for focus-follow float drop-downs (Guake on Wayland).
 * Tiled focus mon → LFT mon → other focus window mon → mon0.
 *
 * @param {object} [opts]
 * @param {number} [opts.focusedTileMonitor=-1]
 * @param {number} [opts.lftMonitor=-1]
 * @param {number} [opts.focusWindowMonitor=-1]
 * @param {number} [opts.fallbackMonitor=0]
 * @returns {number}
 */
export function resolveFloatFocusMonitor(opts = {}) {
  const focusedTileMonitor = opts.focusedTileMonitor ?? -1;
  if (focusedTileMonitor >= 0) return focusedTileMonitor;
  const lftMonitor = opts.lftMonitor ?? -1;
  if (lftMonitor >= 0) return lftMonitor;
  const focusWindowMonitor = opts.focusWindowMonitor ?? -1;
  if (focusWindowMonitor >= 0) return focusWindowMonitor;
  const fallback = opts.fallbackMonitor ?? 0;
  return fallback >= 0 ? fallback : 0;
}

/**
 * wm_class of floats rehomed to focus/LFT (not pointer/GDK seat).
 * @param {string|null|undefined} wmClass
 * @returns {boolean}
 */
export function isFocusMonitorFloatClass(wmClass) {
  if (wmClass == null || wmClass === "") return false;
  const n = String(wmClass).trim().toLowerCase();
  // Guake: mouse-display uses GDK pointer mon; Wayland often ≠ Forge focus/LFT.
  return n === "guake";
}
