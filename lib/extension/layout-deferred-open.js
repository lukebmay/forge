...
/**
 * Pure helpers for hide-place-show (D117) and LayoutBatch leftover hide.
 * No GObject imports — unit tests stay light.
 *
 * Will-TILE maps hide the actor, command Forest dest, then show.
 * LayoutBatch may keep hide until dest if the slot is not placed yet.
 */

/**
 * True when a will-TILE map should hide until Forest dest is commanded.
 * @param {{ willTile?: boolean, openMinFloat?: boolean, openLayoutBatchActive?: boolean }} opts
 * @returns {boolean}
 */
export function shouldDeferHiddenOpen(opts = {}) {
  return !!opts.willTile && !opts.openMinFloat;
}

/**
 * True when dest was commanded so hide-place-show may unhide now.
 * LayoutBatch dest-miss stays hidden until residual/batch release.
 * @param {{ destCommanded?: boolean, openLayoutBatchActive?: boolean }} opts
 * @returns {boolean}
 */
export function shouldShowDeferredAfterDest(opts = {}) {
  if (opts.destCommanded) return true;
  return !opts.openLayoutBatchActive;
}
...
/**
 * Map-time Meta `move_to_monitor` (R036 SEGV on Wayland chrome-null maps).
 * Dock still transfers immediately. Empty-head (D027) is sticky-grace only —
 * Forest admit + `moveLiveToForestSlot` place the slot; idle present follows.
 * LayoutBatch deferred-hidden still uses the home-monitor sticky move.
 *
 * @param {{ isDock?: boolean, homeMonitor?: number }|null|undefined} openPlan
 * @param {boolean} [deferHidden]
 * @returns {boolean}
 */
export function shouldMapTimeStickyMove(openPlan = {}, _deferHidden = false) {
  if (openPlan?.isDock) return true;
  return false;
}
