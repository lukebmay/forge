 ...
 * / `metric agree` / `metric drift` / `metric resync` / `metric present`
 * / `metric chrome-z` / `metric d100-observe` / `gobject-topology-forbidden`.
 */
import { Logger } from "../shared/logger.js";
import { tilesOf, walk } from "../tom/index.js";

/** @typedef {{ applies: number, applyFail: number, applyMs: number, paints: number, paintMs: number, fallbacks: number, invariants: number, agrees: number, agreeFail: number, drifts: number, resyncs: number, resyncFail: number, presents: number }} MetricsCounters */
...
/**
 * Forest-driven Meta present pass (D096 G2). Grep: `metric present`.
 * @param {{ from?: string, moved?: number, skipped?: number, ms?: number }} fields
 */
export function recordPresent(fields) {
  const f = fields && typeof fields === "object" ? fields : {};
  counters.presents += 1;
  const ms = Number(f.ms);
  Logger.info("metric present", {
    fields: {
      from: f.from || "-",
      moved: Number(f.moved) || 0,
...
/**
 * GObject TILES child-list mutate during present. replaceChildren is never
 * excused (G8e — no Forest→GObject topology mirror). Paint may still detach
 * FLOATS / orphan CONs via removeChild/appendChild under `_presentPaintMirror`.
 * Grep: `gobject-topology-forbidden`.
 * @param {any} wm
 * @param {string} op
 * @param {string} [detail]
 */
export function notePresentGobjectMutate(wm, op, detail) {
  if (!wm?._inPresent) return;
  const d = detail ? ` ${detail}` : "";
  // Topology mirror via replaceChildren is forbidden even during paint.
  if (op === "replaceChildren" || !wm._presentPaintMirror) {
    recordInvariant("gobject-topology-forbidden", op || "mutate", detail || op || "mutate");
    Logger.warn(`gobject-topology-forbidden op=${op || "-"}${d}`);
    return;
  }
  Logger.debug(`metric present gobject-paint-cleanup op=${op || "-"}${d}`);
}
...
export function recordApply(fields) {
  ...
  Logger.info("metric apply", {
    fields: {
      ...
      ms: Number.isFinite(ms) ? Math.round(ms) : 0,
      phase: f.phase || "-",
      error: f.error != null && f.error !== "" ? String(f.error) : "-",
...
/**
 * D100: old-architecture Meta handler fired and was not executed.
 * High-frequency kinds (title / size / position / wm-class) are TRACE;
 * others DEBUG. Grep: `metric d100-observe`.
 * @param {string} kind
 * @param {Record<string, unknown>} [extra]
 */
export function recordD100Observe(kind, extra = null) {
  const k = String(kind || "unknown");
  const fields = extra && typeof extra === "object" ? { kind: k, ...extra } : { kind: k };
  const noisy =
    k === "title" ||
    k === "size-changed" ||
    k === "position-changed" ||
  ...
  if (noisy) Logger.trace("metric d100-observe", payload);
  else Logger.debug("metric d100-observe", payload);
}

/**
 * Border/strip Z restack (D096 G5). Grep: `metric chrome-z`.
 * @param {{ kind?: string, id?: string, op?: string }} [fields]
 */
export function recordChromeZ(fields) {
  const f = fields && typeof fields === "object" ? fields : {};
  Logger.info("metric chrome-z", {
    fields: {
      kind: f.kind || "border",
      id: f.id || "-",
      op: f.op || "restack",
