...
/**
 * Live window → parent CON / mon / rect (GetTree-shaped forest).
...
export function matchRequiredTileSlots(opts = {}) {
  ...
  if (
    ...
    for (const id of hardReady.pending || []) {
      const role = byWid[String(id)];
      _pushFailed(failed, (role && role.slot) || `id:${id}`);
...
export function waitHardReadyOnSignals(windowIds, opts, done) {
  ...
  const statusOf = () => {
    ...
    return hardReadyStatus(wins, ids, { requireTile, slots });
  };
