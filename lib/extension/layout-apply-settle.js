...
import { isWindowJoinMove } from "./layout-apply-structure.js";
...
/**
 * Split residual steps into mon-place vs structure (R013 / R011).
 * No focus / open / skeleton / park / close in either bucket.
 * @param {unknown[]} steps
 * @returns {{ place: object[], structure: object[] }}
 */
export function partitionBeltStructureSteps(steps) {
  /** @type {object[]} */
  const place = [];
  /** @type {object[]} */
  const structure = [];
  if (!Array.isArray(steps)) return { place, structure };
  for (const s of steps) {
    if (!s || typeof s !== "object") continue;
    const op = String(s.op || "")
      .trim()
      .toLowerCase();
    if (op === "focus" || op === "skeleton" || op === "open" || op === "park" || op === "close") {
      continue;
    }
    if (op === "move") {
      if (isWindowJoinMove(s)) structure.push(s);
      else place.push(s);
      continue;
    }
    if (op === "layout" || op === "order" || op === "size" || op === "bind") {
      structure.push(s);
    }
  }
...
/**
 * After belt mon-root moves (D014), re-plan place→structure so TABBED groups
 * flattened by rehome are rebuilt (R013). CLI path had beltStructure; ApplyLayout
 * must do the same after pin-role moves.
 *
 * @param {{
 *   profile: object,
 *   forest?: object|null,
 *   snapshotForest?: () => object|null,
 *   flags?: object,
 *   rolePins?: object,
 *   workspace?: number,
 *   forceClose?: boolean,
 ...
 * @returns {object}
 */
export function runBeltStructureRebind(opts) {
  const pins = opts.rolePins && typeof opts.rolePins === "object" ? opts.rolePins : {};
  if (!Object.keys(pins).length) {
    return { ok: true, skipped: true, placeSteps: 0, structureSteps: 0, reason: "no-pins" };
  }
  if (typeof opts.runSteps !== "function") {
    return {
      ok: false,
      ...
      placeSteps: 0,
      structureSteps: 0,
      error: "runSteps missing",
    };
  }

  const snapshot = () => {
    if (typeof opts.snapshotForest === "function") {
      try {
        return opts.snapshotForest();
      } catch (_e) {
        return null;
      }
    }
    return opts.forest && typeof opts.forest === "object" ? opts.forest : null;
  };

  const replanBuckets = (forest) => {
    if (!forest || typeof forest !== "object") {
      return { ok: false, error: "forest required", place: [], structure: [], plan: null };
    }
    let plan;
    try {
      plan = planReconcile(opts.profile, forest, {
        ...(opts.flags && typeof opts.flags === "object" ? opts.flags : {}),
        workspace: opts.workspace ?? 0,
        rolePins: pins,
        justOpenedRoles: Object.keys(pins),
    ...
    if (!plan || plan.ok === false) {
      return {
        ok: false,
        error: plan?.error != null ? String(plan.error) : "belt structure replan failed",
        place: [],
        structure: [],
        plan: plan || null,
      };
    ...
    let steps;
    try {
      steps = planActionsToSteps(plan.actions || [], {
        workspace: opts.workspace ?? 0,
        forceClose: !!opts.forceClose,
      });
    } catch (e) {
    ...
    const parts = partitionBeltStructureSteps(steps);
    return { ok: true, place: parts.place, structure: parts.structure, plan };
  };

  let forest = snapshot();
  let built = replanBuckets(forest);
  if (!built.ok) {
    return {
      ok: false,
      skipped: false,
  ...
  let placeSteps = 0;
  let structureSteps = 0;
  let plan = built.plan;

  if (built.place.length) {
    try {
      const r = opts.runSteps(built.place, { phase: "verify" });
      if (r && r.ok === false) {
        return {
          ok: false,
          skipped: false,
          placeSteps: built.place.length,
          structureSteps: 0,
          error: r.error || "belt place2 failed",
          plan,
        };
      }
    } catch (e) {
  ...
  if (built.structure.length) {
    try {
      const r = opts.runSteps(built.structure, { phase: "order" });
      if (r && r.ok === false) {
        return {
          ok: false,
          skipped: false,
          placeSteps,
          structureSteps: built.structure.length,
          error: r.error || "belt structure rebind failed",
          plan,
        };
      }
    } catch (e) {
  ...
  return { ok: true, skipped: false, placeSteps, structureSteps, plan };
}
