...
import { createLiveTree } from "./tree.js";
...
export class ForgeAdapterGnome extends GObject.Object {
  ...
  _onWindowEnteredMonitor(_display, monitor, metaWindow) {
    ...
    try {
      id =
        metaWindow && typeof metaWindow.get_id === "function"
          ? String(metaWindow.get_id())
          : "-";
    } catch (_e) {
      id = "-";
    }
  ...
  restoreOpenLeafIfWorkspaceFocusSteal(focusNode) {
    ...
    const parent = focusNode
      ? liveParentForPresent(this, focusNode) || focusNode.parentNode
      : null;
    if (!parent) {
  ...
  _insertChildPercent(parentNode, newChild) {
    ...
    if (this._liveForestSeeded && this.forest) {
      const pid = forestIdFromLive(this, parentNode);
      const cid = forestIdFromLive(this, newChild);
      const parentTom = pid ? this.forest.nodes[pid] : null;
      const childTom = cid ? this.forest.nodes[cid] : null;
      if (!parentTom || !childTom) {
    ...
    this.tree.insertChildPercent(parentNode, newChild);
  }
  ...
  _rehomeAttachAfterMonLft(existNodeWindow, destMonNode) {
    ...
    if (minDecision?.kind === "tab" && minDecision.targetUnit) {
      ...
      if (tabCon) {
        ...
        try {
          this._insertChildPercent(existNodeWindow.parentNode, existNodeWindow);
        } catch (_e) {
          /* best-effort share */
        }
        return true;
  ...
  _applyOwningSplitFromGrab(resolved, focusNodeWindow, currentRect, orientation) {
    return grabApplyOwningSplitFromGrab(
      this,
      resolved,
      focusNodeWindow,
      currentRect,
