 ...
 *
 * GetTree / forge tree: Forest+bag via projectForestFromTom (D096 G6).
 * GObject projectNode/projectForest remain for fixtures until G8.
 */
...
export function windowMetaFields(nodeValue) {
  ...
  try {
    const ws = typeof nodeValue.get_workspace === "function" ? nodeValue.get_workspace() : null;
    const idx = typeof ws?.index === "function" ? ws.index() : ws?.index;
    if (typeof idx === "number" && Number.isFinite(idx) && idx >= 0) out.workspace = idx;
  } catch (_e) {
    /* optional */
  }
