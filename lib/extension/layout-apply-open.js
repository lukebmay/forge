...
export function startOpenPhase(opts) {
  ...
  const spawnOne = (action) => {
    ...
    const placeOpts = placeNextOptionsFromLaunchFields(fields);
    // R036: pin PlaceNext to skeleton PH when present (not mon-root only).
    if (!placeOpts.attachSelector && typeof deps.snapshotForest === "function") {
      try {
        const forest = deps.snapshotForest();
        const phId = findLayoutPlaceholderId(forest, {
          role: role || action?.layoutRole,
          slot: action?.slot ?? action?.layoutSlot,
        });
        if (phId) placeOpts.attachSelector = `id:${phId}`;
      } catch (e) {
        Logger.debug?.(`layout-apply-open PH attach lookup: ${e}`);
      }
    }
    if (placeNextHasDest(placeOpts)) {
      let pr;
      try {
        pr = deps.placeNext(placeOpts);
      } catch (e) {
      ...
      if (!pr || pr.ok === false) {
        ...
        opens.push({ ok: false, role, error: err, placeNext: true });
