...
export const SETTLE_LEARN_PAD = 1.25;
...
export const SETTLE_ROLLING_N = 10;
...
export function computeLearnedMinQuietMs(opts = {}) {
  ...
  const raw = opts.latenciesMs;
  if (Array.isArray(raw) && raw.length > 0) {
    let maxLat = 0;
    for (const v of raw) {
      const n = Number(v);
      if (Number.isFinite(n) && n > maxLat) maxLat = n;
    }
    const fromRoll = maxLat * pad;
    return Math.min(cap, Math.max(0, seedFloor, fromRoll));
  }
...
export class AppThrashCatalog {
  ...
  recordSettleSample(wmClass, sample = {}) {
    ...
    if (ms > 0) {
      ...
      if (entry.latenciesMs.length > SETTLE_ROLLING_N) {
        entry.latenciesMs = entry.latenciesMs.slice(-SETTLE_ROLLING_N);
      }
    }
