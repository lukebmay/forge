...
export function summarizeCensusEntry(row = {}) {
  ...
  if (row.workspace != null && Number.isFinite(Number(row.workspace))) {
    out.workspace = Math.floor(Number(row.workspace));
  }
  if (row.floatExempt != null) out.floatExempt = !!row.floatExempt;
  if (row.parentType != null) out.parentType = row.parentType;
  if (row.parentId != null) out.parentId = row.parentId;
