...
import { forestIdFromLive, liveChildrenForPresent, liveParentForPresent } from "./tom-live.js";
import {
  actorAlive,
  chromeGroupEligible,
  isDisposedActorError,
  render as renderChromeLabel,
} from "./node-chrome.js";
...
export class DecorationManager extends GObject.Object {
  ...
  ensureTabChromeLayer() {
    if (this._tabChromeLayer && actorAlive(this._tabChromeLayer)) {
    ...
    try {
      layer.connect("destroy", () => {
        layer._forgeDisposed = true;
        if (this._tabChromeLayer === layer) this._tabChromeLayer = null;
      });
    } catch (_e) {
  ...
  _parkTabChromeLayer() {
    ...
    if (!actorAlive(layer)) {
      if (layer) this._tabChromeLayer = null;
      return;
    }
  ...
  attachTabDecoration(con) {
    ...
    try {
      ...
      if (parent !== layer) {
        if (parent && actorAlive(parent) && typeof parent.remove_child === "function") {
          parent.remove_child(decoration);
        }
        if (actorAlive(layer) && actorAlive(decoration)) layer.add_child(decoration);
      }
    } catch (e) {
  ...
  destroyTabChromeLayer() {
    ...
    for (const child of children) {
      try {
        ...
        if (actorAlive(layer) && actorAlive(child) && typeof layer.remove_child === "function") {
          layer.remove_child(child);
        }
        if (actorAlive(child)) {
          child.hide?.();
          child._forgeDisposed = true;
          child.destroy?.();
        }
      } catch (e) {
        if (child) child._forgeDisposed = true;
  ...
  _borderForMeta(metaWindow) {
    ...
    if (fromBag && actorAlive(fromBag)) return fromBag;
    const actor = metaWindow.get_compositor_private?.();
    const actorBorder = actor?.border ?? null;
    return actorAlive(actorBorder) ? actorBorder : null;
  }
  ...
  restackBorderForMeta(metaWindow) {
    ...
    if (!border || !actorAlive(border)) return false;
    let compositor = null;
    try {
    ...
    try {
      const wg = global.window_group;
      if (!actorAlive(border)) return false;
      ...
      if (isDisposedActorError(e)) {
        if (border) border._forgeDisposed = true;
        recordWarn("deco-disposed", { where: "restack-border" });
      } else {
  ...
  /**
   * Re-pair every WINDOW border in the host bag (D096 G5d). Call after a
   * float/window raise that can insert another actor between a window and
   * its border in `window_group`.
   * @returns {number} how many borders were restacked
   */
  restackAllWindowBorders() {
    const bag = this._extWm?.hostBag;
    if (!bag?.entries || !global.window_group) return 0;
    let n = 0;
    for (const [, entry] of bag.entries()) {
      if (!actorAlive(entry?.border) || !entry?.meta) continue;
      if (!this._tileChromePairLive(entry.meta)) {
        try {
          entry.border.hide?.();
        } catch (_e) {
          entry.border._forgeDisposed = true;
        }
        continue;
      }
      if (this.restackBorderForMeta(entry.meta)) n += 1;
    }
    return n;
  }
  ...
  hideActorBorder(actor) {
    ...
    if (actorAlive(actor.splitBorder)) {
      try {
        actor.splitBorder.hide();
      } catch (e) {
        actor.splitBorder._forgeDisposed = true;
        Logger.warn(`Failed to hide splitBorder: ${e}`);
      }
    } else if (actor.splitBorder) {
      actor.splitBorder._forgeDisposed = true;
    }
  }

  hideWindowBorders() {
    // D096: borders live in hostBag; tree.nodeWindows alone can miss them
    // and leave stale purple/green rings on unfocused windows.
    const bag = this._extWm?.hostBag;
    if (bag?.entries) {
      for (const [, entry] of bag.entries()) {
        const border = entry?.border;
        if (!actorAlive(border)) continue;
        try {
          border.hide?.();
        } catch (_e) {
          border._forgeDisposed = true;
  ...
  /**
   * Parent CON/MONITOR for focus-border class (Forest-first, D096).
   * @param {any} nodeWindow
   * @returns {any|null}
   */
  _parentForBorder(nodeWindow) {
    if (!nodeWindow) return null;
    return liveParentForPresent(this._extWm, nodeWindow) || nodeWindow.parentNode || null;
  }

  /**
   * Focus-border kind from parent layout. Tab groups → green (`tabbed`).
   * @param {any} parentNode
   * @returns {"stacked"|"tabbed"|null}
   */
  _parentBorderKind(parentNode) {
    if (!parentNode) return null;
    if (typeof parentNode.isStacked === "function" && parentNode.isStacked()) return "stacked";
    if (typeof parentNode.isTabbed === "function" && parentNode.isTabbed()) return "tabbed";
    const wm = this._extWm;
    const id = forestIdFromLive(wm, parentNode);
    const tom = id && wm?.forest?.nodes?.[id];
    if (tom && (tom.kind === "CON" || tom.kind === "MONITOR")) {
      if (tom.layout === "STACKED") return "stacked";
      if (tom.layout === "TABBED") return "tabbed";
    }
    if (parentNode.layout === "STACKED") return "stacked";
    if (parentNode.layout === "TABBED") return "tabbed";
    return null;
  }
  ...
  showWindowBorders() {
    ...
    let parentNode = this._parentForBorder(nodeWindow);
    const parentKind = this._parentBorderKind(parentNode);

    const floatingWindow = nodeWindow.isFloat();
    if (!floatingWindow && !this._conOnActiveWorkspace(nodeWindow)) {
      this.hideActorBorder(windowActor);
      return;
    }
    let tiledBorder = this._borderForMeta(metaWindow) || windowActor.border;
    if (tiledBorder && !actorAlive(tiledBorder)) {
      tiledBorder._forgeDisposed = true;
      tiledBorder = null;
    }

    // FLOATS paint may null parentNode; borders still run for focus.
    if (parentKind === "stacked" || parentKind === "tabbed") {
      ...
      if (
        ...
        if (tilingModeEnabled && actorAlive(tiledBorder)) {
          try {
            ...
            tiledBorder._forgeDisposed = true;
            if (!isDisposedActorError(e)) Logger.warn(`showWindowBorders style: ${e}`);
          }
        }
    ...
    borders.forEach((border) => {
      ...
      try {
        ...
        border._forgeDisposed = true;
        if (!isDisposedActorError(e)) Logger.warn(`showWindowBorders: ${e}`);
      }
    });
      ...
      if (!actorAlive(border)) {
        ...
        try {
          ...
          if (
            wg &&
            actorAlive(border) &&
            typeof wg.contains === "function" &&
            wg.contains(border)
          ) {
            wg.remove_child(border);
          }
          if (actorAlive(border)) border.hide?.();
          if (actorAlive(border)) {
            border._forgeDisposed = true;
            border.destroy?.();
          }
          ...
          border._forgeDisposed = true;
          if (isDisposedActorError(e)) recordWarn("deco-disposed", { where: "destroy-border" });
          else Logger.warn(`_destroyActorBorder: ${e}`);
        }
      }
    }
  ...
  _raiseStripToChromeTop(decoration) {
    const layer = this._tabChromeLayer;
    if (!actorAlive(layer) || !actorAlive(decoration)) return;
    try {
      const parent =
        typeof decoration.get_parent === "function" ? decoration.get_parent() : decoration._parent;
  ...
  _conOnActiveWorkspace(con) {
    if (typeof this._tree?._decorationOnActiveWorkspace === "function") {
      return this._tree._decorationOnActiveWorkspace(con);
    }
    try {
      ...
      if (typeof wsIdx !== "number" || wsIdx < 0) return false;
  ...
  /** TILE chrome only on the live `moNwsW` pair; FLOAT always restacks. */
  _tileChromePairLive(metaWindow) {
    const node = this._extWm?.findNodeWindow?.(metaWindow);
    if (!node) return true;
    if (typeof node.isFloat === "function" && node.isFloat()) return true;
    return this._conOnActiveWorkspace(node);
  }
  ...
  _sweepOrphanDecorations(allCons) {
    ...
    for (const actor of children) {
      ...
      if (!actorAlive(actor)) continue;
      try {
        this.untrackTabDecoration(actor);
        if (actorAlive(layer) && typeof layer.remove_child === "function")
        ...
        if (actor) actor._forgeDisposed = true;
