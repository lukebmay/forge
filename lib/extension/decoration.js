...
import { isElevatedSelection, resolveOpsTarget } from "./layout-unit.js";
...
export class DecorationManager extends GObject.Object {
  ...
  /**
   * All focus/split border St actors Forge created (window_group children).
   * Hide/destroy walks this so orphans cannot stick after actor cache loss.
   * @type {Set<object>}
   */
  _borderRegistry;
  ...
  constructor(tree, extWm) {
    ...
    this._tree = tree;
    this._extWm = extWm;
    this._borderRegistry = new Set();
  }

  /**
   * @param {object|null|undefined} border
   */
  _registerBorder(border) {
    if (border) this._borderRegistry.add(border);
  }

  /**
   * @param {object|null|undefined} border
   */
  _unregisterBorder(border) {
    if (border) this._borderRegistry.delete(border);
  }

  /**
   * Hide a border actor; collapse geometry so a missed hide is less visible.
   * @param {object|null|undefined} border
   */
  _hideBorderActor(border) {
    if (!border) return;
    try {
      border.hide();
      if (typeof border.set_size === "function") border.set_size(1, 1);
      if (typeof border.set_position === "function") border.set_position(-10000, -10000);
    } catch (e) {
  ...
  /**
   * Remove + destroy a border actor and drop it from the registry.
   * @param {object|null|undefined} border
   */
  _destroyBorderActor(border) {
    if (!border) return;
    try {
      border.hide();
      // Always try remove_child when window_group exists (matches pre-registry
      // path). contains() can false-negative in tests / mid-teardown.
      if (global.window_group) {
        try {
          global.window_group.remove_child(border);
        } catch (_e) {
          // not a child / already removed
        }
      }
      border.destroy?.();
    } catch (e) {
      Logger.warn(`Failed to destroy border: ${e}`);
    }
    this._unregisterBorder(border);
  }

  hideActorBorder(actor) {
    // Ensure borders are hidden regardless of state (#268)
    if (actor && actor.border) {
      this._hideBorderActor(actor.border);
    }
    this._hideSplitBorders(actor);
  }

  /**
   * Resolve the compositor actor that holds border widgets.
   * Prefer live get_compositor_private over a stale Node._actor cache.
   * Reattach node-owned borders when the live actor lost props.
   * @param {import('./tree.js').Node} nodeWindow
   * @returns {object|null}
   */
  _resolveWindowActor(nodeWindow) {
    if (!nodeWindow) return null;
    let live = null;
    try {
      const meta = nodeWindow.nodeValue;
      live = meta?.get_compositor_private?.() ?? null;
      if (live && nodeWindow._actor !== live) nodeWindow._actor = live;
    } catch (_e) {
      /* finalized */
    }
    const actor = live || nodeWindow.windowActor || null;
    if (actor) this._reattachNodeBorders(nodeWindow, actor);
    return actor;
  }

  /**
   * Borders live on the compositor actor *and* the Node. Restore props if Meta
   * returns a private without them, so hide/show never loses the St.Bin.
   * @param {import('./tree.js').Node} nodeWindow
   * @param {object} actor
   */
  _reattachNodeBorders(nodeWindow, actor) {
    if (!nodeWindow || !actor) return;
    if (nodeWindow._focusBorder) {
      if (!actor.border) {
        actor.border = nodeWindow._focusBorder;
      } else if (actor.border !== nodeWindow._focusBorder) {
        this._destroyBorderActor(nodeWindow._focusBorder);
        nodeWindow._focusBorder = actor.border;
        this._registerBorder(actor.border);
      }
    } else if (actor.border) {
      nodeWindow._focusBorder = actor.border;
      this._registerBorder(actor.border);
    }

    if (Array.isArray(nodeWindow._splitBorders) && nodeWindow._splitBorders.length) {
      if (!Array.isArray(actor.splitBorders) || actor.splitBorders.length === 0) {
        actor.splitBorders = nodeWindow._splitBorders.slice();
        actor.splitBorder = actor.splitBorders[0] || null;
      }
    } else if (Array.isArray(actor.splitBorders) && actor.splitBorders.length) {
      nodeWindow._splitBorders = actor.splitBorders.slice();
      for (const b of actor.splitBorders) this._registerBorder(b);
    }
  }

  /**
   * Create or reuse the focus-border St.Bin. Always starts hidden.
   * @param {object} windowActor
   * @param {import('./tree.js').Node|null} [nodeWindow]
   * @returns {object|null}
   */
  ensureFocusBorder(windowActor, nodeWindow = null) {
    if (!windowActor) return null;
    if (windowActor.border) {
      this._registerBorder(windowActor.border);
      if (nodeWindow) nodeWindow._focusBorder = windowActor.border;
      return windowActor.border;
    }
    if (nodeWindow?._focusBorder) {
      windowActor.border = nodeWindow._focusBorder;
      this._registerBorder(windowActor.border);
      return windowActor.border;
    }
    try {
      const border = new St.Bin({ style_class: "window-tiled-border" });
      border.reactive = false;
      border.track_hover = false;
      if (global.window_group) global.window_group.add_child(border);
      border.hide();
      windowActor.border = border;
      if (nodeWindow) nodeWindow._focusBorder = border;
      this._registerBorder(border);
      return border;
    } catch (e) {
      Logger.warn(`Failed to create focus border: ${e}`);
      return null;
    }
  }

  /** Hide elevated CON selection bag border (S1). */
  hideSelectionBorder() {
    const border = this._selectionBorder;
    if (!border) return;
    this._hideBorderActor(border);
  }

  /** Tear down selection bag actor on disable. */
  destroySelectionBorder() {
    const border = this._selectionBorder;
    if (!border) return;
    this._destroyBorderActor(border);
    this._selectionBorder = null;
  }

  /**
   * Ensure a single St.Bin for selection bag chrome.
   * @returns {St.Bin|null}
   */
  _ensureSelectionBorder() {
    if (this._selectionBorder) return this._selectionBorder;
    try {
      const border = new St.Bin({ style_class: "window-selection-border" });
      border.reactive = false;
      border.track_hover = false;
      if (global.window_group) global.window_group.add_child(border);
      border.hide();
      this._selectionBorder = border;
      this._registerBorder(border);
      return border;
    } catch (e) {
      Logger.warn(`Failed to create selection border: ${e}`);
      return null;
    }
  }
  ...
  hideWindowBorders() {
    // Global registry first: catches orphans when Node/actor lost the prop.
    for (const border of this._borderRegistry) {
      this._hideBorderActor(border);
    }
    ...
    nodeWindows.forEach((nodeWindow) => {
      // Node-owned borders even when compositor private is null.
      this._hideBorderActor(nodeWindow._focusBorder);
      if (Array.isArray(nodeWindow._splitBorders)) {
        for (const b of nodeWindow._splitBorders) this._hideBorderActor(b);
      }

      const actor = this._resolveWindowActor(nodeWindow);
      if (actor) {
  ...
  showWindowBorders() {
    ...
    // Live actor + refresh cache (same path as hide).
    let windowActor = this._resolveWindowActor(nodeWindow);
    ...
    // Ensure focus border exists (map-time create can race null compositor).
    // Floats also get a border actor so floated class can paint when enabled.
    const tiledBorder = this.ensureFocusBorder(windowActor, nodeWindow);
    ...
    // Prefer tree renderRect when Meta frame is still on the wrong mon/size
    // (cross-mon place lag); else Meta frame. Keeps outlines on the live slot.
    let rect = this._borderRectForWindow(nodeWindow, metaWindow);
    ...
    // S1: elevated CON selection bag — full CON rect, separate class from focus.
    this._paintSelectionBorder(nodeWindow, { inset, tilingModeEnabled, focusBorderEnabled });
  }

  /**
   * Paint loud bag chrome when ops target is an elevated CON (≠ focus leaf).
   * Focus border path above is unchanged.
   *
   * @param {object} focusNodeWindow
   * @param {{ inset: number, tilingModeEnabled: boolean, focusBorderEnabled: boolean }} opts
   */
  _paintSelectionBorder(focusNodeWindow, opts) {
    if (!opts.tilingModeEnabled || !opts.focusBorderEnabled) {
      this.hideSelectionBorder();
      return;
    }
    ...
    const attach = this._tree.attachNode;
    if (!isElevatedSelection(attach, focusNodeWindow)) {
      this.hideSelectionBorder();
      return;
    }

    const con = resolveOpsTarget(attach, focusNodeWindow);
    if (!con || con === focusNodeWindow) {
      this.hideSelectionBorder();
      return;
    }
    ...
    let rect = con.rect;
    if (!rect || rect.width <= 0 || rect.height <= 0) {
      // Fallback: union of tiled window frames under the CON
      rect = this._unionTiledFrames(con);
    }
    if (!rect || rect.width <= 0 || rect.height <= 0) {
      this.hideSelectionBorder();
      return;
    ...
    const border = this._ensureSelectionBorder();
    if (!border) return;

    const inset = opts.inset ?? 0;
    const width = Math.max(rect.width + inset * 2, 1);
    const height = Math.max(rect.height + inset * 2, 1);
    border.set_style_class_name("window-selection-border");
    border.set_size(width, height);
    border.set_position(rect.x - inset, rect.y - inset);
    border.show();

    if (global.window_group && global.window_group.contains(border)) {
      // Above focus border so the bag reads as the ops unit.
      global.window_group.remove_child(border);
  ...
  /**
   * Bounding rect of tiled WINDOW frames under `con` (fallback when con.rect missing).
   * @param {object} con
   * @returns {{ x: number, y: number, width: number, height: number }|null}
   */
  _unionTiledFrames(con) {
    if (!con) return null;
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    ...
    const visit = (node) => {
      if (!node) return;
      if (
        typeof node.isWindow === "function" ? node.isWindow() : node.nodeType === NODE_TYPES.WINDOW
      ) {
        if (node.isFloat?.()) return;
        const meta = node.nodeValue;
        const fr = meta?.get_frame_rect?.();
        if (!fr || fr.width <= 0 || fr.height <= 0) return;
        any = true;
        minX = Math.min(minX, fr.x);
        minY = Math.min(minY, fr.y);
        maxX = Math.max(maxX, fr.x + fr.width);
        maxY = Math.max(maxY, fr.y + fr.height);
        return;
      }
  ...
  /**
   * Geometry for focus/split borders.
   * Tiled: prefer tree slot when Meta lags (wrong mon/size/pos) — main cause of
   * leftover smaller red/yellow outlines after resize/move on Wayland.
   * Float / missing slot: Meta frame.
   * @param {import('./tree.js').Node} nodeWindow
   * @param {Meta.Window} metaWindow
   * @returns {{ x: number, y: number, width: number, height: number }|null}
   */
  _borderRectForWindow(nodeWindow, metaWindow) {
    let frame = null;
    try {
      frame = metaWindow?.get_frame_rect?.() ?? null;
    } catch (_e) {
      frame = null;
    }
    const slot = nodeWindow?.renderRect || nodeWindow?.rect || null;
    const slotOk = slot && slot.width > 0 && slot.height > 0;
    const frameOk = frame && frame.width > 0 && frame.height > 0;

    // Floats are not tree-placed; Meta frame is authoritative.
    if (nodeWindow?.isFloat?.()) return frameOk ? frame : slotOk ? slot : null;

    if (slotOk) {
      if (!frameOk) return slot;
      try {
        const metaMon = metaWindow.get_monitor?.();
        const treeMon = this._extWm._monitorIndexOfNode
          ? this._extWm._monitorIndexOfNode(nodeWindow)
          : -1;
        if (typeof metaMon === "number" && typeof treeMon === "number" && treeMon >= 0) {
          if (metaMon !== treeMon) return slot;
        }
      } catch (_e) {
        /* keep comparing geometry */
      }
      // Tight threshold: leftover "smaller" outlines after layout thrash.
      const areaRatio = (frame.width * frame.height) / Math.max(slot.width * slot.height, 1);
      if (areaRatio < 0.85 || areaRatio > 1.2) return slot;
      const dx = Math.abs(frame.x - slot.x);
      const dy = Math.abs(frame.y - slot.y);
      const dw = Math.abs(frame.width - slot.width);
      const dh = Math.abs(frame.height - slot.height);
      if (dx > 16 || dy > 16 || dw > 16 || dh > 16) return slot;
      return frame;
    }
    return frameOk ? frame : null;
  }
  ...
  _paintSplitChromeOnWindow(nodeWindow, axes, opts) {
    ...
    // Rehydrate from node if actor list empty
    if (
      windowActor.splitBorders.length === 0 &&
      Array.isArray(nodeWindow._splitBorders) &&
      nodeWindow._splitBorders.length
    ) {
      windowActor.splitBorders = nodeWindow._splitBorders.slice();
    }
    ...
    while (windowActor.splitBorders.length < axes.length) {
      ...
      splitBorder.reactive = false;
      splitBorder.track_hover = false;
      global.window_group.add_child(splitBorder);
      this._registerBorder(splitBorder);
    ...
    for (let i = axes.length; i < windowActor.splitBorders.length; i++) {
      this._hideBorderActor(windowActor.splitBorders[i]);
    }
    ...
    // Legacy single pointer for destroy/hide paths + node ownership
    windowActor.splitBorder = windowActor.splitBorders[0] || null;
    nodeWindow._splitBorders = windowActor.splitBorders.slice();
  }
  ...
  /**
   * Destroy focus or split borders on an actor (window destroy / disable).
   * @param {object|null|undefined} actor
   * @param {"border"|"splitBorder"} propName
   * @param {import('./tree.js').Node|null} [nodeWindow]
   */
  _destroyActorBorder(actor, propName, nodeWindow = null) {
    if (propName === "splitBorder") {
      ...
      if (nodeWindow && Array.isArray(nodeWindow._splitBorders)) {
        for (const b of nodeWindow._splitBorders) {
      ...
      if (actor) {
        actor.splitBorders = undefined;
        actor.splitBorder = undefined;
      }
      if (nodeWindow) nodeWindow._splitBorders = undefined;
      return;
    }
    // focus border
    const border = (actor && actor[propName]) || nodeWindow?._focusBorder || null;
    this._destroyBorderActor(border);
    if (actor) actor[propName] = undefined;
    if (nodeWindow) nodeWindow._focusBorder = undefined;
  }

  /**
   * Destroy every registered border (disable / hard reset).
   */
  destroyAllBorders() {
    for (const border of [...this._borderRegistry]) {
      this._destroyBorderActor(border);
    }
    this._borderRegistry.clear();
