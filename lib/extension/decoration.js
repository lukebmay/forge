...
export class DecorationManager extends GObject.Object {
  ...
  _resolveWindowActor(nodeWindow) {
    ...
    try {
      ...
      const live = meta?.get_compositor_private?.();
      if (live) {
        if (nodeWindow._actor !== live) nodeWindow._actor = live;
        return live;
      }
    } catch (_e) {
      /* finalized */
    }
    return nodeWindow.windowActor || null;
  }
  ...
  hideSelectionBorder() {
    ...
    try {
      ...
      Logger.warn(`Failed to hide selection border: ${e}`);
    }
  }
  ...
  destroySelectionBorder() {
    ...
    try {
      ...
      Logger.warn(`Failed to destroy selection border: ${e}`);
    }
  ...
  hideWindowBorders() {
    ...
    nodeWindows.forEach((nodeWindow) => {
      // Live actor: map-time _actor can be null while actor.border still shows
      // at an old size after resize/move (obstructs the new tile).
  ...
  showWindowBorders() {
    ...
    // Ensure focus border actor exists (map-time create can race null compositor).
    if (!windowActor.border && !floatingWindow) {
      try {
  ...
  /**
   * Geometry for focus/split borders: tree slot when Meta lags (wrong mon/size).
   * @param {import('./tree.js').Node} nodeWindow
  ...
  _borderRectForWindow(nodeWindow, metaWindow) {
    ...
    if (!slot || slot.width <= 0 || slot.height <= 0) return frame;
    if (!frame || frame.width <= 0 || frame.height <= 0) return slot;

    // Prefer tree slot when Meta is on a different monitor than the tree home.
    try {
      const metaMon = metaWindow.get_monitor?.();
      const monNode = this._tree.findAncestorMonitor?.(nodeWindow);
      const treeMon =
        monNode && this._extWm._monitorIndexOfNode
    ...
    // Or when frame is much smaller than the slot (stale post-resize Meta).
    if (frame.width * frame.height < slot.width * slot.height * 0.5) return slot;
    return frame;
  }
