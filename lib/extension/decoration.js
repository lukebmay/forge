...
export class DecorationManager extends GObject.Object {
   ...
   * Tiled: always use the tree slot (renderRect/rect). Meta frames on Wayland
   * (esp. Chrome PWAs) often report wrong width (~1/3–1/4 mon while the tile is
   * 1/2) — that painted a smaller cyan/blue ring over content.
  ...
  _borderRectForWindow(nodeWindow, metaWindow) {
    ...
    // TILE: tree owns geometry. Never outline from lagging Meta frames.
    if (slotOk) return slot;
