...
export class DecorationManager extends GObject.Object {
  ...
  showWindowBorders() {
    ...
    // Prefer the tile slot (renderRect/rect) over Meta frame. On Wayland clients
    // (Chrome PWAs especially) get_frame_rect() often lags or reports a sliver
    // while the tree slot is already correct — focus border must match the tile.
    let rect = null;
  ...
  /**
   * Destroy window_group forge-deco actors not owned by any live CON.
  ...
  _sweepOrphanDecorations(allCons) {
    ...
    let children;
    try {
      children = global.window_group?.get_children?.() ?? [];
    } catch (_e) {
      return;
    }
