...
export function computeSizes(node, childItems, getTiledChildren) {
  ...
  // Explicit percent when >0 else equal-magic 1/n, then renormalize so mixed
  // [1.0, 0] does not sum >1 and crush remainder into 0/negative widths.
  const equal = n > 0 ? 1.0 / n : 1.0;
  const weights = childItems.map((childNode) =>
    childNode.percent && childNode.percent > 0.0 && !grabTiled ? childNode.percent : equal
  );
  const weightSum = weights.reduce((a, b) => a + b, 0) || 1.0;
  for (let i = 0; i < n; i++) {
    sizes[i] = Math.floor((weights[i] / weightSum) * totalSize);
  }
  ...
  if (totalAllocated !== totalSize && n > 0) {
    let foldIndex = minTotal > 0 ? mostShrinkableIndex(sizes, mins) : n - 1;
    sizes[foldIndex] += totalSize - totalAllocated;
  }

  // Never leave a negative size after remainder fold.
  let clamped = false;
  for (let i = 0; i < n; i++) {
    if (sizes[i] < 0) {
      sizes[i] = 0;
  ...
  if (clamped && n > 0) {
    totalAllocated = sizes.reduce((a, b) => a + b, 0);
    if (totalAllocated !== totalSize) {
      ...
      sizes[foldIndex] = Math.max(0, sizes[foldIndex] + (totalSize - totalAllocated));
    }
  }
