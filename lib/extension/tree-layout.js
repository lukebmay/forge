 ...
 * @param {number} height
 */
export function rememberClassMin(wmClass, width, height) {
  if (!wmClass) return;
  const key = String(wmClass);
  const prev = _classMinFloor.get(key) || { width: 0, height: 0 };
  const w = Number(width);
  const h = Number(height);
  _classMinFloor.set(key, {
    width: Number.isFinite(w) && w > 0 ? Math.max(prev.width || 0, w) : prev.width || 0,
    height: Number.isFinite(h) && h > 0 ? Math.max(prev.height || 0, h) : prev.height || 0,
  });
}
...
/**
 * Best-effort client min size (hints, get_min_size, learned clamp, class floor).
 * Missing → { width: 0, height: 0 } (callers fail-open).
 * @param {any} meta Meta.Window-like
 * @returns {{ width: number, height: number }}
 */
export function readWindowMinSize(meta) {
  ...
  if (Number.isFinite(knownW) && knownW > 1200) {
    delete meta._forgeKnownMinW;
    knownW = 0;
  }
  if (Number.isFinite(knownH) && knownH > 800) {
  ...
  if (!width || !height) {
    const floor = classMinFloor(_wmClassOf(meta));
    if (!width && floor.width > 0) width = floor.width;
    if (!height && floor.height > 0) height = floor.height;
  }
  return { width: width > 0 ? width : 0, height: height > 0 ? height : 0 };
}

/** Ignore size-changed races right after move_resize (ms). */
...
export function noteWindowMinFromClamp(meta, requested, frame, eps = 4, nowMs = Date.now()) {
  ...
  if (Number.isFinite(rw) && Number.isFinite(fw) && Math.abs(fw - rw) <= eps) {
    ...
    // Learn only after delay: settled size still above request.
    // Prefer frame; if still glued to prior, prior is the floor.
    let learned = fw;
    if (Number.isFinite(pw) && Math.abs(fw - pw) <= eps) learned = pw;
    if (learned > rw + eps) {
      meta._forgeKnownMinW = Math.max(Number(meta._forgeKnownMinW) || 0, learned);
    }
  }
  ...
  if (Number.isFinite(rh) && Number.isFinite(fh) && Math.abs(fh - rh) <= eps) {
    ...
    let learned = fh;
    if (Number.isFinite(ph) && Math.abs(fh - ph) <= eps) learned = ph;
    if (learned > rh + eps) {
      meta._forgeKnownMinH = Math.max(Number(meta._forgeKnownMinH) || 0, learned);
    }
  }
