...
/** Absurd learn / persist caps (pre-delay race with large frames). */
export const CLASS_MIN_ABSURD_W = 1200;
export const CLASS_MIN_ABSURD_H = 800;
...
export function noteWindowMinFromClamp(meta, requested, frame, eps = 4, nowMs = Date.now()) {
  ...
  // Probe / grab restore must not raise mins from a racing large frame.
  if (meta._forgeMinProbing) return;
  ...
  if (Number.isFinite(rw) && Number.isFinite(fw) && Math.abs(fw - rw) <= eps) {
    ...
    if (Number.isFinite(pw) && Math.abs(fw - pw) <= eps) {
      ...
      // Frame settled above request and below prior → real clamp.
