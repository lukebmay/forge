...
import { frameUndersizedVsCommand } from "./geom-epsilon.js";
...
/** After D115 jitter, observed undersize floors may exceed clamp caps (Inkscape 651). */
export const CLASS_MIN_HEAL_ABSURD_W = 1920;
export const CLASS_MIN_HEAL_ABSURD_H = 1200;
 ...
 * @param {number} width
 * @param {number} height
 * @param {{ lower?: boolean, silent?: boolean, healUndersize?: boolean }} [opts]
 */
export function rememberClassMin(wmClass, width, height, opts = {}) {
  ...
  const capW = opts.healUndersize ? CLASS_MIN_HEAL_ABSURD_W : CLASS_MIN_ABSURD_W;
  const capH = opts.healUndersize ? CLASS_MIN_HEAL_ABSURD_H : CLASS_MIN_ABSURD_H;
  const nextW =
    Number.isFinite(w) && w > 0 && w <= capW
      ? lower
        ? prev.width > 0
...
function _capFloorDims(width, height) {
  ...
  const h = Number(height);
  return {
    width: Number.isFinite(w) && w > 0 && w <= CLASS_MIN_HEAL_ABSURD_W ? w : 0,
    height: Number.isFinite(h) && h > 0 && h <= CLASS_MIN_HEAL_ABSURD_H ? h : 0,
  };
}
...
export function parseWindowMinsJson(text) {
  try {
    ...
    for (const [k, v] of Object.entries(classes)) {
      ...
      const rawW = Number(v.width);
      const rawH = Number(v.height);
      if (rawW > CLASS_MIN_HEAL_ABSURD_W || rawH > CLASS_MIN_HEAL_ABSURD_H) continue;
      const c = _capFloorDims(v.width, v.height);
      if (c.width > 0 || c.height > 0) out[k] = c;
    }
    return out;
...
export function noteWindowMinFromClamp(meta, requested, frame, eps = 4, nowMs = Date.now()) {
  ...
  const fw = Number(frame.width);
  const fh = Number(frame.height);
  const band = Math.max(Number(eps) || 4, 12);
  if (
    (Number.isFinite(rw) && Number.isFinite(fw) && rw - fw > band) ||
    (Number.isFinite(rh) && Number.isFinite(fh) && rh - fh > band)
  ) {
    Logger.debug(`min-learn skip reason=undersize-vs-command fw=${fw} rw=${rw} fh=${fh} rh=${rh}`);
    return;
  }
...
/**
 * After jitter retries, the observed undersize frame is this class's min (D115 H3).
 * R062 skip in noteWindowMinFromClamp stays until this path.
 * @param {any} meta
 * @param {{ width?: number, height?: number }|null|undefined} observed
 * @param {{ width?: number, height?: number }|null|undefined} desired
 * @param {{ silent?: boolean }} [opts]
 * @returns {boolean}
 */
export function noteWindowMinFromHealUndersize(meta, observed, desired, opts = {}) {
  if (!meta || !observed) return false;
  if (!frameUndersizedVsCommand(desired, observed)) return false;
  const fw = Number(observed.width);
  const fh = Number(observed.height);
  if (!(fw > 0) || !(fh > 0)) return false;
  if (fw > CLASS_MIN_HEAL_ABSURD_W || fh > CLASS_MIN_HEAL_ABSURD_H) return false;
  meta._forgeKnownMinW = Math.max(Number(meta._forgeKnownMinW) || 0, fw);
  meta._forgeKnownMinH = Math.max(Number(meta._forgeKnownMinH) || 0, fh);
  rememberClassMin(_wmClassOf(meta), fw, fh, {
    healUndersize: true,
    silent: !!opts.silent,
  });
  return true;
...
/**
 * Learn mins when a settled TILE frame is larger than its tree slot on an axis.
 * Only axes that overflow by >ε; skips absurd frames (same caps as clamp-learn).
 * @param {any} meta
 * @param {{ width?: number, height?: number }|null} frame
 * @param {{ width?: number, height?: number }|null} slot
 * @param {number} [eps=4]
 * @returns {boolean} true when known mins were raised
 */
export function noteWindowMinFromOversizedFrame(meta, frame, slot, eps = 4) {
  if (!meta || !frame || !slot) return false;
  const fw = Number(frame.width);
  const fh = Number(frame.height);
  ...
  let raised = false;
  if (Number.isFinite(fw) && fw > sw + e) {
    if (fw > CLASS_MIN_ABSURD_W) {
      Logger.debug(`min-learn-oversized skip axis=w reason=absurd fw=${fw} sw=${sw}`);
    } else {
      meta._forgeKnownMinW = Math.max(Number(meta._forgeKnownMinW) || 0, fw);
      raised = true;
      Logger.trace(`min-learn-oversized axis=w fw=${fw} sw=${sw}`);
    }
  }
  if (Number.isFinite(fh) && fh > sh + e) {
    if (fh > CLASS_MIN_ABSURD_H) {
      Logger.debug(`min-learn-oversized skip axis=h reason=absurd fh=${fh} sh=${sh}`);
    } else {
      meta._forgeKnownMinH = Math.max(Number(meta._forgeKnownMinH) || 0, fh);
      raised = true;
      Logger.trace(`min-learn-oversized axis=h fh=${fh} sh=${sh}`);
    }
  }
...
/**
 * Settled frame overflows slot on an axis by >ε and is within learn caps
 * (so oversized-frame learn can raise mins). Absurd frames → false (D026).
 * @param {{ width?: number, height?: number }|null|undefined} frame
 * @param {{ width?: number, height?: number }|null|undefined} slot
 * @param {number} [eps=4]
 * @returns {boolean}
 */
export function frameOverflowsSlotForLearn(frame, slot, eps = 4) {
  const fw = Number(frame?.width);
  const fh = Number(frame?.height);
  const sw = Number(slot?.width);
  const sh = Number(slot?.height);
  if (!(sw > 0) || !(sh > 0)) return false;
  const e = Number.isFinite(Number(eps)) ? Math.abs(Number(eps)) : 4;
  const overW = Number.isFinite(fw) && fw > sw + e && fw <= CLASS_MIN_ABSURD_W;
  const overH = Number.isFinite(fh) && fh > sh + e && fh <= CLASS_MIN_ABSURD_H;
  return overW || overH;
}
...
export function computeSizes(node, childItems, getTiledChildren, opts = {}) {
  ...
  // TABBED/STACKED peers share one full content slot (processTabbed/Stacked).
  // Do not slice by stale sibling percents from a former H/V split.
  if (node.layout === TABBED || node.layout === STACKED) {
    return childItems.map(() => totalSize);
  }

  const rawPercents = childItems.map((childNode) =>
    childNode.percent && childNode.percent > 0.0 && !grabTiled
      ? childNode.percent
      : 1.0 / childItems.length
  );
  let weights = rawPercents;
  if (opts.renormalize && rawPercents.length) {
    const sum = rawPercents.reduce((a, b) => a + b, 0);
    if (sum > 0) weights = rawPercents.map((p) => p / sum);
  }
  childItems.forEach((_childNode, index) => {
    sizes[index] = Math.floor(weights[index] * totalSize);
  });
