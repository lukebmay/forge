 ...
 * @param {Forest} forest
 */
function takeSkeletonLiveWin(queue, forest) {
  while (queue.length) {
    const w = queue.shift();
    if (w && forest.nodes[w.id]?.kind === "WINDOW") return w;
  }
  return null;
}
 ...
 */
function forestSkeletonPlaceWin(wm, forest, parentTom, slot, role, created, liveQueue) {
  const existing = takeSkeletonLiveWin(liveQueue, forest);
  if (existing) {
 ...
 * @returns {{ error?: string }|null}
 */
function forestSkeletonBuildChild(wm, forest, parentTom, spec, created, liveQueue = []) {
  ...
  if (Array.isArray(nested) && nested.length > 0) {
    ...
    for (const sub of nested) {
      const err = forestSkeletonBuildChild(wm, forest, made.con, sub, created, liveQueue);
  ...
  if (mode === "tabbed" || mode === "stacked" || roles.length > 1) {
    ...
    for (const role of roleList) {
      const err = forestSkeletonPlaceWin(wm, forest, made.con, slot, role, created, liveQueue);
      if (err?.error) return err;
    }
    equalizeChildren(forest, made.con, { force: true });
    return null;
  ...
  return forestSkeletonPlaceWin(wm, forest, parentTom, slot, role, created, liveQueue);
}
...
export function forestApplySkeletonMon(wm, monLive, monSpec) {
  ...
  /** @type {object[]} */
  const created = [];
  const liveQueue = [...liveWins];
  const childrenSpec = monSpec.children || [];
  for (const ch of childrenSpec) {
    const err = forestSkeletonBuildChild(wm, forest, mon, ch, created, liveQueue);
