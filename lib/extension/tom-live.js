...
import { processGap as layoutProcessGap } from "./tree-layout.js";
...
export function liveTabOpenLeafForPresent(wm, con) {
  ...
  if (forestSeededWithWindows(wm)) {
    ...
    if (tom?.lastTabFocusId) {
      const focusLive = wm.liveById.get(tom.lastTabFocusId);
      if (focusLive && liveKind(focusLive) === "WINDOW") return focusLive;
    }
    const kids = liveChildrenForPresent(wm, con);
    if (con.lastTabFocus) {
      for (const child of kids) {
        if (liveKind(child) === "WINDOW" && child.nodeValue === con.lastTabFocus) return child;
      }
    }
    for (const child of kids) {
  ...
  try {
    ...
    if (root) {
      walk(forest, root, (node) => {
        ...
        const gap =
          typeof wm.calculateGaps === "function"
            ? wm.calculateGaps(live || { nodeValue: meta })
            : 0;
        let dest = layoutProcessGap({ rect: slot, nodeValue: meta, isWindow: () => true }, gap);
        const mon = ancestorMonitor(forest, node);
        const monGeom = mon ? geomOf(forest, mon) : null;
        if (live?.zoomMode && monGeom) {
