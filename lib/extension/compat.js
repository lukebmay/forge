 ...
 *
 * Shell / Mutter API version-dispatch shims. Meta.Window maximize drift and
 * St.BoxLayout orientation live here. Callers import as
 * `import * as Compat from "./compat.js"` and use Compat.<shim>(…).
 *
...
/** St.BoxLayout.orientation lands in Shell 48; vertical removed at 50. */
export const IS_ST_ORIENTATION = SHELL_MAJOR >= 48;
...
/**
 * Point St.BoxLayout along HORIZONTAL or VERTICAL.
 *
 * GNOME 45–47 only honor `.vertical`. Assigning `.orientation` there is a silent
 * JS own-property no-op, so STACKED chrome stays a horizontal strip at N× bar
 * height ("tabbed but taller"). GNOME 48+ use `.orientation`; `.vertical` is
 * deprecated then removed at 50.
 *
 * @param {St.BoxLayout} box
 * @param {number} orientation Clutter.Orientation.HORIZONTAL | VERTICAL
 */
export function setBoxOrientation(box, orientation) {
  if (!box) return;
  if (IS_ST_ORIENTATION) {
