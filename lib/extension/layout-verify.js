...
/**
 * Exact finite-field equality (desired/projected identity). Nulls never equal.
 *
 * @param {{ x?: number, y?: number, width?: number, height?: number }|null|undefined} a
...
/**
 * Clone a normalized rect, or null.
 * @param {{ x?: number, y?: number, width?: number, height?: number }|null|undefined} r
 * @returns {{ x: number, y: number, width: number, height: number }|null}
 */
export function cloneRect(r) {
...
export function collectTileVerifyInputs(nodes, opts = {}) {
  ...
  for (const node of list) {
    ...
    // AC4: placeholders have no Meta frame — skip sensor scan (not thrash).
    if (isPlaceholderNode(node) || node.placeholder === true) continue;
