...
function walkCollect(node, path, out) {
  ...
  if (isWindowNode(node)) {
    ...
    for (let i = 0; i < nested.length; i++) {
      walkCollect(nested[i], `${path}/${i}`, out);
    }
    return;
  }
...
export function matchNodes(rootOrForest, descriptorOrInput, ctx = {}) {
  ...
  switch (descriptor.kind) {
    ...
    case "id":
      return { matches: matchId(rootOrForest, descriptor, ctx), descriptor };
    case "path":
...
function matchClass(rootOrForest, descriptor, ctx) {
  const all = collectWindows(rootOrForest);
  const want = String(descriptor.value ?? "").toLowerCase();
  let filtered = all.filter((c) => String(c.wmClass ?? "").toLowerCase() === want);
...
/**
 * Detached FLOAT live from bag / liveById when TREE walk misses it.
 * @param {any} ctx
 * @param {any} meta
 * @param {string|null|undefined} nid
 * @param {any} want
 * @returns {MatchCandidate[]}
 */
function candidatesFromDetachedLive(ctx, meta, nid, want) {
  let live = null;
  if (typeof ctx.findNode === "function" && meta) {
    try {
      live = ctx.findNode(meta);
    } catch (_e) {
      live = null;
    }
  }
  if (!live && ctx.liveById instanceof Map) {
    const key = nid || (want != null ? String(want) : "");
    if (key) live = ctx.liveById.get(key) || null;
  }
  if (!live) return [];
  const metaFields = candidateMeta(live);
  return [
    {
      node: live,
      nodeType: metaFields.nodeType || "WINDOW",
      windowId: want != null && want !== "" ? want : metaFields.windowId,
      title: metaFields.title,
      wmClass: metaFields.wmClass,
      path: "FLOATS",
    },
  ];
}

function matchId(rootOrForest, descriptor, ctx = {}) {
  const want = descriptor.value;
  const all = collectWindows(rootOrForest);
  const direct = all.filter((c) => {
    if (c.windowId == null) return false;
    return String(c.windowId) === String(want);
  });
  if (direct.length) return direct;

  // Forest nanoid ↔ Meta id via host bag (C6 Apply keys).
  const bag = ctx.hostBag;
  if (!bag || want == null || want === "") return [];
  let nid = null;
  let meta = bag.get?.(String(want))?.meta;
  if (meta) nid = String(want);
  if (!meta && typeof bag.idFromWindowId === "function") {
    nid = bag.idFromWindowId(want) || null;
    if (nid) meta = bag.get?.(nid)?.meta;
  }
  if (!meta || typeof meta !== "object") {
    return candidatesFromDetachedLive(ctx, null, nid || String(want), want);
  }
  const metaId =
    typeof meta.get_id === "function"
      ? (() => {
          try {
            return meta.get_id();
          } catch (_e) {
      ...
      : meta.id;
  if (metaId == null) {
    return candidatesFromDetachedLive(ctx, meta, nid, want);
  }
  const byMeta = all.filter((c) => c.windowId != null && String(c.windowId) === String(metaId));
  if (byMeta.length) return byMeta;
  // Paint-detached FLOATS: not under MONITOR childNodes.
  return candidatesFromDetachedLive(ctx, meta, nid, want);
}
