 ...
 * - ApplyLayout clears after hard+soft (D043) so jitter/soft notices can show;
 *   Done still clears on error/early exit.
...
/** @deprecated Timed first-apply hint removed; kept for old test imports. */
export const LAYOUT_APPLY_CHROME_HINT_MS = 2500;

/** Settle/soft thrash line under the detail (prod + dev). */
export const LAYOUT_APPLY_CHROME_JITTER_NOTICE = "Settle jitter detected; check logs.";

/** Soft residual failure line. */
export const LAYOUT_APPLY_CHROME_SOFT_FAIL_NOTICE = "Settle soft-failure; check logs.";
...
/**
 * Stage line for the apply chrome checklist (dev).
 * @param {{ label?: string, status?: string }} stage
 * @returns {string}
 */
export function formatApplyChromeStageLine(stage) {
  const label = stage?.label != null ? String(stage.label).trim() : "";
  if (!label) return "";
  const st = String(stage?.status || "pending").toLowerCase();
  let mark = "…";
  if (st === "ok" || st === "done" || st === "pass") mark = "✓";
  else if (st === "fail" || st === "error" || st === "failed") mark = "✗";
  else if (st === "retry" || st === "jitter") mark = "↻";
  return `${label} ${mark}`;
}
 ...
 *   longRunning?: boolean,
 * }} [opts]
 * @returns {{ title: string, detail: string, hint: string, stagesText: string }}
 */
export function formatApplyChromeStatus(layoutName, opts = {}) {
  ...
  const notice = opts.notice != null ? String(opts.notice).trim() : "";
  const stageLines = (Array.isArray(opts.stages) ? opts.stages : [])
    .map((s) => formatApplyChromeStageLine(s))
    .filter((s) => s.length > 0);
  return {
    title: LAYOUT_APPLY_CHROME_TITLE,
    detail: name ? `Loading layout "${name}"...` : "Loading layout...",
    hint: notice,
    stagesText: stageLines.join("\n"),
  };
}
...
export class LayoutApplyChrome {
  ...
  constructor(extWm) {
    ...
    /** @type {string|null} */
    this._notice = null;
    /** @type {Array<{ label: string, status: string }>} */
    this._stages = [];
  ...
  /** Dev install (`production=false`) shows the stage checklist. */
  get debugProgress() {
    return !production;
  }
  ...
  /**
   * Transient settle/jitter line (prod + dev).
   * @param {string|null|undefined} text
   */
  setNotice(text) {
    const raw = text == null ? "" : String(text).trim();
    this._notice = raw.length > 0 ? raw : null;
    this._refreshStatus();
  }

  /**
   * Replace the stage checklist (dev). Ignored when production.
   * @param {Array<{ label?: string, status?: string }>|null|undefined} stages
   */
  setStages(stages) {
    ...
    this._stages = (Array.isArray(stages) ? stages : [])
      .map((s) => ({
        ...
        status: s?.status != null ? String(s.status) : "pending",
      }))
  ...
  /**
   * Upsert one stage row (dev).
   * @param {string} label
   * @param {string} [status]
   */
  setStage(label, status = "pending") {
    if (!this.debugProgress) return;
    const key = label != null ? String(label).trim() : "";
    if (!key) return;
    const st = status != null ? String(status) : "pending";
    const idx = this._stages.findIndex((s) => s.label === key);
    if (idx >= 0) this._stages[idx] = { label: key, status: st };
    else this._stages.push({ label: key, status: st });
    this._refreshStatus();
  }
  ...
  _rebuildStatusHosts() {
    ...
    for (const mon of monitors) {
      ...
      const status = formatApplyChromeStatus(this._layoutName, {
        notice: this._notice,
        stages: this.debugProgress ? this._stages : [],
      });
      ...
      try {
        ...
        if (status.stagesText) {
          const stagesLabel = new St.Label({
            style_class: "forge-layout-apply-chrome-stages",
            text: status.stagesText,
            reactive: false,
            can_focus: false,
            x_align: Clutter.ActorAlign.CENTER,
            style:
              "color: rgba(180, 220, 180, 0.9);" +
              ` font-size: ${Math.max(11, Math.round(m.hintPx * 0.95))}px;` +
              " font-family: monospace;" +
              " font-weight: 400;" +
              " text-align: left;",
          });
          try {
            stagesLabel.clutter_text?.set_line_wrap?.(true);
          } catch (_e) {
            /* */
          }
          panel.add_child(stagesLabel);
        }
      } catch (_e) {
        // St API
      }
