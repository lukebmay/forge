...
function categories() {
  return [
    ...
    { match: /^window-swap-(left|down|up|right)$/, name: _("Join") },
  ...
  _getGroupedKeybindings() {
    ...
    for (const key of keys) {
      ...
      for (const cat of cats) {
        const hit = cat.match ? cat.match.test(key) : key.startsWith(cat.prefix);
        if (hit) {
