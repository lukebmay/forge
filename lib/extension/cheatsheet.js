...
} from "./cheatsheet-group.js";

// Category display names (translatable). Built lazily so gettext resolves at
// render time rather than module load. Pure map lives in cheatsheet-group.js.
function categories() {
  return cheatsheetCategoryDefs().map((d) => ({
    prefix: d.prefix,
    name: _(d.id),
    id: d.id,
  }));
...
export class Cheatsheet extends GObject.Object {
  ...
  _getGroupedKeybindings() {
    ...
    // Map English category id → { displayName, bindings }
    ...
    for (const key of keys) {
      ...
      const categoryId = resolveCategoryId(key) ?? otherId;
      const categoryName = idToName.get(categoryId) ?? otherName;

      if (!groups.has(categoryId)) {
        groups.set(categoryId, { name: categoryName, bindings: [] });
      }
      ...
      groups.get(categoryId).bindings.push({
        key,
    ...
    // Section order from pure defs; within Resize: edges → expand → shrink → golden → reset
    const orderedGroups = [];
    for (const categoryId of [...categoryDisplayOrder(), otherId]) {
      const group = groups.get(categoryId);
      if (!group || group.bindings.length === 0) continue;
      const sorted = sortBindingsInCategory(categoryId, group.bindings).map(
        ({ shortcut, description }) => ({ shortcut, description })
      );
      orderedGroups.push([group.name, sorted]);
    }
