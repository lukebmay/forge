...
export class LayoutApplyRunBag {
  ...
  _runSoftPhase(run) {
    ...
    if (!this._hasSettle()) {
      ...
      // R027: chrome through soft only — verify/belt may still run after.
  ...
  _applySoftResult(run, out) {
    ...
    // Soft is the last "do not touch" gate (R027). Verify/belt can finish without
    // the spinner; user may hover tabs while residual mon moves complete.
    this._clearChrome(run, "soft");
    return { ok: true };
  }

  /**
   * Drop apply chrome once per run (soft settle or terminal). Idempotent.
   * @param {object} run
   * @param {string} reason
   */
