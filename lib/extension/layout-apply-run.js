 ...
 * Product: epoch → materialize forest → maps → focus once (all mapped) →
 * slot machines (Done.ok still forest-match) → soft + verify → Done.
 * Overlay clears at visible-hard (D117). No mid-open/mid-place focus.
...
import {
  ...
  focusAfterAllMappedAllowed as mappedFocusAllowed,
  isVisibleHard,
} from "./layout-apply-visible.js";
import {
  planReconcile,
  planActionsToSteps,
...
/**
 * D008 phase names (logs). Product walk is epoch + maps + focus + machines.
 * D117: focus after all required WINDOW ids exist (mapped), not all-hard.
 */
...
/**
 * True when focus may run: all required WINDOW ids mapped (or no open phase).
 * @param {object|null|undefined} run
 * @param {string[]} [phases]
 * @returns {boolean}
 */
export function focusAfterAllMappedAllowed(run, phases = APPLY_LAYOUT_PHASES) {
  const list = Array.isArray(phases) && phases.length ? phases : APPLY_LAYOUT_PHASES;
  return mappedFocusAllowed(run, list);
}

/** @deprecated D117 — use focusAfterAllMappedAllowed */
export function focusAfterAllHardAllowed(run, phases = APPLY_LAYOUT_PHASES) {
  return focusAfterAllMappedAllowed(run, phases);
}
...
export class LayoutApplyRunBag {
  ...
  _runPhaseWork(run, phase) {
    ...
    if (
      (phase === "focus" || phase === "hard-ready" || phase === "soft") &&
      this._openShouldFail(run)
    ) {
    ...
    if (phase === "soft") {
      if (this._phases.includes("focus") && !run.focusRan) {
        return {
          ok: false,
          error: "soft before focus",
          code: "focus-order",
          phase: "soft",
        };
      }
    ...
    if (phase === "focus") {
      if (!focusAfterAllMappedAllowed(run, this._phases)) {
        return {
          ok: false,
          error: "focus before all mapped",
          code: "focus-order",
          phase: "focus",
        };
      }
    ...
    if (!steps.length) {
      if (phase === "focus") {
        ...
        run.focusRan = true;
        this._maybeClearChromeVisibleHard(run);
      }
   ...
   * Overlay clears at visible-hard (D117); Done.ok stays forest-match.
   * @param {object} run
   */
  _runHardReadyPhase(run) {
    ...
    const session = startSlotMachines(
      machines,
      {
        ...
        onWindowSettled: () => this._maybeClearChromeVisibleHard(run),
   ...
   * After focus. Overlay may already be gone at visible-hard.
   * @param {object} run
   */
  _runSoftPhase(run) {
  ...
  /**
   * Overlay off at visible-hard (D117). Both heads of the apply workspace.
   * Buried TAB members may still settle.
   * @param {object} run
   * @returns {boolean}
   */
  _maybeClearChromeVisibleHard(run) {
    if (!run || run.chromeCleared) return false;
    let forest = null;
    try {
      forest = this._snapshotForSettle(run) || run.forest || null;
    } catch (_e) {
      forest = run.forest || null;
    }
    let wins = [];
    try {
      wins = this._loadSettleWindows(run) || [];
    } catch (_e) {
      wins = [];
    }
    const visibleIds = collectVisibleTileWindowIds(forest, {
      workspace: run.workspace ?? 0,
      includeOtherMonitors: true,
    });
    const { slots } = collectHardReadySlotTargets(run, forest);
    if (!isVisibleHard(wins, visibleIds, { slots })) return false;
    Logger.info("visible-hard overlay clear", {
      fields: {
        applyId: run.applyId || "-",
        reason: "visible-hard",
        n: visibleIds.length,
    ...
    this._clearChrome(run, "visible-hard");
    return true;
  }

  /**
   * Drop apply chrome once per run (visible-hard / Done / error / cancel).
   * @param {object} run
   * @param {string} reason
   */
  ...
  _applyCompletedOpen(run, out) {
    ...
    if (this._openShouldFail(run)) return this._openFailResult(run);
    return { ok: true };
  }
