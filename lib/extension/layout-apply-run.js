...
import { collectSlotMachines, placeSlotWindows, startSlotMachines } from "./layout-apply-slot.js";
import { normalizeProfile } from "../shared/layout-plan.js";
...
export function parseApplyLayoutRequest(raw) {
  ...
  // Normalized layout mon children so hard slot machines key TABBED/STACKED (D040).
  let profile = profileIn;
  try {
    profile = normalizeProfile(profileIn) || profileIn;
  } catch {
    profile = profileIn;
  }
  return {
...
export class LayoutApplyRunBag {
  ...
  _applyHardReadyResult(run, out) {
    ...
    // D043: overlay lifetime ends at all required slots hard-done|hard-failed.
    this._clearChrome(run, "all-hard");
    if (out && out.ok === false) {
      ...
      // Continue spine; Done.ok is required-forest match.
      return { ok: true };
    }
    run.hardPending = [];
    this._emitProgress({
   ...
   * After all-hard + focus only — residual thrash, not mon/structure repair.
   * Overlay already cleared at all-hard (D043); soft must not keep it up.
   * @param {object} run
   */
  _runSoftPhase(run) {
