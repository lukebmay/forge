...
import { validateReconcileProfile } from "../shared/layout-plan.js";
...
export class LayoutApplyRunBag {
  ...
  start(request) {
    ...
    this._live = run;
    Logger.debug(
      `layout-apply start applyId=${applyId} name=${run.name || "-"} ws=${run.workspace ?? 0}`
    );
  ...
  _placeSlot(run, machine) {
    ...
    if (HUNT_TILE_SLOT_FLOAT && n === 0 && attempt > 1) {
      huntTileSlotFloat(
        `slot-place-hollow key=${key} attempt=${attempt} ` +
  ...
  _logSlotPendingWhy(machine, windows, pending) {
    ...
    for (const wid of pending || []) {
      ...
      const key = machine?.key || machine?.id || "-";
      Logger.debug(`hard-ready why id=${id} role=${role} key=${key} ${why.join(",")}`);
      ...
      try {
        ...
        if (!info) continue;
        Logger.debug(
          `hard-ready float id=${id} role=${role} key=${key} ` +
            `float=${info.action}/${info.reason} metaMon=${info.metaMon ?? "?"} ` +
            `treeMon=${info.treeMon ?? "?"} forestMode=${win?.mode ?? "?"} ` +
            `forestMon=${win?.monitor ?? "?"}`
        );
        if (HUNT_TILE_SLOT_FLOAT) {
          huntTileSlotFloat(
            `hard-ready id=${id} role=${role} key=${key} why=${why.join(",")} ` +
              `float=${info.action}/${info.reason} metaMon=${info.metaMon ?? "?"} ` +
              `treeMon=${info.treeMon ?? "?"} flags=${formatFloatFlagTags(info.flags || {})}`
          );
        }
      } catch (_e) {
   ...
   * Overlay drops when machines terminal (D043/SM7) — not during place/hard wait.
   * @param {object} run
   */
  _runHardReadyPhase(run) {
    ...
    if (!this._hasSettle()) {
      ...
      this._clearChrome(run, "all-hard");
      return { ok: true };
    }
   ...
   * After all-hard + focus. Chrome stays until soft finishes so jitter/soft
   * notices remain visible (apply is usually fast now).
   * @param {object} run
   */
  _runSoftPhase(run) {
  ...
  /**
   * Drop apply chrome once per run (all-hard / error / cancel / done). Idempotent.
   * @param {object} run
   * @param {string} reason
   */
  ...
  _finishSpine(run) {
    ...
    const forestMatch = this._matchRequiredTileForest(run);
    const result = {
