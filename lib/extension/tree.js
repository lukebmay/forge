...
export class Node extends GObject.Object {
  ...
  /**
   * Shared header-tab scaffold (icon + title in a styled box). The title widget
   ...
   *
   * Icon/title are non-reactive so the tab BoxLayout owns press/motion (activate
   * + arm drag). Reactive St.Buttons stole press and only activated on release.
   */
  ...
  _createWindowTab() {
    ...
    let metaWin = this.nodeValue;
    let { tabContents } = this._buildTabBase(this.app, this._getTitle());
    ...
    // Primary press: reveal immediately + arm strip reorder / peel (Chrome-like).
    // Icon/title are non-reactive; close stays reactive and is skipped via source.
    ...
    tabContents.connect("button-press-event", (_, event) => {
      ...
      if (btn === Clutter.BUTTON_PRIMARY) {
        clickFn();
        // Press only — DragDropManager owns motion/release for the gesture.
  ...
  _ensureConTab() {
    ...
    let { tabContents } = this._buildTabBase(rep.app, this._getTitle());
  ...
  _activateFromTab(metaWin) {
    ...
    try {
      ...
      const cls =
        typeof metaWin.get_wm_class === "function"
      ...
      if (typeof wm.revealGroupChild === "function") {
        wm.revealGroupChild(node, { keyboard: true, source: "tab-click" });
      } else if (typeof wm.afterFocus === "function") {
  ...
  _createDecoration() {
    if (this.decoration?._forgeDisposed) this.decoration = null;
    ...
    // Parent on tab-chrome layer (not window_group) when DecorationManager is ready.
    try {
      this._resolveExtWm()?.decorationManager?.attachTabDecoration?.(this);
    } catch (e) {
  ...
  _releaseDecorationActor() {
    ...
    this.decoration = null;
    this._tabRowHosts = null;
    const alreadyDead = !!decoration._forgeDisposed;
    decoration._forgeDisposed = true;
    try {
      this._resolveExtWm()?.decorationManager?.untrackTabDecoration?.(decoration);
    } catch (_e) {
      // ignore
    }
    ...
    try {
      ...
      if (parent) parent.remove_child(decoration);
    } catch (_e) {
      // ignore
    }
    try {
...
export class Tree extends Node {
  ...
  findNode(data) {
    ...
    return this.getNodeByValue(data);
  }
  ...
  swapPairs(fromNode, toNode, focus = true) {
    const wm = this.extWm;
    if (wm?._liveForestSeeded && forestSwapWindows(wm, fromNode, toNode)) {
  ...
  processNode(node) {
    ...
    if (node.nodeType === NODE_TYPES.MONITOR || node.nodeType === NODE_TYPES.CON) {
      ...
      params.minTabLabelChars = this.settings.get_uint("min-tab-label-chars") || 0;
      params.maxTabRows = this.settings.get_uint("max-tab-rows") || 0;

      let decoration = node.decoration;
      if (decoration?._forgeDisposed) {
  ...
  measureMinTabWidth({ minChars = 0, dpi = null, fontDesc = null } = {}) {
    ...
    const desc = fontDesc || this._tabTitleFontDesc() || "default";
    const key = `${desc}|${scale}|${chars}`;
    if (this._minTabWidthCache?.key === key) return this._minTabWidthCache.value;

    const avgGlyph = this._avgTabGlyphPx(chars, scale, desc);
    const chromePx = this._tabChromePx(scale);
    const value = TreeLayout.minTabWidthFromChars(chars, avgGlyph, chromePx);
    this._minTabWidthCache = { key, value };
    return value;
  }
  ...
  _avgTabGlyphPx(minChars, scale, _fontDesc) {
    try {
  ...
  _tabChromePx(scale) {
    const s = scale || 1;
    // icon logical 24; close ~16+pad/margin; icon/tab margins
  ...
  _applyDecorationRect(node, child, params, barSize, tabExpand, tabHost = null) {
    ...
    try {
      ...
      if (child.tab && !host.contains(child.tab)) {
        // Mid-drag float chip lives on the tab-chrome layer — do not steal it back.
        const tabDrag = this.extWm?.dragDrop?._tabDrag;
        if (
  ...
  _ensureDecoration(node, orientation) {
    if (node.decoration?._forgeDisposed) node.decoration = null;
  ...
  processTabbed(node, child, params, index) {
    ...
    const minChars =
      params.minTabLabelChars !== undefined
        ...
        : this.settings?.get_uint?.("min-tab-label-chars") || 0;
    const maxRows =
      params.maxTabRows !== undefined
        ? params.maxTabRows
        : this.settings?.get_uint?.("max-tab-rows") || 0;
    // params.minTabWidth lets units inject a floor without St/Pango measure.
    const minTabWidth =
      params.minTabWidth !== undefined ? params.minTabWidth : this.measureMinTabWidth({ minChars });
    const rowInnerWidth =
      params.rowInnerWidth !== undefined ? params.rowInnerWidth : this.processGap(node).width;
    ...
    // Cache on params so sibling processTabbed calls share one plan.
    ...
    if (multiRow) {
      ...
      if (node.decoration && (child.isWindow() || child.isCon())) {
        // Outer host is N×barH; pin each row/tab to barH (same St expand trap as stacked).
      ...
      child.rect = TreeLayout.tabbedChildRect(node.rect, barH, this.tabPosition(), true);
      if (node.decoration && (child.isWindow() || child.isCon())) {
        this._applyDecorationRect(node, child, params, barH, false);
      }
    }
  }
  ...
  computeSizes(node, childItems) {
    return TreeLayout.computeSizes(node, childItems, (items) => this.getTiledChildren(items), {
      skipWriteBack: !!this.extWm?.openLayoutBatchActive,
    });
  }
