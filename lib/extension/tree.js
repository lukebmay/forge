...
export class Node extends GObject.Object {
  ...
  _destroyDecoration() {
    ...
    try {
      ...
      try {
        this._resolveExtWm()?.decorationManager?.untrackTabDecoration?.(this.decoration);
      } catch (_e) {
        // ignore
      }
...
export class Tree extends Node {
  ...
  _treeSnapshotCtx() {
    return {
      ...
      windowIdOf: (node) => TreeSnapshot.windowIdFromMeta(node?.nodeValue),
    };
  }
   ...
   * Skips candidates whose post-move slots would overflow any involved app mins.
   */
  move(node, direction) {
    if (!node) return false;
    if (node.isFloat()) {
