...
export class Tree extends Node {
  ...
  apply(node) {
    if (!node) return;
    // Suppress entered-monitor rehome while move_resize runs (spurious mon
    // events otherwise redistribute mon-level userSized shares to percent=1).
    const wm = this.extWm;
    const prevSuppress = !!wm?._suppressEnteredMonitorRehome;
    if (wm) wm._suppressEnteredMonitorRehome = true;
    try {
      ...
      if (wm) wm._suppressEnteredMonitorRehome = prevSuppress;
    }
  }
