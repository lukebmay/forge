...
export class Node extends GObject.Object {
  ...
  _activateFromTab(metaWin) {
    ...
    try {
      ...
      // LF2: activate-only failed on X11 after multi-mon / layout apply.
...
export class Tree extends Node {
      ...
      tiledChildren.forEach((w) => {
        if (w.renderRect) {
          if (w.renderRect.width > 0 && w.renderRect.height > 0) {
            ...
            const dest = this._paintRectForWindow(w);
  ...
  /** Slot rect, or zoom overlay. Does not mutate percents / node.rect. */
  _paintRectForWindow(w) {
    const slot = w.renderRect;
