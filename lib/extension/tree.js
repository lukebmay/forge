...
export class Node extends GObject.Object {
  ...
  _activateFromTab(metaWin) {
    ...
    // Drive stacked/tabbed restack immediately (focus signal is deferred).
    try {
      let root = this;
...
export class Tree extends Node {
  ...
  computeSizes(node, childItems) {
    ...
    // T4: after min-size paint, store effective percents so stored ratios match
    // frames. Does not set userSized (automatic, not user intent).
  ...
  /**
   * T4 / forge-7m3: Give a newly-added child a share of its parent.
   *
   * Policy (product: equal until user resizes):
   * - If no existing tiled sibling is userSized → equalize (percent 0 = equal
   *   in computeSizes), even if automatic percents were written back.
   * - If any sibling is userSized and setting new-window-size-policy is
   *   "preserve" → carve 1/(n+1) for the newcomer and scale the rest
   *   (forge-7m3).
   * - If any sibling is userSized and policy is "equalize" → re-equalize all.
   *
   * The new child may still be FLOAT at this point (tiled later during render),
   * so it is handled explicitly rather than via getTiledChildren alone.
  ...
  insertChildPercent(parentNode, newChild) {
    ...
    // User-sized siblings + preserve: scale existing proportions, carve fair share.
    let existingTotal = existing.reduce((sum, n) => sum + (n.percent || 0), 0);
    if (existingTotal <= 0) {
      // userSized but still magic-zero percents — materialize equal shares first.
