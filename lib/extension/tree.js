...
export class Tree extends Node {
  ...
  processStacked(node, child, params, _index) {
    ...
    if (node.decoration && (child.isWindow() || child.isCon())) {
      // Decoration keeps full (uncapped) totalBars; content uses cappedBars.
      this._applyDecorationRect(node, child, params, laid.totalBars, true);
    }
  }
  ...
  processTabbed(node, child, params, _index) {
    ...
    if (node.layout === LAYOUT_TYPES.TABBED) {
      // alwaysShowDecorationTab: always reserve bar strip (product default).
   ...
   * New child may still be FLOAT; filtered out of getTiledChildren.
   */
