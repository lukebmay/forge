...
export class Node extends GObject.Object {
  ...
  get windowActor() {
    // Map-time get_compositor_private can be null (Wayland); borders live on the
    // real actor later. Refresh so hideWindowBorders can find actor.border.
    if (!this._actor && this.isWindow() && this._data) {
      try {
        this._actor = this._data.get_compositor_private?.() ?? null;
      } catch (_e) {
        /* finalized */
      }
    }
  ...
  get actor() {
    switch (this.nodeType) {
      case NODE_TYPES.WINDOW:
        // Prefer live compositor private (same refresh as windowActor).
  ...
  _initMetaWindow() {
    if (this.isWindow()) {
      ...
      // Chrome PWAs: tracker often returns Google Chrome; prefer class desktop.
      app = this._preferChromePwaApp(metaWin, app) || app;
      this.app = app;
    }
  }

  /**
   * Resolve Shell.App for chrome-<id>-Default / crx_<id> via .desktop when
   * WindowTracker only has the browser.
   * @param {Meta.Window} metaWin
   * @param {Shell.App|null} tracked
   * @returns {Shell.App|null}
   */
  _preferChromePwaApp(metaWin, tracked) {
    let cls = null;
    try {
      cls = metaWin?.get_wm_class?.() || null;
    } catch (_e) {
      return tracked || null;
    }
    if (!cls || typeof cls !== "string") return tracked || null;
    const n = cls.trim();
    if (!n) return tracked || null;
    const lower = n.toLowerCase();
    let desktopId = null;
    if (lower.startsWith("chrome-") && lower.endsWith("-default")) {
      desktopId = `${n}.desktop`;
    } else if (lower.startsWith("crx_")) {
      // StartupWMClass form; desktop files are usually chrome-<id>-Default.desktop
      const id = n.slice(4);
      if (id) desktopId = `chrome-${id}-Default.desktop`;
    } else {
      return tracked || null;
    }
    try {
      const byDesktop = Shell.AppSystem.get_default().lookup_app(desktopId);
   ...
   * Also rebuild when the resolved app id changes (Chrome PWA was briefly the
   * browser app — wrong icon sticks otherwise).
   */
  refreshApp() {
    const prevId = this.app && typeof this.app.get_id === "function" ? this.app.get_id() : null;
    this._initMetaWindow();
    const nextId = this.app && typeof this.app.get_id === "function" ? this.app.get_id() : null;
    const appChanged = prevId !== nextId;
    if (this.tab && (this._tabFallback || appChanged || !this.app)) {
      this._destroyTab();
    }
    this._createWindowTab();
  }
  ...
  _createWindowTab() {
    ...
    // Whole tab strip is clickable (not only icon/title buttons).
  ...
  _activateFromTab(metaWin) {
    ...
    // Drive stack/tab raise + chrome restack immediately. Focus signal is
    // deferred (~queueEvent 220ms); raise() buries the strip under the window
    // actor until then — and if focus did not change, focus-update never runs.
    try {
      let root = this;
      ...
      // Match focus-update: unfreeze so tab/stack + decoration restack are not no-ops.
      wm.unfreezeRender?.();
...
export class Tree extends Node {
  ...
  /**
   * T6: soft-rehome path — rebuild only monitors whose topology diverged;
   ...
   * restore (snapshotTree) is preferred on reload/soft-rehome; this remains for
  ...
  /**
   * Soft-rehome path: rejoin or re-apply snapshot only for groups that were
   ...
   * CON unit (S2): swaps whole bag with sibling unit; WINDOW path unchanged.
   * @param {Node} node - focused window or elevated CON
   * @param {number} offset - +1 for next, -1 for previous
   */
  swapSibling(node, offset) {
    ...
    if (node.isCon()) {
      ...
      const idx = siblings.indexOf(node);
      if (idx < 0) return null;
      const target = siblings[(idx + offset + siblings.length) % siblings.length];
      if (!target || target === node) return null;
      this.swapUnits(node, target, false);
      return node;
    }
  ...
  move(node, direction) {
    ...
    switch (next.nodeType) {
      case NODE_TYPES.WINDOW:
        // Adjacent sibling: exchange slots (WINDOW↔WINDOW or CON unit ↔ WINDOW).
        if (next === node.previousSibling || next === node.nextSibling) {
          parentTarget = next.parentNode;
          this.swapUnits(node, next);
          if (this.settings.get_boolean("move-pointer-focus-enabled")) {
            this.extWm.movePointerWith(node.isWindow() ? node : this.findFirstNodeWindowFrom(node));
          }
        ...
        break;
      case NODE_TYPES.CON:
        // S2: CON unit vs adjacent CON sibling → swap bags.
        // WINDOW vs adjacent CON → enter the container (legacy leaf path).
        if (node.isCon() && (next === node.previousSibling || next === node.nextSibling)) {
          parentTarget = next.parentNode;
          this.swapUnits(node, next);
          if (this.settings.get_boolean("move-pointer-focus-enabled")) {
            this.extWm.movePointerWith(this.findFirstNodeWindowFrom(node));
          }
          return true;
        }
        parentTarget = next;
      ...
      case NODE_TYPES.MONITOR:
        ...
        if (
          ...
          // CON units: reparent only — renderTree places descendants (S2).
          // WINDOW: Meta geometry first so a throw leaves the tree intact (e3k1).
          if (node.isWindow()) {
  ...
  mergeWindowsIntoGroup(focusNode, partnerNode, layout = LAYOUT_TYPES.TABBED) {
    ...
    if (focusNode.isFloat() || partnerNode.isFloat()) return null;
  ...
  /**
   * One-level dissolve (I2): lift `con` children into grandparent at `con`'s
   * index, remove empty CON, equalize grandparent siblings. Nested CON children
   * stay CONs. Explicit structure op only — not used by setLayout.
   * @param {Node} con
   * @returns {Node|null} grandparent after ungroup, or null on no-op
   */
  ungroupContainer(con) {
    if (!con || con.nodeType !== NODE_TYPES.CON) return null;
    const grandparent = con.parentNode;
    if (!grandparent) return null;

    const children = [...con.childNodes];
    if (con.isStackedOrTabbed()) {
    ...
    for (const child of children) {
      grandparent.insertBefore(child, con);
    }

    try {
      grandparent.removeChild(con);
    } catch (_e) {
      /* already detached */
    }

    this.resetSiblingPercent(grandparent);
    this.attachNode = grandparent;
    return grandparent;
  }

  /**
   * Move-out (C4): reparent layout unit one level up — insert after former
   * parent among grandparent’s children. Does **not** dissolve the parent CON
   * (unlike ungroupContainer).
   * @param {Node} unit - layout unit (window or tab/stack bag)
   * @returns {Node|null} grandparent after reparent, or null on no-op
   */
  moveUnitOut(unit) {
    if (!unit) return null;
    const parent = unit.parentNode;
    if (!parent || parent.nodeType === NODE_TYPES.MONITOR) return null;
    if (parent.nodeType === NODE_TYPES.WORKSPACE || parent.nodeType === NODE_TYPES.ROOT) {
      return null;
    }
    const grandparent = parent.parentNode;
    if (!grandparent) return null;

    // Insert after former parent among grandparent children.
    const afterParent = parent.nextSibling;
    if (afterParent) {
      grandparent.insertBefore(unit, afterParent);
    } else {
      grandparent.appendChild(unit);
    }

    this._finishMove(parent, grandparent);
    this.attachNode = grandparent;
    return grandparent;
  }

  /**
   * Move-in (C4): reparent layout unit into an adjacent sibling CON.
   * No-op when there is no CON sibling (does not invent structure).
   * @param {Node} unit
   * @param {Node} [targetCon] - optional explicit sibling CON
   * @returns {Node|null} target CON after reparent, or null on no-op
   */
  moveUnitIn(unit, targetCon) {
    if (!unit || !unit.parentNode) return null;

    let target = targetCon;
    if (!target) {
      const parent = unit.parentNode;
      ...
      for (let i = idx + 1; i < siblings.length; i++) {
        if (siblings[i]?.nodeType === NODE_TYPES.CON) {
          target = siblings[i];
          break;
        }
      }
    ...
    if (!target || target.nodeType !== NODE_TYPES.CON || target === unit) return null;
    const priorParent = unit.parentNode;
    // Target must be a sibling before reparent (no invent / cross-tree).
    if (target.parentNode !== priorParent) return null;

    target.appendChild(unit);
    this._finishMove(priorParent, target);
  ...
  swap(node, direction) {
    ...
    // S2: CON ops unit swaps with the sibling unit (WINDOW or CON), not a leaf
    // inside the neighbor. Leaf path still resolves CON/MONITOR next → window.
    if (node.isCon()) {
      if (nodeSwapType === NODE_TYPES.MONITOR) {
        return;
      }
      if (nodeSwapType === NODE_TYPES.WINDOW || nodeSwapType === NODE_TYPES.CON) {
        if (!this._sameMonitorUnit(node, nextSwapNode)) {
          return;
        }
        this.swapUnits(node, nextSwapNode, false);
      }
      return nextSwapNode;
    }
  ...
  /**
   * Same-monitor check for WINDOW or CON units (CON has no Meta nodeValue).
   * @param {Node} a
   * @param {Node} b
   * @returns {boolean}
   */
  _sameMonitorUnit(a, b) {
    if (!a || !b) return false;
    const monA = this.findAncestorMonitor(a);
    const monB = this.findAncestorMonitor(b);
    if (monA && monB) return monA === monB;
    const winA = a.isWindow() ? a : this.findFirstNodeWindowFrom(a);
    const winB = b.isWindow() ? b : this.findFirstNodeWindowFrom(b);
    if (winA && winB) return this.extWm.sameParentMonitor(winA, winB);
    return false;
  }

  /**
   * Swap two tiled units (WINDOW or CON). WINDOW↔WINDOW keeps Meta rect dance
   * via swapPairs; any CON involvement is structural (slot + percent) only —
   * renderTree places descendants (S2 elevated bag swap).
   *
   * @param {Node} fromNode
   * @param {Node} toNode
   * @param {boolean} [focus=true] - raise/focus when both are windows
   */
  swapUnits(fromNode, toNode, focus = true) {
    if (!fromNode || !toNode || fromNode === toNode) return;
    if (!this._unitSwappable(fromNode) || !this._unitSwappable(toNode)) return;

    if (fromNode.isWindow() && toNode.isWindow()) {
      this.swapPairs(fromNode, toNode, focus);
      return;
    }

    const parentForFrom = fromNode.parentNode;
    const parentForTo = toNode.parentNode;
    if (!parentForFrom || !parentForTo) return;

    if (!this._sameMonitorUnit(fromNode, toNode)) {
      return;
    }

    const nextIndex = toNode.index;
    const focusIndex = fromNode.index;
    if (nextIndex == null || focusIndex == null) return;

    const transferPercent = fromNode.percent;
    fromNode.percent = toNode.percent;
    toNode.percent = transferPercent;

    // Modes only meaningful on WINDOW leaves; leave CON modes alone.
    if (fromNode.isWindow() && toNode.isWindow()) {
  ...
  /** WINDOW (alive, not minimized) or CON with at least one tiled descendant. */
  _unitSwappable(node) {
    if (!node) return false;
    if (node.isWindow()) {
      return this._swappable(node);
    }
    if (node.isCon()) {
      return this.getTiledChildren(node.childNodes).length > 0;
    }
  ...
  apply(node) {
    if (!node) return;
    // Suppress entered-monitor rehome + size/position retile while move_resize
    // runs (spurious mon events redistribute mon-level userSized shares; geometry
    // signals would schedule another full renderTree → apply feedback loop).
    const wm = this.extWm;
    const prevSuppress = !!wm?._suppressEnteredMonitorRehome;
    const prevGeom = !!wm?._suppressGeometrySignalRetile;
    if (wm) {
      wm._suppressEnteredMonitorRehome = true;
    ...
    try {
      ...
      tiledChildren.forEach((w) => {
        ...
        // Unmanaged/closing windows still appear TILE until prune; move_to_monitor
        // on them SIGSEGVs Wayland (safeMoveToMonitor gates, but skip whole place).
        if (!Utils.isWindowAlive(metaWin)) return;
        if (w.renderRect) {
        ...
        if (metaWin.firstRender) metaWin.firstRender = false;
      });
    } finally {
      if (wm) {
        wm._suppressEnteredMonitorRehome = prevSuppress;
        wm._suppressGeometrySignalRetile = prevGeom;
      }
    }
  }
