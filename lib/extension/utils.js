...
import { ORIENTATION_TYPES, LAYOUT_TYPES, POSITION } from "./tree-types.js";
import { GRAB_TYPES } from "./window-modes.js";
import { DROP_ZONES } from "./drop-zones.js";

// Drop zones for drag-and-drop tiling (canonical source: drop-zones.js)
export { DROP_ZONES };

/** Monotonic µs; Date.now()*1000 when GLib mock lacks get_monotonic_time. */
export function monoTimeUs() {
  return typeof GLib.get_monotonic_time === "function"
...
/** Area of the overlapping region of two rects (0 if no overlap). */
...
/**
 * Monitor index whose geometry best covers `rect` (max intersection area).
 * On ties, prefers the center of `rect` lying inside a monitor; else lowest index.
 * @returns {number} index, or -1 when none of the geometries are usable
 */
export function bestMonitorIndexForRect(rect, monitorGeometries) {
  if (!rect || !monitorGeometries || monitorGeometries.length === 0) return -1;
  let bestIdx = -1;
  let bestArea = 0;
  for (let i = 0; i < monitorGeometries.length; i++) {
    const g = monitorGeometries[i];
    if (!g) continue;
    const area = rectIntersectionArea(rect, g);
    if (area > bestArea) {
      bestArea = area;
      bestIdx = i;
    }
  ...
  // No area overlap: fall back to center containment.
  const cx = rect.x + rect.width / 2;
  const cy = rect.y + rect.height / 2;
  for (let i = 0; i < monitorGeometries.length; i++) {
    const g = monitorGeometries[i];
    if (!g) continue;
    if (rectContainsPoint(g, [cx, cy])) return i;
  }
  return -1;
}
...
export function grabMode(grabOp) {
  ...
  if (RESIZE_GRAB_OPS.has(grabOp)) {
    ...
    // CSD/titlebar: WINDOW_BASE === MOVING on Mutter 46; keep explicit for older mocks.
    raw === Meta.GrabOp.WINDOW_BASE ||
...
/**
 * Tear down forge-deco tab strips (tab-chrome layer) and any leftover typed
 * actors still under window_group. Layer must die before tree drop on disable.
 * @param {import('./decoration.js').DecorationManager|null|undefined} [decorationManager]
 */
export function _disableDecorations(decorationManager) {
  if (decorationManager && typeof decorationManager.destroyTabChromeLayer === "function") {
    try {
      decorationManager.destroyTabChromeLayer();
    } catch (_e) {
      // best-effort
    }
  } else {
    // Fallback: destroy by host name when no manager was passed.
    try {
      const uiGroup = global.Main?.layoutManager?.uiGroup;
      const kids = uiGroup?.get_children?.() ?? [];
      for (const actor of kids) {
        if (actor?.name !== "forge-tab-chrome") continue;
        const stripKids = actor.get_children?.() ? [...actor.get_children()] : [];
        for (const d of stripKids) {
          try {
            actor.remove_child?.(d);
            d.destroy?.();
          } catch (_e) {
            // ignore
          }
  ...
  // Legacy / safety: typed forge-deco left under window_group.
  try {
    const wg = global.window_group;
    if (!wg?.get_children) return;
    const decos = wg.get_children().filter((a) => a.type != null);
    decos.forEach((d) => {
      try {
        wg.remove_child(d);
        d.destroy();
 ...
 * Center wins inside the center band. Among edge bands (including corners),
 * pick the **nearest outer edge** so vertical splits are not starved by a
 * fixed left/right-before-top/bottom priority.
 *
 ...
 *   Full target frame; when omitted, inferred from edge regions.
 * @returns {string} DROP_ZONES value (LEFT, RIGHT, TOP, BOTTOM, CENTER, or NONE)
 */
export function detectDropZone(regions, pointer, targetRect = null) {
  if (!regions || !pointer) return DROP_ZONES.NONE;
  if (rectContainsPoint(regions.center, pointer)) {
    return DROP_ZONES.CENTER;
  }

  const rect = targetRect || inferTargetRectFromDropRegions(regions);
  const px = pointer[0];
  const py = pointer[1];
  // Normalize by axis so tall tiles do not prefer LEFT/RIGHT in bottom corners
  // (px dist to side beats py dist to bottom on a half-height band).
  ...
  if (rectContainsPoint(regions.left, pointer)) {
    candidates.push([DROP_ZONES.LEFT, rect ? (px - rect.x) / w : 0]);
  }
  if (rectContainsPoint(regions.right, pointer)) {
    candidates.push([DROP_ZONES.RIGHT, rect ? (rect.x + rect.width - px) / w : 0]);
  }
  if (rectContainsPoint(regions.top, pointer)) {
    candidates.push([DROP_ZONES.TOP, rect ? (py - rect.y) / h : 0]);
  }
  if (rectContainsPoint(regions.bottom, pointer)) {
    candidates.push([DROP_ZONES.BOTTOM, rect ? (rect.y + rect.height - py) / h : 0]);
  }
  if (!candidates.length) return DROP_ZONES.NONE;
  candidates.sort((a, b) => a[1] - b[1]);
  return candidates[0][0];
}

/**
 * Reconstruct the full target rect from edge bands when callers omit it.
 * @param {Object} regions
 * @returns {{ x: number, y: number, width: number, height: number }|null}
 */
export function inferTargetRectFromDropRegions(regions) {
  if (!regions?.left || !regions?.right || !regions?.top || !regions?.bottom) return null;
  const x = regions.left.x;
  const y = regions.top.y;
  const width = regions.right.x + regions.right.width - x;
  const height = regions.bottom.y + regions.bottom.height - y;
