...
export function collectSlotMachines(run, forest) {
  ...
  Logger.debug("slot-machines", { fields: { n: out.length } });
  return out;
}
...
/**
 * Slot hard-retry may need structure (ensure_layout/order), not only Meta moves.
 * Same-mon but wrong parent (TABBED peer as MONITOR sibling) emits ensure_layout
 * with 0 moves — R042.
 * @param {object} action
 * @param {{ roles: Set<string>, wids: Set<string>, machineKey: string }} ctx
 * @returns {boolean}
 */
function _actionTouchesSlotMachine(action, ctx) {
  if (!action || typeof action !== "object") return false;
  const op = String(action.op || "")
    .trim()
    .toLowerCase();
  if (op === "move") {
    if (action.role != null && ctx.roles.has(String(action.role))) return true;
    const wid = _actionWindowId(action);
    return !!(wid && ctx.wids.has(wid));
  }
  if (op === "ensure_layout" || op === "ensure_order") {
    // Slot key only — do not match by windowId overlap (a mon sibling's id can
    // appear in another slot's ensure_layout and steal the Meta-hollow path).
    const slot = action.slot != null ? String(action.slot).trim() : "";
    return !!(slot && ctx.machineKey && slot === ctx.machineKey);
  }
  return false;
}

/** Steps that repair place / group topology for one slot. */
const SLOT_PLACE_STEP_OPS = new Set(["move", "layout", "order"]);

/**
 * Summarize why a hollow slot-place (moves=0) may still be unsettled.
 * @param {object|null|undefined} machine
 * @param {object|null|undefined} forest
 * @returns {string}
 */
export function slotPlaceHollowSummary(machine, forest) {
  const wids = (machine?.windowIds || []).map((w) => String(w));
  if (!wids.length) return "no-windowIds";
  const wins = windowsFromForestSafe(forest);
  const parts = [];
  for (const id of wids) {
    const win = wins.find((w) => w && String(w.windowId) === id);
    if (!win) {
      parts.push(`${id}:missing-forest`);
      continue;
    }
    const mode = win.mode != null ? String(win.mode) : "none";
    const mon = win.monitor != null ? String(win.monitor) : "?";
    parts.push(`${id}:mode=${mode},mon=${mon}`);
  }
  return parts.join(";") || "-";
}

function windowsFromForestSafe(forest) {
  if (!forest || typeof forest !== "object") return [];
  const out = [];
  const walk = (n) => {
    if (!n || typeof n !== "object") return;
    if (n.nodeType === "WINDOW" || n.windowId != null) out.push(n);
    for (const c of n.children || []) walk(c);
  };
  for (const m of forest.monitors || []) walk(m);
  for (const o of forest.orphanWindows || []) walk(o);
  return out;
}
 ...
 *   run?: object,
 *   unsettled?: boolean,
 *   ensureMetaInSlot?: (machine: object) => { ok?: boolean, steps?: number }|void,
 * }} opts
 * @returns {object}
 */
export function placeSlotWindows(opts = {}) {
  ...
  const key = machine?.id || machine?.key || "-";
  const machineKey = String(key);
  const actions = (plan.actions || []).filter((a) =>
    _actionTouchesSlotMachine(a, { roles, wids, machineKey })
  );
  ...
  steps = (Array.isArray(steps) ? steps : []).filter((s) => {
    ...
    return SLOT_PLACE_STEP_OPS.has(
      String(s.op || "")
        .trim()
        .toLowerCase()
    );
  ...
  const nMove = steps.filter((s) => String(s.op).toLowerCase() === "move").length;
  const nLayout = steps.filter((s) => String(s.op).toLowerCase() === "layout").length;
  const nOrder = steps.filter((s) => String(s.op).toLowerCase() === "order").length;
  Logger.trace(
    `slot-place key=${key} moves=${nMove} layout=${nLayout} order=${nOrder} ws=${workspace}`
  );
  if (!steps.length && opts.unsettled) {
    const hollow = slotPlaceHollowSummary(machine, opts.forest);
    Logger.debug(`slot-place hollow key=${key} unsettled ${hollow}`);
    // Forest replan has no move/layout/order for this slot while Meta mode/mon
    // may still be wrong (D040 retry). Re-assert via WM ensureMetaInSlot —
    // do not invent a twin place planner. Wrong-parent same-mon thrash is
    // ensure_layout above (R042), not this Meta-only path.
    if (typeof opts.ensureMetaInSlot === "function") {
      try {
        const forced = opts.ensureMetaInSlot(machine) || {};
        const n = typeof forced.steps === "number" ? forced.steps : 1;
        Logger.debug(`slot-place ensure-meta key=${key} steps=${n}`);
        if (forced.ok === false) {
          return {
            ok: false,
            skipped: false,
            steps: n,
            error: forced.error || "ensureMetaInSlot failed",
            plan,
          };
        }
        return { ok: true, skipped: false, steps: n, reason: "ensure-meta", plan };
      } catch (e) {
        return {
          ok: false,
          skipped: false,
  ...
  const reason = nLayout > 0 || nOrder > 0 ? "structure" : "moves";
  return { ok: true, skipped: false, steps: steps.length, reason, plan };
}
...
export function startSlotMachines(inputMachines, opts, done) {
  ...
  const startHardWait = (machine) => {
    ...
    if (!machine.windowSettle || typeof machine.windowSettle !== "object") {
      machine.windowSettle = {};
    }
    const ids = (Array.isArray(machine.windowIds) ? machine.windowIds : []).map((x) => String(x));
    const todo = ids.filter((id) => machine.windowSettle[id] !== SLOT_STATE.HARD_DONE);
    if (!todo.length) {
    ...
    let inflight = todo.length;
    let anyFail = false;
    for (const id of todo) {
      machine.windowSettle[id] = SLOT_STATE.HARD_WAIT;
      waitHardReadyOnSignals(
        [id],
        {
          ...
          getSlots: () => ({ [id]: machine.slots?.[id] }),
          slots: { [id]: machine.slots?.[id] },
        },
        (out) => {
          if (disposed) return;
          if (out?.cancelled) {
          ...
          if (out && out.ok) {
            machine.windowSettle[id] = SLOT_STATE.HARD_DONE;
            try {
              opts.onWindowSettled?.(machine, id);
            } catch {
              /* */
            }
          } else {
            anyFail = true;
            const pending = out?.pending || [id];
            try {
