...
export function placeSlotWindows(opts = {}) {
  ...
  Logger.trace(`slot-place key=${key} moves=${steps.length} ws=${workspace}`);
  if (!steps.length && opts.unsettled) {
    ...
    // Forest replan is empty while Meta mode/mon may still be wrong (D040 retry).
    // Re-assert via WM ensureMetaInSlot — do not invent a twin place planner.
