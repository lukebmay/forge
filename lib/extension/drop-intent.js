...
/** @param {object|null|undefined} parent @param {object} [ctx] */
function kidsOf(parent, ctx) {
  const wm = ctx?.wm ?? ctx?.extWm;
  if (wm && parent) return liveChildrenForPresent(wm, parent);
  return Array.isArray(parent?.childNodes) ? parent.childNodes : [];
}

/** @param {object|null|undefined} node @param {object} [ctx] */
function parentOf(node, ctx) {
  const wm = ctx?.wm ?? ctx?.extWm;
  if (wm && node) return liveParentForPresent(wm, node);
...
export function dropChangesStructure(source, target, operation, ctx) {
  ...
  const parent = parentOf(source, ctx);
  if (!parent || parent !== parentOf(target, ctx)) return true;
  ...
  const si = kids.indexOf(source);
  const ti = kids.indexOf(target);
  if (si < 0 || ti < 0) return true;
  if (operation.isBefore) {
    return kids[si + 1] !== target;
  }
  return kids[ti + 1] !== source;
}
 ...
 * @returns {boolean}
 */
export function shouldMergeCenterGroup(source, target, operation, ctx) {
  if (!source || !target || !operation?.isCenter || operation.isSwap) return false;
  const parent = parentOf(source, ctx);
  if (!parent || parent !== parentOf(target, ctx)) return false;
...
function childIndex(parent, child, ctx) {
  const kids = kidsOf(parent, ctx);
...
function isAdjacentSibling(parent, source, target, dir, ctx) {
  const i = childIndex(parent, source, ctx);
  const j = childIndex(parent, target, ctx);
 ...
 */
function resolveSwapAsMove(source, target, ctx) {
  const srcParent = parentOf(source, ctx);
  if (!srcParent || srcParent !== parentOf(target, ctx)) return null;
  if (isMonNode(srcParent) || isGrouped(srcParent)) return null;
  if (!(isHSplit(srcParent) || isVSplit(srcParent))) return null;
  const dir =
    dirFromIndices(
      srcParent,
      childIndex(srcParent, source, ctx),
      childIndex(srcParent, target, ctx)
    ) || dirFromGeometry(source, target);
  if (!dir || !isAdjacentSibling(srcParent, source, target, dir, ctx)) return null;
  return { op: "move", dir };
}
...
export function resolveDropMark2(source, target, operation, ctx = {}) {
  ...
  const srcParent = parentOf(source, ctx);
  const tgtParent = parentOf(target, ctx);
  if (!srcParent || !tgtParent) return null;

  if (operation.isSwap) return resolveSwapAsMove(source, target, ctx);

  if (operation.isCenter) {
    const group = tgtParent;
    // MONITOR parent: wrapMonitorMax1 would wrap every live mon child first.
    const groupParent = parentOf(group, ctx);
    if (isGrouped(group) && srcParent === groupParent && !isMonNode(srcParent)) {
      const dir =
        dirFromIndices(
          srcParent,
          childIndex(srcParent, source, ctx),
          childIndex(srcParent, group, ctx)
        ) || dirFromGeometry(source, group);
      if (dir && isAdjacentSibling(srcParent, source, group, dir, ctx)) {
        return { op: "join", dir };
      }
    }
    return null;
  ...
  if (!isAdjacentSibling(srcParent, source, target, dir, ctx)) return null;
  return { op: "move", dir };
}
...
export function resolveDropSurface(source, target, operation, ctx = {}) {
  ...
  if (shouldMergeCenterGroup(source, target, operation, ctx)) return { op: "group" };
