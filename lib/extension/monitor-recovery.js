...
import { ensureLiveForest, forestAdmitMonitor } from "./tom-live.js";
import { forestCollectMonLoss, forestWindowMetasUnder } from "./forest-restore.js";
...
/** While still locked, lock shield must not expire (overnight DPMS thrash). */
export const LOCK_SHIELD_WHILE_LOCKED_US = 86_400_000_000;
/**
...
/** Lock + post-unlock until first workareas settle: no present/chrome/ε. */
export function hostPresentHoldActive(wm) {
  return !!(wm && (wm._sessionLocked || wm._hostPresentHold));
}

export function armHostPresentHold(wm) {
  if (!wm) return;
  wm._hostPresentHold = true;
}

export function clearHostPresentHold(wm) {
  if (!wm) return;
  wm._hostPresentHold = false;
}
...
export class MonitorRecoveryManager extends GObject.Object {
  ...
  shouldSkipWorkareasAsNoop() {
    ...
    if (wm._sessionLocked || wm._unlockWorkareasSettleBoost || wm._sessionLayoutRestoring) {
      return false;
    }
  ...
  recoverAfterWorkareas() {
    ...
    // Unlock settle: present was frozen; release before skip or reapply/present.
    clearHostPresentHold(wm);
    // Post-restore / post-lock thrash: reapply shield forest instead of
    // snapshotTree() freezing Meta-piled windows into last-good.
    // If quiet geom + homes already match, skip — desk is already correct.
    if (wm._sessionLayoutShieldActive()) {
      const prevFp = wm._lastQuietWorkareasFp;
      const liveFp = this._buildDisplayWorkareasFingerprint();
      const geomQuiet = !!(prevFp?.monitors?.length && workareasGeometryEqual(prevFp, liveFp));
      if (geomQuiet && this.homesMatchLastGood()) {
        sessionLayoutTrace("session-layout: monitor-recovery → shield skip (quiet+homes)");
        wm._workareasThrashPending = false;
        this._storeQuietWorkareasFingerprint(liveFp);
        return;
      }
    ...
    if (kind === "retile" || kind === "mon_gain") {
      ...
      // Structure is truth after scale/mode: pull Meta onto Forest/tree mon.
  ...
  /**
   * Move Meta windows to match Forest/tree monitor (structure-preserving retile).
   * Does not reparent topology.
   */
  _alignMetaMonsToTree() {
    ...
    if (wm._liveForestSeeded && wm.forest) {
      for (const tom of Object.values(wm.forest.nodes || {})) {
        if (tom?.kind !== "WINDOW") continue;
        const meta = wm.hostBag?.get?.(tom.id)?.meta;
        if (!Utils.isWindowAlive(meta)) continue;
        if (wm._validWindow && !wm._validWindow(meta)) continue;
        let cur = tom;
        while (cur && cur.kind !== "MONITOR") {
          cur = cur.parentId ? wm.forest.nodes[cur.parentId] : null;
        }
        if (!cur || cur.kind !== "MONITOR") continue;
        const treeMon = MonitorIdentity.monIndexFromId(cur.id);
        if (treeMon < 0 || treeMon >= n) continue;
        safeMoveToMonitor(meta, treeMon, "workareas-retile align Meta→forest");
      }
      return;
    }
  ...
  _runH1MonitorRecovery(treeSnapshot, nextFp) {
    ...
    for (const wNode of windowNodes) {
      ...
      const treeMon =
        typeof wm._monitorIndexOfNode === "function" ? wm._monitorIndexOfNode(wNode) : -1;
      if (treeMon >= 0 && treeMon !== targetMon) {
        const home = wm._lastGoodHomes?.get?.(metaWindow);
        Logger.trace(
          `monitor-recovery wrong-mon tree=${treeMon} → ${targetMon} ` +
            `key=${home?.stableKey || "-"}`,
          {
            fields: {
              treeMon,
              targetMon,
              stableKey: home?.stableKey ?? null,
              lastGoodIndex: home?.monitorIndex ?? -1,
            },
          }
        );
      }
  ...
  _collectMonLossToSurvivor(prevFp, nextFp, _treeSnapshot) {
    ...
    for (const deadIdx of deadIdxs) {
      for (const deadMon of monNodes) {
        ...
        if (!wm.forest?.nodes?.[survivorId] && !wm.tree.findNode(survivorId)) continue;

        const collectedMetas = forestWindowMetasUnder(wm, deadMon.nodeValue);
        if (collectedMetas.length === 0) {
          this._pruneEmptyMonitorNode(deadMon);
          continue;
        }

        // Forest-only (D096 G7): no createNode/appendChild invent on mon-loss.
        if (!forestCollectMonLoss(wm, deadMon.nodeValue, survivorId)) {
          recordFallback("mon-loss", "forest-fail");
          continue;
        }

        for (const meta of collectedMetas) {
          if (!Utils.isWindowAlive(meta)) continue;
  ...
  _ensureLiveMonitorNodes() {
    ...
    if (n <= 0) return;
    const wsIds = new Set();
    for (const wsNode of wm.tree.getNodeByType(NODE_TYPES.WORKSPACE) || []) {
      const wsVal = wsNode?.nodeValue;
      if (typeof wsVal === "string" && wsVal.startsWith("ws")) wsIds.add(wsVal);
    }
    for (const [id, tom] of Object.entries(wm.forest?.nodes || {})) {
      if (tom?.kind === "WORKSPACE" && id.startsWith("ws")) wsIds.add(id);
    }
    for (const wsVal of wsIds) {
      ...
      for (let mi = 0; mi < n; mi++) {
        ...
        if (wm.liveById?.get?.(id) || wm.forest?.nodes?.[id] || wm.tree.findNode(id)) continue;
        try {
          ...
          const layout = wm.determineSplitLayoutForRect?.(monitorRect) || LAYOUT_TYPES.HSPLIT;
          forestAdmitMonitor(wm, wsIdx, mi, { layout, tree: wm.tree });
        } catch (e) {
