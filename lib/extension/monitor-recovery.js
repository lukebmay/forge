...
import { forestEnsureSpineNode } from "./tom-live.js";
import { forestCollectMonLoss } from "./forest-restore.js";
  ...
  _collectMonLossToSurvivor(prevFp, nextFp, _treeSnapshot) {
    ...
    for (const deadIdx of deadIdxs) {
      for (const deadMon of monNodes) {
        ...
        const forested = seeded && forestCollectMonLoss(wm, deadMon.nodeValue, survivorId);
        if (!forested) {
        ...
        if (!forested) wm.tree.redistributeSiblingPercent?.(survivorMon);
        this._pruneEmptyMonitorNode(deadMon);
      }
    }
