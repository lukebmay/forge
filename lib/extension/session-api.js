...
/** Combined control-plane version (+ LayoutBatch open multi-open / CL5). */
export const SESSION_API_VERSION = 7;
...
export class SessionApi {
   ...
   *
   * @param {string} action "begin" | "end" (optional reason via "end:reason")
   */
  LayoutBatch(action) {
    try {
      const wm = this._wm();
      ...
      return JSON.stringify({
        ok: false,
        error: `LayoutBatch action want begin|end (got ${raw || "empty"})`,
      });
    } catch (e) {
      return JSON.stringify({ ok: false, error: String(e?.message || e) });
    }
