...
export class SessionApi {
  ...
  _snapshotForestForApply(run = {}) {
    ...
    try {
      const extra = this._orphanWindowProjections(tree, forest, { workspace });
...

    {
      ...
      const fp = `ws=${workspace ?? "-"} mons=${monN} orphans=${orphanN} activeWs=${
        activeWorkspace ?? "-"
  ...
  _moveInOp(selector, opts = {}) {
    ...
    const tree = wm?.tree;
    if (!tree?.moveIn) {
      return { error: "Tree moveIn not available" };
    }

    const dest = tree.moveIn(focusNode);
    if (!dest) {
  ...
  _moveOutOp(selector, opts = {}) {
    ...
    const tree = wm?.tree;
    if (!tree?.moveOut) {
      return { error: "Tree moveOut not available" };
    }

    const unit = tree.moveOut(focusNode);
    if (!unit) {
      return { ok: true, changed: false, reason: "at MONITOR/ROOT (no-op)" };
    }
