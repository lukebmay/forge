...
import { recordFallback, scanForestInvariants } from "./metrics.js";
...
export class SessionApi {
  ...
  _ensureLayoutApplyRuns() {
    ...
    this._layoutApplyRuns = new LayoutApplyRunBag({
      ...
      onApplyLive: (active, run) => {
        ...
        // D069/D071: force tab/stack peer Meta frames to shared slot before the
        // user can click — heal was skipped while ApplyEpoch was live.
        if (!active) {
          try {
          ...
          try {
            const n = wm.reassertAllTabStackSlots?.({ force: true }) ?? 0;
            if (n > 0) {
              Logger.info(`session-api ApplyLayout epoch-end tab-slot reassert groups=${n}`);
            }
            wm._schedulePostRenderTabSlotHeal?.();
          } catch (e) {
            Logger.debug?.(`session-api ApplyLayout epoch-end tab-slot: ${e}`);
          }
          try {
  ...
  _ensureApplySlotMetaInSlot(machine, _run) {
    ...
    for (const raw of wids) {
      ...
      if (typeof wm.reassertNodeToSlot === "function") {
        if (wm.reassertNodeToSlot(node, { force: true })) steps += 1;
      }
    }
    try {
  ...
  _applyNestClientLaunchEnv(launcherOrCtx) {
    ...
    try {
      if (typeof launcherOrCtx.setenv === "function") {
        launcherOrCtx.setenv("GSK_RENDERER", "cairo", true);
        launcherOrCtx.setenv("LIBGL_ALWAYS_SOFTWARE", "1", true);
      }
    } catch (_e) {
      /* */
    }
  ...
  /**
   * ApplyLayout planner IR from live Forest (DBus GetTree is Surface-only).
   * @param {object} [run]
   * @returns {object}
   */
  ...
  /**
   * Windows in the tree but not under projected monitors.
   * When `workspace` is set, only same-ws orphans (never leak other desks into clean/claim).
   * @param {object} tree
   * @param {object} forest
   * @param {{ workspace?: number|null, liveMap?: any }} [options]
   * @returns {object[]}
   */
  _orphanWindowProjections(tree, forest, options = {}) {
    const targetWs =
    ...
    const extra = [];
    const seen = new Set();
    let skippedOtherWs = 0;
    let skippedUnknownWs = 0;
    const walk = (n) => {
      if (!n) return;
    ...
    for (const node of tree.getNodeByType?.(NODE_TYPES.WINDOW) || []) {
      ...
      try {
        ...
        const wsObj = meta?.get_workspace?.();
        if (wsObj && typeof wsObj.index === "function") {
          const idx = wsObj.index();
          if (Number.isFinite(idx)) nodeWs = idx;
        }
      } catch (_e) {
        /* ignore */
      }
      if (nodeWs == null) {
        const mon =
          typeof tree.findParent === "function" ? tree.findParent(node, NODE_TYPES.MONITOR) : null;
        const mid = typeof mon?.nodeValue === "string" ? mon.nodeValue : null;
        if (mid) {
          const wsi = MonitorIdentity.workspaceFromId(mid);
          if (wsi >= 0) nodeWs = wsi;
        }
      }
      if (targetWs != null && Number.isFinite(targetWs)) {
        if (nodeWs == null || nodeWs !== targetWs) {
          if (nodeWs == null) skippedUnknownWs += 1;
          else skippedOtherWs += 1;
          continue;
        }
      }
      if (nodeWs != null) proj.workspace = nodeWs;
      seen.add(String(id));
      extra.push(proj);
    }
    if (targetWs != null && Number.isFinite(targetWs)) {
      const skipped = skippedOtherWs + skippedUnknownWs;
      // Summary only (per-id skip TRACE flooded hard-ready polls).
      const line = `orphan-filter want=${targetWs} kept=${extra.length} skipped=${skipped} otherWs=${skippedOtherWs} unknownWs=${skippedUnknownWs}`;
      if (extra.length > 0) Logger.debug(line);
      else Logger.trace(line);
    }
    return extra;
  }
  ...
  _setLayoutStructureOp(mode, selector, opts = {}) {
    ...
    if (forested?.ok) {
      const parent = forested.parentLive || focusNode.parentNode;
      try {
  ...
  /**
   * DBus GetTree — Surface dump for CLI/debug. Apply IR is `_snapshotForestForApply`.
  ...
  GetTree(optionsJson) {
    try {
      ...
      try {
        const extra = this._orphanWindowProjections(tree, forest, options);
  ...
  _sizeOp(windowIds, shares, opts = {}) {
    ...
    recordFallback("size", forestOut?.error || "forest-miss");
    ...
    if (sameParent) {
      ...
      for (const node of winNodes) {
        ...
        if (seen.has(monDirect)) {
          Logger.info(
            `size skip reason=duplicate mon-direct for size targets n=${winNodes.length}`
          );
          return {
            ok: true,
            sized: false,
      ...
      for (const md of monDirects) {
        if (md.parentNode !== parent) {
          Logger.debug(
            `size skip reason=size targets not under common parent n=${winNodes.length}`
          );
          return {
            ok: true,
            sized: false,
            reason: "size targets not under common parent",
          };
        }
      }
      targets = monDirects;
  ...
  _unwrapSingleChildSplit(node, mon, tree) {
    ...
    for (let guard = 0; guard < 8 && cur; guard++) {
      ...
      only.percent = cur.percent || 0;
      only.userSized = !!cur.userSized;
  ...
  _mergeGroupOp(selector, withSel, opts = {}) {
    ...
    const group = tree.group(focusNode, partner);
    if (!group) {
  ...
  _moveInOp(selector, opts = {}) {
    ...
    let dest = forestMoveIn(wm, focusNode);
    if (!dest && tree.moveIn) dest = tree.moveIn(focusNode);
    if (!dest) {
  ...
  _moveOutOp(selector, opts = {}) {
    ...
    let unit = forestMoveOut(wm, focusNode);
    if (!unit && tree.moveOut) unit = tree.moveOut(focusNode);
    if (!unit) {
      return { ok: true, changed: false, reason: "at MONITOR/ROOT (no-op)" };
    }
  ...
  _dndDropOp(tileSel, ontoSel, zone, opts = {}) {
    ...
    try {
      ...
      if (dropChangesStructure(focusNode, ontoNode, operation, ctx)) {
        commitDrop(focusNode, ontoNode, operation, ctx);
      }
    } catch (e) {
  ...
  _skeletonOp(mons, opts = {}) {
    ...
    try {
      for (const monSpec of mons) {
        ...
        if (forested && !forested.fallback && forested.error) return forested;
