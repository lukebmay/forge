...
export class SessionApi {
  ...
  _runStepHandlers() {
    return {
      ...
      "clear-selection": (step) => this._clearSelectionOp(step.selector, { quiet: true }),
  ...
  _orderMonChildrenOp(windowIds, opts = {}) {
    ...
    for (const md of monDirects) {
      if (md.parentNode !== parent) {
        return { error: "mon-directs not under same MONITOR" };
      }
    }
  ...
  _layoutOp(mode, selector, opts = {}) {
    ...
    // S2: elevated attach CON is the layout target when focus is default selector.
    const elevated = isElevatedSelection(tree.attachNode, focusNode);
    let parent = elevated
      ? resolveLayoutOpsTarget(tree.attachNode, focusNode)
      : focusNode.parentNode;
    if (!parent) {
      return { error: "window has no parent container" };
    }

    // Leaf-only: monitor parent → wrap in split first for tab/stack.
    if (
      !elevated &&
    ...
    // I1: mode only — no silent flatten, no percent wipe (explicit ungroup = C2).
    ...
    if (layoutValue === LAYOUT_TYPES.TABBED || layoutValue === LAYOUT_TYPES.STACKED) {
      ...
      layoutOpts.clearLastTabFocus = true;
    }
    if (!setLayout(parent, layoutValue, layoutOpts)) {
      return { error: `unsupported layout mode: ${mode}` };
    }
  ...
  _layoutCycleOp(axis, selector, opts = {}) {
    ...
    // S2: elevated CON when selected; else focus parent.
    const parent = resolveLayoutOpsTarget(tree.attachNode, focusNode);
    if (!parent) {
      return { error: "window has no parent container" };
    }
    ...
    if (ax === "group") {
      ...
      // I1: H↔V mode only — no percent wipe (equalize is window-reset-sizes).
    ...
    if (!setLayout(parent, next, layoutOpts)) {
      return { error: `failed to set layout ${next}` };
    }
    try {
      tree.attachNode = parent;
      if (!quiet) {
  ...
  /**
   * Explicit one-level ungroup (I2): dissolve nearest parent CON of selector.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _ungroupOp(selector, opts = {}) {
    ...
    const wm = this._wm();
    const tree = wm?.tree;
    if (!tree?.ungroupContainer) {
      return { error: "Tree ungroup not available" };
    }

    // S2: elevated CON when selected; else nearest parent of focus.
    const con = resolveUngroupOpsTarget(tree.attachNode, focusNode);
    if (!con) {
      return { ok: true, changed: false, reason: "no ungroup target" };
    }

    const grandparent = tree.ungroupContainer(con);
    if (!grandparent) {
      return { ok: true, changed: false, reason: "ungroup no-op" };
    }

    try {
      tree.attachNode = grandparent;
      if (!quiet) {
        wm?.unfreezeRender?.();
  ...
  /**
   * Focus parent CON (set attachNode; activate representative window).
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _focusParentOp(selector, opts = {}) {
    ...
    const focusNode = resolved.match.node;
    if (!focusNode) return { error: "not found", candidates: [] };
    ...
    let start = focusNode;
    const attach = tree.attachNode;
    if (
      attach &&
      attach !== focusNode &&
      (attach.nodeType === NODE_TYPES.CON || attach.isCon?.()) &&
      typeof attach.contains === "function" &&
      attach.contains(focusNode)
    ) {
      start = attach;
    }
    ...
    if (!parent) {
      return { ok: true, changed: false, reason: "no focus parent" };
    }

    const win = resolveRepresentativeWindow(parent, focusNode, {
      findNode: (meta) => tree.findNode?.(meta) || null,
    });
    if (win?.nodeValue) {
      try {
        win.nodeValue.raise?.();
        win.nodeValue.activate?.(global.display?.get_current_time?.() || 0);
        if (!quiet) {
    ...
    // Attach after activate so same-window re-focus keeps elevated CON (S1).
    tree.attachNode = parent;
    if (wm) wm._lastFocusNodeWindow = win || focusNode;
    if (!quiet) wm?.updateBorderLayout?.();
  ...
  /**
   * Focus preferred child of attach/parent CON.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _focusChildOp(selector, opts = {}) {
    ...
    if (
      ...
      (attach.nodeType === NODE_TYPES.CON || attach.isCon?.()) &&
      typeof attach.contains === "function" &&
      (attach.contains(focusNode) || attach === focusNode)
    ) {
      con = attach;
    } else if (
      focusNode.parentNode &&
      (focusNode.parentNode.nodeType === NODE_TYPES.CON || focusNode.parentNode.isCon?.())
    ) {
      con = focusNode.parentNode;
    }
    if (!con) {
      return { ok: true, changed: false, reason: "no focus child con" };
    }

    const child = resolveFocusChild(con, con.lastTabFocus ?? focusNode);
    if (!child || child === con) {
      return { ok: true, changed: false, reason: "no focus child" };
    }
    ...
    if (child.nodeType === NODE_TYPES.WINDOW || child.isWindow?.()) {
      ...
      nextAttach = child;
      win = resolveRepresentativeWindow(child, focusNode, {
        findNode: (meta) => tree.findNode?.(meta) || null,
      });
    }
    ...
    if (nextAttach) tree.attachNode = nextAttach;
    if (wm) wm._lastFocusNodeWindow = win || focusNode;
    if (!quiet) wm?.updateBorderLayout?.();
  ...
  /**
   * Clear elevated CON selection → focused (or selected) window leaf.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _clearSelectionOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    tree.attachNode = clearOpsTarget(focusNode);
    if (!quiet) wm?.updateBorderLayout?.();
  ...
  /**
   * Move layout unit one level out (parent CON kept).
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _moveOutOp(selector, opts = {}) {
    ...
    const tree = wm?.tree;
    if (!tree?.moveUnitOut) {
      return { error: "Tree moveUnitOut not available" };
    }

    const unit = resolveMoveUnit(tree.attachNode, focusNode);
    const gp = tree.moveUnitOut(unit);
    if (!gp) {
      return { ok: true, changed: false, reason: "move-out no-op" };
    }
  ...
  /**
   * Move layout unit into adjacent sibling CON.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _moveInOp(selector, opts = {}) {
    ...
    const tree = wm?.tree;
    if (!tree?.moveUnitIn) {
      return { error: "Tree moveUnitIn not available" };
    }

    const unit = resolveMoveUnit(tree.attachNode, focusNode);
    const target = tree.moveUnitIn(unit);
    if (!target) {
      return { ok: true, changed: false, reason: "move-in no-op" };
    }

    try {
      tree.attachNode = target;
      if (!quiet) {
        wm?.unfreezeRender?.();
