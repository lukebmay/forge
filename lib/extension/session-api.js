 ...
 * Log (session level / persist / truncate).
 */
...
import { scanForestInvariants } from "./metrics.js";
...
import {
  ...
  setSessionLevel,
  truncateLogFile,
} from "../shared/plog-adapter.js";
import { assertApplyForestWorkspace, assertionFailed } from "../shared/assert.js";
...
} from "../shared/nest-client-launch-env.js";
...
import { chromePwaDesktopIds, isChromeFamilyClass } from "./place-hint.js";
...
import { TREE_QUERY_API_VERSION, windowMetaFields } from "./tree-query.js";
import { projectForestFromTom } from "./forest-apply-snapshot.js";
import {
  ensureLiveForest,
  ...
  forestUnwrapUnaryLive,
  forestHoistNestedMonPanesLive,
  forestIdFromLive,
...
import { getOpSet, worldPointInMark2Zone } from "../opsets/index.js";
...
/** Combined control-plane version (+ Log session control). */
export const SESSION_API_VERSION = 11;
...
export class SessionApi {
  ...
  /**
   * Overlay FORGE_LAYOUT_* env onto ApplyLayout flags (nest / host hunts).
   * Request flags win when already set.
   * @param {{ flags?: object }|null|undefined} request
   */
  _mergeApplyLayoutEnvFlags(request) {
    if (!request || typeof request !== "object") return;
    if (!request.flags || typeof request.flags !== "object") request.flags = {};
    const flags = request.flags;
    const envTruth = (name) => {
      try {
        const raw = String(GLib.getenv(name) || "")
          .trim()
          .toLowerCase();
        if (!raw) return null;
    ...
    if (flags.forestFailsafe === undefined) {
      const v = envTruth("FORGE_LAYOUT_FAILSAFE");
      if (v != null) flags.forestFailsafe = v;
    }
    if (flags.chaos === undefined) {
      const v = envTruth("FORGE_LAYOUT_CHAOS");
      if (v === true) flags.chaos = true;
    }
    if (flags.chaosSeed == null || String(flags.chaosSeed).trim() === "") {
      try {
        const seed = String(GLib.getenv("FORGE_LAYOUT_CHAOS_SEED") || "").trim();
        if (seed) flags.chaosSeed = seed;
      } catch (_e) {
        /* */
      }
    }
  ...
  ApplyLayout(request_json) {
    try {
      ...
      // Env overlays for nest/dev hunts (do not require CLI flag plumbing).
      try {
        this._mergeApplyLayoutEnvFlags?.(parsed.request);
      } catch (_e) {
        /* optional */
      }
      Logger.debug(
        `ApplyLayout start name=${parsed.request?.name || "-"} ws=${
          parsed.request?.workspace ?? "?"
        }`
      );
  ...
  _ensureLayoutApplyRuns() {
    ...
    this._layoutApplyRuns = new LayoutApplyRunBag({
      ...
      onApplyLive: (active, run) => {
        ...
        // D095 S5: no opportunistic epoch-end geometry-force / post-render heal.
        if (!active) {
          try {
          ...
          try {
            ...
            if (forest && wm._liveForestSeeded) {
            ...
            Logger.debug?.(`session-api ApplyLayout unary-settle: ${e}`);
          }
          try {
            scanForestInvariants(wm.forest);
          } catch (e) {
            Logger.debug?.(`session-api ApplyLayout invariant scan: ${e}`);
          }
        }
      },
      onDone: (payload) => {
      ...
      onChromeShow: ({ name, hardMs }) => {
        ...
        try {
          ...
          if (wm?.layoutApplyChrome?.showForApplyRun) {
            ...
            if (enabled) {
              wm.layoutApplyChrome.setNotice?.(null);
              wm.layoutApplyChrome.setStages?.([]);
      ...
      onChromeNotice: ({ notice } = {}) => {
        try {
          this._wm()?.layoutApplyChrome?.setNotice?.(notice ?? null);
        } catch (e) {
          Logger.debug?.(`session-api ApplyLayout chrome notice: ${e}`);
        }
      },
      onChromeStage: ({ label, status } = {}) => {
        try {
          if (label != null) this._wm()?.layoutApplyChrome?.setStage?.(label, status || "pending");
        } catch (e) {
          Logger.debug?.(`session-api ApplyLayout chrome stage: ${e}`);
        }
      },
      onChromeClear: ({ reason } = {}) => {
        ...
        try {
          ...
          try {
            wm?.clearAllPlaceHints?.(reason || "apply-end");
          } catch (e2) {
            Logger.debug?.(`session-api ApplyLayout place-hint clear: ${e2}`);
          }
      ...
      onPhaseEnter: ({ phase } = {}) => {
        ...
        try {
          ...
          if (phase != null) wm?.layoutApplyChrome?.setStage?.(String(phase), "pending");
        } catch (e) {
  ...
  _layoutApplyOpenDeps() {
    return {
      ...
      loadWindows: () => this._loadApplyPinWindows(this._applyLiveRun()),
  ...
  _layoutApplySettleDeps() {
    return {
      ...
      explainWindowFloat: (windowId) => this._explainApplyWindowFloat(windowId),
      ensureMetaInSlot: (machine, run) => this._ensureApplySlotMetaInSlot(machine, run),
    };
  }

  /**
   * D040 hollow retry: forest replan emitted no moves, but Meta mode/mon may
   * still fail in-slot. Use WM tile policy + reassertNodeToSlot (not a twin planner).
   * @param {object|null|undefined} machine
   * @param {object|null|undefined} [_run]
   * @returns {{ ok: boolean, steps: number, error?: string }}
   */
  _ensureApplySlotMetaInSlot(machine, _run) {
    const wm = this._wm();
    if (!wm?.tree) return { ok: false, steps: 0, error: "no-wm" };
    const wids = Array.isArray(machine?.windowIds) ? machine.windowIds : [];
    if (!wids.length) return { ok: true, steps: 0 };
    let steps = 0;
    for (const raw of wids) {
      ...
      try {
        ...
        for (const n of nodes) {
          const m = n?.nodeValue;
          if (!m || typeof m.get_id !== "function") continue;
          if (String(m.get_id()) !== id) continue;
          node = n;
          meta = m;
          break;
        }
      ...
      if (typeof wm._ensureTiledForSlotPlace === "function") {
        wm._ensureTiledForSlotPlace(meta, node);
      }

      let wantMon = machine?.slots?.[id]?.monitor;
      if (wantMon == null && machine?.key) {
        const m = String(machine.key).match(/^mon(\d+)/);
        if (m) wantMon = Number(m[1]);
      }
      const monN = Number(wantMon);
      if (Number.isFinite(monN) && monN >= 0) {
        try {
          if (typeof meta.get_monitor === "function" && meta.get_monitor() !== monN) {
            safeMoveToMonitor(meta, monN, "slot-ensure-meta");
            steps += 1;
          }
        } catch (_e) {
          /* */
      ...
      if (typeof wm.reassertNodeToSlot === "function") {
        // D095: evidence-only (no ε-bypass force). Log write vs in-slot skip.
        const wrote = wm.reassertNodeToSlot(node, { force: false });
        if (wrote) {
          steps += 1;
          Logger.debug?.(`slot-ensure-meta evidence-write id=${id}`);
        } else {
          Logger.debug?.(`slot-ensure-meta skip-in-slot id=${id}`);
        }
      }
    }
    try {
      wm.commitLayout?.("slot-ensure-meta", { force: true });
    } catch (_e) {
      try {
        wm.renderTree?.("slot-ensure-meta", true);
      } catch (_e2) {
        /* */
      }
    }
    return { ok: true, steps: Math.max(1, steps) };
  }

  /**
   * Live Meta float decision for hard-ready diagnostics.
   * @param {string|number} windowId
   * @returns {object|null}
   */
  _explainApplyWindowFloat(windowId) {
    const id = windowId != null ? String(windowId) : "";
    if (!id) return null;
    const wm = this._wm();
    if (!wm?.tree || typeof wm._processFloatDecision !== "function") return null;
    try {
      const nodes = wm.tree.getNodeByType?.(NODE_TYPES.WINDOW) || [];
      for (const n of nodes) {
        const meta = n?.nodeValue;
        if (!meta || typeof meta.get_id !== "function") continue;
        if (String(meta.get_id()) !== id) continue;
        const decision = wm._processFloatDecision(n, meta);
        let metaMon = null;
        try {
          metaMon = typeof meta.get_monitor === "function" ? meta.get_monitor() : null;
        } catch (_e) {
          metaMon = null;
        }
        let treeMon = null;
        try {
          treeMon = wm._monitorIndexOfNode?.(n);
        } catch (_e) {
          treeMon = null;
        }
        return {
  ...
  _loadApplyPinWindows(run) {
    ...
    const live = run || this._applyLiveRun();
    const forest = this._snapshotForestForApply(live);
    try {
    ...
    return collectWindows(forest, ws != null && ws !== "" ? { workspace: ws } : {});
  }
  ...
  _spawnApplyLaunch(fields, action) {
    ...
    try {
      // Ghostty: never DesktopAppInfo.launch (stock .desktop is single-instance
      // and ignores --gtk-single-instance=false). Path-like: argv, not Name search.
      const argvPlan = argvForApplyLaunch(app);
      if (argvPlan && argvPlan.length) {
        const role = action?.role != null ? String(action.role) : "-";
        Logger.info(`apply spawn argv role=${role} exe=${argvPlan[0]}`);
        const pid = this._spawnArgvDetached(argvPlan);
        return { ok: true, pid, ...payload };
      }

      const info = this._desktopAppInfoForLaunch(app, fields);
      if (info) {
        // Nest: never DesktopAppInfo.launch — DBusActivatable apps (Nautilus)
        // are spawned by dbus-daemon with host WAYLAND and map on the host desk.
        if (this._isNestShellEnv()) {
          const nestArgv =
            this._nestChromeLaunchArgv(info, fields, action) || this._desktopInfoArgv(info);
          if (nestArgv?.length) {
            const pid = this._spawnArgvDetached(nestArgv);
            return {
              ok: true,
              pid,
              desktop: info.get_id?.() || app,
              nestDirect: true,
              ...payload,
            };
          }
        }
        const nestChromeArgv = this._nestChromeLaunchArgv(info, fields, action);
        if (nestChromeArgv) {
          const pid = this._spawnArgvDetached(nestChromeArgv);
          return {
            ok: true,
            pid,
            desktop: info.get_id?.() || app,
        ...
        const ctx =
          typeof global.create_app_launch_context === "function"
            ...
            : null;
        this._applyNestClientLaunchEnv(ctx);
      ...
      // Bare PATH cmds that are not Ghostty / path-like (desktop lookup missed).
  ...
  /**
   * Nest dummy-mon shell: GTK4 GL often never maps clients (ghostty open-miss).
   * Force cairo when FORGE_HOST is nest-scoped or dummy monitors are active.
   */
  _isNestShellEnv() {
    try {
      const host = String(GLib.getenv("FORGE_HOST") || "");
      if (host.includes("-sub-")) return true;
    } catch (_e) {
      /* */
    }
    try {
      const n = String(GLib.getenv("MUTTER_DEBUG_NUM_DUMMY_MONITORS") || "");
  ...
  _applyNestClientLaunchEnv(launcherOrCtx) {
    if (!this._isNestShellEnv() || !launcherOrCtx) return;
    try {
      if (typeof launcherOrCtx.setenv !== "function") return;
      // Full nest client_env rewrite — host WAYLAND/XDG on the nest Shell
      // process must not leak to DesktopAppInfo / Subprocess children (spill).
      let nestWl = "";
      let nestState = "";
      let forgeHost = "";
      let forgeConfigHome = "";
      try {
        nestWl = String(GLib.getenv("FORGE_NEST_WAYLAND_DISPLAY") || "").trim();
      } catch (_e) {
        /* */
      }
      try {
        nestState = String(GLib.getenv("FORGE_NEST_STATE_DIR") || "").trim();
      } catch (_e) {
        /* */
      }
      try {
        forgeHost = String(GLib.getenv("FORGE_HOST") || "").trim();
      } catch (_e) {
        /* */
      }
      try {
        forgeConfigHome = String(GLib.getenv("FORGE_CONFIG_HOME") || "").trim();
      } catch (_e) {
        /* */
      }
      if (typeof launcherOrCtx.unsetenv === "function") {
        for (const key of NEST_CLIENT_UNSET_KEYS) {
          try {
            launcherOrCtx.unsetenv(key);
          } catch (_e) {
            /* */
          }
        }
      ...
      const vars = nestClientLaunchEnvVars({
        nestStateDir: nestState,
        nestWaylandDisplay: nestWl,
        forgeHost,
        forgeConfigHome,
      });
      for (const [key, value] of Object.entries(vars)) {
        try {
          launcherOrCtx.setenv(key, String(value), true);
        } catch (_e) {
          /* */
        }
      }
      if (nestWl && nestState) {
        try {
          Logger.info(`nest-client-launch env wayland=${nestWl} runtime=${nestState}/runtime`);
        } catch (_e) {
          /* */
        }
      }
  ...
  /** DesktopAppInfo commandline without DBus activation (nest spill guard). */
  _desktopInfoArgv(info) {
    if (!info) return null;
    let cmdline = "";
    try {
      cmdline = String(info.get_commandline?.() || "");
    } catch (_e) {
      cmdline = "";
    }
    let argv = shellSplit(cmdline.replace(/%\w/g, " ").trim());
    if (!argv.length) {
      const bin = info.get_executable?.();
      if (!bin) return null;
      argv = [String(bin)];
    }
    return argv;
  ...
  _nestChromeLaunchArgv(info, fields, action) {
    if (!this._isNestShellEnv() || !info) return null;
    const desktopId = String(info.get_id?.() || "").toLowerCase();
    const exe = String(info.get_executable?.() || "").toLowerCase();
    const wc = String(fields?.wm_class ?? fields?.wmClass ?? action?.match?.class ?? "");
    const chrome =
      desktopId.includes("chrome") ||
      exe.includes("chrome") ||
      exe.includes("chromium") ||
      isChromeFamilyClass(wc);
    ...
    const configHome = String(GLib.getenv("FORGE_CONFIG_HOME") || "").trim();
    if (!configHome) return null;
    const profile = GLib.build_filenamev([GLib.path_get_dirname(configHome), "chrome-profile"]);
    try {
      GLib.mkdir_with_parents(profile, 0o755);
    } catch (_e) {
      /* */
    }
    if (!argv.some((a) => String(a).startsWith("--user-data-dir="))) {
      argv = [argv[0], `--user-data-dir=${profile}`, ...argv.slice(1)];
    }
    if (!argv.includes("--no-first-run")) argv.push("--no-first-run");
    if (!argv.some((a) => String(a).startsWith("--ozone-platform="))) {
      argv.push("--ozone-platform=wayland");
    }
    return argv;
  }
  ...
  _spawnArgvDetached(argv) {
    ...
    try {
      // Nest: cwd under nest HOME so relative paths / GLib home stay isolated.
      let cwd = GLib.get_home_dir();
      if (this._isNestShellEnv()) {
        const nestState = String(GLib.getenv("FORGE_NEST_STATE_DIR") || "").trim();
        if (nestState) {
          cwd = GLib.build_filenamev([nestState, "home"]);
          try {
            GLib.mkdir_with_parents(cwd, 0o755);
          } catch (_e) {
            /* */
          }
        }
    ...
    for (const key of applyLaunchUnsetEnvKeys(this._isNestShellEnv())) {
      try {
    ...
    this._applyNestClientLaunchEnv(launcher);
    const proc = launcher.spawnv(argv);
    if (!this._spawnedSubprocs) this._spawnedSubprocs = new Set();
    this._spawnedSubprocs.add(proc);
    try {
      proc.wait_async(null, () => {
        this._spawnedSubprocs.delete(proc);
      });
    } catch (_e) {
      /* */
    }
  ...
  /**
   * ApplyLayout planner IR from live Forest (same projector as GetTree).
   * @param {object} [run]
   * @returns {object}
   */
  _snapshotForestForApply(run = {}) {
    const wm = this._wm();
    const tree = wm?.tree ?? this._ext?.extWm?.tree;
    if (!tree) {
    ...
    let liveMap = null;
    try {
      liveMap = wm?.getMonitorLiveMap?.() ?? this._ext?.extWm?.getMonitorLiveMap?.() ?? null;
    } catch (_e) {
      liveMap = null;
    }
    ...
    let tom = null;
    try {
      tom = wm ? ensureLiveForest(wm) : null;
    } catch (e) {
      Logger.warn(`apply-snapshot forest seed: ${e}`);
      tom = wm?.forest || null;
    }
    if (!tom) tom = wm?.forest || null;
    ...
    try {
      const focusMeta = wm?.focusMetaWindow ?? null;
      if (focusMeta && wm?.hostBag) {
        focusWindowId = wm.hostBag.idFromMeta(focusMeta) || null;
      }
      if (focusWindowId == null && tom?.focusId) focusWindowId = tom.focusId;
      if (focusWindowId == null && focusMeta) {
    ...
    try {
      ...
      if (lftMeta && wm?.hostBag) {
        lastTileFocusWindowId = wm.hostBag.idFromMeta(lftMeta) || null;
      }
      if (lastTileFocusWindowId == null && lftMeta) {
    ...
    const forest = projectForestFromTom(tom, wm?.hostBag, {
      liveById: wm?.liveById || null,
      liveMap,
      workspace,
      focusWindowId,
    ...
    {
      const monN = Array.isArray(forest.monitors) ? forest.monitors.length : 0;
      const orphanN = Array.isArray(forest.orphanWindows) ? forest.orphanWindows.length : 0;
      const monIds = (forest.monitors || [])
        .map((m) => {
          const kids = Array.isArray(m?.children) ? m.children.length : 0;
          let wins = 0;
          const walk = (n) => {
            if (!n || typeof n !== "object") return;
            if (n.nodeType === "WINDOW" || n.kind === "WINDOW") {
              wins += 1;
              return;
            }
          ...
          walk(m);
          return `${m?.id || "?"}:c${kids}w${wins}`;
        })
        .join(",");
      const via = wm?._liveForestSeeded && tom ? "tom" : "empty";
      const fp = `via=${via} ws=${workspace ?? "-"} mons=${monN} orphans=${orphanN} activeWs=${
        activeWorkspace ?? "-"
      } nWs=${nWorkspaces ?? "-"} monIds=${monIds}`;
      if (fp !== this._lastApplySnapshotFp) {
        this._lastApplySnapshotFp = fp;
        Logger.debug(`apply-snapshot ${fp}`);
      } else {
        Logger.trace(`apply-snapshot repeat ${fp}`);
      }
    }

    if (workspace != null) {
      assertApplyForestWorkspace(forest, workspace, { ws: workspace });
    }
  ...
  _runApplyLayoutSteps(steps, ctx = {}) {
    if (assertionFailed()) {
      return { ok: false, error: "assertion failed", code: "assert-failed" };
    }
    const wm = this._wm();
    if (!wm) {
    ...
    Logger.debug(
      `apply-steps phase=${ctx.phase || "-"} n=${Array.isArray(steps) ? steps.length : 0}`
    );
    ...
    if (!dispatchResult?.ok) {
      ...
      const failIdx = failed?.index != null ? failed.index : dispatchResult?.stoppedAt;
      const failOp =
        Number.isFinite(failIdx) && Array.isArray(steps) && steps[failIdx]
          ? String(steps[failIdx].op || "-")
          : "-";
      const err = failed?.error || `${ctx.phase || "structure"} steps failed`;
      Logger.warn("apply-steps failed", {
        fields: {
          phase: ctx.phase || "-",
          index: Number.isFinite(failIdx) ? failIdx : -1,
          op: failOp,
          error: String(err),
          which: failed?.which || "-",
        },
  ...
  _setLayoutStructureOp(mode, selector, opts = {}) {
    ...
    if (!this._hasForestOrLiveParent(wm, focusNode)) {
    ...
    const forested = forestApplyLayoutStructure(wm, focusNode, layoutValue, { structure: true });
    if (forested?.ok) {
      const parent =
        forested.parentLive || this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
      try {
        if (parent) tree.attachNode = parent;
        if (!quiet) {
          wm?.unfreezeRender?.();
          if (typeof wm?.commitLayout === "function") {
    ...
    if (isTabOrStack && parent) {
      ...
      // Nested H/V bags (free-open aspect-split): lift *this window* to the
      // monitor, then wrap — do not peel sibling CONs (REG-ensure-flatten).
      if (hasNestedCon && !isMon) {
        const mon =
          (typeof tree.findParent === "function" &&
            tree.findParent(liveNode, NODE_TYPES.MONITOR)) ||
          null;
        if (mon && parent !== mon && typeof mon.appendChild === "function") {
          try {
            mon.appendChild(liveNode);
          } catch (e) {
            return {
              error: `ensure_layout lift nested: ${e?.message || e}`,
              code: "ensure-lift-failed",
            };
          }
          try {
  ...
  /**
   * DBus GetTree — Forest+bag dump for CLI/debug (D096 G6).
  ...
  GetTree(optionsJson) {
    try {
      ...
      const wm = this._wm() ?? this._ext?.extWm;
      if (!wm) {
      ...
      let tom = null;
      try {
        tom = ensureLiveForest(wm);
      } catch (e) {
        Logger.warn(`GetTree forest seed: ${e}`);
        tom = wm.forest || null;
      }
      if (!tom) {
        return JSON.stringify({ error: "Forest not available" });
      }
      ...
      // Do NOT sync lastTabFocus from Meta keyboard focus (D018 / R014).
      let focusWindowId = null;
      try {
        const focusMeta = wm.focusMetaWindow ?? null;
        if (focusMeta && wm.hostBag) {
      ...
      if (workspace != null && workspace !== "") {
        assertApplyForestWorkspace(forest, workspace);
      }

      if (options.includeMeta || options.metaWindows) {
        try {
          forest.metaWindows = wm.censusMetaWindows?.() ?? [];
        } catch (_e) {
          forest.metaWindows = [];
        }
      }
  ...
  /**
   * DBus: Log(request_json) → status / session set / reset / truncate.
   * request: { op: "status"|"set"|"reset"|"truncate", level?, persist? }
   * Session set does not write gsettings; persist uses SetSetting path.
   * @param {string} requestJson
   */
  Log(requestJson) {
    try {
      let req = {};
      const raw = requestJson == null ? "" : String(requestJson).trim();
      if (raw) {
        try {
          req = JSON.parse(raw);
        } catch {
          return JSON.stringify({ error: "Log request must be JSON object" });
        }
      }
      ...
      if (op === "status") {
        return JSON.stringify({ ok: true, ...getLogStatus() });
      }

      if (op === "reset") {
        const status = clearSessionLevel();
        return JSON.stringify({ ok: true, op: "reset", ...status });
      }

      if (op === "truncate") {
        const status = truncateLogFile();
        return JSON.stringify({ ok: true, op: "truncate", ...status });
      }

      if (op === "set") {
        const parsed = parseLevelName(req.level);
        if (!parsed.ok) {
        ...
        if (persist) {
          ...
          if (parsed.num === LOG_LEVELS.OFF) {
            const off = sync.setPortable("logging-enabled", false);
            if (!off.ok) return JSON.stringify({ error: off.error });
          } else {
            const en = sync.setPortable("logging-enabled", true);
            if (!en.ok) return JSON.stringify({ error: en.error });
            const lv = sync.setPortable("log-level", parsed.num);
            if (!lv.ok) return JSON.stringify({ error: lv.error });
          }
          // changed:: may already have reconfigured; status reflects durable.
          return JSON.stringify({
            ok: true,
            op: "set",
            persist: true,
        ...
        const status = setSessionLevel(parsed.num);
        return JSON.stringify({ ok: true, op: "set", persist: false, ...status });
      }

      return JSON.stringify({
        error: `unknown Log op ${JSON.stringify(op)} (status|set|reset|truncate)`,
      });
    } catch (e) {
      return JSON.stringify({ error: String(e?.message || e) });
    }
  ...
  _settleAfterRunSteps(wm) {
    ...
    try {
      ...
      const cons = liveStackedOrTabbedConsForPresent(wm, { root: wm.currentWsNode });
  ...
  /**
   * Sync each TABBED/STACKED strip attach/track (no raise). Layer owns Z vs Meta.
   * @param {import('./window.js').WindowManager} wm
   */
  ...
  _tabSettleFocusNode(wm, con) {
    return liveTabOpenLeafForPresent(wm, con);
  }
  ...
  _runStepHandlers() {
    return {
      ...
      group: (step) => this._mergeGroupOp(step.selector, step.with, { quiet: true }),
      ungroup: (step) => this._ungroupOp(step.selector, { quiet: true }),
      "focus-parent": (step) => this._focusParentOp(step.selector, { quiet: true }),
      "focus-child": (step) => this._focusChildOp(step.selector, { quiet: true }),
      "move-in": (step) => this._moveInOp(step.selector, { quiet: true }),
      "move-out": (step) => this._moveOutOp(step.selector, { quiet: true }),
      "dnd-drop": (step) =>
  ...
  _closeOp(selector, opts = {}) {
    ...
    let forestId = null;
    try {
      const d = parseSelector(selector);
      if (d?.kind === "id" && d.value != null && String(d.value) !== "") {
        forestId = String(d.value);
      }
    } catch (_e) {
      /* bad selector handled below */
    }

    const dropForest = (liveOrMetaOrId) => {
      if (!seeded || liveOrMetaOrId == null) return false;
      try {
        return !!forgetHostWindow(wm, liveOrMetaOrId, "close");
      } catch (_e) {
        /* fall through */
      }
      const removed = forestRemoveWindow(wm, liveOrMetaOrId);
      if (!removed && typeof liveOrMetaOrId === "string") return false;
      try {
        for (const m of wm.forest?.monitors || []) {
          if (m && wm.forest.nodes[m.id]) mark2CleanupUnder(wm.forest, m);
        }
      } catch (_e2) {
        /* best-effort */
      }
      try {
        resyncWmToReality(wm, "close");
      } catch (_e2) {
        /* best-effort */
      }
      try {
    ...
    if (!resolved.ok) {
      // Resolve miss: still drop Forest by id: nanoid when bag/Forest has it.
      if (seeded && forestId) {
        const entry = wm.hostBag?.get?.(forestId);
        const live = wm.liveById?.get?.(forestId);
        const meta = entry?.meta ?? live?.nodeValue ?? null;
        if (meta && typeof meta.delete === "function") {
          ...
          try {
            meta.delete(closeStamp());
          } catch (e) {
            return { error: `delete failed: ${e.message || e}` };
          }
          dropForest(live || meta || forestId);
          return {
            ok: true,
            closed: true,
            force,
        ...
        if (wm.forest?.nodes?.[forestId]?.kind === "WINDOW" || live || entry) {
          dropForest(live || forestId);
          return {
            ok: true,
            closed: false,
            alreadyGone: true,
    ...
    if (!forestId) {
      forestId = forestWindowId(wm, node) || forestIdFromLive(wm, node) || null;
    }
    if (!meta) {
      const removed = dropForest(node || forestId);
      return {
        ok: true,
        closed: false,
        alreadyGone: true,
        forestRemoved: !!removed,
        selector: String(selector || ""),
      };
    }
    ...
    // Eager Forest drop — actor-destroy may miss GObject-detached lives after join.
    const forestRemoved = dropForest(node || meta || forestId);

    return {
      ok: true,
      closed: true,
      force,
      forestRemoved: !!forestRemoved,
      candidate: candidatePublic(resolved.match),
    };
  }
  ...
  _swapOp(selectorA, selectorB, opts = {}) {
    ...
    if (!forestSwapWindows(wm, a.match.node, b.match.node)) {
      return { error: "Forest swap ids miss" };
    }
   ...
   * Forest membership first (G8o); GObject parentNode is stale after seed.
   * @param {object} node
   * @returns {number}
   */
  _monitorIndexOfNode(node) {
    const wm = this._wm?.() ?? this._wm;
    let n = node;
    const seen = new Set();
    while (n && !seen.has(n)) {
      seen.add(n);
      ...
      const next = this._membershipParentLive(wm, n) || n.parentNode;
    ...
    if (forest && tom) {
      ...
      if (mon) {
        const liveMon = wm.liveById?.get?.(mon.id);
        const idx = Utils.monitorIndex(liveMon?.nodeValue ?? mon.id);
  ...
  _moveOp(selector, dest, opts = {}) {
    ...
    const forestMoved = forestReparent(wm, sourceNode, destNode, {
      position: opts.position,
      destIsWindow: destType === "WINDOW",
    });
    if (!forestMoved) {
      return { error: "Forest reparent ids miss" };
    }

    // Align Mutter before layout render (paint may leave Meta on prior mon).
    try {
  ...
  _monDirectAncestor(node) {
    ...
    while (n && !seen.has(n)) {
      seen.add(n);
      const p = this._membershipParentLive(wm, n) || n.parentNode;
      if (!p) break;
  ...
  _sizeOp(windowIds, shares, opts = {}) {
    ...
    const forestOut = forestSizeWindows(wm, winNodes, fracs);
    if (!forestOut?.ok) {
      return { error: forestOut?.error || "Forest size miss" };
    }
    if (forestOut.sized === false && forestOut.reason) {
      Logger.info(
        `size skip reason=${forestOut.reason} n=${winNodes.length}` +
          (forestOut.distinct != null ? ` distinct=${forestOut.distinct}` : "")
      );
    }
    try {
      if (!quiet && forestOut.sized) {
        wm?.unfreezeRender?.();
        if (typeof wm?.commitLayout === "function") {
  ...
  _orderMonChildrenOp(windowIds, opts = {}) {
    ...
    const forestOut = forestOrderWindows(wm, winNodes);
    if (forestOut?.ok) {
      try {
        if (!quiet && forestOut.reordered) {
          wm?.unfreezeRender?.();
          if (typeof wm?.commitLayout === "function") {
    ...
    const parentOf = (n) => this._membershipParentLive(wm, n) || n.parentNode;
    const sharedParent = parentOf(winNodes[0]);
    const sameParent = sharedParent && winNodes.every((n) => parentOf(n) === sharedParent);
    if (sameParent) {
    ...
    {
      const monParent = parentOf(monDirects[0]);
      if (monParent) {
        const unwrapped = [];
        const seenU = new Set();
    ...
    const parent = parentOf(monDirects[0]);
    if (!parent) {
   ...
   */
  _hoistNestedMonPanes(winNodes, _tree) {
    return forestHoistNestedMonPanesLive(this._wm(), winNodes);
  }
  ...
  _unwrapMonDirectSingleChildSplits() {
    ...
    for (const mon of mons) {
      if (!mon) continue;
      const kids = [...liveChildrenForPresent(wm, mon)];
   ...
   * Copy wrapper percent/userSized onto the promoted child; never equalize mon siblings.
   * @param {object} node
   * @param {object} [_mon]
   * @param {object} [_tree]
   * @returns {object} mon-direct node after unwrap
   */
  _unwrapSingleChildSplit(node, _mon, _tree) {
    return forestUnwrapUnaryLive(this._wm(), node) || node;
  }
  ...
  _reorderParentChildren(parent, orderedNodes, ctx = {}) {
    ...
    const wm = ctx.wm || this._wm();
    const kids = [...liveChildrenForPresent(wm, parent)];
    const ordered = [];
    ...
    if (!forestOrderLiveChildren(wm, parent, ordered)) {
      return { error: "Forest order unavailable" };
    }
  ...
  _layoutOp(mode, selector, opts = {}) {
    ...
    const forested = forestApplyLayoutStructure(wm, focusNode, layoutValue, { structure: false });
    if (forested?.ok) {
      const parent =
        forested.parentLive || this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
      try {
    ...
    // Tab/stack: lift nested non-MONITOR bags (parity with _setLayoutStructureOp);
    // wrap mon-direct / multi-window H/V. Never peel sibling CONs.
    let liveNode = focusNode;
    let parent = liveNode.parentNode;
    if (isTabOrStack && parent) {
      let isMon =
      ...
      if (hasNestedCon && !isMon) {
        ...
        if (mon && parent !== mon && typeof mon.appendChild === "function") {
          try {
            ...
            return {
              error: `layout lift nested: ${e?.message || e}`,
              code: "ensure-lift-failed",
            };
          }
          try {
        ...
        if (hasNestedCon && !isMon) {
          return {
            error: "layout needs flatten of nested CONs; refused (use ungroup or fix structure)",
            code: "ensure-flatten-refused",
          };
        }
      }
  ...
  _layoutCycleOp(axis, selector, opts = {}) {
    ...
    const parent = this._membershipParentLive(wm, focusNode);
    if (!parent) {
      return { error: "window has no parent container" };
    }
    ...
    const setOpts = { resetPercents: !!layoutOpts.resetPercents };
    if (Object.prototype.hasOwnProperty.call(layoutOpts, "lastTabFocus")) {
      setOpts.lastTabFocusLive = layoutOpts.lastTabFocus == null ? null : focusNode;
    }
    const forested = forestSetLayout(wm, parent, next, setOpts);
    if (!forested) tree.setLayout(parent, next, layoutOpts);
    try {
      tree.attachNode = parent;
      if (!quiet) {
        wm?.unfreezeRender?.();
  ...
  _mergeGroupOp(selector, withSel, opts = {}) {
    ...
    const tree = wm?.tree;
    if (!wm) {
      return { error: "no window manager" };
    }
    ...
    if (withSel != null && String(withSel).trim() !== "") {
      ...
      if (
        ...
        const parent = this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
        const kids = parent ? liveChildrenForPresent(wm, parent) : parent?.childNodes || [];
        const siblings = (tree.getTiledChildren?.(kids) || kids).filter(
          (n) => n.nodeType === NODE_TYPES.WINDOW && n !== focusNode
        );
    ...
    // D044: dest = focus mon; cross-mon partner rehomes then joins (no straddle).
    let group = null;
    if (wm._liveForestSeeded) {
      group = forestMergeWindowsIntoGroup(wm, focusNode, partner, LAYOUT_TYPES.TABBED);
    } else if (tree?.group) {
      group = tree.group(focusNode, partner);
    } else {
      return { error: "Tree group not available" };
    }
    if (!group) {
    ...
    return {
      ok: true,
      mode: group.layout || LAYOUT_TYPES.TABBED,
      candidate: candidatePublic(resolved.match),
      homeMonitor:
        typeof tree?.groupHomeMonitor === "function" ? tree.groupHomeMonitor(group) : undefined,
    };
  }

  /**
   * I2: dissolve the selector's parent CON (or the CON itself).
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _ungroupOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    const tree = wm?.tree;
    if (!tree?.ungroup) {
      return { error: "Tree ungroup not available" };
    }

    const parent = tree.ungroup(focusNode);
    if (!parent) {
      return { ok: true, changed: false, reason: "not a CON (MONITOR/ROOT no-op)" };
    }

    try {
      tree.attachNode = focusNode;
      if (!quiet) {
        wm?.unfreezeRender?.();
        if (typeof wm?.commitLayout === "function") {
          wm.commitLayout("session-ungroup", { force: true });
        } else {
          wm?.renderTree?.("session-ungroup", true);
        }
      }
    } catch (e) {
      Logger.warn(`session-api ungroup post: ${e}`);
    }
  ...
  /**
   * C4: elevate focus to parent CON unit; activate leaf via reveal/afterFocus.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _focusParentOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    const tree = wm?.tree;
    if (!tree?.focusParent) {
      return { error: "Tree focusParent not available" };
    }

    const win = tree.focusParent(focusNode);
    if (!win) {
      return { ok: true, changed: false, reason: "at MONITOR/ROOT (no-op)" };
    }

    try {
      if (!quiet) {
        const winParent = this._membershipParentLive(wm, win) || win.parentNode;
        if (winParent?.isStackedOrTabbed?.()) {
          wm?.revealGroupChild?.(win, { keyboard: true, source: "session-focus-parent" });
        } else {
          tree._activateWindowNode?.(win, undefined);
          wm?.afterFocus?.(win, { source: "session-focus-parent" });
        }
      }
    } catch (e) {
      Logger.warn(`session-api focus-parent post: ${e}`);
    }
    ...
    return {
      ...
      changed: true,
      candidate: candidatePublic(resolved.match),
      focusUnitType: tree.focusUnit?.nodeType,
      windowId: win?.nodeValue?.get_id?.() ?? win?.nodeValue?.get_stable_id?.(),
    };
  }

  /**
   * C4: descend focus into child unit.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _focusChildOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    const tree = wm?.tree;
    if (!tree?.focusChild) {
      return { error: "Tree focusChild not available" };
    }

    const win = tree.focusChild(focusNode);
    if (!win) {
      return { ok: true, changed: false, reason: "no child (window or empty CON)" };
    }
    ...
    try {
      if (!quiet) {
        const winParent = this._membershipParentLive(wm, win) || win.parentNode;
        if (winParent?.isStackedOrTabbed?.()) {
          wm?.revealGroupChild?.(win, { keyboard: true, source: "session-focus-child" });
        } else {
          tree._activateWindowNode?.(win, undefined);
          wm?.afterFocus?.(win, { source: "session-focus-child" });
        }
      }
    } catch (e) {
      Logger.warn(`session-api focus-child post: ${e}`);
    }
  ...
  /**
   * C4: move layout unit into sibling CON.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _moveInOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    const dest = forestMoveIn(wm, focusNode);
    if (!dest) {
      return { ok: true, changed: false, reason: "no sibling CON" };
    }

    try {
      if (typeof dest.isStackedOrTabbed === "function" && dest.isStackedOrTabbed()) {
        wm?.normalizeGroupToHomeMonitor?.(dest);
      }
      tree.attachNode = focusNode;
      if (!quiet) {
        wm?.unfreezeRender?.();
        if (typeof wm?.commitLayout === "function") {
          wm.commitLayout("session-move-in", { force: true });
        } else {
          wm?.renderTree?.("session-move-in", true);
        }
        if (typeof dest.isStackedOrTabbed === "function" && dest.isStackedOrTabbed()) {
          wm?.revealGroupChild?.(focusNode, { keyboard: true, source: "session-move-in" });
        }
      }
    } catch (e) {
      Logger.warn(`session-api move-in post: ${e}`);
    }
    ...
    return {
      ...
      changed: true,
      candidate: candidatePublic(resolved.match),
      destLayout: dest.layout,
    };
  }

  /**
   * C4: move layout unit out to grandparent.
   * @param {string} [selector]
   * @param {{ quiet?: boolean }} [opts]
   */
  _moveOutOp(selector, opts = {}) {
    const quiet = !!opts.quiet;
    const sel = selector != null && String(selector).trim() !== "" ? String(selector) : "focus";
    const resolved = this._resolveWindow(sel);
    if (!resolved.ok) return resolved;
    ...
    const unit = forestMoveOut(wm, focusNode);
    if (!unit) {
      return { ok: true, changed: false, reason: "at MONITOR/ROOT (no-op)" };
    }
    ...
    try {
      ...
      if (!quiet) {
        wm?.unfreezeRender?.();
        if (typeof wm?.commitLayout === "function") {
          wm.commitLayout("session-move-out", { force: true });
        } else {
          wm?.renderTree?.("session-move-out", true);
        }
      }
    } catch (e) {
      Logger.warn(`session-api move-out post: ${e}`);
    }
  ...
  _dndDropOp(tileSel, ontoSel, zone, opts = {}) {
    ...
    Logger.debug(
      `dnd-drop tile=${tileSel || "focus"} onto=${ontoSelStr || "-"} zone=${zone || "-"} destMon=${
        destMonOpt ?? "-"
      }`
    );
    ...
    const wm = this._wm();
    const dragDrop = wm?.dragDrop;
    const pointerApi = getOpSet("mark2")?.pointer;
    if (!dragDrop?._commitPointerOp || !pointerApi?.release) {
      return { error: "dnd-drop: OpSet.pointer not available" };
    }

    const parentBefore = this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
    ...
    if (simulateEntered) {
      try {
        ...
        if (typeof destMon === "number" && destMon >= 0) {
          ...
          const parentAfter = this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
          enteredSkipped = parentAfter === parentBefore;
        }
      } catch (e) {
    ...
    /** @type {{ op: string|null, args?: object }|null} */
    let resolved = null;
    try {
      ensureLiveForest(wm, liveForestPaintHooks(wm, { treatGrabTileAsTiles: true }));
      const forest = wm.forest;
      if (!forest?.nodes || !wm._liveForestSeeded) {
        focusNode.mode = prevMode || WINDOW_MODES.TILE;
        wm._draggedNodeWindow = prevDragged || null;
        return { error: "dnd-drop: forest not ready" };
      }
      const grabId = forestIdFromLive(wm, focusNode);
      const ontoId = forestIdFromLive(wm, ontoNode);
      if (!grabId || !ontoId || !forest.nodes[grabId] || !forest.nodes[ontoId]) {
        focusNode.mode = prevMode || WINDOW_MODES.TILE;
        wm._draggedNodeWindow = prevDragged || null;
        return { error: "dnd-drop: missing forest ids" };
      }
      const targetRect =
        dropTargetHitRect(ontoNode, ontoNode.nodeValue, true) ||
      ...
      if (!targetRect || !(targetRect.width > 0) || !(targetRect.height > 0)) {
        focusNode.mode = prevMode || WINDOW_MODES.TILE;
        wm._draggedNodeWindow = prevDragged || null;
        return { error: "dnd-drop: onto has no pane rect" };
      }
      const paneRect = {
        x: Number(targetRect.x) || 0,
        y: Number(targetRect.y) || 0,
        width: Number(targetRect.width) || 0,
        height: Number(targetRect.height) || 0,
      };
      const world = worldPointInMark2Zone(paneRect, zoneKey);
      if (!world) {
        focusNode.mode = prevMode || WINDOW_MODES.TILE;
        wm._draggedNodeWindow = prevDragged || null;
        return { error: `dnd-drop: no world point for zone ${zoneKey}` };
      }
      const mins = unitMins(focusNode);
      const ev = {
        world,
        grab: {
          id: grabId,
          kind: "window",
          mins: mins.width > 0 || mins.height > 0 ? mins : undefined,
        },
        hit: { tag: "window", id: ontoId, paneRect },
      };
      resolved = pointerApi.release(forest, ev);
      if (resolved?.op) {
        const committed = dragDrop._commitPointerOp(focusNode, resolved);
        if (!committed) {
          focusNode.mode = prevMode || WINDOW_MODES.TILE;
          wm._draggedNodeWindow = prevDragged || null;
          return { error: "dnd-drop: commit failed" };
        }
      }
    } catch (e) {
    ...
    const focusParent = liveParentForPresent(wm, focusNode) || focusNode.parentNode;
    const ontoParent = liveParentForPresent(wm, ontoNode) || ontoNode.parentNode;
    return {
      ok: true,
      zone: zoneKey,
      op: resolved?.op || null,
      enteredMonitorSkippedRehome: enteredSkipped,
      parentLayout: focusParent?.layout || null,
      sameParentAsOnto: focusParent === ontoParent,
      candidate: candidatePublic(tileResolved.match),
    };
  }

  /**
   * R015 synthetic: empty-mon grab-tile drop → Mark 2 move (onto MONITOR).
   * Host `_commitEmptyMonitorDrop` is last-resort Meta transfer only.
  ...
  _dndEmptyMonDropOp(focusNode, tileResolved, destMon, opts = {}) {
    ...
    try {
      ...
      let workArea = null;
      try {
        workArea = focusNode.nodeValue?.get_work_area_for_monitor?.(destMon);
      } catch (_e) {
        workArea = null;
      }
      const wsIndex = focusNode.nodeValue?.get_workspace?.()?.index?.() ?? 0;
      const destId = Utils.createMonitorWorkspaceId(destMon, wsIndex);
      const destNode = wm.tree?.findNode?.(destId);
      const monId = destNode ? forestIdFromLive(wm, destNode) : null;
      if (!monId || !forest.nodes[monId] || !workArea || !(workArea.width > 0)) {
        // Fall through to host Meta transfer when Forest mon geometry missing.
        const commit =
          dragDrop._commitEmptyMonitorDrop?.bind(dragDrop) || wm._commitEmptyMonitorDrop?.bind(wm);
        rehomed = !!commit?.(focusNode, destMon);
        opId = rehomed ? "move" : null;
      } else {
        const wa = {
        ...
        const mins = unitMins(focusNode);
        const ev = {
          world: { x: wa.x + wa.width / 2, y: wa.y + wa.height / 2 },
          grab: {
            id: grabId,
            kind: "window",
            mins: mins.width > 0 || mins.height > 0 ? mins : undefined,
          },
          hit: { tag: "empty-monitor", id: monId, workArea: wa },
        };
        const resolved = pointerApi.release(forest, ev);
        opId = resolved?.op || null;
        if (resolved?.op) {
          rehomed = !!dragDrop._commitPointerOp(focusNode, resolved);
          if (!rehomed && resolved.op === "move") {
            const commit =
              dragDrop._commitEmptyMonitorDrop?.bind(dragDrop) ||
  ...
  _skeletonOp(mons, opts = {}) {
    ...
    Logger.debug(`skeleton ws=${workspace} mons=${mons.length}`);
    ...
    try {
      for (const monSpec of mons) {
        ...
        // Always rewrite. A leftover PH from a failed apply must not skip
        // (that left PlaceNext dest mon-root / open-miss).
        const forested = forestApplySkeletonMon(wm, monNode, monSpec);
        if (forested?.ok) {
          if (forested.created) created.push(...forested.created);
          continue;
        }
        if (forested?.error) return forested;
        return { error: "skeleton requires live Forest (GObject invent retired)" };
      }
  ...
  _monitorNodeFor(monIdx, workspace) {
    ...
    if (byVal) {
      Logger.trace(`monitorNode hit id=${id}`);
      return byVal;
    }
    const mons = tree.getNodeByType?.(NODE_TYPES.MONITOR) || [];
    for (const m of mons) {
      ...
      if (idx === monIdx) {
        ...
        if (!mid || mid.includes(`ws${workspace}`)) {
          Logger.trace(`monitorNode scan idx=${monIdx} ws=${workspace} used=${mid}`);
          return m;
        }
      }
    }
    Logger.debug(`monitorNode miss mo${monIdx}ws${workspace}`);
    return null;
  }

  /** @param {object} monNode */
  _monHasLayoutSkeleton(monNode) {
    const wm = typeof this._wm === "function" ? this._wm() : this._wm;
    const walk = (n) => {
      if (!n) return false;
      if (isPlaceholderNode(n) && (n.layoutRole || n.layoutSlot)) return true;
      for (const c of liveChildrenForPresent(wm, n)) {
   ...
   * @returns {object|null}
   */
  _membershipParentLive(wm, live) {
    if (!live) return null;
    const fromForest = liveParentForPresent(wm, live);
    ...
    if (!tom || !wm?.forest) return null;
    const p = tomParent(wm.forest, tom);
    if (!p || (p.kind !== "CON" && p.kind !== "MONITOR")) return null;
    return wm.liveById?.get?.(p.id) ?? null;
  }
   ...
   * @returns {boolean}
   */
  _hasForestOrLiveParent(wm, live) {
    if (this._membershipParentLive(wm, live)) return true;
    const id = forestIdFromLive(wm, live);
    const tom = id ? wm?.forest?.nodes?.[id] : null;
    if (!tom || !wm?.forest) return false;
    const p = tomParent(wm.forest, tom);
    return !!(p && (p.kind === "CON" || p.kind === "MONITOR"));
  }
  ...
  _bindOp(tile, opts = {}) {
    ...
    const parentLive = liveParentForPresent(wm, phNode);
    const phForestId = forestIdFromLive(wm, phNode);
    const phTom = phForestId && wm.forest?.nodes?.[phForestId] ? wm.forest.nodes[phForestId] : null;
    const forestParent = phTom && wm.forest ? tomParent(wm.forest, phTom) : null;
    const seeded = !!(wm._liveForestSeeded && wm.forest);
    if (seeded) {
      if (!forestParent) {
        this._clearLayoutBindPendingIfIdle(wm, tree);
        return { error: "placeholder has no parent" };
      }
    } else if (!parentLive && !phNode.parentNode) {
      this._clearLayoutBindPendingIfIdle(wm, tree);
      return { error: "placeholder has no parent" };
    }

    const destMon = this._monitorIndexOfBindDest(wm, phNode, parentLive, forestParent);

    wm._suppressRehome.enter();
    try {
      const bound = forestBindWindow(wm, winNode, phNode);
      if (!bound) {
        if (seeded) {
          return { error: "Forest bind failed" };
        }
        const parent = phNode.parentNode || parentLive;
        if (!parent) {
          return { error: "placeholder has no parent" };
        }
        // Unseeded fixtures only: GObject insert before PH.
  ...
  /**
   * Dest monitor for bind: Forest MONITOR ancestor, not GObject parentNode.
   * @param {object} wm
   * @param {object} phNode
   * @param {object|null} parentLive
   * @param {object|null} forestParent
   * @returns {number}
   */
  _monitorIndexOfBindDest(wm, phNode, parentLive, forestParent) {
    let n = parentLive || phNode;
    const seen = new Set();
    while (n && !seen.has(n)) {
      seen.add(n);
      const isMon =
        n.nodeType === NODE_TYPES.MONITOR ||
        n.nodeType === "MONITOR" ||
        (typeof n.isMonitor === "function" && n.isMonitor());
      if (isMon) {
        const idx = Utils.monitorIndex(n.nodeValue);
      ...
      const next = liveParentForPresent(wm, n);
      if (!next || next === n) break;
      n = next;
    }
    const forest = wm?.forest;
    if (forest && forestParent) {
      const mon =
        forestParent.kind === "MONITOR" ? forestParent : ancestorMonitor(forest, forestParent);
      if (mon) {
        const liveMon = wm.liveById?.get?.(mon.id);
        const idx = Utils.monitorIndex(liveMon?.nodeValue ?? mon.id);
        if (Number.isFinite(idx) && idx >= 0) return idx;
    ...
    return this._monitorIndexOfNode(parentLive || phNode);
  }
  ...
  _findLayoutPlaceholder(tree, want = {}) {
    const wm = this._wm();
    return findLiveLayoutPlaceholder(wm, want, tree);
  }
  ...
  _detachedWindowLives() {
    ...
    if (wm?.liveById instanceof Map) {
      for (const live of wm.liveById.values()) {
        ...
        const isWin =
          live.nodeType === NODE_TYPES.WINDOW ||
  ...
  _resolveWindow(selector) {
    ...
    if (all.length) {
      const { matches: more } = matchWindows(all, descriptor, ctx);
      const picked2 = pickMatch(more, { first: !!descriptor.first });
      if (picked2.ok) return picked2;
    }
    // Paint-detached FLOATS: not under MONITOR / Tree index.
    const detached = this._detachedWindowLives();
    if (!detached.length) return picked;
    const { matches: floatMatches } = matchWindows(detached, descriptor, ctx);
    return pickMatch(floatMatches, { first: !!descriptor.first });
  }
