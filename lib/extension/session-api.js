...
export class SessionApi {
  ...
  /**
   * DBus: SaveSessionLayout() → JSON — flush last-good tile topology to disk
   * before install/Shell HUP so restore can re-apply splits/tabs.
   */
  SaveSessionLayout() {
    try {
      ...
      return JSON.stringify({
        ok,
        error: ok ? undefined : "nothing to save (empty tree or no configMgr)",
      });
    } catch (e) {
      return JSON.stringify({ ok: false, error: String(e?.message || e) });
    }
  ...
  _runStepHandlers() {
    return {
      ...
      move: (step) => this._moveOp(step.tile, step.dest, { quiet: true }),
