...
/** Combined control-plane version (+ GetThrashCatalog / SL2). */
export const SESSION_API_VERSION = 9;
...
export class SessionApi {
   ...
   *
   * chrome-clear | clear-chrome — hide apply chrome (CLI after residual
   * bind/place; also error/finally if still up). end() does not clear.
   *
   * @param {string} action "begin" | "end" | "release-deferred" | "chrome-clear"
   */
  LayoutBatch(action) {
    try {
      const wm = this._wm();
      ...
      return JSON.stringify({
        ok: false,
        error: `LayoutBatch action want begin|end|release-deferred|chrome-clear (got ${
          raw || "empty"
        })`,
      });
    } catch (e) {
  ...
  RunSteps(stepsJson) {
    try {
      ...
      try {
        ...
        // Single residual Cf when we own freeze state (StructureChanged).
        if (!prevFrozen && wm) {
          try {
          ...
          // After idle C: settle open leaves (F+Dfocus+B) — not a second C.
          this._scheduleRunStepsSettle(wm);
        }
        // Never leave sticky bind-suppress after a completed RunSteps wave
        // (failed mid-bind with leftover PHs, or skeleton-only with no batch).
        if (wm) {
  ...
  /**
   * @param {string} selector
   * @param {{ keyboard?: boolean }} [opts]
   *   keyboard (default true): Meta.activate + full afterFocus (LFT/pointer).
   *   keyboard false: open-leaf only (lastTabFocus + pin + raise + tab chrome)
   *   without stealing keyboard focus from the profile target.
   * @returns {object}
   */
  _focusOp(selector, opts = {}) {
    ...
    // Pin open leaf before activate so late chrome cannot race the data model.
    const parent = node.parentNode;
    if (parent && typeof parent.isStackedOrTabbed === "function" && parent.isStackedOrTabbed()) {
      parent.lastTabFocus = meta;
      try {
        // Pin for soft-focus wall (SE5); fixed 3.5s expired mid soft-barrier.
    ...
    if (wm) {
      try {
        if (keyboard) {
          ...
          // Open leaf only: tab/stack chrome + raise; do not steal keys/LFT.
          if (typeof wm.settleTabFocus === "function") {
  ...
  _unwrapSingleChildSplit(node, mon, tree) {
    ...
    for (let guard = 0; guard < 8 && cur; guard++) {
      ...
      const only = kids[0];
      if (!only || !mon?.childNodes) break;
      const idx = mon.childNodes.indexOf(cur);
      if (idx < 0) break;
      only.parentNode = mon;
      mon.childNodes[idx] = only;
      cur.childNodes = [];
      cur.parentNode = null;
      cur = only;
    }
  ...
  _layoutOp(mode, selector, opts = {}) {
    ...
    if (isTabOrStack) {
      ...
      const prevFocus = parent.lastTabFocus;
      ...
      if (!focusStillChild) {
        parent.lastTabFocus = liveNode.nodeValue ?? parent.lastTabFocus;
      }
    }
  ...
  /**
   * Synthetic grab-tile drop (R012 / live matrix). Optionally fires
   * window-entered-monitor while GRAB_TILE so mid-drag rehome cannot steal the drop.
  ...
  _dndDropOp(tileSel, ontoSel, zone, opts = {}) {
    ...
    const ontoResolved = this._resolveWindow(
      ontoSel != null && String(ontoSel).trim() !== "" ? String(ontoSel) : ""
    );
    if (!ontoResolved.ok) return ontoResolved;

    const focusNode = tileResolved.match.node;
    const ontoNode = ontoResolved.match.node;
    if (!focusNode?.nodeValue || !ontoNode?.nodeValue) {
    ...
    try {
      ...
      const noOp = wm.dragDrop?._isNoOpDrop?.(focusNode, ontoNode, operation, ctx);
      if (!noOp) {
  ...
  _bindOp(tile, opts = {}) {
    ...
    if (!phNode) {
      // No PH for this bind: clear if wave has no layout PHs left.
