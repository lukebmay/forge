...
export class SessionApi {
  ...
  _snapshotForestForApply(run = {}) {
    ...
    Logger.debug(
      `apply-snapshot ws=${workspace ?? "-"} mons=${
        Array.isArray(forest.monitors) ? forest.monitors.length : 0
      } orphans=${Array.isArray(forest.orphanWindows) ? forest.orphanWindows.length : 0} activeWs=${
        activeWorkspace ?? "-"
      } nWs=${nWorkspaces ?? "-"}`
    );
    Logger.trace(
      `apply-snapshot monIds=${(forest.monitors || []).map((m) => m?.id || "?").join(",")}`
    );
  ...
  _orphanWindowProjections(tree, forest, options = {}) {
    ...
    for (const node of tree.getNodeByType?.(NODE_TYPES.WINDOW) || []) {
      ...
      if (targetWs != null && Number.isFinite(targetWs)) {
        if (nodeWs == null || nodeWs !== targetWs) {
          skipped += 1;
          Logger.trace(
            `orphan skip id=${id} nodeWs=${nodeWs ?? "-"} want=${targetWs} reason=${
              nodeWs == null ? "unknown-ws" : "other-ws"
            }`
          );
          continue;
        }
    ...
    if (targetWs != null && Number.isFinite(targetWs)) {
      Logger.debug(`orphan-filter want=${targetWs} kept=${extra.length} skipped=${skipped}`);
    }
    return extra;
  }
