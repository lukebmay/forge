...
export class SessionApi {
      ...
      onApplyLive: (active, run) => {
        ...
        if (!active) {
          ...
          try {
            ...
            if (r?.steps?.length) paintWmForest(wm);
          } catch (e) {
            Logger.debug?.(`session-api ApplyLayout resync: ${e}`);
          }
          try {
    ...
    try {
      ...
      if (info) {
        ...
        if (this._isNestShellEnv()) {
          const nestArgv =
            this._nestChromeLaunchArgv(info, fields, action) ||
            this._desktopInfoArgv(info);
          if (nestArgv?.length) {
  ...
  _applyNestClientLaunchEnv(launcherOrCtx) {
    ...
    try {
      ...
      if (nestWl && nestState) {
        try {
          Logger.info(
            `nest-client-launch env wayland=${nestWl} runtime=${nestState}/runtime`
          );
        } catch (_e) {
          /* */
        }
  ...
  _snapshotForestForApply(run = {}) {
...

    {
      ...
      const monIds = (forest.monitors || []).map((m) => m?.id || "?").join(",");
  ...
  _mergeGroupOp(selector, withSel, opts = {}) {
    ...
    if (withSel != null && String(withSel).trim() !== "") {
      ...
      if (
        ...
        const kids = parent
          ? liveChildrenForPresent(wm, parent)
          : parent?.childNodes || [];
  ...
  _dndDropOp(tileSel, ontoSel, zone, opts = {}) {
    ...
    const wm = this._wm();
    const commitDrop =
      wm?._commitResolvedDrop?.bind(wm) || wm?.dragDrop?._commitResolvedDrop?.bind(wm.dragDrop);
    if (!wm?._buildDropOperation || !commitDrop) {
      return { error: "dnd-drop: drag-drop not available" };
    }

    const parentBefore =
      this._membershipParentLive(wm, focusNode) || focusNode.parentNode;
    ...
    try {
      ...
      const parentNodeTarget =
        liveParentForPresent(wm, ontoNode) || ontoNode.parentNode;
      if (!parentNodeTarget) {
      ...
      const ctx = {
        ...
        parentNodeTarget,
        // Same ctx as pointer — wrap vs reorder needs the dragged node.
        focusNodeWindow: focusNode,
      ...
      if (dropChangesStructure(focusNode, ontoNode, operation, ctx)) {
        const committed = commitDrop(focusNode, ontoNode, operation, ctx);
        if (committed === false) {
          focusNode.mode = prevMode || WINDOW_MODES.TILE;
          wm._draggedNodeWindow = prevDragged || null;
          return { error: "dnd-drop: commit failed (no TILES parent)" };
        }
      }
    } catch (e) {
