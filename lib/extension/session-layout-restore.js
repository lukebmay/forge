...
/** Session restore diagnostics (Logger.debug + file when !production). Soft rehome imports this. */
...
/**
 * SessionLayoutRestoreManager owns save/restore/rehome/raise/shield/trace
 * orchestration for portable session-layout.json. Extracted from window.js
 * (WindowManager) so the cluster lives in one place.
 *
 * Shared WindowManager state is read LIVE via this._extWm:
 *   _sessionLayoutShield, _sessionLayoutRestoring, _sessionLayoutSaveHoldUntil,
 *   _sessionLayoutSaveSrcId, _lastGoodHomes, disabled, tree, ext, …
 * Soft rehome (SoftRehomeManager) sets shield/thrash flags on WM and calls thin
 * WM wrappers for shield active/reapply.
 */
