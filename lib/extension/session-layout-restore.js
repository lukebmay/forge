...
export class SessionLayoutRestoreManager extends GObject.Object {
  ...
  saveSessionLayoutForReload(opts = {}) {
    ...
    try {
      ...
      try {
        focusWindowId = SessionLayout.windowStableId(wm.focusMetaWindow);
      } catch (_e) {
        focusWindowId = null;
      }
      if (focusWindowId == null) {
        ...
        try {
          ...
          const lftMeta = lftNode?.nodeValue ?? lftNode ?? null;
          focusWindowId = SessionLayout.windowStableId(lftMeta);
        } catch (_e) {
          focusWindowId = null;
        }
      }
      ...
      sessionLayoutTrace(
        `session-layout: save focusId=${focusWindowId ?? "none"} ${metaWinLabel(
          wm.focusMetaWindow
        )}`
      );
      return true;
  ...
  _scheduleSessionFocus(focusMeta) {
    const wm = this._extWm;
    if (!focusMeta) return;
    wm._clearTimeoutId?.("_sessionFocusRetrySrcId");
    try {
      wm._sessionFocusRetrySrcId = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
        wm._sessionFocusRetrySrcId = 0;
        try {
          const focus =
      ...
      // GLib unavailable in some unit contexts
      try {
