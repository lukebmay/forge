...
import {
  LOCK_SHIELD_AFTER_UNLOCK_US,
  LOCK_SHIELD_WHILE_LOCKED_US,
  armHostPresentHold,
  safeMoveToMonitor,
} from "./monitor-recovery.js";
import { rehomeWmForestWindows, restoreWmForestStrict } from "./forest-restore.js";
...
export class SessionLayoutRestoreManager extends GObject.Object {
  ...
  armLockLayoutShield() {
    ...
    if (!forest?.monitors?.length) {
      // Common when enable/HUP runs already in unlock-dialog before track/restore.
      sessionLayoutTrace("session-layout: lock shield empty (no monitors in snapshot)");
      return false;
    }
    ...
    wm._sessionLayoutShield = {
      liveForest: forest,
      focusMeta,
      untilMonoUs: now + LOCK_SHIELD_WHILE_LOCKED_US,
      fromLock: true,
    };
    sessionLayoutTrace(
  ...
  /** Enter unlock-dialog: arm shield; freeze present + thrash recovery until unlock. */
  onSessionLocked() {
    const wm = this._extWm;
    wm._sessionLocked = true;
    armHostPresentHold(wm);
    try {
  ...
  saveSessionLayoutForReload(opts = {}) {
    ...
    try {
      ...
      // Fill empty/dead LTF only — never stomp live open leaf from keyboard (D018).
      ...
      try {
        const keys = TreeSnapshot.portableWindowKeys(focusMeta, wm.hostBag);
        focusWindowId = keys.windowId ?? null;
      } catch (_e) {
        focusWindowId = null;
      }
  ...
  /** Cancel debounce and write now (install flush). Always persist. */
  flushSessionLayout() {
    const wm = this._extWm;
    wm._wmSources.cancel("sessionLayoutSave");
    return wm._saveSessionLayoutForReload({ immediate: true, force: true });
  }
  ...
  restoreSessionLayoutAfterTrack() {
    ...
    try {
      ...
      if (wm._sessionLocked) {
        // Enable/HUP while already locked: onSessionLocked often armed an empty
        // shield before track/restore. Replace with a real fromLock shield so
        // overnight DPMS thrash stays covered (3s post-HUP TTL would expire).
        wm._sessionLayoutShield = {
          liveForest,
          focusMeta,
          untilMonoUs: nowMono + LOCK_SHIELD_WHILE_LOCKED_US,
          fromLock: true,
        };
        sessionLayoutTrace(
          `session-layout: lock shield armed after restore mon=${
            liveForest.monitors.length
          } focus=${metaWinLabel(focusMeta)}`
        );
      } else {
  ...
  rehomeWindowsForSessionForest(liveForest) {
    ...
    const moveMeta = ({ window: metaWin, monIndex }, pass) => {
      if (!metaWin) return;
      try {
        const before = typeof metaWin.get_monitor === "function" ? metaWin.get_monitor() : -1;
        // Never raw move_to_monitor — Wayland unready windows SEGV Shell.
        const moved = safeMoveToMonitor(metaWin, monIndex, `session-layout-rehome pass=${pass}`);
        const after = typeof metaWin.get_monitor === "function" ? metaWin.get_monitor() : before;
        sessionLayoutTrace(
          `session-layout: rehome pass=${pass} plan=${monIndex} meta ${before}→${after}` +
            ` moved=${moved} ${metaWinLabel(metaWin)}`
        );
      } catch (e) {
        sessionLayoutTrace(`session-layout: rehome move failed pass=${pass}: ${e}`);
      }
    };
    for (const home of homes) moveMeta(home, 1);
    rehomeWmForestWindows(wm, homes, liveForest);
    for (const home of homes) {
      const { window: metaWin, monIndex } = home;
      if (!metaWin) continue;
      let needRetry = false;
      try {
      ...
      if (needRetry) moveMeta(home, 2);
    }
    rehomeWmForestWindows(wm, homes, liveForest);
  }

  /** Strict mon resolve only (no majority pile remap). Forest-only (D096 G7). */
  restoreSessionForestStrict(liveForest) {
    const wm = this._extWm;
    ensureLiveForest(wm);
    if (!restoreWmForestStrict(wm, liveForest)) {
      recordFallback("session-restore", "forest-fail");
    }
  }
  ...
  raiseAfterSessionRestore(liveForest, opts = {}) {
    ...
    try {
      ...
      if (focus) {
        ...
        const node = wm.tree?.findNode?.(focus);
        // Fill empty/dead LTF; update* adopts focus as open leaf after install raise.
