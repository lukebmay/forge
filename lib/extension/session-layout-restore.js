...
import { LOCK_SHIELD_AFTER_UNLOCK_US, LOCK_SHIELD_WHILE_LOCKED_US } from "./monitor-recovery.js";
...
export class SessionLayoutRestoreManager extends GObject.Object {
  ...
  rehomeWindowsForSessionForest(liveForest) {
    ...
    const ctx = seeded ? null : wm.tree._treeSnapshotCtx();
    const moveMeta = ({ window: metaWin, monIndex }, pass) => {
      ...
      try {
        ...
        sessionLayoutTrace(
          `session-layout: rehome pass=${pass} plan=${monIndex} meta ${before}→${after} ${metaWinLabel(
            metaWin
          )}`
        );
      } catch (e) {
    ...
    if (seeded) {
      ...
      const applyTree = ({ window: metaWin, monIndex, monId }, pass) => {
        if (!metaWin) return;
        try {
          const node = wm.tree.findNode(metaWin);
          if (!node) {
            sessionLayoutTrace(
              `session-layout: rehome pass=${pass} NO_NODE plan=${monIndex} ${metaWinLabel(
                metaWin
              )}`
            );
            return;
          }
          const monDesc = (liveForest.monitors || []).find((m) => m.id === monId) || { id: monId };
      ...
      for (const home of homes) applyTree(home, 1);
    }
    for (const home of homes) {
      ...
      if (needRetry) {
        ...
        if (!seeded && ctx) {
          ...
          const monDesc = (liveForest.monitors || []).find((m) => m.id === monId) || { id: monId };
          const mon = SessionLayout.resolveStrictMonitor(monDesc, ctx) || wm.tree.findNode(monId);
          if (node && mon && !TreeSnapshot.hasAncestor(node, mon)) mon.appendChild(node);
        }
      }
    }
    if (seeded) rehomeWmForestWindows(wm, homes, liveForest);
  }
