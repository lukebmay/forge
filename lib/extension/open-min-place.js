...
/**
 * @typedef {object} OpenMinPlacementOpts
 * @property {"horizontal"|"vertical"} [orientation]
 * @property {{ width?: number, height?: number }} [newMins]
 * @property {object|null} [lftUnit]
 * @property {(unit: object) => ({ width?: number, height?: number }|null|undefined)} [slotRectFor]
 * @property {object[]} [candidates] BFS-ordered tab units (lft first)
 * @property {(unit: object) => ({ width: number, height: number })} [unitMinsFor]
 * @property {"split-or-tab"|"tab-only"} [mode]
 * @property {object|null} [selfUnit] overflowing unit (overflow path only)
 */
 ...
 *
 * @param {OpenMinPlacementOpts} [opts]
 ...
 * @param {object} [wm] Forest-first parent when seeded
 * @returns {object|null}
 */
export function tabJoinUnit(node, layoutTypes, wm) {
  if (!node) return null;
 ...
 * @param {object} [wm] Forest-first parent/kids when seeded
 * @returns {object[]}
 */
export function bfsOpenMinTabCandidates(start, monitorNode, layoutTypes, wm) {
  if (!start) return [];
  const out = [];
  const seen = new Set();
  ...
  const parentOf = (n) => (wm && liveParentForPresent(wm, n)) || n?.parentNode || null;
  const kidsOf = (n) => {
    if (wm && n) return liveChildrenForPresent(wm, n);
    return n?.childNodes || [];
  };
  ...
  while (q.length) {
    ...
    if (n.isCon?.() && !n.isStackedOrTabbed?.()) {
      for (const child of kidsOf(n)) push(child);
    }
  }
