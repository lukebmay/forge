...
/** Drop every PlaceNext hint (apply end / fail). */
export function clearAllPlaceHints(wm, reason = "clear") {
  const n = clearPlaceHintQueue(wm?._pendingPlaceHints);
  if (n) Logger.info(`place-hint clear n=${n} reason=${reason}`);
  return n;
}
...
export function placePlanFromConsumedHint(wm, hint) {
  ...
  // Mon-root PlaceNext: empty LFT(m) still has dest tiles (R036 late class).
  if (!attachLft) {
    attachLft = lastTileOnMonitor(wm, homeMonitor);
    if (attachLft) attachVia = attachVia || "mon-last";
  }
...
export function tryAdoptLatePlaceHint(wm, metaWindow) {
  ...
  // Already a TILE leaf: pending PlaceNext is for new maps (R063).
  if (!wm._pendingPlaceHints?.length) return false;
  Logger.debug(
    `place-hint late skip-pending tracked winClass=${cls} ` +
...
export function applyPlacePlanToExistingWindow(wm, metaWindow, plan) {
  ...
  try {
    ...
    const floated = !!(node.isFloat?.() || node.mode === WINDOW_MODES.FLOAT);
    const treeMon = wm._monitorIndexOfNode(node);
    // FLOATS has no MONITOR ancestor — not dest TILES (D096 / D117 admit).
    if (
      !floated &&
      treeMon === homeMonitor &&
      placeDeskMatches(wm, node, homeMonitor, plan.workspace)
    ) {
      Logger.info(
    ...
    try {
      ...
      // Live mapped window: Meta dest before notify renderTree RESYNC (D117).
      try {
        if (
          typeof metaWindow.get_monitor === "function" &&
          metaWindow.get_monitor() !== homeMonitor
        ) {
          safeMoveToMonitor(metaWindow, homeMonitor, "late place-hint apply");
        }
      } catch (_e) {
        /* idle retry */
      }
