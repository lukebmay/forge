  ...
  if (existNodeWindow) return;
    ...
    if (willTile && !placePinned && !deferHidden) {
      ...
      if (minDecision?.kind === "tab" && minDecision.targetUnit) {
        ...
        const leftoverLay = wm._layoutFromOrientation(
          wm._orientationFromUnit(leftoverSlot)
        );
    ...
    if (placePinned && openPlan.attachLft) {
      ...
      if (pin) {
        if (pin.isWindow?.() || pin.nodeType === NODE_TYPES.WINDOW) {
          attachTarget =
            wm._membershipParentLive(pin) || pin.parentNode || metaMonWsNode;
          placeBefore = pin;
    ...
    if (!attachTarget && openMinTabTarget) {
      const tabLive =
        openMinTabTarget.nodeValue != null
          ...
          : openMinTabTarget;
      const tabParent = tabLive
        ? wm._membershipParentLive(tabLive) || tabLive.parentNode
        : null;
...
export function resolvePlaceSlotAttachFromHint(wm, hint, homeMonitor = -1) {
  ...
  const ph = findLiveLayoutPlaceholder(wm, want, wm.tree);
  if (ph) {
    const monOf = wm._monitorIndexOfNode(ph);
    if (homeMonitor < 0 || monOf < 0 || monOf === homeMonitor) {
  ...
  for (const bag of bags) {
    if (!bag) continue;
    const monOf = wm._monitorIndexOfNode(bag);
    if (homeMonitor >= 0 && monOf >= 0 && monOf !== homeMonitor) continue;
  ...
  if (ph) return { attachLft: ph, via: "role-ph" };
  return null;
}
...
export function ensureMetaOnWorkspace(wm, metaWindow, wantWs) {
  ...
  try {
    ...
    const wsMgr =
      global.workspace_manager || global.display?.get_workspace_manager?.() || null;
...
export function consumeLeftoverLayoutPlaceholder(wm, winNode, plan = {}) {
  ...
  if (
    ...
    try {
      const kids =
        liveChildrenForPresent(wm, plan.attachLft) || plan.attachLft.childNodes || [];
  ...
  if (!ph) {
    ...
    const sibs = parent
      ? liveChildrenForPresent(wm, parent) || parent.childNodes || []
      : [];
  ...
  if (!ph) {
    ...
    const picked = findLiveLayoutPlaceholder(wm, want, wm.tree);
    if (accept(picked)) ph = picked;
  }
  ...
  try {
    ...
    Logger.debug(
      `place-hint late state id=${winId} class=${metaWmClass(metaWindow)} ` +
        `wantMon=${homeMonitor} metaMon=${metaMonBefore} treeMon=${wm._monitorIndexOfNode(
          node
        )} ` +
...
export function decideOpenMinPlacement(wm, metaWindow, lftNode) {
  ...
  const orientation =
    wm._orientationFromUnit(insertUnit) === ORIENTATION_TYPES.VERTICAL
      ? "vertical"
      : "horizontal";
...
export function resolveInsertUnit(wm, node) {
  ...
  if (isTabOrStackParent(parent0, LAYOUT_TYPES)) return parent0;
  let unit = live;
  for (let i = 0; i < 8; i++) {
...
export function adoptOpenIntoTileSlot(wm, nodeWindow) {
  ...
  const alreadySlotted =
    !!liveUnit &&
    (nodeParent === liveUnit ||
      (nodeParent === liveUnitParent && wm._isHvCon(liveUnitParent)));
...
export function lastTileOnMonitor(wm, monIndex) {
  ...
  const monNode = monId ? wm.tree.findNode(monId) : null;
...
export function emptyTileMonitorIndices(wm) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!monId || !wm.tree?.findNode(monId)) continue;
