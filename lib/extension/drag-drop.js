...
import { DROP_ZONES } from "./utils.js";
import {
  buildDropZones,
  hitTestDropZone,
  zonePaintRect,
...
import { dropWouldOverflowMins, unitMins } from "./drop-intent.js";
...
/**
 * Tree MONITOR index, else Meta `get_monitor()` when the ancestor is missing.
 * @param {{
 *   sourceTreeMonIndex?: number,
 *   sourceMetaMonIndex?: number,
 * }|null|undefined} p
 * @returns {number}
 */
export function sourceMonIndexFromDrop(p) {
  const tree = p?.sourceTreeMonIndex;
  if (typeof tree === "number" && tree >= 0) return tree;
  const meta = p?.sourceMetaMonIndex;
  if (typeof meta === "number" && meta >= 0) return meta;
  return -1;
}

/**
 * Why `resolveEmptyMonitorDrop` returned null (`src-miss` / `same-mon` / `pointer-miss`).
 * @param {{
 ...
 *   sourceMetaMonIndex?: number,
 * }|null|undefined} p
 * @returns {"src-miss"|"same-mon"|"pointer-miss"|null}
 */
export function emptyMonitorDropNoDecisionReason(p) {
  if (!p || p.hasWindowTarget) return null;
  const dest = p.pointerMonIndex;
  const src = sourceMonIndexFromDrop(p);
  if (typeof dest !== "number" || dest < 0) return "pointer-miss";
  if (src < 0) return "src-miss";
  if (dest === src) return "same-mon";
  return null;
}
 ...
 * Tree ancestor miss falls back to Meta `sourceMetaMonIndex`. Dest occupancy ignored.
 *
 * @param {{
 *   hasWindowTarget: boolean,
 *   pointerMonIndex: number,
...
export class DragDropManager extends GObject.Object {
  ...
  /** Prefer hostBag tab/tabChip; fall back to Node.tab (D096 G8c). */
  _tabForNode(node) {
  ...
  swapWindowsUnderPointer(focusNodeWindow) {
    ...
    const focusParent = liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode;
    const ptrParent = liveParentForPresent(wm, nodeWinAtPointer) || nodeWinAtPointer.parentNode;
    if (!focusParent || !ptrParent) {
      Logger.warn("swapWindowsUnderPointer: missing parent node");
      return;
    }
    if (!forestSwapWindows(wm, focusNodeWindow, nodeWinAtPointer)) {
      Logger.warn("swapWindowsUnderPointer: Forest ids miss; fail closed");
      try {
        resyncWmToReality(wm, "dnd-swap-ids-miss");
      } catch (_e) {
        /* best-effort */
      }
    }
  ...
  /**
   * Build Mark 2 pointer event (D101). Host fills grab/coords/hit — no zone names.
   * @param {object} focusNodeWindow
   * @param {{ preferStrip?: boolean }} [opts]
   */
  _buildPointerEv(focusNodeWindow, opts = {}) {
    const wm = this._extWm;
    const forest = wm.forest;
    const pointer = wm.getDragPointer(focusNodeWindow);
    const [wx, wy] = _pointerXY(pointer);
    const grabId = forestIdFromLive(wm, focusNodeWindow);
    if (!forest?.nodes || !grabId) return null;

    const mins = unitMins(focusNodeWindow);
    /** @type {any} */
    const ev = {
      world: { x: wx, y: wy },
      grab: {
    ...
    if (nodeWinAtPointer?.nodeValue && nodeWinAtPointer !== focusNodeWindow) {
      const ontoId = forestIdFromLive(wm, nodeWinAtPointer);
      const targetRect =
        dropTargetHitRect(nodeWinAtPointer, nodeWinAtPointer.nodeValue, true) ||
        nodeWinAtPointer.nodeValue.get_frame_rect?.() ||
        null;
      if (ontoId && forest.nodes[ontoId] && targetRect && targetRect.width > 0) {
        windowHit = {
          tag: "window",
          id: ontoId,
          paneRect: {
    ...
    // Pane under the pointer wins over strip chrome (titlebar peel onto a tile).
    if (windowHit && pointInRect(windowHit.paneRect)) {
      ev.hit = windowHit;
      return ev;
    }

    if (opts.preferStrip) {
      const origin = this._originStripCommit;
      if (origin?.group && origin.insertIndex != null) {
        const conId = forestIdFromLive(wm, origin.group);
        if (conId && forest.nodes[conId]) {
          const axis =
            typeof origin.group.isStacked === "function" && origin.group.isStacked() ? "y" : "x";
          ev.hit = { tag: "strip", id: conId, axis, insertIndex: origin.insertIndex };
          return ev;
        }
      }
      if (this._foreignStripCommit?.group) {
        const dest = this._foreignStripCommit.group;
        const conId = forestIdFromLive(wm, dest);
        if (conId && forest.nodes[conId]) {
          const axis = typeof dest.isStacked === "function" && dest.isStacked() ? "y" : "x";
          const insertIndex =
            this._foreignStripCommit.insertIndex != null
              ? this._foreignStripCommit.insertIndex
              : this._computeForeignStripInsertIndex(dest, wx, wy, focusNodeWindow);
          ev.hit = { tag: "strip", id: conId, axis, insertIndex };
          return ev;
        }
      }
      const dest = this._hitTestForeignTabStrip(
        focusNodeWindow,
        pointer,
      ...
      if (dest) {
        ...
        if (conId && forest.nodes[conId]) {
          const axis = typeof dest.isStacked === "function" && dest.isStacked() ? "y" : "x";
          const insertIndex = this._computeForeignStripInsertIndex(dest, wx, wy, focusNodeWindow);
          ev.hit = { tag: "strip", id: conId, axis, insertIndex };
          return ev;
        }
      }
    ...
    // Empty-monitor / gap (R015).
    const decision = this._resolveEmptyMonitorDropDecision(focusNodeWindow);
    if (decision) {
      let workArea = null;
      try {
        workArea = focusNodeWindow.nodeValue?.get_work_area_for_monitor?.(decision.destMonIndex);
      } catch (_e) {
        workArea = null;
      }
      const destId = Utils.createMonitorWorkspaceId(
        decision.destMonIndex,
        focusNodeWindow.nodeValue?.get_workspace?.()?.index?.() ?? 0
      );
  ...
  /**
   * Map Mark 2 hover descriptor → existing St preview bins.
   * @param {object} focusNodeWindow
   * @param {any} desc
   * @param {object|null} [nodeWinAtPointer]
   */
  _paintPointerHover(focusNodeWindow, desc, nodeWinAtPointer = null) {
    const wm = this._extWm;
    if (!desc || desc.paint === "none") {
      this._hidePreviewActors(focusNodeWindow);
      return;
    }
    if (desc.paint === "empty-monitor") {
      if (!this._previewHintsWanted()) {
        this.clearAllPreviewHints();
        return;
      }
      const rect = desc.preview?.rect;
      if (!rect) {
        this._hidePreviewActors(focusNodeWindow);
        return;
      }
      ...
      if (!previewHint) return;
      const cls =
        desc.refuse || desc.preview?.style === "invalid"
          ? "window-tilepreview-invalid"
          : "window-tilepreview-tiled";
      previewHint.set_style_class_name?.(cls);
      previewHint.set_position(rect.x, rect.y);
      previewHint.set_size(rect.width, rect.height);
      previewHint.show?.();
      this._armPreviewHintFailsafe();
      return;
    }
    if (desc.paint === "strip") {
      // Tab-strip chrome owns strip preview; tile bins stay hidden.
      this._hidePreviewActors(focusNodeWindow);
      return;
    }
    if (desc.paint !== "tile-zones" || !nodeWinAtPointer) {
      this._hidePreviewActors(focusNodeWindow);
      return;
    }
    const targetRect = desc.preview?.rect
      ? null
      : dropTargetHitRect(nodeWinAtPointer, nodeWinAtPointer.nodeValue, true);
    const pane =
      nodeWinAtPointer &&
      (dropTargetHitRect(nodeWinAtPointer, nodeWinAtPointer.nodeValue, true) ||
        nodeWinAtPointer.nodeValue?.get_frame_rect?.());
    if (!pane || !(pane.width > 0)) {
      this._hidePreviewActors(focusNodeWindow);
      return;
    }
    const dropZones = buildDropZones(pane);
    const zoneMap = {
    ...
    const zone = desc.zone ? zoneMap[desc.zone] : DROP_ZONES.NONE;
    if (!zone || zone === DROP_ZONES.NONE) {
      this._hidePreviewActors(focusNodeWindow);
      return;
    }
    const style = desc.preview?.style;
    let previewClass = "window-tilepreview-tiled";
    if (style === "invalid") previewClass = "window-tilepreview-invalid";
    else if (style === "tabbed") previewClass = "window-tilepreview-tabbed";
    else if (style === "stacked") previewClass = "window-tilepreview-stacked";
    else if (desc.zone === "center")
      previewClass = wm._getDragDropCenterPreviewStyle?.() || previewClass;

    const zoneOverflow = desc.zoneOverflow
      ? {
          [DROP_ZONES.TOP]: !!desc.zoneOverflow.top,
          [DROP_ZONES.RIGHT]: !!desc.zoneOverflow.right,
          [DROP_ZONES.BOTTOM]: !!desc.zoneOverflow.bottom,
          [DROP_ZONES.LEFT]: !!desc.zoneOverflow.left,
          [DROP_ZONES.CENTER]: !!desc.zoneOverflow.center,
        }
      : undefined;
    const operation = {
      zone,
      isCenter: zone === DROP_ZONES.CENTER,
      dropZones,
      previewRect: desc.preview?.rect || zonePaintRect(dropZones, zone) || pane,
      previewClass,
      blockedByMins: !!desc.refuse,
  ...
  /**
   * Commit `{op,args}` from OpSet.pointer.release into named Mark 2 Ops.
   * @param {object} focusNodeWindow
   * @param {{ op: string|null, args?: { dir: string, onto?: string, insertIndex?: number, place?: string } }} resolved
   * @returns {boolean}
   */
  _commitPointerOp(focusNodeWindow, resolved) {
    if (!resolved?.op || !resolved.args?.dir) {
      Logger.debug(`dnd commit failed reason=${!resolved?.op ? "no-op" : "no-dir"}`);
      return false;
    }
    return this._commitDropMark2(
      focusNodeWindow,
      { isCenter: resolved.op === "group", isSwap: false },
      {
        op: resolved.op,
        dir: resolved.args.dir,
        onto: resolved.args.onto,
        insertIndex: resolved.args.insertIndex,
        place: resolved.args.place,
      }
    );
  }

  /**
   * GRAB_TILE stays on TILES for pointer ops. A seed without
   * treatGrabTileAsTiles parks it in FLOATS; admit back to the live slot.
   * @param {object} focusNodeWindow
   */
  _admitGrabTileOntoTiles(focusNodeWindow) {
    const wm = this._extWm;
    const forest = wm?.forest;
    if (!forest?.nodes || !focusNodeWindow) return;
    const grabId = forestIdFromLive(wm, focusNodeWindow);
    const grab = grabId ? forest.nodes[grabId] : null;
    if (!grab) return;
    const p = grab.parentId ? forest.nodes[grab.parentId] : null;
    if (!p || p.kind !== "FLOATS") return;
    const destLive =
      (typeof wm._membershipParentLive === "function"
        ? wm._membershipParentLive(focusNodeWindow)
        : null) || focusNodeWindow.parentNode;
    if (!destLive || destLive === focusNodeWindow) return;
    forestReparent(wm, focusNodeWindow, destLive);
  }

  /**
   * Preview / commit via OpSet.pointer (D101). Zone→op is Mark 2 data.
   */
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    // Ensure Forest before pointer policy (same gate runMark2 uses).
    ensureLiveForest(wm, liveForestPaintHooks(wm, { treatGrabTileAsTiles: true }));
    this._admitGrabTileOntoTiles(focusNodeWindow);
    const forest = wm.forest;
    ...
    const ev = this._buildPointerEv(focusNodeWindow, { preferStrip: !preview });
    if (!ev) {
      if (preview) this._hidePreviewActors(focusNodeWindow);
      return;
    }
    ...
    if (preview) {
      const desc = pointerApi.hover(forest, ev);
      this._paintPointerHover(focusNodeWindow, desc, wm.nodeWinAtPointer);
      return;
    }

    // Consume strip stashes once we have a strip hit (or after attempt).
    const wasStrip = ev.hit?.tag === "strip";
    if (wasStrip) {
      this._originStripCommit = null;
      this._clearForeignStripPreview();
      this._foreignStripCommit = null;
    ...
    if (!resolved?.op) {
      if (ev.hit?.tag === "none") Logger.debug("dnd commit miss reason=pointer-none");
      else if (ev.hit?.tag === "window") Logger.debug("dnd commit miss reason=pointer-noop");
      return;
    }

    Logger.debug(`dnd commit pointer op=${resolved.op} dir=${resolved.args?.dir || "-"}`);
    if (this._commitPointerOp(focusNodeWindow, resolved)) return;

    // Empty-mon: Mark 2 transfer may fail if Meta mon move needed — last-resort host path.
    if (resolved.op === "move" && ev.hit?.tag === "empty-monitor") {
  ...
  /**
   * Run named Mark 2 Ops from pointer.release `{op,args}`.
   * @param {object} focusNodeWindow
   * @param {object} operation
   * @param {{ op: string, dir: string, onto?: string, insertIndex?: number, place?: string }} resolved
   * @returns {boolean}
   */
  _commitDropMark2(focusNodeWindow, operation, resolved) {
    ...
    if (!applied) {
      Logger.debug(`dnd commit failed op=${resolved.op} reason=mark2`);
      return false;
    }

    const tabIntent = resolved.op === "group" || (operation.isCenter && !operation.isSwap);
    if (tabIntent) {
      const joined = liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode;
      if (!joined?.isStackedOrTabbed?.()) {
        Logger.debug("dnd commit failed reason=parent-not-tabbed");
        return false;
      }
      if (joined.isStacked?.() && !wm.ext?.settings?.get_boolean?.("stacked-tiling-mode-enabled")) {
        if (!forestSetLayout(wm, joined, LAYOUT_TYPES.TABBED)) {
  ...
  getDragPointer(focusNodeWindow) {
    ...
    if (inside) return [inside.x, inside.y, pointerCoord[2]];
    // Bug #151: frame follows the touch point when `_data`/inside is missing.
    try {
      const frame = focusNodeWindow?.nodeValue?.get_frame_rect?.();
      if (frame && Number(frame.width) > 0 && Number(frame.height) > 0) {
        const x = Number(frame.x) || 0;
        const y = Number(frame.y) || 0;
        const w = Number(frame.width) || 0;
        const h = Number(frame.height) || 0;
        return [x + w / 2, y + Math.min(8, Math.max(0, h / 2)), pointerCoord[2]];
      }
    } catch (_e) {
      /* finalized Meta */
    }
  ...
  /**
   * Pointer / tree / Meta monitor fields for empty-mon / gap drops (R015).
   * @param {Object} focusNodeWindow
   * @param {number|null} [destMonOverride]
   * @returns {{
   *   hasWindowTarget: boolean,
   *   pointerMonIndex: number,
   *   sourceTreeMonIndex: number,
   ...
   * }|null}
   */
  _emptyMonitorDropParams(focusNodeWindow, destMonOverride = null) {
    const wm = this._extWm;
    ...
    let sourceMetaMon = -1;
    try {
      const m = focusNodeWindow.nodeValue.get_monitor?.();
      if (typeof m === "number") sourceMetaMon = m;
    } catch (_e) {
      sourceMetaMon = -1;
    }
    let pointerMon = -1;
    ...
    return {
      ...
      sourceMetaMonIndex: sourceMetaMon,
    };
  }

  /**
   * Resolve pointer mon vs source tree/Meta mon for empty-mon / gap drops (R015).
   * @param {Object} focusNodeWindow
  ...
  _resolveEmptyMonitorDropDecision(focusNodeWindow, destMonOverride = null) {
    const p = this._emptyMonitorDropParams(focusNodeWindow, destMonOverride);
    if (!p) return null;
    return resolveEmptyMonitorDrop(p);
  }
  ...
  _commitEmptyMonitorDrop(focusNodeWindow, destMonOverride = null) {
    ...
    const wm = this._extWm;
    const p = this._emptyMonitorDropParams(focusNodeWindow, destMonOverride);
    const decision = p ? resolveEmptyMonitorDrop(p) : null;
    if (!decision) {
      const src = sourceMonIndexFromDrop(p);
      const dest = p?.pointerMonIndex ?? -1;
      const reason = emptyMonitorDropNoDecisionReason(p) || "pointer-miss";
      Logger.debug(
        `dnd empty-mon no-decision src=${src} dest=${dest} pointerMon=${dest} ` +
          `hasWindowTarget=${!!p?.hasWindowTarget} reason=${reason}`
      );
      return false;
    }
    ...
    if (!attached && !forestReparent(wm, focusNodeWindow, destNode)) {
      recordFallback("empty-mon-dnd", "forest-fail");
      Logger.warn("empty-mon-dnd: Forest reparent fail; fail closed");
      try {
        resyncWmToReality(wm, "empty-mon-dnd-forest-fail");
      } catch (_e) {
        /* best-effort */
      }
      return false;
    ...
    this._tree.resetSiblingPercent?.(previousParent);
    this._tree.resetSiblingPercent?.(
      liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode || destNode
    );
  ...
  _findNodeWindowAtPointer(metaWindow, pointer) {
    ...
    const draggedNode = this._tree.findNode(metaWindow);
    const duringGrab = !!(
    ...
    for (let i = 0, n = sortedWindows.length; i < n; i++) {
      ...
      const node = this._tree.findNode(w);
  ...
  _handleGrabOpBegin(_display, metaWindow, grabOp) {
    ...
    if (focusMetaWindow) {
      ...
      const seenTabbed = new Set();
      while (
        tabbedAncestor &&
        tabbedAncestor.isStackedOrTabbed() &&
        !seenTabbed.has(tabbedAncestor)
      ) {
        seenTabbed.add(tabbedAncestor);
  ...
  armTabDrag(metaWindow, event) {
    ...
    try {
      ...
      pressedTab = this._tabForNode(ctx?.unit) || null;
  ...
  _resolveTabStripReorderContext(metaWindow) {
    const wm = this._extWm;
    const winNode = wm?.findNodeWindow?.(metaWindow) || null;
    if (!winNode) return null;

    let unit = winNode;
    let group = liveParentForPresent(wm, unit) || unit.parentNode;
    const seen = new Set();
    while (group && typeof group.isStackedOrTabbed === "function" && !group.isStackedOrTabbed()) {
      ...
      seen.add(group);
      unit = group;
      const up = liveParentForPresent(wm, unit) || unit.parentNode;
      if (!up || up === group) {
        group = null;
        break;
      }
    ...
    if ((liveParentForPresent(wm, unit) || unit.parentNode) !== group) return null;
  ...
  _collectGroupTabRects(group) {
    const rects = [];
    for (const c of liveChildrenForPresent(this._extWm, group)) {
      const r = tabActorScreenRect(this._tabForNode(c));
      if (r) rects.push(r);
    }
    return rects;
  }
  ...
  _tryEnterTabStripReorder(state, x, y) {
    ...
    const kids = liveChildrenForPresent(this._extWm, ctx.group);
    const fromIndex = kids.indexOf(ctx.unit);
    if (fromIndex < 0) return false;

    const tab = this._tabForNode(ctx.unit);
  ...
  _snapshotReorderSiblings(state) {
    ...
    const unitTab = this._tabForNode(unit);
    const unitHome = tabActorScreenRect(unitTab);
    const groupKids = liveChildrenForPresent(this._extWm, group);
    const unitInGroup = !!(unit && groupKids.indexOf(unit) >= 0);
    ...
    for (const c of groupKids) {
      if (c === unit) continue;
      const cTab = this._tabForNode(c);
      const r = tabActorScreenRect(cTab);
      ...
      snaps.push({
        ...
        expandX: cTab?.x_expand,
        expandY: cTab?.y_expand,
      });
      // Freeze expand so BoxLayout cannot swallow the gap mid-gesture.
      try {
        if (cTab) {
          if (axis === "y") cTab.y_expand = false;
          else cTab.x_expand = false;
        }
      } catch (_e) {
        // mock / disposed
      }
  ...
  _beginTabReorderFloat(state, x, y) {
    const tab = this._tabForNode(state.unitNode);
    if (!tab) return;
  ...
  _plannedTabStripHitRects(state, ctx) {
    ...
    if (state.siblingSnap?.length) {
      ...
      for (const c of liveChildrenForPresent(this._extWm, ctx?.group)) {
  ...
  _updateTabReorderFromPointer(state, x, y) {
    ...
    const kids = liveChildrenForPresent(this._extWm, group);
    ...
    for (let i = 0; i < kids.length; i++) {
      ...
      if (s) {
        ...
        tabs.push(tabActorScreenRect(this._tabForNode(kids[i])));
      }
    }
  ...
  _restoreTabReorderChip(state) {
    if (!state) return;
    const unit = state.unitNode;
    const tab = this._tabForNode(unit);
    const host = state.chipHomeParent;
    const axis = state.axis === "y" ? "y" : "x";
  ...
  /** Commit strip reorder via Forest order; percents and open-leaf stay put. */
  _commitTabStripReorder(state) {
    ...
    const unitParent = liveParentForPresent(wm, unit) || unit?.parentNode;
    if (!group || !unit || unitParent !== group || insertIndex == null) {
      this._disarmTabDrag({ keepSynthetic: false });
    ...
    const kids = liveChildrenForPresent(wm, group);
    ...
    // D023: percents travel with the children — do not equalize.
    if (!same && !forestOrderLiveChildren(wm, group, next)) {
      recordFallback("tab-strip-reorder", "forest-fail");
      Logger.warn("tab-strip-reorder: Forest order fail; fail closed");
      try {
        resyncWmToReality(wm, "tab-strip-reorder-forest-fail");
      } catch (_e) {
        /* best-effort */
      }
      this._relayoutAfterTabStripGesture("tab-strip-reorder");
  ...
  _commitOriginStripReorder(focusNodeWindow) {
    ...
    let unit = stash.unit;
    const parentOf = (n) => liveParentForPresent(wm, n) || n?.parentNode;
    if (!unit || parentOf(unit) !== group) {
      let u = focusNodeWindow;
      const seen = new Set();
      while (u && parentOf(u) !== group) {
        if (seen.has(u)) {
          u = null;
          break;
        }
    ...
    if (!unit || parentOf(unit) !== group) return false;
    const kids = liveChildrenForPresent(wm, group);
    const fromIndex = kids.indexOf(unit);
    if (fromIndex < 0) return false;
    ...
    if (!same && !forestOrderLiveChildren(wm, group, next)) {
      recordFallback("origin-strip-reorder", "forest-fail");
      Logger.warn("origin-strip-reorder: Forest order fail; fail closed");
      try {
        resyncWmToReality(wm, "origin-strip-reorder-forest-fail");
      } catch (_e) {
        /* best-effort */
      }
      this._relayoutAfterTabStripGesture("tab-strip-reorder");
  ...
  _clearTabPressedClasses(state) {
    ...
    const unitTab = this._tabForNode(state?.unitNode);
    if (unitTab) tabs.add(unitTab);
  ...
  _originGroupOf(node) {
    const wm = this._extWm;
    let p = liveParentForPresent(wm, node) || node?.parentNode;
    const seen = new Set();
    while (p && typeof p.isStackedOrTabbed === "function" && !p.isStackedOrTabbed()) {
      if (seen.has(p)) return null;
      seen.add(p);
      const up = liveParentForPresent(wm, p) || p.parentNode;
      if (!up || up === p) return null;
      p = up;
    }
  ...
  _computeForeignStripInsertIndex(destGroup, x, y, focusNodeWindow) {
    ...
    const tabs = [];
    for (const c of liveChildrenForPresent(this._extWm, destGroup)) {
      if (c === focusNodeWindow) continue;
      tabs.push(tabActorScreenRect(this._tabForNode(c)));
    }
    const home = tabActorScreenRect(this._tabForNode(focusNodeWindow));
  ...
  _beginForeignStripPreview(focusNodeWindow, destGroup, x, y) {
    const axis = destGroup?.isStacked?.() ? "y" : "x";
    const tab = this._tabForNode(focusNodeWindow);
    const home = tabActorScreenRect(tab);
    const chipW = this._tabDragChipMinWidth(home?.width);
    ...
    const destLen = liveChildrenForPresent(this._extWm, destGroup).length;
    const state = {
      groupNode: destGroup,
  ...
  _commitForeignStripJoin(focusNodeWindow, destGroup, pointer) {
    ...
    if ((liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode) === destGroup) {
      return false;
    }
    ...
    const destMember =
      liveChildrenForPresent(wm, destGroup).find(
    ...
    const previousParent = focusNodeWindow.parentNode;
    const focusTab = this._tabForNode(focusNodeWindow);
    if (focusTab) {
      try {
        const decoParent = focusTab.get_parent();
        if (decoParent) decoParent.remove_child(focusTab);
      } catch (e) {
    ...
    const joined = forestMergeWindowsIntoGroup(wm, destMember, focusNodeWindow, layout, {
      insertIndex,
      group: destGroup,
    });
    if (!joined) {
      recordFallback("foreign-strip-join", "forest-fail");
      Logger.warn("foreign-strip-join: Forest merge fail; fail closed");
      try {
        resyncWmToReality(wm, "foreign-strip-join-forest-fail");
      } catch (_e) {
        /* best-effort */
      }
      return false;
    ...
    previousParent?.resetLayoutSingleChild?.();
    return (liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode) === destGroup;
  }
}
