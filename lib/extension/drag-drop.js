...
export class DragDropManager extends GObject.Object {
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    // Probe dest mins when unread (async; later motion paints red).
    try {
      wm.ensureWindowMinSizeKnown?.(nodeWinAtPointer.nodeValue);
      const destGroup =
        nodeWinAtPointer.parentNode?.isStackedOrTabbed?.() && nodeWinAtPointer.parentNode;
      if (destGroup) {
        for (const child of destGroup.childNodes || []) {
          if (child?.nodeValue) wm.ensureWindowMinSizeKnown?.(child.nodeValue);
        }
      }
    } catch (_e) {
      /* ignore */
   ...
   * Skipped when tab-chrome already owns the gesture.
   */
  ...
  _handleGrabOpBegin(_display, metaWindow, grabOp) {
    ...
    wm.decorationManager?.setSplitChromeForceShowAll?.(true);
    ...
    if (focusMetaWindow) {
      ...
      if (
        ...
        // Wayland: no size hints — probe clamp so DnD red zones can arm.
        try {
          wm.ensureWindowMinSizeKnown?.(focusMetaWindow);
        } catch (_e) {
          /* ignore */
        }
      }
  ...
  _handleGrabOpEnd(_display, metaWindow, grabOp) {
    ...
    try {
      ...
      // I5: restore prior split-chrome mode even if drop cancels / peel aborts
      wm.decorationManager?.setSplitChromeForceShowAll?.(false);
    }
  }
