...
export class DragDropManager extends GObject.Object {
  ...
  /** Hit-test rects: sibling tabs, else decoration strip. */
  ...
  _tabDragPointerOnStrip(state, x, y) {
    ...
    state.unitNode = ctx.unit;
    state.axis = ctx.axis;
    // Prefer live sibling+spacer union; fall back to decoration when floating.
    const hitRects = this._collectGroupStripHitRects(ctx.group);
    if (state.siblingSnap?.length) {
      const axis = state.axis === "y" ? "y" : "x";
      const sizes = state.siblingSnap.map((s) => s.size);
      const segs = tabStripFlowLayoutWithGap({
        sizes,
        gapIndex: state.previewGap ?? 0,
        chipSize: axis === "y" ? state.chipH : state.chipW,
        origin: state.stripOrigin,
      });
      const rects = segs.map((seg, i) => {
        if (axis === "y") {
          return {
            x: state.siblingSnap[i]?.cross ?? state.stripCross,
            y: seg.start,
            width: state.siblingSnap[i]?.crossSize || 1,
            height: seg.end - seg.start,
          };
        }
        return {
          x: seg.start,
          y: state.siblingSnap[i]?.cross ?? state.stripCross,
          width: seg.end - seg.start,
          height: state.siblingSnap[i]?.crossSize || 1,
        };
      });
      // Include gap slot so the hole still counts as on-strip.
      const gapAt = Math.max(0, Math.min(sizes.length, Number(state.previewGap) || 0));
      let gapStart = state.stripOrigin;
      for (let i = 0; i < gapAt; i++) gapStart += sizes[i];
      const chipSize = axis === "y" ? state.chipH : state.chipW;
      if (axis === "y") {
        rects.push({
          x: state.stripCross,
          y: gapStart,
          width: state.siblingSnap[0]?.crossSize || 1,
          height: chipSize,
        });
      } else {
        rects.push({
          ...
          y: state.stripCross,
          width: chipSize,
          height: state.siblingSnap[0]?.crossSize || state.chipH || 1,
        });
      }
      if (rects.length) return pointerOnTabStrip({ tabs: rects, pointer: [x, y] });
    }
  ...
  _updateTabReorderFromPointer(state, x, y) {
    ...
    const flowTabs = segs.map((seg, i) => {
      ...
      return {
        x: seg.start,
        y: snaps[i]?.cross ?? state.stripCross,
        width: seg.end - seg.start,
        height: snaps[i]?.crossSize || state.chipH || 1,
      };
    });
    ...
    if (state.previewGap !== gapIndex) {
      state.previewGap = gapIndex;
      this._applyTabReorderGapVisual(state, true);
    }
  ...
  _applyTabReorderGapVisual(state, animate) {
    ...
    for (let i = 0; i < snaps.length; i++) {
      ...
      // translation = target screen - home screen (layout still at home)
      const dx = axis === "x" ? targetStart - snap.homeStart : 0;
      const dy = axis === "y" ? targetStart - snap.homeStart : 0;
  ...
  _placeGapSpacerInHost(state) {
    ...
    const host = state.chipHomeParent;
    const snaps = state.siblingSnap || [];
    if (!spacer || !host) return;

    try {
      if (typeof host.remove_child === "function" && host.contains?.(spacer)) {
    ...
    // Child order among remaining tabs may not match snaps if BoxLayout reordered;
    // insert before the tab at gapAt when possible.
    try {
      if (typeof host.insert_child_at_index === "function") {
        // Build desired order: remaining tabs in snap order with spacer at gapAt.
        const kids = typeof host.get_children === "function" ? [...host.get_children()] : [];
        for (const k of kids) {
          if (k === spacer) continue;
          // leave non-tab chrome alone; only ensure spacer index among snaps
        }
        let insertAt = 0;
        const children = typeof host.get_children === "function" ? host.get_children() : [];
        // Count snap tabs before gapAt that are still on host.
        let seen = 0;
        for (let i = 0; i < children.length; i++) {
          ...
          const isSnapTab = snaps.some((s) => s.tab === children[i]);
          if (isSnapTab) seen++;
          insertAt = i + 1;
        }
