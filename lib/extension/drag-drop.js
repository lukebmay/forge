...
/** CSS marker on the sibling tab we would insert before (preview only). */
const TAB_REORDER_INSERT_CLASS = "window-tabbed-tab-reorder-insert";
...
export class DragDropManager extends GObject.Object {
  ...
  noteTabDragMotion(x, y) {
    ...
    if (state.reorder) {
      if (!this._tabDragPointerOnStrip(state, x, y)) {
        this._clearTabReorderPreview(state);
        state.reorder = false;
  ...
  _updateTabReorderFromPointer(state, x, y) {
    ...
    for (const c of group.childNodes || []) {
      ...
      if (r) {
        ...
        // Keep index alignment when a sibling lacks a tab actor.
        const prev = segments[segments.length - 1];
        if (state.axis === "y") {
          const y0 = prev ? prev.y + prev.height : 0;
          segments.push({ x: 0, y: y0, width: 1, height: 1 });
        } else {
          const x0 = prev ? prev.x + prev.width : 0;
          segments.push({ x: x0, y: 0, width: 1, height: 1 });
        }
      }
    }

    const insertIndex = tabStripInsertIndex({
      tabs: segments,
      pointer: { x, y },
      axis: state.axis || "x",
    });
    state.insertIndex = insertIndex;
    this._paintTabReorderPreview(group, insertIndex, state.unitNode);
  }
   ...
   * @param {any} unitNode
   */
  _paintTabReorderPreview(group, insertIndex, unitNode) {
    for (const c of group?.childNodes || []) {
      try {
        c?.tab?.remove_style_class_name?.(TAB_REORDER_INSERT_CLASS);
      } catch (_e) {
        // disposed
      }
    }
    const kids = group?.childNodes || [];
    if (insertIndex < 0 || insertIndex >= kids.length) return;
    const target = kids[insertIndex];
    if (!target || target === unitNode) return;
    try {
      target.tab?.add_style_class_name?.(TAB_REORDER_INSERT_CLASS);
    } catch (_e) {
      // disposed
    }
  }
