...
/**
 * TOP/BOTTOM (or edge) on a tab window: the slot is the TABBED/STACKED bag.
 * @param {object|null|undefined} referenceNode
 * @param {object|null|undefined} nodeWinAtPointer
 * @param {object} [wm] Forest-first parent when seeded (D096)
 * @returns {object|null|undefined}
 */
export function dropSlotSplitUnit(referenceNode, nodeWinAtPointer, wm) {
  const unit = referenceNode || nodeWinAtPointer;
  if (!unit) return unit;
  const parent = (wm && liveParentForPresent(wm, unit)) || unit.parentNode;
  if (
    parent &&
...
export class DragDropManager extends GObject.Object {
  ...
  /**
   * Named Host SurfaceOps (swapPairs / group / slotSplit / split / wrap / insert).
   * Forest-only (D096/G4) — fail closed + resync on ids miss.
   *
   * @param {Object} focusNodeWindow
   * @param {Object} operation
   * @param {Object} nodeWinAtPointer
   * @param {Object} ctx
   * @param {{ op: string }} surface
   */
  _commitDropSurface(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface) {
    const wm = this._extWm;
    const { isCenter, isHorizontal, isBefore } = operation;
    const { centerLayout, stackedOrTabbed } = ctx;
    const op = surface?.op || "insert";
    Logger.trace(
      `dnd surface=${op} center=${!!isCenter} h=${!!isHorizontal} before=${!!isBefore} ` +
    ...
    if (
      !this._commitDropSurfaceForest(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface)
    ) {
      Logger.warn(`dnd surface=${op}: Forest ids miss; fail closed`);
      try {
        resyncWmToReality(wm, "dnd-surface-ids-miss");
      } catch (_e) {
        /* best-effort */
      }
      return;
    ...
    this._finishDropMutate(focusNodeWindow, previousParent, operation);
  }

  /**
   * Forest SurfaceOp. False → caller fails closed (no GObject twin).
   * @returns {boolean}
   */
  _commitDropSurfaceForest(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface) {
    const wm = this._extWm;
    ...
    if (op === "swapPairs") {
      return forestSwapWindows(wm, referenceNode, focusNodeWindow);
    }
    if (op === "group") {
      const layout = centerLayout === "STACKED" ? LAYOUT_TYPES.STACKED : LAYOUT_TYPES.TABBED;
      return !!forestMergeWindowsIntoGroup(wm, focusNodeWindow, nodeWinAtPointer, layout);
    }
    if (op === "wrap") {
      const layout = isHorizontal
        ...
        : !isCenter
        ? LAYOUT_TYPES.VSPLIT
        : LAYOUT_TYPES[centerLayout];
      const parentKids = liveChildrenForPresent(wm, parentNodeTarget);
      const numWin = parentKids.filter((c) => c.nodeType === NODE_TYPES.WINDOW).length;
      const numChild = parentKids.length;
      const reuseHost = !isCenter && isConParent && numWin === 1 && numWin === numChild;
      return forestWrapInsert(wm, nodeWinAtPointer, focusNodeWindow, layout, {
        before: !!isBefore,
        reuseHost,
      });
    }
    if (op === "slotSplit") {
      const targetCon = dropSlotSplitUnit(referenceNode, nodeWinAtPointer, wm);
      const parent = liveParentForPresent(wm, targetCon) || targetCon?.parentNode || containerNode;
      if (!targetCon || !parent) {
        Logger.warn("slotSplit: invalid target/parent");
        return false;
      }
      const orientation = isHorizontal ? ORIENTATION_TYPES.HORIZONTAL : ORIENTATION_TYPES.VERTICAL;
      const ok =
        forestSlotSplit(wm, targetCon, orientation, {
          insertLive: focusNodeWindow,
          before: !!isBefore,
        }) ||
        forestSlotSplit(wm, targetCon, orientation, {
          insertLive: focusNodeWindow,
      ...
      if (ok) {
        Logger.info(`dnd surface=slotSplit ok orient=${orientation} before=${!!isBefore}`);
      }
      return ok;
    }
    if (op === "split") {
      const orientation = isHorizontal ? ORIENTATION_TYPES.HORIZONTAL : ORIENTATION_TYPES.VERTICAL;
      return forestSplit(wm, focusNodeWindow, orientation, {
        moveToLive: containerNode,
        moveBeforeLive: referenceNode || undefined,
      });
    }

    const dest = referenceNode || containerNode;
    if (
      !forestReparent(wm, focusNodeWindow, dest, {
        destIsWindow: !!referenceNode,
        position: referenceNode ? "before" : undefined,
      })
    ) {
      return false;
    }
    let nextLayout = null;
    if (isHorizontal) nextLayout = LAYOUT_TYPES.HSPLIT;
    else if (!isCenter) {
      if (!stackedOrTabbed) nextLayout = LAYOUT_TYPES.VSPLIT;
    } else if (isCenter && containerNode.isStacked() && centerLayout === "TABBED") {
      nextLayout = LAYOUT_TYPES.TABBED;
    }
    if (nextLayout && containerNode.layout !== nextLayout) {
      forestSetLayout(wm, containerNode, nextLayout);
    }
    return true;
  }
  ...
  /**
   * @param {Object} focusNodeWindow
   * @param {Object|null} previousParent
   * @param {Object} operation
   */
  _finishDropMutate(focusNodeWindow, previousParent, operation) {
    const wm = this._extWm;
    const isCenter = !!operation?.isCenter;
    const stackOff =
      isCenter && !operation.isSwap && !wm.ext.settings.get_boolean("stacked-tiling-mode-enabled");
    const joinedNow = liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode;
    if (stackOff) {
      if (joinedNow && joinedNow.isStacked()) {
        if (!forestSetLayout(wm, joinedNow, LAYOUT_TYPES.TABBED)) {
          joinedNow.layout = LAYOUT_TYPES.TABBED;
        }
      }
    }
    ...
    if (isCenter && !operation.isSwap) {
      if (joinedNow?.isStackedOrTabbed?.()) {
        // D095 S5: primary present sizes peers; no join geometry-force.
        wm.normalizeGroupToHomeMonitor?.(joinedNow);
      }
    }
  ...
  _buildDropOperation(zone, ctx) {
    ...
    if (isCenter) {
      ...
      // MONITOR or H/V CON: wrap dest+source. Same-parent 2-window CENTER
      // still hits shouldMergeCenterGroup → group (in-place flip).
      return {
        ...baseOp,
    ...
    if (stackedOrTabbed) {
      if (!isMonParent) {
        ...
        const bagParent = liveParentForPresent(wm, parentNodeTarget) || parentNodeTarget.parentNode;
        return {
          ...baseEdgeOp,
          shouldWrapTargetCon: true,
          containerNode: bagParent,
    ...
    const draggedParent = liveParentForPresent(ctx.wm ?? this._extWm, dragged);
    const sameParent = !!(dragged && draggedParent === parentNodeTarget);
    const destHasSiblings =
      liveChildrenForPresent(ctx.wm ?? this._extWm, parentNodeTarget).length >= 2;
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    // Bug #328 / D096: Meta required; TILES parent may be Forest-only (GObject null).
    if (!nodeWinAtPointer.nodeValue) {
      Logger.warn("moveWindowToPointer: invalid nodeWinAtPointer structure");
      return;
    }

    const parentNodeTarget =
      liveParentForPresent(wm, nodeWinAtPointer) || nodeWinAtPointer.parentNode;
    if (!parentNodeTarget) {
    ...
    if (preview) {
      ...
      this._commitResolvedDrop(focusNodeWindow, nodeWinAtPointer, operation, ctx);
    }
  }

  /**
   * Pointer + synthetic drop: Mark 2 Join/Move, else named Host SurfaceOp.
   * @returns {boolean}
   */
  _commitResolvedDrop(focusNodeWindow, nodeWinAtPointer, operation, ctx) {
    const wm = this._extWm;
    const resolved = resolveDropMark2(focusNodeWindow, nodeWinAtPointer, operation, ctx);
    if (resolved && this._commitDropMark2(focusNodeWindow, operation, resolved)) return true;
    // D096: Forest parent is TILES SoT; GObject parentNode may be null after seed.
    const focusParent = liveParentForPresent(wm, focusNodeWindow) || focusNodeWindow.parentNode;
    if (!focusParent) return false;
    const surface = resolveDropSurface(focusNodeWindow, nodeWinAtPointer, operation, ctx);
    if (surface?.op === "emptyMonitorDrop") {
      const dest =
        typeof ctx?.destMonitor === "number" && ctx.destMonitor >= 0 ? ctx.destMonitor : null;
      return !!this._commitEmptyMonitorDrop(focusNodeWindow, dest);
    }
    let op = operation;
    if (surface?.op === "slotSplit" && operation) {
      const unit = dropSlotSplitUnit(operation.referenceNode, nodeWinAtPointer, wm);
      if (unit && unit !== operation.referenceNode) {
        op = {
          ...operation,
          referenceNode: unit,
          containerNode:
            liveParentForPresent(wm, unit) || unit.parentNode || operation.containerNode,
        };
      }
    }
    this._commitDropSurface(
      focusNodeWindow,
      op,
      nodeWinAtPointer,
      ctx,
  ...
  /**
   * TILES Join/Move for a resolved drop. False → caller keeps SurfaceOp.
   * @param {object} focusNodeWindow
   * @param {object} operation
