 ...
 */
export function dropSlotSplitUnit(referenceNode, nodeWinAtPointer) {
...
export class DragDropManager extends GObject.Object {
  ...
  swapWindowsUnderPointer(focusNodeWindow) {
    ...
    if (!forestSwapWindows(wm, focusNodeWindow, nodeWinAtPointer)) {
      recordFallback("dnd-swap", "ids-miss");
      this._tree.swapPairs(focusNodeWindow, nodeWinAtPointer);
    }
  }

  /**
   * One body for named Host SurfaceOps (swapPairs / group / slotSplit / split / wrap / insert).
   *
  ...
  _commitDropSurface(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface) {
    ...
    if (
      ...
      recordFallback("dnd", "ids-miss");
      this._commitDropSurfaceGObject(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface);
    }
  ...
  /**
   * Forest-first SurfaceOp. False → GObject fallback (ids miss / unseeded).
   * @returns {boolean}
   */
  _commitDropSurfaceForest(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface) {
    ...
    if (op === "slotSplit") {
      const targetCon = dropSlotSplitUnit(referenceNode, nodeWinAtPointer);
      const parent = targetCon?.parentNode || containerNode;
  ...
  _commitDropSurfaceGObject(focusNodeWindow, operation, nodeWinAtPointer, ctx, surface) {
    const { containerNode, referenceNode, isCenter, isHorizontal, isBefore } = operation;
    const { isConParent, centerLayout, parentNodeTarget, stackedOrTabbed } = ctx;
    const op = surface?.op || "insert";
    const previousParent = focusNodeWindow.parentNode;
  ...
  _finishDropMutate(focusNodeWindow, previousParent, operation) {
    ...
    if (isCenter && !operation.isSwap) {
      ...
      if (joined?.isStackedOrTabbed?.()) {
        ...
        // D069: size every peer on join so the next tab-click stays raise-only.
        try {
          wm.focusManager?._reassertTabStackSiblingSlots?.(joined, {
            mode: "all",
            force: true,
          });
        } catch (_e) {
  ...
  _commitResolvedDrop(focusNodeWindow, nodeWinAtPointer, operation, ctx) {
    ...
    if (!focusNodeWindow.parentNode) return false;
    ...
    if (surface?.op === "slotSplit" && operation) {
      const unit = dropSlotSplitUnit(operation.referenceNode, nodeWinAtPointer);
      if (unit && unit !== operation.referenceNode) {
        op = {
          ...operation,
          referenceNode: unit,
          containerNode: unit.parentNode || operation.containerNode,
        };
      }
    }
