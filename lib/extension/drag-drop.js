...
export class DragDropManager extends GObject.Object {
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    // Mid-drag: use known/class floors only. Queuing dest probes here caused
    // grab-end to shrink-probe every hovered window (Chrome never learns →
    // forever-retry → fights tile sizes / next DnD).
   ...
   * Skipped when tab-chrome already owns the gesture. Also drives
   * `_handleMoving` so overlays do not depend solely on Meta geom signals.
   */
    ...
    if (stage && typeof stage.connect === "function") {
      const id = stage.connect("captured-event", (_actor, ev) => {
        ...
        if (isMotion && !this._tabDrag) {
          const dragged = this._extWm?._draggedNodeWindow;
          if (dragged?.mode === WINDOW_MODES.GRAB_TILE) {
            try {
  ...
  _handleGrabOpBegin(_display, metaWindow, grabOp) {
    ...
    if (focusMetaWindow) {
      ...
      if (
        ...
        // Abort any in-flight shrink probe so restore cannot fight this grab.
        try {
          wm._cancelMinSizeProbes?.();
        } catch (_e) {
          /* ignore */
        }
        wm.freezeRender();
        ...
        // Do not probe/move_resize during MOVING grab — fights Wayland grab and
        // suppresses position-changed overlays via _forgeMinProbing. Probe after
        // grab-end / open settle; mid-drag uses known/class floor only.
      }
  ...
  _handleGrabOpEnd(_display, metaWindow, grabOp) {
    ...
    wm.grabOp = null;
    // Wayland mins: drop any mid-drag / leftover queue, then queue only the
    // dragged window after tile settle — never flush immediately (that raced
    // commitLayout with 32×32 probes on every hovered dest).
    try {
      wm._minSizeProbeQueue?.clear?.();
      try {
        wm._wmSources?.cancel?.("minSizeProbeFlush");
      } catch (_e2) {
        /* ignore */
      }
      if (focusMetaWindow) {
        wm._queueMinSizeProbe?.(focusMetaWindow, { delayMs: 450 });
      }
    } catch (_e) {
      /* ignore */
    }
  ...
  _armTabDragPointerPoll() {
    ...
    const tick = () => {
      ...
      const xy = this._tabDragPointerXY();
      if (xy) this.noteTabDragMotion(xy[0], xy[1]);
      // Re-arm one-shot (~120 Hz). Cancelled on residual clear / disarm.
