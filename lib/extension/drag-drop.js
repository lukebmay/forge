...
export class DragDropManager extends GObject.Object {
  ...
  /**
   * True when drop would not change tree topology/order (e.g. already bottom of
   * VSPLIT with that sibling). Prevents re-drop thrash (HSPLIT↔VSPLIT) when the
   * pointer lands on the same relative slot as the source already occupies.
   */
  _isNoOpDrop(focusNodeWindow, nodeWinAtPointer, operation, ctx) {
    if (!focusNodeWindow || !nodeWinAtPointer || !operation) return true;
    // Dropping on self / same tile unit — paint OK, never restructure.
    if (focusNodeWindow === nodeWinAtPointer) return true;

    const parent = focusNodeWindow.parentNode;
    if (!parent || parent !== nodeWinAtPointer.parentNode) return false;

    // Center join into same tab/stack parent: already a member.
    if (operation.isCenter && !operation.isSwap && !operation.shouldCreateCon) {
      if (ctx?.stackedOrTabbed && parent === operation.containerNode) return true;
    }

    if (operation.isSwap) return false;
    if (operation.shouldWrapTargetCon || operation.shouldDetachWindow) return false;

    // Simple insert / createCon among existing siblings.
    if (operation.shouldCreateCon) {
      // Would nest a new H/V split — only no-op if already alone with target under
      // a matching split parent (two kids, correct layout, correct order).
      if (parent.childNodes.length !== 2) return false;
      const wantH = operation.isHorizontal;
      const haveH = parent.isHSplit?.() || parent.layout === LAYOUT_TYPES.HSPLIT;
      const haveV = parent.isVSplit?.() || parent.layout === LAYOUT_TYPES.VSPLIT;
      if (wantH && !haveH) return false;
      if (!wantH && !haveV) return false;
    } else {
      // Layout change on same parent would still be a real op.
      if (operation.isHorizontal && parent.isVSplit?.()) return false;
      if (!operation.isHorizontal && !operation.isCenter && parent.isHSplit?.()) return false;
    }

    if (operation.isBefore) {
      // Want focus immediately before target.
      return focusNodeWindow.nextSibling === nodeWinAtPointer;
    }
    // Want focus immediately after target.
    return nodeWinAtPointer.nextSibling === focusNodeWindow;
  }
  ...
  _commitEmptyMonitorDrop(focusNodeWindow, destMonOverride = null) {
    ...
    // Same attach policy as Meta rehome: after dest LFT when present, else mon root.
    if (typeof wm._rehomeWindowPreservingContainer === "function") {
      wm._rehomeWindowPreservingContainer(focusNodeWindow, meta, destNode);
    } else {
