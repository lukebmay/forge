...
import { layoutUnit } from "./layout-resize.js";
...
export class CommandHandler extends GObject.Object {
  ...
  _buildHandlers() {
    ...
    const swapSibling = (action, ctx) => {
      ...
      // S2: elevated CON swaps as unit.
      if (!focusNodeWindow) return;
      wm.unfreezeRender();
      const offset = action.name === "SwapNext" ? 1 : -1;
      const unit = resolveMoveUnit(wm.tree.attachNode, focusNodeWindow) || focusNodeWindow;
      const swapped = wm.tree.swapSibling(unit, offset);
      if (swapped) {
        if (Utils.isWindowAlive(focusNodeWindow.nodeValue)) {
    ...
    this._handlers = {
      ...
      Move: (action, ctx) => {
        ...
        // S2: elevated CON moves as one unit; leaf path unchanged.
        const moveUnit = resolveMoveUnit(wm.tree.attachNode, focusNodeWindow) || focusNodeWindow;
        let moved = wm.tree.move(moveUnit, moveDirection);
        if (!focusNodeWindow) {
          focusNodeWindow = wm.findNodeWindow(wm.focusMetaWindow);
        }
        wm.queueEvent({
          ...
          callback: () => {
            if (wm.eventQueue.length <= 0) {
              ...
              if (focusNodeWindow.parentNode.layout === LAYOUT_TYPES.STACKED) {
                // Raise only — keep childNodes order stable (stack label order).
      ...
      Swap: (action, ctx) => {
        ...
        // S2: elevated CON swaps as one unit; leaf path unchanged.
        const swapUnit = resolveMoveUnit(wm.tree.attachNode, focusNodeWindow) || focusNodeWindow;
        wm.tree.swap(swapUnit, swapDirection);
        if (Utils.isWindowAlive(focusNodeWindow.nodeValue)) {
          focusNodeWindow.nodeValue.raise();
        }
        wm.updateTabbedFocus(focusNodeWindow);
      ...
      LayoutToggle: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;
        const parent = resolveLayoutOpsTarget(wm.tree.attachNode, focusNodeWindow);
        if (!parent) return;
        const currentLayout = parent.layout;
        if (currentLayout === LAYOUT_TYPES.HSPLIT) {
          setLayout(parent, LAYOUT_TYPES.VSPLIT);
        } else if (currentLayout === LAYOUT_TYPES.VSPLIT) {
          setLayout(parent, LAYOUT_TYPES.HSPLIT);
        }
      ...
      SplitChromeShowAllToggle: (action, ctx) => {
        const { wm } = ctx;
        const showAll = wm.ext.settings.get_boolean("split-chrome-show-all");
        wm.ext.settings.set_boolean("split-chrome-show-all", !showAll);
      },
      ...
      // I1: mode only via setLayout — no percent wipe; MONITOR still force-splits.
      LayoutStackedToggle: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;
        if (!wm.ext.settings.get_boolean("stacked-tiling-mode-enabled")) return;

        const elevated = resolveOpsTarget(wm.tree.attachNode, focusNodeWindow);
        const useElevated = elevated && elevated !== focusNodeWindow;
        // Leaf-only: wrap direct MONITOR children before tab/stack mode.
        if (!useElevated && focusNodeWindow.parentNode?.isMonitor?.()) {
          wm.tree.split(focusNodeWindow, ORIENTATION_TYPES.HORIZONTAL, true);
        }

        const parent =
          resolveLayoutOpsTarget(wm.tree.attachNode, focusNodeWindow) || focusNodeWindow.parentNode;
        if (!parent) return;
        const currentLayout = parent.layout;

        if (currentLayout === LAYOUT_TYPES.STACKED) {
          setLayout(parent, wm.determineSplitLayout());
        } else {
          setLayout(parent, LAYOUT_TYPES.STACKED, {
            clearLastTabFocus: currentLayout === LAYOUT_TYPES.TABBED,
          });
      ...
      LayoutTabbedToggle: (action, ctx) => {
        ...
        if (currentLayout === LAYOUT_TYPES.TABBED) {
          setLayout(parent, wm.determineSplitLayout(), { clearLastTabFocus: true });
        } else {
          setLayout(parent, LAYOUT_TYPES.TABBED, {
            lastTabFocus: focusNodeWindow.nodeValue,
          });
        }
        wm.unfreezeRender();
        wm.tree.attachNode = parent;
      ...
      // Uses setLayout (I1): mode only, no reparent/flatten.
      LayoutStackTabToggle: (action, ctx) => {
        ...
        if (current === LAYOUT_TYPES.STACKED) {
          ...
          if (!stackOn) return;
          setLayout(parent, LAYOUT_TYPES.STACKED, { clearLastTabFocus: true });
          const lastChild = parent.lastChild;
          if (lastChild && lastChild.nodeType === NODE_TYPES.WINDOW) {
            lastChild.nodeValue.activate(global.display.get_current_time());
          }
        ...
        wm.unfreezeRender();
        wm.tree.attachNode = parent;
        wm.renderTree("layout-stack-tab-toggle");
      },
      ...
      WindowMergeGroup: (action, ctx) => {
        ...
        wm.renderTree("window-merge-group");
      },

      // Dissolve elevated CON when selected; else nearest parent of focus (I2/S2).
      WindowUngroup: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || focusNodeWindow.isFloat()) return;

        const con = resolveUngroupOpsTarget(wm.tree.attachNode, focusNodeWindow);
        if (!con) return;

        const grandparent = wm.tree.ungroupContainer(con);
        if (!grandparent) return;

        wm.unfreezeRender();
        wm.tree.attachNode = grandparent;
        wm.renderTree("window-ungroup");
      },

      // i3 focus parent ($mod+a): elevate ops target to parent CON.
      FocusParent: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;

        // Walk from elevated attach when already above focus.
        let start = focusNodeWindow;
        const attach = wm.tree.attachNode;
        const elevated = resolveOpsTarget(attach, focusNodeWindow);
        if (elevated && elevated !== focusNodeWindow) {
          start = elevated;
        }

        const parent = resolveFocusParent(start);
        if (!parent) return;

        const win = resolveRepresentativeWindow(parent, focusNodeWindow, {
          findNode: (meta) => wm.tree.findNode(meta),
        });
        if (win && win.nodeValue) {
          try {
            win.nodeValue.raise();
            win.nodeValue.activate(global.display.get_current_time());
          } catch (_e) {
            /* best-effort activate */
          }
          wm.updateStackedFocus(win);
          wm.updateTabbedFocus(win);
        }
        // Set attach after activate so focus-signal reset (same window) keeps elevation.
        wm.tree.attachNode = parent;
        wm._lastFocusNodeWindow = win || focusNodeWindow;
        wm.updateBorderLayout?.();
      },

      // Focus into preferred child of attach/parent CON.
      FocusChild: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;
        ...
        if (elevated && elevated !== focusNodeWindow) {
          ...
          focusNodeWindow.parentNode &&
          (focusNodeWindow.parentNode.nodeType === NODE_TYPES.CON ||
            focusNodeWindow.parentNode.isCon?.())
        ) {
          con = focusNodeWindow.parentNode;
        }
        if (!con) return;

        const child = resolveFocusChild(con, con.lastTabFocus ?? focusNodeWindow);
        if (!child || child === con) return;

        let win = null;
        let nextAttach = null;
        if (child.nodeType === NODE_TYPES.WINDOW || child.isWindow?.()) {
          win = child;
          nextAttach = layoutUnit(child) || child;
        } else if (child.nodeType === NODE_TYPES.CON || child.isCon?.()) {
          nextAttach = child;
          win = resolveRepresentativeWindow(child, focusNodeWindow, {
            findNode: (meta) => wm.tree.findNode(meta),
          });
        }
        if (win && win.nodeValue) {
        ...
        // Attach after activate: nested CON stays elevated; leaf window is default.
        if (nextAttach) wm.tree.attachNode = nextAttach;
        wm._lastFocusNodeWindow = win || focusNodeWindow;
        wm.updateBorderLayout?.();
      },

      // Explicit clear: ops target → focused window leaf (S1; chords in S3).
      ClearSelection: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        wm.tree.attachNode = clearOpsTarget(focusNodeWindow);
        wm.updateBorderLayout?.();
      },

      // Lift unit one level (parent CON stays; not ungroup).
      // Unit = attach CON when focus-parent selected it; else focused WINDOW
      // (not layoutUnit bag — that would no-op tab groups under MONITOR).
      WindowMoveOut: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || focusNodeWindow.isFloat()) return;

        const unit = resolveMoveUnit(wm.tree.attachNode, focusNodeWindow);
        if (!unit) return;
        const gp = wm.tree.moveUnitOut(unit);
        if (!gp) return;

        wm.unfreezeRender();
        wm.tree.attachNode = gp;
        wm.renderTree("window-move-out");
      },

      // Reparent unit into adjacent sibling CON (no invent).
      WindowMoveIn: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || focusNodeWindow.isFloat()) return;

        const unit = resolveMoveUnit(wm.tree.attachNode, focusNodeWindow);
        if (!unit) return;
        const target = wm.tree.moveUnitIn(unit);
        if (!target) return;

        wm.unfreezeRender();
        wm.tree.attachNode = target;
        wm.renderTree("window-move-in");
      },
