...
import { ancestorMonitor, fail, isUnderFloats } from "../tom/index.js";
...
function canonicalCommandName(action) {
  ...
  if (name === "LayoutToggle") return "toggleSplit";
  if (name === "LayoutStackTabToggle") return "toggleTabStack";
 ...
 * @param {string} reason
 */
function runLiveForest(wm, focusNodeWindow, mutate, reason) {
  if (!focusNodeWindow || !wm?.tree) return;
  ...
  if (cur && isUnderFloats(forest, cur)) return;
...
function runMark2(wm, focusNodeWindow, op, dir, reason) {
  ...
  runLiveForest(
    wm,
    focusNodeWindow,
    (draft, api) => getOpSet("mark2").ops[op](draft, api, dir),
    reason
  );
}
