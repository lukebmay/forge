...
export class CommandHandler extends GObject.Object {
  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      // FC2: mode exit first; else clear TILE keyboard focus (best-effort).
      WindowUnfocus: (_action, ctx) => {
        const { wm } = ctx;
        if (typeof wm.exitForgeMode === "function" && wm.exitForgeMode()) return;
        if (typeof wm.unfocusTiles === "function") {
          wm.unfocusTiles();
        }
      },
