...
import { ancestorMonitor } from "../tom/index.js";

// App imports
import { NODE_TYPES, LAYOUT_TYPES, ORIENTATION_TYPES, Node } from "./tree.js";
 ...
 * @param {string} reason
 */
function runMark2(wm, focusNodeWindow, op, dir, reason) {
  if (!focusNodeWindow || !wm?.tree || !dir) return;
  if (typeof focusNodeWindow.isFloat === "function" && focusNodeWindow.isFloat()) return;
  ...
  const r = runOpAbstract(forest, api, (draft) => {
    const cur = draft.focusId ? draft.nodes[draft.focusId] : null;
    const mon = cur ? ancestorMonitor(draft, cur) : null;
...
export class CommandHandler extends GObject.Object {
  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      // C4: reparent layout unit into existing sibling CON (D044 normalize on tab/stack).
      WindowMoveIn: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || !wm.tree?.moveIn) return;
        const dest = wm.tree.moveIn(focusNodeWindow);
        if (!dest) return;
        ...
        wm.commitLayout("window-move-in", { force: true });
        const leaf =
          focusNodeWindow.parentNode === dest || dest.contains?.(focusNodeWindow)
            ? focusNodeWindow
            : wm.tree._windowForFocusUnit?.(dest, focusNodeWindow);
        if (leaf && typeof dest.isStackedOrTabbed === "function" && dest.isStackedOrTabbed()) {
          wm.revealGroupChild(leaf, { keyboard: true, source: "command-move-in" });
        }
      },

      WindowMoveOut: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || !wm.tree?.moveOut) return;
        const unit = wm.tree.moveOut(focusNodeWindow);
        if (!unit) return;

        wm.unfreezeRender();
        wm.tree.attachNode = focusNodeWindow;
        wm.commitLayout("window-move-out", { force: true });
      },
      ...
      WindowMergeGroup: (action, ctx) => {
        ...
        const group = wm.tree.group(focusNodeWindow, partner);
        if (!group) return;
      ...
      // I2: dissolve the focused unit's parent CON (promote children to grandparent).
      WindowUngroup: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;
        const parent = wm.tree.ungroup(focusNodeWindow);
        if (!parent) return;

        wm.unfreezeRender();
        wm.tree.attachNode = focusNodeWindow;
        wm.commitLayout("window-ungroup", { force: true });
      },
