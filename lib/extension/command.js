...
export class CommandHandler extends GObject.Object {
  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      // i3 focus parent ($mod+a): select parent CON for open/split attach.
      FocusParent: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;

        // Walk from attachNode when it already selects a CON above focus.
        let start = focusNodeWindow;
        const attach = wm.tree.attachNode;
        if (
          attach &&
          attach !== focusNodeWindow &&
          (attach.nodeType === NODE_TYPES.CON || attach.isCon?.()) &&
          (typeof attach.contains === "function" ? attach.contains(focusNodeWindow) : false)
        ) {
          start = attach;
        }
      ...
      FocusChild: (action, ctx) => {
        ...
        if (
          ...
          (typeof attach.contains === "function"
            ? attach.contains(focusNodeWindow) || attach === focusNodeWindow
            : false)
        ) {
          con = attach;
        } else if (
        ...
        if (child.nodeType === NODE_TYPES.WINDOW || child.isWindow?.()) {
          win = child;
          wm.tree.attachNode = layoutUnit(child) || child;
        } else if (child.nodeType === NODE_TYPES.CON || child.isCon?.()) {
          wm.tree.attachNode = child;
