...
// App imports
import { equalizeChildren } from "../tom/index.js";
...
function resetSiblingPercent(wm, parentNode) {
  if (!parentNode) return;
  if (typeof wm._resetSiblingPercent === "function") {
    wm._resetSiblingPercent(parentNode);
    return;
  }
  if (wm._liveForestSeeded && wm.forest) {
    const fid = forestIdFromLive(wm, parentNode);
    const tom = fid ? wm.forest.nodes[fid] : null;
    if (!tom) {
      recordFallback("resetSiblingPercent", "ids-miss");
      return;
    }
    equalizeChildren(wm.forest, tom, { force: true });
    paintWmForest(wm);
    return;
  }
  wm.tree?.resetSiblingPercent?.(parentNode);
}
...
/** @param {import('./window.js').WindowManager} wm @param {any} focusNodeWindow @param {any} partner */
function joinDirTowardSibling(wm, focusNodeWindow, partner) {
  const parent = liveParentForPresent(wm, focusNodeWindow);
  if (!parent || !partner || liveParentForPresent(wm, partner) !== parent) return null;
...
function partnerForJoin(wm, focusNodeWindow) {
  ...
  const sibs = [];
  for (const n of liveChildrenForPresent(wm, parent)) {
...
/** @param {import('./window.js').WindowManager} wm @param {any} focusNodeWindow */
function adjacentSiblingCon(wm, focusNodeWindow) {
  const parent = liveParentForPresent(wm, focusNodeWindow);
...
/** @param {import('./window.js').WindowManager} wm @param {any} focusNodeWindow @param {number} offset */
function inAxisMoveDir(wm, focusNodeWindow, offset) {
  const parent = liveParentForPresent(wm, focusNodeWindow);
  if (!parent) return null;
  const vert = parent.layout === LAYOUT_TYPES.VSPLIT;
...
/** @param {import('./window.js').WindowManager} wm @param {any} focusNodeWindow */
function crossAxisMoveDir(wm, focusNodeWindow) {
  const parent = liveParentForPresent(wm, focusNodeWindow);
...
export class CommandHandler extends GObject.Object {
  ...
  _buildHandlers() {
    ...
    const floatToggle = (action, ctx) => {
      ...
      let existParent = liveParentForPresent(wm, focusNodeWindow);
      // FLOATS members may already be detached from TILES (Forest SoT).
      if (existParent) {
        const tiled = wm.tree.getTiledChildren(liveChildrenForPresent(wm, existParent));
        if (tiled.length <= 1) {
          existParent.percent = 0.0;
          existParent.userSized = false;
          resetSiblingPercent(wm, liveParentForPresent(wm, existParent));
        }
        resetSiblingPercent(wm, existParent);
      }
    ...
    const swapAlongAxis = (action, ctx) => {
      ...
      if (!focusNodeWindow) return;
      const dir = inAxisMoveDir(wm, focusNodeWindow, action.name === "SwapNext" ? 1 : -1);
      if (!dir) return;
    ...
    this._handlers = {
      ...
      WindowMoveIn: (_action, ctx) => {
        ...
        const dir = joinDirTowardSibling(
          wm,
          focusNodeWindow,
          adjacentSiblingCon(wm, focusNodeWindow)
        );
        if (!dir) return;
      ...
      WindowMoveOut: (_action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || focusNodeWindow.isFloat?.()) return;
        const dir = crossAxisMoveDir(wm, focusNodeWindow);
        if (!dir) return;
        runMark2(wm, focusNodeWindow, "move", dir, "move-window");
      },
      ...
      WindowResetSizes: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        const parent = liveParentForPresent(wm, focusNodeWindow);
        if (focusNodeWindow && parent) {
          resetSiblingPercent(wm, parent);
          const grand = liveParentForPresent(wm, parent);
          if (grand) resetSiblingPercent(wm, grand);
      ...
      WindowMergeGroup: (_action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow || focusNodeWindow.isFloat?.()) return;
        const dir = joinDirTowardSibling(wm, focusNodeWindow, partnerForJoin(wm, focusNodeWindow));
        if (!dir) return;
        runMark2(wm, focusNodeWindow, "join", dir, "join");
      },
      ...
      WindowSwapLastActive: (action, ctx) => {
        ...
        if (focusNodeWindow) {
          ...
          if (!forestSwapWindows(wm, lastActiveNodeWindow, focusNodeWindow)) {
            Logger.warn("WindowSwapLastActive: Forest ids miss; fail closed");
            return;
          }
