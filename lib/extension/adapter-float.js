    ...
    (overrides, wmClass, wmId, withWmId) => {
      for (let override of overrides) {
        ...
        if (withWmId) {
          ...
          if (override.wmClass === wmClass && !override.wmId && !override.wmTitle) return overrides;
        }
      }
      overrides.push({
        wmClass: wmClass,
  ...
  if (changed) {
    Logger.debug(
      `float-reason id=${id} class=${cls} ${wasFloat ? "FLOAT" : "TILE"}→${
        ...
        formatFloatFlagTags(decision.flags || {})
    );
...
export function matchesSpecificFloatOverride(wm, metaWindow) {
  return wm.windowProps.overrides.some(
    (kf) => kf.mode === "float" && (kf.wmTitle || kf.wmId) && matchesFloatRule(wm, kf, metaWindow)
  );
}
