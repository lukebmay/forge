 ...
 * @param {string} s already lower/trimmed
 * @returns {boolean}
 */
...
/**
 * Chrome PWA / extension app window class (Wayland often maps these instead of Google-chrome).
 * @param {string} s already lower/trimmed
 * @returns {boolean}
 */
export function isChromePwaClass(s) {
  ...
  const n = String(s).trim().toLowerCase();
  if (!n) return false;
  if (n.startsWith("crx_")) return true;
  // chrome-<appid>-Default
  if (n.startsWith("chrome-") && n.endsWith("-default")) return true;
  return false;
}
...
/**
 * Stable Chrome PWA app id shared by StartupWMClass `crx_<id>` and Meta
 * `chrome-<id>-Default` (same app, different surfaces).
...
export function chromePwaAppId(s) {
  ...
  if (n.startsWith("chrome-") && n.endsWith("-default") && n.length > "chrome--default".length) {
    return n.slice("chrome-".length, -"-default".length);
  }
  return null;
}
