 ...
 * (ghostty ↔ com.mitchellh.ghostty), plus Chrome family ↔ PWA ids
 * (Google-chrome ↔ chrome-*-Default / crx_*).
 * @param {any} a
 * @param {any} b
 * @returns {boolean}
 */
export function wmClassEqual(a, b) {
  ...
  // Chrome browser class matches PWA / other chrome-family ids either side.
  if (isChromeFamilyClass(sa) && isChromeFamilyClass(sb)) return true;
  return false;
}
