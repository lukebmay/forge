 ...
 *
 * Hint shape:
 *   { wmClass?, titleContains?, titleExact?, monitor?, treePath?, attachSelector?,
 *     workspace?, expiresAt, first? }
 *
...
export { isMonRootTreePath, placeNextDestKind, placeNextHasSlotDest };
 ...
 * @returns {PlaceHint[]}
 */
/** Empty a PlaceNext queue. Returns how many were dropped. */
export function clearPlaceHintQueue(queue) {
...
/**
 * True when Meta already exposes class or title for PlaceNext identity match.
 * Wayland Chrome/PWA often maps with both null — provisional FIFO is needed.
 * @param {any} metaLike
 * @returns {boolean}
 */
export function metaHasPlaceIdentity(metaLike) {
  return !!(metaWmClass(metaLike) || metaTitle(metaLike));
}

/**
 * Titles that mean "Chrome is still booting" — not yet decidable for match/mismatch.
 * Re-queue on these caused role thrash + move_to_monitor storms (R036 Shell death).
 * @param {string|null|undefined} title
 * @returns {boolean}
 */
export function isLoadingPlaceTitle(title) {
  if (title == null) return true;
  const t = String(title).trim().toLowerCase();
  if (!t) return true;
  if (t === "new tab" || t === "new tab - google chrome" || t === "new tab - chromium") {
    return true;
  }
  if (t === "about:blank" || t === "untitled" || t === "loading" || t === "loading…") {
    return true;
  }
  if (t === "loading..." || t.startsWith("loading ")) return true;
  return false;
}

/**
 * True when Meta has every field the hint needs to decide match vs mismatch.
 * Incomplete identity must keep a provisional pin (do not re-queue on partial title).
 * Loading titles (New Tab / about:blank) are not ready when the hint requires a title.
 * @param {any} metaLike
 * @param {PlaceHint|null|undefined} hint
 * @returns {boolean}
 */
export function placeHintIdentityReady(metaLike, hint) {
  if (!hint || typeof hint !== "object") return false;
  const wantClass = hint.wmClass != null && String(hint.wmClass).trim() !== "";
  const wantTitle =
    (hint.titleExact != null && String(hint.titleExact).trim() !== "") ||
    (hint.titleContains != null && String(hint.titleContains).trim() !== "");
  if (wantClass && !metaWmClass(metaLike)) return false;
  if (wantTitle) {
    const title = metaTitle(metaLike);
    if (title == null || isLoadingPlaceTitle(title)) return false;
  }
  // Unconstrained hint: any class or title is enough to settle.
  if (!wantClass && !wantTitle) return metaHasPlaceIdentity(metaLike);
  return true;
}

/**
 * Compact one-line PlaceNext hint for journal diagnosis.
 * @param {PlaceHint|null|undefined} hint
 * @returns {string}
 */
export function formatPlaceHint(hint) {
  if (!hint || typeof hint !== "object") return "null";
  const parts = [];
  if (hint.wmClass) parts.push(`class=${hint.wmClass}`);
  if (hint.titleContains) parts.push(`title~=${hint.titleContains}`);
  if (hint.titleExact) parts.push(`title=${hint.titleExact}`);
  if (hint.monitor != null && hint.monitor !== "") parts.push(`mon=${hint.monitor}`);
  if (hint.attachSelector) parts.push(`attach=${hint.attachSelector}`);
  if (hint.treePath) parts.push(`path=${hint.treePath}`);
  if (hint.workspace != null && Number.isFinite(Number(hint.workspace))) {
    parts.push(`ws=${Number(hint.workspace)}`);
  }
  parts.push(`dest=${placeNextDestKind(hint)}`);
  return parts.join(" ") || "{}";
}

/**
 * True when a hint has a slot dest (PH attach / non-mon-root path), not mon-only.
 * @param {PlaceHint|null|undefined} hint
 * @returns {boolean}
 */
export function placeHintHasSlotDest(hint) {
  if (!hint || typeof hint !== "object") return false;
  return placeNextDestKind(hint) === "slot";
}

/**
 * FIFO index of oldest unexpired slot PlaceNext (spawn order).
 * Used when map has null class/title so free-open cannot match by identity.
 * Does not mutate. Returns -1 if none.
 *
 * @param {PlaceHint[]} queue
 * @param {number} [now]
 * @returns {number}
 */
export function findProvisionalPlaceHintIndex(queue, now = Date.now()) {
  if (!queue || !queue.length) return -1;
  for (let i = 0; i < queue.length; i++) {
    const h = queue[i];
    ...
    if (placeHintHasSlotDest(h)) return i;
  }
  return -1;
}

/**
 * Consume oldest slot PlaceNext for a null-identity map. Mutates queue.
 * @param {PlaceHint[]} queue
 * @param {number} [now]
 * @returns {PlaceHint|null}
 */
export function consumeProvisionalPlaceHint(queue, now = Date.now()) {
  pruneExpiredPlaceHints(queue, now);
  const idx = findProvisionalPlaceHintIndex(queue, now);
  if (idx < 0) return null;
  const [hint] = queue.splice(idx, 1);
  return hint || null;
}
...
export function normalizePlaceHint(options, now = Date.now()) {
  ...
  let layoutRole = options.layoutRole ?? options.layout_role ?? options.role ?? null;
  if (layoutRole != null) layoutRole = String(layoutRole).trim() || null;
  let layoutSlot = options.layoutSlot ?? options.layout_slot ?? options.slot ?? null;
  if (layoutSlot != null) layoutSlot = String(layoutSlot).trim() || null;
