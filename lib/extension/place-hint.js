 ...
 * Expired hints never match. When hint.wmClass is set, exact class match
 * (Meta get_wm_class / wmClass / wm_class). No class → any window.
 *
 * @param {any} metaLike
...
export function findMatchingPlaceHintIndex(queue, metaLike, now = Date.now()) {
  ...
  for (let i = queue.length - 1; i >= 0; i--) {
    ...
    if (want) {
      // Specific match — prefer immediately.
      if (got && got === want) return i;
      continue;
    }
