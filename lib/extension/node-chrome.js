...
import { isWindowAlive } from "./utils.js";
...
function deleteMetaFromTab(metaWin) {
  if (!isWindowAlive(metaWin)) return;
  try {
...
export function createWindowTab(node) {
  ...
  let clickFn = () => {
    if (!actorAlive(tabContents)) return;
  ...
  let closeFn = () => {
    if (!actorAlive(tabContents) && !actorAlive(closeButton)) return;
    deleteMetaFromTab(metaWin);
  };

  let middleClickCloseFn = (_, event) => {
    if (!actorAlive(closeButton) && !actorAlive(tabContents)) return;
  ...
  const isCloseControl = (source) => {
    if (!source || !actorAlive(closeButton)) return false;
  ...
  closeButton.connect("button-press-event", (_, event) => {
    if (!actorAlive(closeButton)) return Clutter.EVENT_STOP;
  ...
  tabContents.connect("button-press-event", (_, event) => {
    if (!actorAlive(tabContents)) return Clutter.EVENT_STOP;
...
export function ensureConTab(node) {
  ...
  let clickFn = () => {
    ...
    try {
      ...
      if (isDisposedActorError(e)) {
        tabContents._forgeDisposed = true;
        dropDeadTab(node, "con-tab-click");
        return;
      }
      throw e;
    }
...
/** Kids that may own tab chips. Seeded: Forest-only (no GObject leftovers). */
export function collectChromeKids(node) {
  const wm = resolveLiveWm(node);
