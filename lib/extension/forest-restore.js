...
function forestRebuildNode(wm, descriptor, cohortSet) {
  ...
  // appendChild detach clears userSized + repairs shares — re-apply after.
  const planned = kids.map((k) => ({
    id: k.id,
    percent: k.percent ?? 0,
  ...
  for (const p of planned) {
    const n = forest.nodes[p.id];
    if (!n) continue;
    n.percent = p.percent;
    n.userSized = p.userSized;
  }
...
export function forestApplyMonitorSnapshot(wm, mon, monDesc) {
  ...
  // replaceChildren → repairShares on MONITOR H/V (D032). Re-apply the
  // renormalized shares afterwards (same pattern as forestRebuildNode).
  const planned = rebuilt.map((n) => ({
    id: n.id,
    percent: n.percent ?? 0,
    userSized: !!n.userSized,
  }));
...
function forestRestoreMonitors(wm, snapshot, opts = {}) {
  if (!snapshot?.monitors?.length) return false;
  const forest = ensureLiveForest(wm);
  if (!forest?.nodes) return false;
  let any = false;
...
export function restoreWmForest(wm, snapshot) {
  if (!ensureLiveForest(wm)) return false;
...
export function rehomeWmForestWindows(wm, homes, liveForest) {
  if (!Array.isArray(homes)) return false;
  const forest = ensureLiveForest(wm);
...
/**
 * Meta windows under a Forest MONITOR (bag lookup).
 * @param {any} wm
 * @param {string} monId
 * @returns {any[]}
 */
export function forestWindowMetasUnder(wm, monId) {
  const forest = wm?.forest;
  const mon = forestFindMonitor(wm, monId);
  if (!forest || !mon) return [];
  const out = [];
  for (const n of forestWindowsUnder(forest, mon)) {
    const meta = wm.hostBag?.get?.(n.id)?.meta;
