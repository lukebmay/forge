...
/**
 * Whether `focus` is under `ancestor` (inclusive).
 * @param {object|null|undefined} focus
 * @param {object|null|undefined} ancestor
 * @returns {boolean}
 */
function nodeIsUnder(focus, ancestor) {
  if (!focus || !ancestor) return false;
  if (focus === ancestor) return true;
  if (typeof ancestor.contains === "function") return !!ancestor.contains(focus);
  let p = focus.parentNode;
  while (p) {
...
/**
 * True when attachNode is an elevated CON ops target above the focused leaf
 * (focus-parent selection). Default leaf selection is not elevated.
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode - focused WINDOW node
 * @returns {boolean}
 */
export function isElevatedSelection(attachNode, focusNode) {
  if (!focusNode || !attachNode || attachNode === focusNode) return false;
  if (!isUngroupCon(attachNode)) return false;
  return nodeIsUnder(focusNode, attachNode);
}

/**
 * Ops target for tiling commands (S1): elevated CON when valid, else focus leaf.
 * Queryable surface over tree.attachNode + focus.
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
 */
export function resolveOpsTarget(attachNode, focusNode) {
  if (!focusNode) return null;
  if (isElevatedSelection(attachNode, focusNode)) return attachNode;
  return focusNode;
}

/**
 * Clear elevated selection → focused window leaf (or null).
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
 */
export function clearOpsTarget(focusNode) {
  return focusNode || null;
}

/**
 * Meta focus change selection rule (S0/S1):
 * - different window → reset attach to the new focus leaf
 * - same window re-focus (raise/activate after focus-parent) → keep elevated CON
 *   if it still contains the leaf
 *
 * @param {object|null|undefined} prevFocusNode - previous focused WINDOW node
 * @param {object|null|undefined} newFocusNode - new focused WINDOW node
 * @param {object|null|undefined} currentAttach - tree.attachNode before the change
 * @returns {object|null} next attachNode
 */
export function resolveAttachOnFocusChange(prevFocusNode, newFocusNode, currentAttach) {
  if (!newFocusNode) return null;

  const sameWindow =
    prevFocusNode === newFocusNode ||
    (prevFocusNode?.nodeValue != null &&
      newFocusNode?.nodeValue != null &&
      prevFocusNode.nodeValue === newFocusNode.nodeValue);

  if (sameWindow) {
    if (isElevatedSelection(currentAttach, newFocusNode)) return currentAttach;
    return newFocusNode;
  }

  // Different Meta focus window: always snap ops target to that leaf.
  return newFocusNode;
}
 ...
 *
 * Same surface as resolveOpsTarget for directional move/swap (S2).
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
...
export function resolveMoveUnit(attachNode, focusNode) {
  return resolveOpsTarget(attachNode, focusNode);
}

/**
 * Layout cycle / setLayout target (S2): elevated CON when selected; else the
 * focused window’s parent (existing leaf path).
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
 */
export function resolveLayoutOpsTarget(attachNode, focusNode) {
  if (!focusNode) return null;
  if (isElevatedSelection(attachNode, focusNode)) return attachNode;
  return focusNode.parentNode || null;
}

/**
 * Ungroup target (S2): dissolve elevated CON when selected; else nearest
 * parent CON of focus (I2).
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
 */
export function resolveUngroupOpsTarget(attachNode, focusNode) {
  if (!focusNode) return null;
  if (isElevatedSelection(attachNode, focusNode) && isUngroupCon(attachNode)) {
    return attachNode;
  }
  return resolveUngroupTarget(focusNode);
}

/**
 * Resize unit seed (S2): elevated CON when selected; else `focusNode` for
 * layoutUnit / owning-split walk. Callers still run layoutUnit on the result
 * when they need automatic tab-bag promotion on the leaf path.
 *
 * @param {object|null|undefined} attachNode
 * @param {object|null|undefined} focusNode
 * @returns {object|null}
 */
export function resolveResizeOpsSeed(attachNode, focusNode) {
  return resolveOpsTarget(attachNode, focusNode);
}
