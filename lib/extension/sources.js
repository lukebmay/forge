 ...
 *
 * Logging (dev builds, logging-enabled):
 *  - TRACE: interesting set / replace / cancel / fire (not high-churn names)
 *  - DEBUG: cancelAll / dispose
 *  - WARN on use-after-dispose and callback throws
 * snapshot() is always available for failure dumps (no log gate).
 */
/** High-churn slots — skip routine TRACE (set/fire/cancel/miss). */
const SOURCE_TRACE_QUIET = new Set([
  "queue",
  "renderTree",
  "sessionLayoutSave",
  "wsWindowAdd",
 ...
 * @returns {boolean}
 */
export function sourceTraceInteresting(name) {
  const key = String(name || "");
  if (!key) return false;
  if (SOURCE_TRACE_QUIET.has(key)) return false;
  const base = key.includes(":") ? key.slice(0, key.indexOf(":")) : key;
  if (SOURCE_TRACE_QUIET.has(base)) return false;
  return true;
}
...
export class SourceBag {
  ...
  cancel(name) {
    ...
    if (!slot) {
      // Misses are routine (failsafe / optional cancel) — do not TRACE.
      return false;
    }
    this._slots.delete(key);
    this._safeCancel(slot.id, key, "cancel");
    this._cancelCount += 1;
    if (sourceTraceInteresting(key)) {
      Logger.trace(
  ...
  _arm(key, kind, delayMs, cb) {
    ...
    const interesting = sourceTraceInteresting(key);
    if (prev) {
      this._slots.delete(key);
