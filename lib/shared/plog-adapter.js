...
export function init(s, opts = {}) {
  ...
  // Extension enable: rotate current → *.prev.* then empty for this Shell session.
...
/**
 * Previous-session sibling: `forge.log` → `forge.prev.log`.
 * @param {string} filePath
 * @param {{
 *   pathJoin?: (a: string, b: string) => string,
 *   dirname?: (p: string) => string,
 ...
 * @returns {string}
 */
export function previousSessionPath(filePath, rt = {}) {
  const pathJoin =
    rt.pathJoin || ((a, b) => `${String(a).replace(/\/+$/, "")}/${String(b).replace(/^\/+/, "")}`);
  const dirname =
    rt.dirname ||
  ...
  const dir = dirname(s);
  const i = base.lastIndexOf(".");
  if (i > 0) return pathJoin(dir, `${base.slice(0, i)}.prev${base.slice(i)}`);
  return pathJoin(dir, `${base}.prev`);
}
...
export function getLogStatus() {
  ...
  return {
    ...
    file,
    jsonl,
    prevFile: file ? previousSessionPath(file, rt) : null,
    prevJsonl: jsonl ? previousSessionPath(jsonl, rt) : null,
  };
}
...
/** Rotate current → *.prev.* then empty hunt tapes (same as enable). */
/** @returns {ReturnType<typeof getLogStatus>} */
...
/**
 * Copy non-empty tape to `*.prev.*` before session truncate.
 * Empty/missing current leaves any existing previous tape alone.
 * @param {string} filePath
 */
function rotateTapeToPrevious(filePath) {
  if (!runtime?.exists?.(filePath)) return;
  let content;
  try {
    content = runtime.readFile(filePath);
  } catch {
    return;
  }
  if (content == null || String(content).length === 0) return;
  const prev = previousSessionPath(filePath);
  const tmp = `${prev}.tmp`;
  if (runtime.writeFileAtomic) {
    runtime.writeFileAtomic(prev, String(content), tmp);
    return;
  }
  if (runtime.writeFileInPlace) {
    try {
      runtime.truncateFile?.(prev);
    } catch {
      /* create may fail; writeFileInPlace may still work on some runtimes */
    }
    runtime.writeFileInPlace(prev, String(content));
  }
}
...
function reconfigure(opts = {}) {
  ...
  if (opts.truncateFile && runtime) {
    for (const p of [file, jsonl]) {
      if (!p) continue;
      try {
        rotateTapeToPrevious(p);
      } catch {
        /* best-effort */
      }
      if (runtime.truncateFile) {
        try {
          runtime.truncateFile(p);
        } catch {
          /* best-effort — first write creates */
        }
      }
    }
  }
