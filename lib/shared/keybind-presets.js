...
import { MARK2_TABLE } from "../keybinds/mark2.js";
import { asAccels } from "../keybinds/strip-super.js";
import { KEYBINDING_KEYS } from "./settings-keys.js";

/** Mark 2 action id → vim gsettings key (window-swap-* is the Join surface). */
export const MARK2_VIM_KEYS = Object.freeze({
  ...
  toggleSplit: "con-split-layout-toggle",
  toggleTabStack: "con-stack-tab-layout-toggle",
  "layout.cycle-": "con-layout-cycle-prev",
  "layout.cycle+": "con-layout-cycle-next",
  promote: "window-ungroup",
  promoteRecursive: "window-ungroup-recursive",
  "size.nudge.x-": "size-nudge-x-minus",
  "size.nudge.y-": "size-nudge-y-minus",
  "size.nudge.y+": "size-nudge-y-plus",
  "size.nudge.x+": "size-nudge-x-plus",
  "size.share": "size-share",
  "size.shareSiblings": "size-share-siblings",
  "size.shareSiblingsOnly": "size-share-siblings-only",
  "size.shareSelfSiblingsParent": "size-share-self-siblings-parent",
  "size.shareParent": "size-share-parent",
  "size.shareParentGroup": "size-share-parent-group",
  "size.shareParentSiblingsOnly": "size-share-parent-siblings-only",
  "size.shareBothGroups": "size-share-both-groups",
  "size.shareAll": "size-share-all",
  "size.preset.7": "size-preset-7",
  "size.preset.8": "size-preset-8",
  "size.preset.9": "size-preset-9",
  "size.preset.0": "size-preset-0",
});

/** New Mark 2 gsettings keys: unbound on Safe/i3 (vim overlay fills them). */
const UNBOUND_MARK2_KEYS = Object.freeze({
...
function vimMark2Bindings() {
  /** @type {Record<string, string[]>} */
  const out = {};
  for (const [id, key] of Object.entries(MARK2_VIM_KEYS)) {
    out[key] = asAccels(MARK2_TABLE[id]);
  }
  return out;
}
...
const VIM_BINDINGS = {
  ...
  "window-ungroup": ["<Ctrl><Shift><Super>m"],
  "window-move-in": ["<Shift><Super>comma"],
  "window-move-out": ["<Ctrl><Shift><Super>comma"],
...
const SAFE_BINDINGS = {
  ...
  "window-ungroup": ["<Ctrl><Shift><Super>m"],
  "window-focus-parent": ["<Ctrl><Super>a"],
  "window-focus-child": ["<Ctrl><Shift><Super>a"],
  "window-move-in": ["<Ctrl><Super>comma"],
  "window-move-out": ["<Ctrl><Shift><Super>comma"],
...
const I3_BINDINGS = {
  ...
  "window-merge-group": ["<Shift><Super>m"],
  "window-ungroup": ["<Ctrl><Shift><Super>m"],
  "window-focus-parent": ["<Super>a"],
  "window-focus-child": ["<Shift><Super>a"],
  "window-move-in": ["<Shift><Super>comma"],
  "window-move-out": ["<Ctrl><Shift><Super>comma"],
  ...
  "prefs-config-reload": ["<Shift><Super>r"],
  "window-pointer-to-focus": [],
  // Super+m free (REG-i3-super-m); Wave Z owns zoom via Super+Enter
...
export const RESERVED_KIT_IDS = Object.freeze(["safe", "vim", "i3"]);

/**
 * Safe filename stem: alnum, dash, underscore only.
 * Strips a trailing `.json` if present.
 * @param {string} name
 * @returns {string|null}
 */
...
/**
 * True when name is a built-in kit id (case-insensitive).
 * @param {string} name
 * @returns {boolean}
 */
export function isReservedKitName(name) {
  const stem = sanitizeProfileName(name);
  return stem != null && RESERVED_KIT_IDS.includes(stem.toLowerCase());
}
...
/**
 * JSON shape for saved keybind profiles (GUI Save + CLI `forge keybind save`).
 * Matches Python ``build_profile_props`` / portable keybindings.json (no $schema).
...
/**
 * Snapshot live GSettings into a saveable profile object (GUI/CLI parity).
 * @param {{ get_strv: Function, get_string?: Function }} kbdSettings
 * @param {string} [name] - profile stem stored in JSON
 * @returns {Object}
 */
export function liveProfileProps(kbdSettings, name) {
  const snap = bindingsFromSettings(kbdSettings);
  return buildProfileProps({
