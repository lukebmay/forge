 ...
 * Lock: unbound in kits — GNOME media-keys screensaver is kit-aware (KB0):
 * Safe keeps Super+L + Super+Delete; Vim/i3 Super+Delete only (frees Super+L).
 * Float (Safe/Vim): Ctrl+Super+Space — primary Ctrl+Super family + Space.
 * Float (i3 only): Shift+Super+Space — i3 floating toggle tradition.
 * Always-float: Ctrl+Shift+Super+Space — secondary twin (Ctrl+Shift on Space).
 * Focus border: Ctrl+Super+b — "b"order (rare; multi-mod).
 * Tiling master: Ctrl+Super+e — "e"nable tiling (rare; multi-mod).
 */
const LOCK_SCREEN = [];
const FLOAT_CTRL = ["<Ctrl><Super>space"];
...
const VIM_BINDINGS = {
  ...
  "window-toggle-float": FLOAT_CTRL,
  ...
  "prefs-app-launch": ["<Shift><Super>Return"],
