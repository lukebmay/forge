...
/** allocRoleId suffix: ghostty-2 → ghostty. Not reverse-DNS hyphens. */
export function roleTokenBase(token) {
  const t = String(token ?? "").trim();
  const m = t.match(/^(.*)-(\d+)$/);
  if (!m || !m[1]) return t;
  return m[1];
...
function inferOpenAndMatch(token) {
  token = roleTokenBase(token.trim());
...
export function planReconcile(profile, forestJson, flags) {
  ...
  const needOpenSkeleton = !safe && anyRoleOpen && !hasLayoutPh && !thrashed;
