...
/**
 * Versioned layout profile / plan action JSON bag.
 * Escape: full IR typedef deferred — prefer Record over hand-rolled Meta-sized types.
 * @typedef {Record<string, any>} LayoutJson
 */

/**
 * @typedef {{ op: string, [key: string]: any }} PlanAction
 */
...
const ROLE_CELL_KEYS = new Set(["id", "open", "app", "match", "class", "title", "title~=", "slot"]);

/** Top-level profile keys validate keeps / sugar consumes. */
const PROFILE_TOP_KEYS = new Set([
  "version",
  "mode",
  "roles",
  "layout",
 ...
 * (belong under floating[] via `forge layout save --keep-floats`).
 * Title-scoped float rules (e.g. Firefox About) are omitted.
 */
const FLOAT_OR_IGNORE_CLASSES = new Set([
  "guake",
...
const DUAL_MON_HINT =
  'use [[mon0…],[mon1…]] (e.g. [["inkscape"],[{"hsplit":["ghostty","YouTube"]}]])';
...
/**
 * Split a slot id into monitor head + optional rest path.
 * @param {string|null|undefined} slot
 * @param {Iterable<string>|null} [knownHeads] stableKey / alias heads when not monN|primary
...
function cellToRole(cell, usedIds, where) {
  ...
  const unknownKeys = Object.keys(cell).filter((k) => !ROLE_CELL_KEYS.has(k));
  if (unknownKeys.length) {
    throw new Error(
      `${where}: unknown key(s) ${unknownKeys.map((k) => JSON.stringify(k)).join(", ")} ` +
...
function rawTilesArray(raw) {
  if (Array.isArray(raw)) return raw;
  if (isPlainObject(raw) && Array.isArray(raw.tiles)) return raw.tiles;
  return null;
}
...
function isAllowedProfileTopKey(k) {
  if (PROFILE_TOP_KEYS.has(k)) return true;
  if (typeof k !== "string") return false;
  const ks = k.trim();
...
function preflightUnknownTopKeys(raw) {
  if (!isPlainObject(raw)) return;
  const unknown = Object.keys(raw).filter((k) => !isAllowedProfileTopKey(k));
  if (!unknown.length) return;
  throw new Error(
    `profile unknown key(s): ${unknown.map((k) => JSON.stringify(k)).join(", ")} ` +
      "(normalize would drop them; remove or rename)"
  );
}

function isHorVSplitBody(item) {
  if (!isPlainObject(item)) return false;
  const tag = taggedContainerMode(item);
  if (tag === "hsplit" || tag === "vsplit") return true;
  let modeRaw = item.split;
  if (modeRaw === undefined || modeRaw === null) modeRaw = item.layout;
  if (modeRaw === undefined || modeRaw === null) return false;
  const mode =
    SPLIT_ALIASES[String(modeRaw).trim().toLowerCase()] ?? String(modeRaw).trim().toLowerCase();
  return mode === "hsplit" || mode === "vsplit";
}

function preflightAmbiguousDualMon(raw, opts) {
  const tilesArr = rawTilesArray(raw);
  if (!tilesArr || tilesArr.length < 2) return;
  if (tilesArr.every((x) => Array.isArray(x))) return;

  const monCount = opts.mon_count !== undefined ? opts.mon_count : opts.monCount ?? null;
  let n = null;
  if (monCount != null && Number.isFinite(Number(monCount))) {
    n = Math.trunc(Number(monCount));
  }
  if (n != null && n >= 2 && tilesArr.length === n && tilesArr.every(looksLikeMonBody)) {
    return;
  }
  // Vinyl-style: flat [role, hsplit|vsplit] loads as one mon; dual needs [[…],[…]].
  // Tab/stack beside a role is a normal single-mon pattern — do not warn.
  if (!(tilesArr.some(isRoleCell) && tilesArr.some(isHorVSplitBody))) return;

  const msg =
    "ambiguous dual-mon tiles: flat array loads as one monitor; " +
    `if you meant one desk per head, ${DUAL_MON_HINT}`;
  const strict = opts.strictAmbiguousDualMon === true || opts.strict_ambiguous_dual_mon === true;
  if (strict) throw new Error(msg);
  pushWarning(opts.warnings, msg);
}

function classTokensFromRole(role) {
  const out = [];
  if (!isPlainObject(role)) return out;
  const match = isPlainObject(role.match) ? role.match : {};
  for (const k of ["class", "wmClass", "wm_class"]) {
    if (match[k] != null && String(match[k]).trim()) out.push(String(match[k]).trim());
  }
  const openSpec = isPlainObject(role.open) ? role.open : {};
  for (const k of ["wmClass", "wm_class", "app", "desktop", "command"]) {
    if (openSpec[k] != null && String(openSpec[k]).trim()) {
      out.push(String(openSpec[k]).trim());
    }
  }
  if (role.id != null && String(role.id).trim()) out.push(String(role.id).trim());
  return out;
}

export function isFloatOrIgnoreClassToken(token) {
  const raw = String(token || "").trim();
  if (!raw) return false;
  const cf = casefold(raw);
  if (FLOAT_OR_IGNORE_CLASSES.has(cf)) return true;
  if (cf.includes("guake")) return true;
  const stem = cf.split(/\s+/)[0];
  if (FLOAT_OR_IGNORE_CLASSES.has(stem)) return true;
  const short = stem.includes(".") ? stem.split(".").pop() : stem;
  return Boolean(short && FLOAT_OR_IGNORE_CLASSES.has(short));
}

function preflightFloatTiles(roles, opts) {
  const allow =
    opts.allowFloatTiles === true ||
    opts.allow_float_tiles === true ||
    opts.keepFloats === true ||
    opts.keep_floats === true;
  if (allow || !Array.isArray(roles)) return;
  const hits = [];
  for (const role of roles) {
    if (!isPlainObject(role)) continue;
    for (const tok of classTokensFromRole(role)) {
      if (isFloatOrIgnoreClassToken(tok)) {
        const label = role.id != null ? String(role.id) : tok;
        hits.push(label);
        break;
      }
    }
  ...
  throw new Error(
    `float/ignore-class role(s) in tiles: ${uniq.join(", ")} ` +
 ...
 * ambiguous dual-mon flat tiles (warn; `strictAmbiguousDualMon` refuses).
 *
 * @param {unknown} raw
 * @param {{
 *   mon_count?: number|null,
 *   mon_indices?: number[]|null,
 *   monCount?: number|null,
 *   monIndices?: number[]|null,
 *   warnings?: string[],
 *   strictAmbiguousDualMon?: boolean,
 *   strict_ambiguous_dual_mon?: boolean,
 *   allowFloatTiles?: boolean,
 *   allow_float_tiles?: boolean,
 *   keepFloats?: boolean,
 *   keep_floats?: boolean,
 * }} [opts]
 * @returns {LayoutJson}
 */
export function validateReconcileProfile(raw, opts = {}) {
  const monCount = opts.mon_count !== undefined ? opts.mon_count : opts.monCount ?? null;
  const monIndices = opts.mon_indices !== undefined ? opts.mon_indices : opts.monIndices ?? null;

  preflightUnknownTopKeys(raw);
  preflightAmbiguousDualMon(raw, opts);
  ...
  preflightFloatTiles(data.roles, opts);
...
/** True when forest already has a TABBED/STACKED group (mid-session structure). */
function forestHasTabOrStackGroup(forest) {
  let found = false;
  const walk = (n) => {
    if (!n || found) return;
    const lay = String(n.layout || "")
      .trim()
      .toUpperCase();
    if (lay === "TABBED" || lay === "STACKED") {
      found = true;
      return;
    }
  ...
  for (const m of iterForestMonitors(forest)) walk(m);
  return found;
}
...
function windowWorkspaceIndex(w) {
  if (!isPlainObject(w)) return null;
  if (isInt(w.workspace)) return w.workspace | 0;
  const path = w.path != null ? String(w.path) : "";
  const pm = /mo\d+ws(\d+)/.exec(path);
  if (pm) return parseInt(pm[1], 10);
  return null;
}

function filterExtraWindowsByWorkspace(list, ws) {
  if (!Array.isArray(list)) return list;
  return list.filter((w) => {
    if (!isPlainObject(w)) return false;
    const ows = windowWorkspaceIndex(w);
    // Fail-closed: unknown-ws extras must not leak into a filtered desk.
    return ows === ws;
  });
}

export function filterForestWorkspace(forest, workspace = 0) {
  ...
  const out = Object.assign({}, forest, { monitors: kept });
  if (Array.isArray(forest.orphanWindows)) {
    out.orphanWindows = filterExtraWindowsByWorkspace(forest.orphanWindows, ws);
  }
  if (Array.isArray(forest.metaWindows)) {
    out.metaWindows = filterExtraWindowsByWorkspace(forest.metaWindows, ws);
  }
  return out;
}
...
/**
 * Role token → window (bind tag, classEq, title~= via inferOpenAndMatch).
 * @param {object} win
 * @param {unknown} token
 * @returns {boolean}
 */
export function windowMatchesRoleToken(win, token) {
  if (!win || typeof win !== "object") return false;
  const t = String(token ?? "").trim();
  if (!t || t === "_slot") return false;
  const tCf = casefold(t);
  const tagged = win.layoutRole != null ? win.layoutRole : win.layout_role;
  if (tagged != null && casefold(String(tagged).trim()) === tCf) return true;
  const inferred = inferOpenAndMatch(t);
  const match = inferred && inferred.match;
  if (match && typeof match === "object" && windowMatches(win, match)) return true;
  // Role ids sanitize reverse-DNS (org.gnome.Foo → org-gnome-Foo).
  if (t.includes("-")) {
    const dotted = t.replace(/-/g, ".");
    if (dotted !== t && windowMatches(win, { class: dotted })) return true;
  }
  return false;
}
...
/**
 * Flatten forest WINDOW nodes (optionally one workspace).
 * @param {unknown} forest
 * @param {{ workspace?: number|null }} [opts]
 * @returns {LayoutJson[]}
 */
export function collectWindows(forest, opts) {
  ...
  function walk(n, path, monIdx) {
    ...
    if (ntype === "WINDOW") {
      ...
      if (n.metaWindowId != null && n.metaWindowId !== "") w.metaWindowId = n.metaWindowId;
  ...
  if (isPlainObject(forest)) {
    const bag = /** @type {LayoutJson} */ (forest);
    const mons = bag.monitors;
  ...
  if (isPlainObject(forest)) {
    ...
    for (const w of out) {
      if (w.metaWindowId != null && String(w.metaWindowId).trim() !== "") {
        seen.add(String(w.metaWindowId));
      }
    }
    const wantWs =
      workspace !== undefined && workspace != null ? normalizeWorkspace(workspace) : null;
    for (const extra of [...(bag.orphanWindows || []), ...(bag.metaWindows || [])]) {
      ...
      if (wantWs != null) {
        const ows = windowWorkspaceIndex(extra);
        if (ows !== wantWs) continue;
      }
      seen.add(String(wid));
      const row = {
        windowId: wid,
      ...
      if (isInt(extra.workspace)) row.workspace = extra.workspace;
      out.push(row);
    }
  }
  return out;
...
function isMax1SplitWrap(mon) {
  const kids = mon && Array.isArray(mon.children) ? mon.children : [];
  if (kids.length !== 1) return false;
  const c = kids[0];
  if (!isPlainObject(c)) return false;
  const ntype = c.nodeType || c.type;
  if (ntype !== "CON") return false;
  const lay = String(c.layout || "")
    .trim()
    .toUpperCase();
  return lay === "HSPLIT" || lay === "VSPLIT";
}

function forestMonByKey(forest, monKey) {
  const want = String(monKey || "");
  if (!want) return null;
  for (const m of iterForestMonitors(forest)) {
    if (!isPlainObject(m)) continue;
    const idx = monitorNodeIndex(m);
    if (idx != null && "mon" + idx === want) return m;
  }
  return null;
}

function peeledMonChildLoc(path, mon) {
  const loc = monChildLoc(path);
  if (!loc || !isMax1SplitWrap(mon)) return loc;
  const parts = String(path).split("/");
  if (parts.length < 3) return loc;
  const gidx = Number.parseInt(parts[2], 10);
  if (!Number.isFinite(gidx)) return loc;
  return [loc[0], gidx];
}
...
function bagConChildMismatches(forest) {
  const mismatches = [];
  function walk(n, monKey) {
    if (!isPlainObject(n)) return;
    const ntype = n.nodeType || n.type;
    const lay = String(n.layout || "")
      .trim()
    ...
    let nextMon = monKey;
    if (ntype === "MONITOR") {
      const idx = monitorNodeIndex(n);
      nextMon = idx != null ? "mon" + idx : monKey;
    }
    if (ntype === "CON" && (lay === "TABBED" || lay === "STACKED")) {
      for (const c of n.children || []) {
        if (!isPlainObject(c)) continue;
        const ct = c.nodeType || c.type;
        if (ct === "CON") {
          mismatches.push({
            kind: "bag-con-child",
            slot: nextMon || "mon",
            want: "WINDOW",
            got: String(c.layout || "CON"),
            detail: lay + " bag has CON child",
          });
        }
      }
    }
    for (const c of n.children || []) walk(c, nextMon);
  }
  if (isPlainObject(forest)) {
    for (const m of forest.monitors || []) walk(m, null);
  }
  return mismatches;
}

function monChildTopologyMismatches(roleResults, parentInfo, prof, forest) {
  const mismatches = [];
  ...
  for (const monKey of monKeys) {
    ...
    const children = monBody.children || [];
    if (!Array.isArray(children) || children.length < 2) continue;
    const liveMon = forestMonByKey(forest, monKey);
    const byChild = {};
    for (const r of roleResults) {
      if (r.windowId == null) continue;
      const slot = String(r.slot || "");
    ...
    for (const [childId, roles] of Object.entries(byChild)) {
      for (const r of roles) {
        ...
        const loc = peeledMonChildLoc(path, liveMon);
        if (loc != null) {
...
function compareLayoutStructure(forest, profile, opts) {
  ...
  mismatches.push(...monChildTopologyMismatches(roleResults, pinfo, prof, forest));
  mismatches.push(...bagConChildMismatches(forest));
 ...
 * @param {unknown} forestJson
 * @param {LayoutJson} [flags]
 * @returns {LayoutJson}
 */
export function planReconcile(profile, forestJson, flags) {
  ...
  const anyRoleOpen = roleResults.some((r) => String(r.status || "") === "open");
  ...
  // Not for Mode B thrash.
  const needOpenSkeleton =
    !safe && anyRoleOpen && !hasLayoutPh && !thrashed && !forestHasTabOrStackGroup(forestJson);
  ...
  // Skeleton owns topology while opening into PH (cold empty or partial opens).
  ...
  const skipWindowStructure = coldEmpty || needOpenSkeleton;
  ...
  if ((coldEmpty || needOpenSkeleton) && !safe) {
 ...
 *
 * @param {PlanAction[]|unknown} actions
 * @param {LayoutJson} [opts]
 * @returns {PlanAction[]}
 */
