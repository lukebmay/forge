...
/**
 * Argv spawn when DesktopAppInfo.launch would ignore flags (Ghostty GTK
 * single-instance) or the app is a filesystem path. Else null → desktop.
 * @param {string} app
 * @param {{ desktop?: string|null, exePath?: string|null }} [opts]
 * @returns {string[]|null}
 */
export function argvForApplyLaunch(app, opts = {}) {
  const raw = String(app ?? "").trim();
  if (!raw) return null;
  if (isGhosttyLaunchTarget(raw)) return ghosttyMultiInstanceArgv(raw, opts);
  if (isPathLikeLaunchApp(raw)) {
    const parts = shellSplit(raw);
...
export function openActionToLaunchFields(action, opts = {}) {
  ...
  if (a.role != null && String(a.role).trim() !== "") fields.role = String(a.role).trim();
  if (slot != null && String(slot).trim() !== "") fields.slot = String(slot).trim();
  ...
  if (!fields.wm_class) {
    const mc = oaMatch.class || oaMatch.wmClass || oaMatch.wm_class;
    if (mc != null && String(mc).trim() !== "") fields.wm_class = String(mc).trim();
  }
  if (!fields.wm_class && isGhosttyLaunchTarget(appS)) {
    fields.wm_class = "com.mitchellh.ghostty";
  }
  return fields;
}
...
export function placeNextOptionsFromLaunchFields(fields) {
  ...
  const ws = f.workspace != null ? f.workspace : f.ws;
  if (ws != null && ws !== "" && Number.isFinite(Number(ws)) && Number(ws) >= 0) {
    out.workspace = Math.floor(Number(ws));
  }
  ...
  const role = f.layoutRole || f.layout_role || f.role;
  if (role != null && String(role).trim() !== "") out.layoutRole = String(role).trim();
  const slot = f.layoutSlot || f.layout_slot || f.slot;
  if (slot != null && String(slot).trim() !== "") out.layoutSlot = String(slot).trim();
  return out;
}
...
export function applyPlaceNextOptions(action, fields, forest) {
  ...
  const role = action?.role ?? action?.layoutRole;
  const slot = action?.slot ?? action?.layoutSlot;
  if (role != null && String(role).trim() !== "" && !placeOpts.layoutRole) {
    placeOpts.layoutRole = String(role).trim();
  }
  if (slot != null && String(slot).trim() !== "" && !placeOpts.layoutSlot) {
    placeOpts.layoutSlot = String(slot).trim();
  }
  const dest = findLayoutSlotDest(forest, { role, slot });
