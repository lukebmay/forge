...
/**
 * Bare mon path for PlaceNext (moNwsW). Slot PH pin is attachSelector (R036).
 * @param {string} slot
...
/**
 * Find a layout placeholder windowId in a GetTree-shaped forest.
...
export function findLayoutPlaceholderId(forest, want = {}) {
  ...
  const mons = Array.isArray(forest?.monitors)
    ...
    ? forest
    : [];
  /** @type {{ id: string, role: string|null, slot: string|null }[]} */
  const hits = [];
  const walk = (n) => {
    if (!n || typeof n !== "object") return;
    if (n.nodeType === "WINDOW" || n.nodeType === "window") {
      const isPh =
        n.placeholder === true ||
      ...
      if (isPh) {
        ...
        if (id) hits.push({ id, role: r, slot: s });
      }
    }
...
export function openActionIsChromeFamily(action) {
  ...
  const openSpec =
    action.open && typeof action.open === "object" && !Array.isArray(action.open)
      ? action.open
      : {};
...
export function placeNextHasDest(opts) {
  ...
  if (opts.monitor != null && String(opts.monitor).trim() !== "") return true;
  if (opts.treePath != null && String(opts.treePath).trim() !== "") return true;
  if (opts.attachSelector != null && String(opts.attachSelector).trim() !== "") return true;
  return false;
}
