...
export class ThemeManagerBase extends GObject.Object {
  ...
  setCssProperty(selector, propertyName, propertyValue) {
    ...
    if (cssProperty && cssProperty.value !== undefined) {
      // Idempotent: if effective value already matches, do not rewrite or bump css-updated.
      if (cssProperty.value === propertyValue) {
        return true;
      }
      // Always persist on the user AST (base is read-only). Color-only user files get
      // the rule/decl added here so Appearance still works without a full fork.
   ...
   * Never writes the bundled install sheet.
   */
  _updateCss() {
    if (!this.cssAst) {
      return;
