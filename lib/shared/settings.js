...
import { sanitizeProfileName } from "./keybind-presets.js";
...
export class ConfigManager extends GObject.Object {
  ...
  /** Named profiles: ~/.config/forge/config/keybinding-profiles/<name>.json */
  ...
  listKeybindingProfiles() {
    ...
    try {
      ...
      while ((info = enumerator.next_file(null)) !== null) {
        ...
        if (sanitizeProfileName(stem)) names.push(stem);
      }
  ...
  saveKeybindingProfile(name, props) {
    ...
    try {
      this._saveJsonConfig(file, props, `keybinding profile ${safe}`);
