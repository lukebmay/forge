...
function showAboutWindow(parent, { "version-name": versionName, description: comments }) {
  ...
  abt.add_link(_("Documentation"), "https://github.com/lukebmay/forge/tree/master/docs");
  abt.present();
}
  ...
  constructor({ settings, aboutButton }) {
    ...
    this.add_group({
      ...
      children: [
        ...
        new SwitchRow({
          title: _("Tiny-pane tab fallback"),
          subtitle: _(
            "When quarter tiling would make panes too small, open as tabs instead (opt-in)"
          ),
          experimental: true,
          settings,
          bind: "tiny-pane-tab-fallback-enabled",
        }),
        new SwitchRow({
        ...
        new SwitchRow({
          ...
          subtitle: _("Allow stacked (i3-style) groups; tabbed remains the default group type"),
          settings,
          bind: "stacked-tiling-mode-enabled",
        }),
        new SwitchRow({
          title: _("Tabbed tiling"),
          subtitle: _("Allow tabbed groups (default for DnD center and layout sugar)"),
          settings,
        ...
        new DropDownRow({
          ...
          subtitle: _(
            "Generic opens follow the last focused tile’s monitor (default). Window’s own monitor respects app-restored geometry. Dock launches always stick to the dock’s monitor when detected."
          ),
          settings,
          type: "s",
          bind: "new-window-placement",
          items: [
            { id: "pointer", name: _("Last focused tile (default)") },
            { id: "window-actual", name: _("Window's own monitor") },
          ],
        }),
        new DropDownRow({
          title: _("New window size"),
          subtitle: _(
            "After you resize tiles: keep their proportions or re-equalize when a new window opens"
          ),
          settings,
          type: "s",
          bind: "new-window-size-policy",
          items: [
            { id: "preserve", name: _("Preserve resized proportions") },
            { id: "equalize", name: _("Equalize all tiles") },
          ],
        }),
        new DropDownRow({
    ...
    this.add_group({
      ...
      description: _(
        "Empty opens GNOME Run a Command (Alt+F2). Or set a command (ghostty, rofi, …)."
      ),
      children: [
        new EntryRow({
          title: _("Command (empty = run dialog)"),
          settings,
    ...
    this.add_group({
      title: _("Debugging"),
      description: _("Layout overlay for diagnosing tile ratios and monitor homes"),
      children: [
        new SwitchRow({
          title: _("Layout debug overlay"),
          subtitle: _("Show parent layout, percent, and monitor id on each tiled window"),
          settings,
          bind: "layout-debug-overlay-enabled",
        }),
        new SwitchRow({
          title: _("Layout apply chrome"),
          subtitle: _(
            "Soft dim during multi-open layout apply. On by default; clears after residual place; hard-clears within 30s"
          ),
          settings,
          bind: "layout-apply-chrome-enabled",
        }),
      ],
    });
    if (!production) {
