...
export class AppearancePage extends PreferencesPage {
  ...
  static getCssSelectorAsMessage(selector) {
    switch (selector) {
      ...
      case ".window-zoomed-border":
        return _("Zoomed window");
  ...
  constructor({ settings, dir }) {
    ...
    this.add_group({
      ...
      children: [
        ...
        new SwitchRow({
          title: _("Show all split chrome"),
          subtitle: _("Draw H/V indicators on every split (not only focus ancestry)"),
          settings,
          bind: "split-chrome-show-all",
        }),
        new SpinButtonRow({
        ...
        new SpinButtonRow({
          title: _("Max tabs per line"),
          subtitle: _("Wrap tab labels after N (0 = single row, unlimited)"),
          range: [0, 50, 1],
          settings,
          bind: "max-tabs-per-line",
        }),
        new SpinButtonRow({
          title: _("Min tab label characters"),
          subtitle: _("Width-wrap when filled tabs would show fewer (0 = width wrap off)"),
          range: [0, 80, 1],
          settings,
          bind: "min-tab-label-chars",
        }),
        new SpinButtonRow({
          title: _("Max tab rows"),
          subtitle: _("Hard row cap for tabbed bars (0 = unbounded)"),
          range: [0, 10, 1],
          settings,
          bind: "max-tab-rows",
        }),
  ...
  _createColorOptionWidget(prefix) {
    ...
    borderSizeRow.add_suffix(
      new ResetButton({
        onReset: () => {
          // Clear user override so bundled base cascade shows (delta sheet).
          theme.removeCssProperty(selector, "border-width");
          const borderDefault = theme.removePx(
    ...
    borderColorRow.add_suffix(
      new ResetButton({
        onReset: () => {
          // Remove overrides (not write defaults) so base colors cascade.
          theme.removeCssProperty(selector, "border-color");
          if (colorScheme === "tabbed") {
            theme.removeCssProperty(`.window-${colorScheme}-tab`, "border-color");
            theme.removeCssProperty(`.window-${colorScheme}-tab-active`, "background-color");
          }
          theme.removeCssProperty(`.window-tilepreview-${colorScheme}`, "border-color");
          theme.removeCssProperty(`.window-tilepreview-${colorScheme}`, "background-color");
          const selectorColor = theme.getCssProperty(selector, "border-color").value;
          const rgba = new Gdk.RGBA();
          if (selectorColor && rgba.parse(selectorColor)) {
