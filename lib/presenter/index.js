...
import { ancestorMonitor, hasWindowDescendant } from "../tom/queries.js";
...
export function paneRect(f, node) {
  ...
  for (const child of chain) {
    // MONITOR with HSPLIT/VSPLIT splits like CON (layout profiles use mon-direct
    // kids). Only bag layouts fill; a non-split MONITOR also fills (sole child).
    const axis = splitAxis(p);
    const fill = isBagLayout(p) || (p.kind === "MONITOR" && !axis);
    if (fill) {
      p = child;
      continue;
    }
    const kids = children(f, p).filter((k) => k.kind !== "CON" || hasWindowDescendant(f, k));
    if (!axis || !kids.length) {
      p = child;
      continue;
    }
    // Sole child fills (common MONITOR → one HSPLIT CON wrap; unset percent).
    if (kids.length === 1) {
      p = child;
      continue;
    }
