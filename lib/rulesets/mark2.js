...
import { appendChild } from "../tom/atomics.js";
import { children, makeCon, makeIdFactory, registerTree, walk } from "../tom/kernel.js";
import { promoteChildren, wrapNodes } from "../tom/composed.js";
...
export function coerceSameTypeUnder(f, root, aspectTieBreak = "HSPLIT") {
  ...
  walk(f, root, (node) => {
    ...
    for (const ch of children(f, node)) {
      ...
      if (
        node.layout === ch.layout &&
        (ch.layout === "HSPLIT" || ch.layout === "VSPLIT") &&
        children(f, ch).some((k) => k.kind === "CON")
      ) {
        flatten.push(ch);
        continue;
      }
  ...
  for (const ch of flatten) {
    if (!f.nodes[ch.id]) continue;
    if (promoteChildren(f, ch).ok) n++;
  }
  return n;
}
...
export function wrapMonitorMax1(f, root) {
  ...
  for (const mon of mons) {
    ...
    const cons = kids.filter((k) => k.kind === "CON");
    const wins = kids.filter((k) => k.kind === "WINDOW");
    const host = cons.length === 1 ? cons[0] : null;
    const hostLay = host?.layout || "HSPLIT";
    const hostSplit = hostLay === "HSPLIT" || hostLay === "VSPLIT";
    if (
      host &&
      hostSplit &&
      hostLay === layout &&
      wins.length &&
      cons.length + wins.length === kids.length
    ) {
      for (const w of wins) appendChild(f, host, w);
      wrapped++;
      continue;
    }
