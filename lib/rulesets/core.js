...
import { children, parent, walk } from "../tom/kernel.js";
...
export function pruneEmptyCons(f, root) {
  ...
  let guard = 0;
  while (guard++ < 64) {
    scrubDanglingChildIds(f);
    /** @type {Node[]} */
    const empties = [];
    const seen = new Set();
    walk(f, root, (n) => {
      if (n.kind === "CON" && children(f, n).length === 0) {
        empties.push(n);
        seen.add(n.id);
      }
    });
    for (const n of Object.values(f.nodes)) {
      if (n.kind !== "CON" || seen.has(n.id)) continue;
      if (children(f, n).length === 0) empties.push(n);
    }
    if (!empties.length) break;
    empties.sort((a, b) => depthOf(f, b) - depthOf(f, a));
    for (const con of empties) {
      if (!f.nodes[con.id] || children(f, con).length !== 0) continue;
      ...
      if (f.selectionId === con.id) f.selectionId = p?.id ?? null;
      pruned++;
    }
  }
  return pruned;
...
/** Drop childIds whose nodes are gone so empty/unary settle can see them. */
function scrubDanglingChildIds(f) {
  for (const n of Object.values(f.nodes)) {
    if (!Array.isArray(n.childIds) || !n.childIds.length) continue;
    const next = n.childIds.filter((id) => !!f.nodes[id]);
    if (next.length !== n.childIds.length) n.childIds = next;
  }
}
