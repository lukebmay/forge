...
describe("keybind kits", () => {
  ...
  describe("shared low-frequency chords", () => {
    ...
    it("shared rare chords; float is Ctrl-primary except i3", () => {
      for (const id of ["safe", "vim", "i3"]) {
  ...
  describe("vim and i3 kits", () => {
    it("vim has Super+h on focus-left and Ctrl+Super+Space float", () => {
      expect(KITS.vim.bindings["window-focus-left"]).toContain("<Super>h");
      expect(KITS.vim.bindings["window-toggle-float"]).toEqual(["<Ctrl><Super>space"]);
      expect(kitUsesBareSuper(KITS.vim)).toBe(true);
    });

    it("i3 has Super+hjkl focus and Shift+Super+Space float", () => {
