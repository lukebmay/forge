 ...
 * Prototype-only until an adopt/abandon lock. Uses TreeOps; does not splice.
 */

import {
  isAtMonitorEdge,
  neighborMonitor,
  transferLeafToMonitor,
  monitorsSiblingAxis,
} from "../monitors.mjs";
import { motionLog } from "../plog.mjs";
import {
  ancestorMonitor,
  appendChild,
...
/**
 * @param {Forest} f
 * @returns {import('../tom/kernel.mjs').Decisions}
 */
...
export function preferredSplitVsParent(parentLayout, f) {
  ...
  return d.aspectTieBreak === "VSPLIT" ? "VSPLIT" : "HSPLIT";
}
...
export function coerceSameTypeUnder(f, root) {
  ...
  walk(f, root, (node) => {
    ...
    for (const ch of children(f, node)) {
      ...
      const next = coerceDifferentType(node.layout, ch.layout, f, {
...
/**
 * Mark 2 settle: prune empty, collapse unary, coerce same-type. Repeat.
 * @param {Forest} f
 * @param {Node} root
 */
...
export function mark2CleanupForest(f) {
  ...
  for (const mon of f.monitors) {
    const r = mark2CleanupUnder(f, mon);
    pruned += r.pruned;
...
export function mark2Move(f, api, dir) {
  ...
  if (isInAxis(p, dir)) {
    ...
    if (sib) {
      ...
      keepLeafFocus(f, cur);
      motionLog.debug("mark2-move swap", { leaf, dir, parent: p.layout });
      return ok("Move", { dir, mode: "swap" });
    }
    const edge = f.decisions.edgeMove || "wrap";
    if (edge === "noop") {
      motionLog.debug("mark2-move edge-noop", { leaf, dir });
    ...
    if (edge === "wrap") {
      if (p.childIds.length >= 2) {
        ...
        keepLeafFocus(f, cur);
        motionLog.debug("mark2-move wrap", { leaf, dir, parent: p.layout });
        return ok("Move", { dir, mode: "wrap" });
      }
    }
  }

  if (isAtMonitorEdge(f, api, cur, dir)) {
    const mon = ancestorMonitor(f, cur);
    const dest = mon ? neighborMonitor(f, mon, dir) : null;
    if (dest) {
      const oldMon = mon;
      const r = transferLeafToMonitor(f, api, cur, dest, dir, { join: false });
      if (!r.ok) return r;
      if (oldMon && f.nodes[oldMon.id]) mark2CleanupUnder(f, oldMon);
      if (f.nodes[dest.id]) mark2CleanupUnder(f, dest);
      keepLeafFocus(f, cur);
      motionLog.debug("mark2-move cross-mon", { leaf, dir, to: dest.id });
      return {
        ...r,
        ok: true,
        op: "Move",
        dir,
        mode: "cross-mon",
        siblingAxis: monitorsSiblingAxis(f),
      };
    }
  }
  ...
  if (!br.ok) {
    motionLog.debug("mark2-move fail", { leaf, dir, reason: br.reason });
    return br;
  }
  const mon = ancestorMonitor(f, cur);
  if (mon) mark2CleanupUnder(f, mon);
  keepLeafFocus(f, cur);
  motionLog.debug("mark2-move breakout", { leaf, dir });
...
export function mark2Join(f, api, dir) {
  ...
  if (isAtMonitorEdge(f, api, cur, dir)) {
    ...
    if (dest) {
      const oldMon = mon;
      const r = transferLeafToMonitor(f, api, cur, dest, dir, { join: true });
      if (!r.ok) return r;
      if (oldMon && f.nodes[oldMon.id]) mark2CleanupUnder(f, oldMon);
      if (f.nodes[dest.id]) mark2CleanupUnder(f, dest);
      keepLeafFocus(f, cur);
      motionLog.debug("mark2-join cross-mon", { leaf, dir, to: dest.id });
      return {
        ...r,
        ok: true,
        op: "Join",
  ...
  if (pairSib && cannotBreakoutLeaf(f, cur)) {
    ...
    if (!r.ok) return r;
    keepLeafFocus(f, cur);
    motionLog.debug("mark2-join wrap-pair", { leaf, dir, why: "no-breakout" });
  ...
  if (inAxis) {
    ...
    if (sib) {
      if (sib.kind === "WINDOW") {
        ...
        if (!r.ok) return r;
        keepLeafFocus(f, cur);
        motionLog.debug("mark2-join wrap-pair", { leaf, dir, why: "toward-window" });
        return { ok: true, op: "Join", dir, mode: "wrap-pair", ...r };
      }
      if (sib.kind === "CON") {
        ...
        if (!r.ok) return r;
        keepLeafFocus(f, cur);
        motionLog.debug(`mark2-join ${r.mode}`, { leaf, dir });
        return { ok: true, op: "Join", dir, ...r };
      }
    }
    const br = breakoutLeaf(f, api, cur, dir);
    if (!br.ok) {
      motionLog.debug("mark2-join fail", { leaf, dir, reason: br.reason });
      return br;
    }
  } else {
    const br = breakoutLeaf(f, api, cur, dir);
  ...
  if (!sib) {
    keepLeafFocus(f, cur);
    motionLog.debug("mark2-join breakout-only", { leaf, dir });
  ...
  if (sib.kind === "WINDOW") {
    ...
    if (!r.ok) return r;
    keepLeafFocus(f, cur);
    motionLog.debug("mark2-join breakout-wrap", { leaf, dir });
  ...
  if (sib.kind === "CON") {
    ...
    motionLog.debug(`mark2-join ${mode}`, { leaf, dir });
...
function launchNextTo(f, api, slot, win) {
  ...
  if (p.layout === "HSPLIT") {
    ...
    if (wider) {
      if (extraFloaterWouldViolate(f, p)) {
...
function launchAtEnd(f, api, last, win) {
  ...
  if ((p.layout === "HSPLIT" || p.layout === "VSPLIT") && extraFloaterWouldViolate(f, p)) {
...
function pickMonitorWrapLayout(f, slot) {
  const rect = paneRect(f, slot);
  /** @type {import('../tom/kernel.mjs').Layout} */
  let layout = f.decisions?.aspectTieBreak || "HSPLIT";
  if (rect) {
 ...
 * @param {Node} slot
 * @param {Node} win
 * @param {import('../tom/kernel.mjs').Layout} layout
 */
...
function insertAsLastChild(f, api, slot, win) {
  if (extraFloaterWouldViolate(f, slot)) {
    return wrapSlotWithNew(f, api, slot, win, "TABBED");
  }
  appendChild(f, slot, win);
  win.userSized = false;
  const r = redistributeFloaters(f, slot);
  if (!r.ok) return r;
...
function insertAsNextSibling(f, api, slot, win) {
  ...
  if (p.layout === "HSPLIT" || p.layout === "VSPLIT") {
    const r = redistributeFloaters(f, p);
    if (!r.ok) return r;
  } else {
