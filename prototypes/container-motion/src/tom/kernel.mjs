 ...
 * No tiling policy. No monitor geometry. No keybinds.
 */
...
export function makeIdFactory(start = 1) {
  ...
  return {
    ...
    hydrate(f) {
      ...
      f._seq = seq;
      if (!f.decisions) f.decisions = defaultDecisions();
      else f.decisions = { ...defaultDecisions(), ...f.decisions };
    },
  };
}
...
export function applyForestSnapshot(live, draft, api) {
  ...
  live.mergeTags = draft.mergeTags ? [...draft.mergeTags] : [];
  live.decisions = draft.decisions ? { ...draft.decisions } : live.decisions;
