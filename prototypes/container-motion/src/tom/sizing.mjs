// @ts-check
/**
 * In-axis shares on HSPLIT/VSPLIT children. Cross-axis size is the parent's
 * share in its split. Float = not userSized; remaining space splits equally.
 */

import { children, fail, ok, parent, selectionNode } from "./kernel.mjs";
import { ancestorMonitor } from "./queries.mjs";
...
/**
 * Pane rect from MONITOR geom + in-axis percents. TAB/STACK fill the slot.
 * @param {Forest} f
 * @param {Node} node
...
/**
 * True if adding one floater to this H/V would put a floater below 10%.
 * @param {Forest} f
 * @param {Node} parentNode
 */
export function extraFloaterWouldViolate(f, parentNode) {
  ...
  const floats = kids.filter((k) => !k.userSized);
  let used = 0;
  for (const k of sized) used += k.percent ?? 0;
  const remain = 1 - used;
  const nFloat = floats.length + 1;
  if (nFloat <= 0) return true;
  return remain / nFloat + EPS < SIZE_MIN;
}
 ...
 * @param {Forest} f
 * @param {Node} slot
 * @param {import('./kernel.mjs').Layout} wrapLayout
 */
 ...
 * @param {Forest} f
 * @param {Node} parentNode
 * @param {{ rescaleIfNoFloat?: boolean }} [opts]
 */
export function redistributeFloaters(f, parentNode, opts = {}) {
  const kids = children(f, parentNode);
  ...
  let used = 0;
  for (const k of sized) used += k.percent ?? 0;
  if (used > SIZE_MAX + EPS) return fail("sized shares exceed 100%");
  const remain = 1 - used;
  if (!floats.length) {
    if (opts.rescaleIfNoFloat) return rescaleSizedToFill(f, parentNode);
    if (Math.abs(remain) > EPS) return fail("leftover with no floaters");
    return ok("redistributeFloaters", { id: parentNode.id, n: 0 });
  }
  const share = remain / floats.length;
  if (share + EPS < SIZE_MIN) return fail("floater below 10%");
  for (const k of floats) k.percent = share;
  return ok("redistributeFloaters", { id: parentNode.id, n: floats.length, share });
}
...
/**
 * After a child is added or removed from an H/V split: give leftover to
 * floaters, or rescale sized kids so they fill 100%.
 * @param {Forest} f
 * @param {Node} parentNode
 */
export function repairSharesAfterChildChange(f, parentNode) {
  ...
  const r = redistributeFloaters(f, parentNode, { rescaleIfNoFloat: true });
...
export function setChildShare(f, parentNode, target, share) {
  ...
  const r = redistributeFloaters(f, parentNode);
  if (!r.ok) {
    for (const s of snap) {
 ...
 * @param {Node} [node]
 */
export function floatCombo(f, spec, node) {
  const start = node || selectionNode(f);
  if (!start || start.kind === "ROOT" || start.kind === "WORKSPACE") {
    return fail("bad selection");
  }
  ...
  if (inAx) {
    ...
    const r = redistributeFloaters(f, inAx.parent);
    if (!r.ok) {
      restore();
      return r;
    }
  ...
  if (cross && (wantParent || wantParentSib)) {
    ...
    const r = redistributeFloaters(f, cross.parent);
    if (!r.ok) {
      restore();
      return r;
    }
 ...
 * @param {Node} [node]
 */
export function floatSize(f, node) {
  return floatCombo(f, { self: true }, node);
}
 ...
 * @param {Node} [node]
 */
export function floatSiblingSizes(f, node) {
  return floatCombo(f, { self: true, siblings: true }, node);
}
 ...
 * @param {Forest} f
 */
export function floatAllSizes(f) {
  /** @type {Node[]} */
  const splits = [];
  ...
  return ok("floatAllSizes", { splits: splits.length });
}
