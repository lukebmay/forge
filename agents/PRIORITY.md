...
**Workon WR8 shipped** (stableKey / alias mon keys). Remaining polish is
**WR9** (env) or **regression watch** if thrash returns —
plan [forge-workon-reconcile.md](./plans/forge-workon-reconcile.md).

| Order | ID | Task | Why first |
| ---: | --- | --- | --- |
| 1 | **WR9** | shellrc `FORGE_WORKON_DIR` env snippet | Out of critical path |
| — | residual | mon1 tab `roleOrder` settle | Non-blocking; opened stays 0 |

**WR1–WR8 + WR10–WR15 Done** (planner through live accept + capture + stableKey).
...
| Doc | Role |
| --- | --- |
| [forge-workon-reconcile.md](./plans/forge-workon-reconcile.md) | **P1** — WR1–WR8 + WR10–15 **Done**; next WR9 later |
...
| 3 | **High (shipped core)** | **Idempotent `forge workon` + sugar + coexist + capture + stableKey mons** | Morning desk + lived-in companions + quick profiles | **WR1–WR8 + WR10–15 Done**; polish WR9 later |
...
| later | WR9 env snippet | Later | Polish |
...
| **WR8 stableKey mons** | [completed](./plans/forge-workon-reconcile/completed/forge-workon-reconcile_wr8-stablekey.md) — plan-time mon key resolve |
...
- **WR8 Done:** stableKey / alias mon keys at plan-time; longest-prefix slot
  heads for dotted `name:…` keys; CLI unit green.
- **WR7 Done:** `forge workon capture` → tiles sugar; `workon_capture.py`;
  round-trip tree-perfect opened=0; docs/help.
- **WR6 Done earlier:** live black accept; chrome-luke `title~=`.
- **WR10–WR15** Done (sugar through clean).
- **Next:** WR9 later; thrash/regression watch outranks polish.
