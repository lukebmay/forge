...
**Updated:** 2026-08-05 (handoff: X11-first CL7 live; PWA open/wait fixed)  
...
| Order | Work | Why |
| ---: | --- | --- |
| **1** | **CL7 live on X11** | HUP-friendly; sole Ghostty + `forge layout dev` acceptance |
| **2** | **Merge control-loop → master** | Queue + product path after X11 CL7 (or when human says) |
| **3** | **Wayland residual pass** | After X11 green; borders / W4 / place timing only |
...
| Order | ID | Task | Why |
| ---: | --- | --- | --- |
| **1** | **CL7** | **X11** sole Ghostty + `forge layout dev` | [task](./tasks/forge-layout-control-loop_cl7-live-ghostty.md) · [HANDOFF](./HANDOFF.md) |
| **2** | **Merge** | `plan/forge-layout-control-loop` → master when smoke OK | local only unless push asked |
| **3** | **Wayland** | One logout smoke after X11 green | stash holds extra WIP — do not pop onto control-loop |
| **4** | **MR0** | monitor-recovery rename (own branch/PR) | [plan](./plans/forge-monitor-recovery-rename.md) |
| **5** | **CON S3** | After reliability green enough | [container-selection](./plans/forge-container-selection.md) |
...
| Unit tests (PWA slice) | CLI 358 + place-hint 24 |
...
| [forge-layout-control-loop.md](./plans/forge-layout-control-loop.md) | **Active** — CL7 live X11 left |
...
| Pri | Item | Status |
| --- | --- | --- |
| **P0** | [layout control loop](./plans/forge-layout-control-loop.md) | CL0–CL6 + PWA fix **code done** — **CL7 live X11** |
| **P0** | Wayland residual | **After** X11 CL7; stash holds WIP |
