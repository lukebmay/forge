...
3. Atomic ops match keybind command surface so scripts and hands share one
   engine.  
4. **`forge workon <name>`** = thin wrapper: display scene + mixed forge steps.

**Non-goals:** full i3 IPC parity; live GUI recorder; declarative full-tree restore.
...
| `workon` | **`forge workon`** profiles — FC5 Done |
...
| **FC5** | [completed/…](./forge-command/completed/forge-command_fc5-workon.md) | **Done (B AGREE)** | FC1–FC4 | S–M | `forge workon` + mixed `forge run` |
...
```text
1. FC0–FC5 complete. Prefer live smoke of `forge workon` on black mornings.
2. Optional: OP-opt tiny-pane (daily-driver) — not this plan.
3. No new DBus surface unless a real script gap appears.
```
