...
| R020 | 2026-08-13 | Tiled app (e.g. VLC) Meta-fullscreens or resizes off-slot without a grab | `tree.apply` skipped fullscreen; `onExternalGeometry` log-only; max floated | **Shipped:** D026 `shouldRestoreTileSlot` + `_restoreTileToSlot` | L0 `bug-461-edge-snap` / `layout-sensors` restore; lone-max still `bug-dyt2` |
