...
| R016 | 2026-08-11 | **Wayland/X11** dual-mon: **no-op** monitor re-apply (e.g. `gdisplays load` when layout already correct, or Mutter re-`ApplyMonitorsConfig` with identical geometry) **thrashes** tiled desk — windows jump / tabs reshuffle / focus storm — instead of calm retile or true no-op | `workareas-changed` (and related) arms **H1 thrash-pending + monitor-recovery** even when mon geometry fingerprint is **unchanged**. H1 is correct for lock/wake GPU peel; wrong for false thrash. shellrc gdisplays **6.4.2** skips no-op apply at caller; Forge must still short-circuit | **Open** — task [forge-monitor-noop-apply-thrash](./tasks/forge-monitor-noop-apply-thrash.md): fingerprint no-op before thrash-pending; L0 pure + `LIVE_CASES` **R016** | *pending* L0 no-op workareas; live `--tags R016` |
