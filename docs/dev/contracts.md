...
| **Would this drop overflow app mins?** | `dropWouldOverflowMins` / `swapWouldOverflowMins` / `unitMins` (`drop-intent.js`) + `readWindowMinSize` / `noteWindowMinFromClamp` / class floor (`tree-layout.js`); `ensureWindowMinSizeKnown` probe when Wayland has no hints; preview paints **per-zone** `.window-tilepreview-invalid` (HSPLIT / VSPLIT / TAB independent); refuse execute. Checks **dragged + dest window/group**. Tab join = full pane; edge = half on axis. Unreadable mins = fail-open | Poisoned full-frame “mins”; refuse path outside `moveWindowToPointer`; painting all zones red from one axis overflow |
