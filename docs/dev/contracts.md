...
| **Logging sinks + levels** | Vendored plog (`third_party/pansi/`, PLOG 1.2.0) via `lib/shared/plog-adapter.js` (GJS Gio `plog.gjs.js`) / `cli/plog.mjs` (Node). **Dual sink:** independent file (`~/.local/state/forge/forge.log`, `$FORGE_LOG_FILE`, or nest sibling) = at/above effective; journal / CLI tee = **WARN/ERROR/fatal only** (not INFO). Custom plog levels table mirrors prefs OFF/FATAL/ALL; gsettings `log-level` gates min. `production=true` does **not** force OFF. Levels: INFO = lifecycle; DEBUG = feature/hunt (silent on prod INFO); TRACE = opt-in path/id detail. **D052:** regular/`--prod` → INFO; `--dev` → DEBUG; enable `truncateFile:true` empties hunt file (CLI does not). **D053:** live change via `forge log` / DBus `Log` — session override (in-memory; cleared on disable/enable) wins over durable; `--persist` writes gsettings; `changed::log-level`/`logging-enabled` → `reconfigure()` (no truncate). CLI `FORGE_LOG_LEVEL` is process-only. | Parallel forge-only logger; INFO/DEBUG/TRACE to journal; import Node `plog.js` into GJS; CLI truncating the shared file; top-level `forge --log-level` driving Shell; `production` silencing logs |
