...
| **Tab drag pointer events (gesture lifetime)** | `DragDropManager` only (`drag-drop.js`): stage `captured-event` + `tabDragPointer` SourceBag poll → `noteTabDragMotion` / `finishTabDragRelease`. Tab actors **press-arm only** (`armTabDrag`) | Tab-actor `motion-event` / `button-release` for drag; dual event owners; drop stage listeners mid-gesture |
...
| **Would this drop overflow app mins?** | `dropWouldOverflowMins` / `swapWouldOverflowMins` / `unitMins` (`drop-intent.js`) + `readWindowMinSize` / `noteWindowMinFromClamp` / class floor (`tree-layout.js`); durable `window-mins.json`; `ensureWindowMinSizeKnown` **after** grab/open settle (never mid MOVING grab); cancel in-flight probes on grab-begin; **do not** queue dest probes mid-drag; grab-end queues **dragged only** (delayed); failed shrink → `_forgeMinProbeGaveUp` (no forever-retry). Preview paints **per-zone** `.window-tilepreview-invalid` (HSPLIT / VSPLIT / TAB independent); refuse execute. Checks **dragged + dest window/group**. Tab join = full pane; edge = half on axis. Unreadable mins = fail-open | Probe `move_resize` during MOVING grab; mid-drag dest probe queue; immediate grab-end flush racing `commitLayout`; forever-retry when Mutter ignores tiny resize; learning prior frame before clamp lands; refuse path outside `moveWindowToPointer`; painting all zones red from one axis overflow |
...
| **Free open when split would overflow mins** | `resolveOpenMinPlacement` / `bfsOpenMinTabCandidates` (`open-min-place.js`) via `wm._decideOpenMinPlacement` → tab on first same-mon unit that fits, else float (`addFloatOverride`). Always on when mins known; fail-open when unknown. Tiny-pane QoL stays separate. **Not** PlaceNext/ApplyLayout pins; **not** DnD (DnD still refuse) | Retargeting pinned apply slots; cross-mon tab walk (D044); replacing tiny-pane setting |
