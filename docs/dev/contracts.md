...
| Execute a tile drop | `DragDropManager.moveWindowToPointer` → `resolveDropMark2` + Mark 2 Join/Move (`runLiveForest`, `treatGrabTileAsTiles`) when mapped; else `_executeDropOperation` | Parallel session-only structure; rewrite of gesture/preview |
