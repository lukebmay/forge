...
| **Close / unmanage TILE** | `forgetHostWindow(wm, meta\|live\|id, reason)` — Forest remove → `mark2CleanupUnder` ancestor MONITOR → `commitLayout(force)` / `presentSeededForest`. `unmanaged` + `windowDestroy` both call it (idempotent) | Ad-hoc `forestRemoveWindow` + `paintWmForest` only; skip RuleSet settle; leave GObject ghost when seeded |
| **Seeded TILE/CON slot AABB** | `forestSlotPaintRect` / `forestSlotRect` / `paneRect` / `seededChildRect` (`present-chrome.js`). Chrome H/V + Meta + `reassertNodeToSlot` when `_liveForestSeeded`. First willTile map: `moveLiveToForestSlot` | Parallel `computeSizes(live.percent)` as a second SoT; remainder-to-last pixel split for Meta/chrome; default-map geom then idle snap |
| **Active pair chrome gate** | Forest MONITOR id `moNwsW` via `forestMonitorIdForChrome` / `decorationOnActiveWorkspace`. Show strips **and** TILE borders iff that pair is live (Shell active WS + that output). FLOAT = FLOATS Meta frame only. Inactive MONITOR: hide chrome, do not paint workarea rects. `showWindowBorders` / `restackAllWindowBorders` pair-gated | Default-show when Forest `moNwsW` exists; key chrome by monitor index alone; restack other-WS TILE borders; `processNode` sizes inactive `moNwsW` onto the physical workarea |
| **TABBED wrap Meta dest** | `planTabbedWrap` + `tabbedChildRect` + `forestSlotPaintRect` (`rowCount × stacked-tab-bar-height`). Wrap 1↔2 must rewrite `presentWmSlots` dest | Chrome St rows grow over content without Meta dest rewrite; 1-row-only inset |
| **Open share after admit** | `_insertChildPercent` via Forest **direct** parent (`tomParent` of the WINDOW). D032 wrap: carve the wrap only; sibling MONITOR/CON percents stay | `nodeWindow.parentNode` only; n+1-carve an ancestor split when the WINDOW already sits in a wrap |
| **Seeded tab open-leaf / buried present** | Forest parent + `lastTabFocusId` via `forestSlotPresentBuried` / `presentWmSlots` / `liveTabOpenLeafForPresent` (`liveById` / host bag) | GObject `parentNode` + duck `lastTabFocus` / `contains` as SoT when Forest parent exists |
...
| **Would this drop overflow app mins?** | `dropWouldOverflowMins` / `swapWouldOverflowMins` / `unitMins` (`drop-intent.js`) + `readWindowMinSize` / `noteWindowMinFromClamp` / `noteWindowMinFromOversizedFrame` / `noteWindowMinFromHealUndersize` / class floor (`tree-layout.js`) + env floor `defaultMinTileSize` (`min-tile-size.js`; unset → **256×144**). Durable `window-mins.json`. Passive learn only (clamp vs request / settled frame larger than slot). Do **not** learn when the frame is far **smaller** than the commanded dest (map-size stuck) **until D115 jitter retries are exhausted**, then `noteWindowMinFromHealUndersize` records that observed size as the class min. **No shrink-probe.** Preview paints **per-zone** `.window-tilepreview-invalid` (HSPLIT / VSPLIT / TAB independent); refuse execute. Checks **dragged + dest window/group**. Tab join = full pane; edge = half on axis. Floor always applies — never “unknown → allow” | Shrink-probe / mid-drag dest discover-resize queue; fail-open for unreadable mins; refuse path outside `moveWindowToPointer`; painting all zones red from one axis overflow |
| **After-act undersize heal (D115)** | `observeHealAfterSettle` / `decideHealStep` (`heal-ladder.js`) after tiling present `move` settle (`_scheduleGeomEpsilonObserve`). Desired = `forestSlotPaintRect`. Agree → stop. Else (1) jitter same dest (`decideUndersizeDestRetry` / `geomUndersizeRetry`, D111) (2) `noteWindowMinFromHealUndersize` (3) Mark 2 `group` enter nearest same-MONITOR TAB if slot ≥ min (4) `group` wrap nearest TILE neighbor if wrap slot ≥ min (D108 `place: "end"`) (5) FLOAT via `forestSetWindowFloating` — FLOAT **is** Agree. Hunt: `heal-ladder` `rung=`. | `force: true`; `wm_class=` product branch; second retry loop; merge with overflow (`rehomeIfSlotTooSmall` is frame > slot); `Mark2Drop*`; workspace hop / other-head steal; whole-forest `MON_MISMATCH` RESYNC |
...
| Execute a tile drop | Host → `OpSet.pointer.hover\|release(ev)` → named Mark 2 Ops (`move` / `join` / `group`) via `_commitPointerOp` / `runMark2` (`treatGrabTileAsTiles`). Live: `moveWindowToPointer`. Synthetic: session `_dndDropOp` / nest `dnd-drop` / e2e `fuzzDrag` synthesize `ev` (zone harness places `world` in onto pane) then same release path. No `resolveDropSurface` / `_commitDropSurface` user path. No `Mark2Drop*`. | Zone-named host commands; SurfaceOp fallthrough; invent `Mark2Drop*`; dual Forest←GObject write |
| Empty-monitor drop | Pointer `empty-monitor` hit → Mark 2 `move` with `onto`=MONITOR; host `_commitEmptyMonitorDrop` is Meta-transfer last-resort only (leaf) | Mid-drag rehome (R012); `_rehomeWindowPreservingContainer` (R022); empty-mon as parallel user SurfaceOp |
...
| **Hide-place-show (will-TILE map)** | `shouldDeferHiddenOpen` / `hideDeferredActor` / `showDeferredActor` (`layout-deferred-open.js`) then `moveLiveToForestSlot` (`forestSlotPaintRect`). Layout PlaceNext **and** ordinary TILE Launch (D117). Unhide after dest command, not at LayoutBatch end if already placed | Minimize-as-hide; FLOAT-as-hide; show-then-move; map-time raw `move_to_monitor` as the product place |
| **Apply overlay hide** | `isVisibleHard` / `collectVisibleTileWindowIds` (`layout-apply-visible.js`). Visible-hard (D117): every visible TILE on the apply workspace in-slot (D040) or honest FLOAT (D115). Hunt: `visible-hard overlay clear`. `_finish` still hides leftover (cancel / error / Done). Overlay ≠ Done.ok | Overlay until Done (D071 lifetime); overlay = all-hard (D043); hold overlay on buried ε |
| **Apply keyboard focus** | `focusAfterAllMappedAllowed` (`layout-apply-visible.js`). Once all **required** apps have **mapped** windows (D117). Pin still applies. Open-leaf **raise** is earlier (`revealGroupChild` / `pickOpenLeafToRaise` when the group has ≥1 mapped WINDOW) and is not keyboard focus | Focus during first open/place; gate on buried ε / all-hard (SM5) |
| **Visible-first spawn** | `orderOpenActionsVisibleFirst` (`layout-apply-visible.js`). Bands: open leaf → other visible TILE → buried. Chaos `launch-order` shuffle is test-only | Product apply shuffle; chrome-serialize as product order |
...
| **Restore TILE to paint target** | `wm.reassertNodeToSlot` dest = `forestSlotPaintRect` / `forestSlotRect` (+ gap/zoom, same as `presentWmSlots`) when seeded; else `tree.paintRectForWindow` (zoom-aware) | Move to GObject `computeSizes` leftover (1/3\|2/3); bare `renderRect` while `zoomMode` set (undoes D030); `onExternalGeometry` reassert (AC1: verify is log-only) |
| **H/V share after child change** | `repairSharesAfterChildChange` — CON H/V **and** MONITOR HSPLIT/VSPLIT (`redistributeShare` / rescale). Skip MONITOR when layout is not H/V (child-CON hint) | Skip all MONITOR; GObject remainder-to-last as share repair |
...
| ApplyLayout (D037–D043, D117) | Yes — Meta signals + bags | ApplyEpoch + in-slot hard + slot machines (SM1–SM4). Independent WINDOW settle. Keyboard focus once all required apps are **mapped** (D117; amends SM5 all-hard). Overlay hide = **visible-hard** (D117; D043/D071 overlay lifetime superseded). Overlay ≠ Done.ok (D041). Belt deleted (D042/SM6; D014 superseded) |
...
| Live grab resize/move | Grab handlers (percents / preview). Grab-time max/fs reject stays |
| Unsolicited size / maximize / Meta fullscreen | **D100:** observe only (`metric d100-observe`). No idle restore-to-slot / `renderTree` / `onExternalGeometry`. Slot authority remains D095 present/ε (RESYNC after settle-heuristics — not this handler) |
| Lone-tile maximize-on-single | Setting still exists; idle path no longer restores (D100) |
| Forge zoom full/width/height | `wm.toggleZoom` + `zoomRect` (D030) — presentation flag, **not** Meta fullscreen |

Idle D026 restore-to-slot as a Meta-signal path is **disconnected** (D100).
`LayoutController.onExternalGeometry` is not driven from idle size-changed.
