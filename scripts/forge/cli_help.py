...
def print_forge_help(*, stream: TextIO | None = None) -> None:
    ...
    _out(s, heading("forge", **kw),
         " - tiling control CLI (DBus -> Forge extension)")
    _out(s,
         dim("Distinct from forge-ctl / ./install (install & migrate).", **kw))
    _blank(s)
    ...
    rows = [
        ...
        (
            "thrash",
            "Thrash catalog / settle heuristics status / reset-heuristics",
        ),
        ...
        ("update",
         "Fetch origin/master; pull if new; always install (even if git current)"
         ),
    ...
    _out(s, "  ", cmd("forge layout help", **kw), "   ",
         dim("# full layout guide + minimal config", **kw))
    _out(s, "  ", cmd("forge layout list", **kw))
    _out(s, "  ", cmd("forge layout mydesk --dry-run", **kw))
    _out(s, "  ", cmd("forge layout mydesk", **kw))
    _out(s, "  ", cmd("forge layout mydesk --detach", **kw), "  ",
         dim("# job continues if TTY dies", **kw))
    _out(s, "  ", cmd("forge jobs", **kw), "                   ",
    ...
    terms = [
        ("LFT",
         "Last Focused Tile - the tile that last held focus; default open-app attach point"
         ),
        ("OP1",
         "Open-app policy: new windows place after LFT unless PlaceNext path/monitor is set"
         ),
        ("CON",
         "Container node in the tiling tree (HSPLIT/VSPLIT/TABBED/STACKED group)"
         ),
        ("HSPLIT / VSPLIT",
         "Horizontal / vertical split layout of sibling panes"),
        ("TABBED / STACKED", "Tab strip / stack of windows sharing one pane"),
        ("PlaceNext",
         "One-shot attach hint for the next matching window (monitor / tree path)"
         ),
        ...
        ("reconcile",
         "Desired-state layout: match roles, open gaps, move/park - not a launch script"
         ),
        ("slot",
         "Home for a role, e.g. mon0.term or mon1.comms (monitor + layout child id)"
         ),
        ("moNwsW",
         "Monitor N, workspace W tree id (mo0ws0 = primary-ish mon 0, workspace 0)"
         ),
        ("HUP",
         "Shell reload on X11 (killall -HUP gnome-shell) after install"),
        ("wmClass",
         "Window class string used for matching (see forge tree titles/classes)"
         ),
    ]
    for k, v in terms:
    ...
    _out(
        s, "  ",
        dim(
            "Path: moN = monitor, wsW = workspace, /i/j = child indices (0-based).",
            **kw))
    _out(s, "  ", dim("Discover: ", **kw), cmd("forge tree", **kw))
    _blank(s)

    _out(s, heading("Dependencies", **kw))
    _out(s, "  python3; python3-gi ", dim("(preferred)", **kw),
         " or gdbus; Forge extension enabled")
    _blank(s)

    _out(s, heading("Global flags", **kw))
    _out(s, "  ", cyan("--color=auto|always|never", **kw), "  ",
         dim("default auto (TTY on)", **kw))
    _out(s, "  ", cyan("--first", **kw), "                 ",
         dim("ambiguous match -> first candidate", **kw))
...
def print_layout_help(*, stream: TextIO | None = None) -> None:
    ...
    _out(s, heading("forge layout", **kw),
         " - named layout profiles (desired state)")
    _out(
        s,
        dim(
    ...
    _out(s, "  park extras. Run twice ~= no-op. Apps and layout live in ",
         bold("your JSON", **kw), ",")
    _out(
        s,
        "  not in Forge source. Forge never hardcodes Ghostty, Chrome, host names, etc."
    )
    _blank(s)
    ...
    for line, desc in (
        ...
        ("forge layout list",
         "This host: Name + Description table (JSON when piped)"),
        ("forge layout show <name>", "Resolved path + validated profile"),
        ("forge layout save <name>",
         "Snapshot current desk → host profile (no : or @ in name)"),
        ("forge layout save <name> --tree-file F",
         "Offline save from GetTree JSON"),
        ("forge layout save <name> --stdout", "Print JSON only (no write)"),
        ("forge layout save <name> --keep-floats",
         "Include FLOAT windows (Guake, …)"),
        ("forge layout <name> --focus TOKEN",
         "Override keyboard focus on load"),
        ("forge layout save <name> --description T",
         "Set description (no prompt)"),
        ("forge layout save <name> --no-description", "Omit description key"),
        ("forge layout <name> --dry-run",
         "Plan only (human + plan JSON; mode A/B)"),
        ("forge layout <name>",
         "Apply on current workspace; short human summary (stderr)"),
        ("forge layout a b", "Sequential: a→current, b→current+1 (all bare)"),
        ("forge layout 1:a 3:b",
         "Static: explicit 1-based workspaces (all numbered)"),
        ("forge layout a@1 b@3", "Static: same as 1:a 3:b"),
        ("forge layout <name> --verbose",
         "Also dump plan/apply JSON (or FORGE_VERBOSE=1)"),
        ("forge layout <name> --force-launch",
         "Imperative steps[] only (escape hatch)"),
        ("forge layout <name> --safe",
         "Open+move roles only (no park/structure/ensure)"),
        (
        ...
        ("forge layout <name> --keep-others",
         "Park residuals into last mon unit (tabs)"),
        ("forge layout <name> --clean --force",
         "Stronger Meta delete; never process-kill"),
    ):
        _out(s, "  ", cmd(line, **kw))
        _out(s, "      ", dim(desc, **kw))
    _blank(s)

    _out(s, heading("Workspace targeting", **kw), " ",
         dim("(exclusive modes)", **kw))
    _out(
        s,
        "  All layout args are ",
        bold("bare", **kw),
    ...
    for line, desc in (
        ("bare names only",
         "Sequential from current: name1 → now, name2 → now+1, …"),
        ("N:name  /  name@N",
         "Static on workspace N (CLI is 1-based; tree uses 0-based)"),
        ("mix bare + N:name", "Error — apply nothing"),
        ("preflight",
         "Every profile must exist; every workspace in range; else no apply"),
        ("scope",
         "Each apply/save only sees windows on that workspace (no cross-ws steal)"
         ),
    ):
    ...
    _out(s, heading("Where profiles live", **kw), " ",
         dim("(first hit wins)", **kw))
    for line in (
    ...
    _out(
        ...
        "  ",
        dim(
            "Export FORGE_LAYOUT_DIR from shellrc for multi-machine trees. Host: FORGE_HOST or hostname.",
            **kw),
    )
    _blank(s)

    _out(s, heading("Minimal profile", **kw), " ",
         dim("(bare array - preferred)", **kw))
...
[ { "tab": ["firefox", "code"] }, "ghostty" ]"""
    for line in code.splitlines():
        _out(s, "  ", cyan(line, **kw))
    _blank(s)
    _out(s, "  Then: ", cmd("forge layout simple --dry-run", **kw), "  ->  ",
         cmd("forge layout simple", **kw))
    _blank(s)

    _out(s, heading("Layout sugar", **kw), " ",
         dim("(desugars to roles + layout)", **kw))
    for line, desc in (
        ("bare dual-mon array",
         "[[panes…], [panes…]] when len==live mon count"),
        ("bare single-mon panes",
         "[ pane, pane ] → mon0 (default mon split hsplit)"),
        ('{ "tab": ["a","b"] }',
         "tabbed pane (also t / tabbed); save uses tab"),
        ('{ "stack": ["a","b"] }', "stacked (also s / stacked)"),
        ('{ "tab": […], "active": "Grok" }', "open leaf (1st match in group)"),
        ('{ "tab": […], "active": 1 }', "open 2nd child in group (0-based)"),
        ('{ "tab": […], "active": ["Grok", 1] }',
         "2nd Grok in this group only"),
        ('"focus": "Grok"', "keyboard focus (1st match desk-wide)"),
        ('"focus": ["Grok", 1]', "2nd Grok desk-wide (0-based)"),
        ('{ "hsplit"|"vsplit": […] }',
         "split CON (also h/horizontal, v/vertical)"),
        ('{ "hsplit": […], "share": [0.67, 0.33] }',
         "custom widths (ratio OK: [2,1])"),
    ...
    _out(s, heading("Defaults", **kw), " ",
         dim("(omit noise; Forge fills these in)", **kw))
    defaults = [
...
[
  ...
  [ "term-right", ["mail", "chat"] ]
    ...
    _out(s, "  ", bold("v2 bare array / tiles / roles+layout", **kw),
         "  reconcile (daily default)")
    _out(s, "  ", bold("v1 steps[]", **kw),
         "         imperative replay (can double apps)")
    _out(s, "  ", bold("--force-launch", **kw),
         "      force steps[] path for debug")
    _out(s, "  ", bold("--safe", **kw),
         "             open+move roles only (no park/structure/ensure)")
    _out(
        s,
    ...
    _out(s, "  ", bold("default", **kw),
         "             close non-layout windows (desk = profile only)")
    _out(s, "  ", bold("--keep-others", **kw),
         "       park residuals onto last mon unit (tab join)")
    _out(s, "  ", bold("--clean --force", **kw),
         "     stronger Meta delete; never process-kill")
    _out(s, "  ", bold("marginal.mode=strict", **kw),
         "  no companion keep; residual policy still applies")
    _blank(s)

    _out(s, heading("Thrash modes", **kw), " ",
         dim("(auto; stderr on dry-run/apply)", **kw))
    _out(s, "  ", bold("mode=A collect", **kw),
         "       desk sane: open gaps, move roles, tab marginals into views")
    _out(
        s,
        "  ",
    ...
    _out(s, "  ", bold("thrashState", **kw),
         "            one line when thrashed (score + reasons)")
    _out(s, "  ", bold("thrashRisk", **kw),
         "             plan risk when score > 0 (structure/moves)")
    _out(s, "  ", bold("--safe", **kw),
         "                 skips park/structure; still reports A/B detection")
    _blank(s)
    ...
    _out(s, "  • Profile names must not contain ", cyan(":", **kw), " or ",
         cyan("@", **kw), " (workspace targeting).")
    _out(s, "  • Match titles with ", cyan('title~="substr"', **kw),
         " when several windows share a class.")
    _out(s, "  • Counts: reused / opened / moved / kept / parked / closed.")
    _out(
        s,
        "  • Default closes non-layout windows; roles stay. --keep-others parks instead."
    )
    _out(
        s,
    ...
    _out(s, "  • Optional: ", cyan('"displays": "scene"',
                                   **kw), " -> gdisplays load; ",
         cyan('"settings": "name"', **kw), " -> SettingsLoad.")
    _out(
        s,
        "  • Offline plan: ",
    ...
    _out(s, "  • Save sketch: ", cmd("forge layout save mydesk", **kw),
         "  (bare array when possible)")
    _out(
        s,
        "  • Keep Guake on save: ",
        cmd("forge layout save mydesk --keep-floats", **kw),
    )
    _out(s, "  • In-tree examples: ",
         cyan("scripts/forge/examples/layout-tiles-minimal.json", **kw),
         dim(" (bare)", **kw))
    _out(s, "                    ",
         cyan("scripts/forge/examples/layout-tiles-nested.json", **kw))
    _out(s, "                    ",
         cyan("scripts/forge/examples/layout-minimal.json", **kw),
         dim(" (IR)", **kw))
    _blank(s)
